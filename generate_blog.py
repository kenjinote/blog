...
filepath = r"c:\work\kenji.blog\content\post\programming-languages-history-paradigm-evolution\index.md"
...
本記事では、プログラミング言語の歴史と、その根底にある **パラダイム** の変遷について、アセンブリ言語から始まり、C言語、Java、そして現代のシステムプログラミングを牽引するRustやGoに至るまで、極めて詳細かつ体系的に解説します。
...
この構造化プログラミングのパラダイムを体現し、さらにシステムプログラミングに革命をもたらしたのが、1972年にデニス・リッチーによって開発された **C言語** です。

C言語は、UNIXオペレーティングシステムを記述するために作られました。アセンブリ言語に近い低レベルなメモリアクセス能力（ポインタなど）を持ちながら、ハードウェアに依存しない移植性を備えていました。
...
C言語の成功により、「手続き型プログラミング」は長らくプログラミングの標準的なパラダイムとして定着しました。しかし、システムがさらに巨大化し、複雑化するにつれて、データとそれを操作する手続き（関数）が分離していることによる保守性の低下が課題となってきました。

""")

# Java and OOP
content.append("""
## 3. オブジェクト指向の台頭：複雑性への対処とJavaの登場

データと手続きをひとまとめにし、プログラムを「オブジェクト」の相互作用としてモデル化する **オブジェクト指向プログラミング（OOP）** というパラダイムが注目を集めました。

SimulaやSmalltalkといった言語がOOPの概念を築き、C言語にOOPの機能を追加した **C++** が広く普及しました。しかし、C++は複雑な言語仕様と、ポインタによるメモリ管理の難しさ（メモリリークやセグメンテーションフォルトなど）という問題を抱えていました。
...
Javaの登場により、エンタープライズシステムの大規模開発においてオブジェクト指向は絶対的な主流パラダイムとなりました。
...
同時に、状態を持たない関数の評価として計算をモデル化する **関数型プログラミング** のパラダイム（HaskellやScalaなど）も、並行処理の容易さから再評価されるようになりました。
...
数学的な厳密性を持つ関数型言語は、副作用のない純粋関数を中心に構築され、バグの発生しにくい堅牢なコードを書きやすいという利点があります。
...
Googleによって開発された **Go** は、システムプログラミング言語でありながら、C言語のようなシンプルさと、動的言語のような書きやすさを兼ね備えています。
...
Goはガベージコレクションを持ち、メモリ管理を自動化していますが、その実行速度は非常に高速であり、マイクロサービスやクラウドインフラ（KubernetesやDockerなど）の開発において事実上の標準言語となっています。
...
Rustのメモリ管理モデルとガベージコレクション（GC）の比較を以下の図に示します。
...
例えば、RustやGoは関数型プログラミングの要素（クロージャ、高階関数など）を取り入れており、JavaやC++も後のバージョンで関数型的な機能（ラムダ式など）を追加しています。
...
アセンブリ言語によるハードウェアとの直接の対話から始まり、C言語による構造化と移植性の獲得、Javaによるオブジェクト指向とメモリ管理の抽象化、そしてRustやGoによる並行処理と安全性の追求に至るまで、プログラミング言語は絶えず進化を続けてきました。
