...
Главное преимущество `std::unique_ptr` — его производительность. В состоянии по умолчанию, без кастомного удалителя (custom deleter), размер `std::unique_ptr` абсолютно идентичен размеру сырого указателя (Raw [Pointer](https://kenji.blog/ru/p/c-language-pointers-memory-management-stack-heap/)). Он не имеет никаких лишних переменных-членов и не использует виртуальные функции. Благодаря оптимизациям компилятора, доступ через `std::unique_ptr` разворачивается в ассемблерный код, эквивалентный сырому указателю.
