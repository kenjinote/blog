...
No entanto, há uma armadilha grave aqui, que pode ser chamada de "armadilha da abstração". O fato cruel é que "a transição da programação visual para linguagens de programação reais baseadas em texto (Python, JavaScript, C++, [Rust](https://kenji.blog/pt/p/webassembly-wasm-current-future/), etc.) é extremamente difícil, e muitos alunos desistem nesta fase".
