...
[4색 정리(Four Color Theorem)](https://kenji.blog/p/four-color-theorem/)는 수학, 특히 그래프 이론 및 위상 수학에서 가장 유명하고 매력적인 문제 중 하나입니다. 그 주장은 매우 단순하여 초등학생도 이해할 수 있을 정도로 직관적입니다. "어떠한 평면상의 지도라도, 인접한 영역이 다른 색이 되도록 칠하려면 최대 **4색** 이면 충분하다"는 것입니다.
...
4색 정리를 수학적으로 엄밀하게 다루기 위해, 문제는 그래프 이론의 언어로 번역됩니다. 지도상의 각 영역을 "정점(Vertex)"으로 하고, 경계선을 공유하는 영역끼리를 "간선(Edge)"으로 연결합니다. 이렇게 만들어진 그래프는 "평면 그래프(Planar Graph)"라고 불립니다.
...
여기서 [오일러의 다면체 정리](https://kenji.blog/p/eulers-polyhedron-formula/) $V - E + F = 2$ ($V$ 는 정점의 수, $E$ 는 간선의 수, $F$ 는 면의 수)가 평면 그래프의 성질을 조사하는 데 중요한 역할을 합니다.
