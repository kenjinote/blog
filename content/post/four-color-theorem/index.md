...
## 4. [グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)理論への変換

四色問題を数学的に厳密に扱うために、問題はグラフ理論の言葉に翻訳されます。地図上の各領域を「頂点（Vertex）」とし、境界線を共有する領域同士を「辺（Edge）」で結びます。このようにして作られたグラフは「平面グラフ（Planar [Graph](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)）」と呼ばれます。
...
ここで、[オイラー](https://kenji.blog/p/euler/)の多面体定理 $V - E + F = 2$ （$V$ は頂点の数、$E$ は辺の数、$F$ は面の数）が、平面[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)の性質を調べる上で重要な役割を果たします。
