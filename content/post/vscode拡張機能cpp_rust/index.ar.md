...
في برمجة الأنظمة الحديثة، رسخت لغتا C++ و [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) مكانتهما كأهم اللغات. تمتلك C++ تاريخاً طويلاً ونظاماً بيئياً ضخماً، ولا غنى عنها في أنظمة التشغيل، ومحركات الألعاب، وأنظمة التداول عالي التردد (HFT). ومن ناحية أخرى، تنتشر [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) بسرعة بفضل أمان الذاكرة من خلال نموذج الملكية (Ownership) ومواصفات اللغة الحديثة، ويتقدم اعتمادها في نواة لينكس. عند التطوير بهاتين اللغتين، يرتبط اختيار المحرر وإعداداته ارتباطاً مباشراً بإنتاجية التطوير.
...
- **التسريع من خلال فهرسة الخلفية**: يقوم بحساب مسبق (فهرسة) لمعلومات الرموز للمشروع بأكمله في الخلفية، لذا فإن عمليات مثل "الانتقال إلى التعريف ([Go](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) to Definition)" و "البحث عن كل المراجع (Find All References)" تكتمل على الفور حتى في المشاريع الضخمة.
...
### ② rust-analyzer (المعيار الفعلي لتطوير [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/))

في تطوير [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/)، خادم اللغة الرسمي المعتمد حاليًا هو **`rust-analyzer`**. في الماضي، كان المعيار RLS (خادم لغة [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)) ذا استجابة محدودة لأنه كان يعتمد على بنية تستدعي المترجم (rustc) مباشرة، ولكن تمت إعادة تصميم `rust-analyzer` من الصفر من أجل بيئات التطوير المتكاملة (IDE)، ولديه ميزات قوية قادرة على تحليل الكود تدريجياً حتى لو كان غير مكتمل.

#### مجموعة من الميزات التي تخلق إنتاجية هائلة
1. **Inlay Hints (تلميحات مضمنة)**: في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، حيث استنتاج النوع قوي، يوصى بعدم كتابة أنواع المتغيرات صراحة، ولكن هذا قد يقلل من قابلية القراءة. تعرض التلميحات المضمنة الأنواع المستنتجة وأسماء وسيطات استدعاء الدالة كنص خفيف متراكب على المحرر.
...
```
تشغيل `cargo clippy` تلقائياً في الخلفية عند الحفظ يعتبر أمراً حيوياً. يتيح لك ذلك ليس فقط اكتشاف انتهاكات الملكية، بل أيضاً اقتراحات تحسين الأداء وتعلم طرق الكتابة الشائعة (Idiomatic) لـ [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) فوراً.

---

### ③ CodeLLDB (مصحح أخطاء قوي متعدد المنصات)

سواء كنت تطور بـ C++ أو [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/)، فإن مصحح الأخطاء لفحص حالة الذاكرة أثناء وقت التشغيل ضروري. خاصةً، **`CodeLLDB`** يعمل باستقرار عبر جميع المنصات: Windows، Mac، و Linux، ولديه توافق عالٍ للغاية مع [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/).

يستخدم مترجم [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) (rustc) LLVM كخلفية، وصيغة معلومات التصحيح التي تم إنشاؤها (DWARF / PDB) متوافقة تمامًا مع LLDB، وهو أيضًا جزء من مشروع LLVM.

#### مثال متقدم لإعدادات launch.json

هذا إعداد لـ `.vscode/launch.json` لبدء التصحيح في VSCode. هنا نعرض تكويناً متكاملاً لتصحيح أخطاء الملفات التنفيذية لكل من C++ و [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/).
...
```
لاحظ كتلة تكوين [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/). نظرًا لأن `CodeLLDB` يدعم خيار `cargo` بشكل أصلي، فليست هناك حاجة لتحديد مسار الثنائي الذي يحتوي على قيم تجزئة معقدة بعد التجميع مباشرة. يقوم المحرر تلقائيًا بتشغيل `cargo build`، ويلتقط أحدث ملف تنفيذي تم إنشاؤه ويرفق مصحح الأخطاء.
...
### ⑤ crates (إدارة تبعيات حزم [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) في الوقت الفعلي)

إضافة تجعل `Cargo.toml`، ملف إدارة التبعيات في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، مريحًا للغاية.
...
`Error Lens` هي إضافة ثورية تبرز أخطاء القوالب الطويلة في C++ وأخطاء فحص الاستعارة ([Borrow Checker](https://kenji.blog/ar/p/memory-management-garbage-collection/)) الصارمة في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) مضمنةً مباشرة على يمين السطر ذي الصلة في المحرر.
...
باستخدام `Error Lens`، تُعرض رسائل الخطأ على حافة رؤيتك أثناء كتابة الكود دون رفع يديك عن لوحة المفاتيح. في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) بشكل خاص، يمكنك فهم أخطاء دورة الحياة المعقدة على الفور مثل "`cannot borrow 'x' as mutable because it is also borrowed as immutable`" أثناء النظر إلى السطر ذي الصلة، مما يحسن سرعة الإصلاح بشكل كبير.
...
عند مواجهة كتلة `unsafe` في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) أو عمليات صب (Casting) صعبة في C++، فإن القدرة على الإشارة الفورية إلى طلب السحب (Pull Request) أو رسالة الالتزام التفصيلية من وقت دمج هذا الكود هي سلاح قوي في الهندسة العكسية.

---

### ⑧ GitHub Copilot

حتى في برمجة الأنظمة، أصبح إدخال مساعدي الذكاء الاصطناعي التوليدي نقلة نوعية لا مفر منها. تدعم إضافة **`GitHub Copilot`** بناء الأكواد المكررة في C++ وبناء سلاسل المكررات المعقدة في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) بدقة عالية جدًا.
...
إضافة توفر تسليط الضوء على بناء الجملة، والتنسيق التلقائي، والتحقق القوي من المخطط (Schema Validation) لـ `Cargo.toml`، ملف إعداد مشروع [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، و `rust-toolchain.toml`، ملف إعداد سلسلة الأدوات.
...
في أنماط التصميم التي تستخدم السلاسل النصية كمفاتيح في `std::unordered_map` لـ C++ أو `HashMap` لـ [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، فإن الأخطاء الإملائية (Typos) تمر عبر المترجم ويصعب ملاحظتها حتى تظهر كأخطاء في وقت التشغيل، وهي طبيعة مزعجة للغاية. من خلال تقديم مدقق إملائي وعرض تحذير بخط متعرج على المحرر، يمكن القضاء تمامًا على هذه الأخطاء السهلة في مرحلة البرمجة.
...
يوجد أدناه مثال متقدم لإعداد `tasks.json` للتعايش بين بناء C++ باستخدام CMake وبناء [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) باستخدام Cargo.
...
كما يوضح مخطط التسلسل هذا، تتم اتصالات لا حصر لها (Debug Adapter Protocol - DAP) بين VSCode و CodeLLDB أثناء جلسة تصحيح الأخطاء. حتى هياكل البيانات المعقدة التي هي عبارة عن مجموعات من المؤشرات، مثل `std::map` في C++ أو `Vec<T>` في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، يتم عرضها بشكل بديهي جداً (كشجرة مع محتويات المصفوفة الموسعة) على واجهة مستخدم VSCode بواسطة ميزة المنسق (Formatter) المدمجة في CodeLLDB.

لجعل ذلك ممكناً، يقوم مترجم [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) بتضمين معلومات تخطيط النوع (مثل الحجم والحشو) بالتفصيل داخل صيغة DWARF، ويقوم CodeLLDB وفقاً لذلك بتحويل سلسلة البايتات الخام على الذاكرة المستهدفة ببراعة إلى صيغة يمكن للبشر قراءتها.
...
تعتبر لغتا C++ و [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) لغتين صارمتين تهدفان إلى "استخراج أقصى أداء للأجهزة"، وتتطلبان من المطورين مستوى عالٍ من الفهم والبرمجة الدقيقة.
...
1. **clangd** (خادم لغة C++)
2. **rust-analyzer** (خادم لغة [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/))
