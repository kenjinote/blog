...
L'une des réalisations initiales les plus importantes de von Neumann a été ses recherches sur l'axiomatisation de la théorie des ensembles. La théorie des ensembles, fondée par Georg Cantor, devait être le fondement des mathématiques, mais elle était confrontée à des contradictions logiques (paradoxes) telles que le paradoxe de Russell. Pour résoudre ce problème, Ernst Zermelo, Adolf Fraenkel et d'autres construisaient la théorie axiomatique des ensembles, mais von Neumann a adopté une approche différente.
...
```mermaid
flowchart LR
    A["Joueur A (Player A)"] -->|"Choisir une stratégie (Choose Strategy)"| B["Environnement de jeu (Game Environment)"]
    C["Joueur B (Player B)"] -->|"Choisir une stratégie (Choose Strategy)"| B
    B -->|"Calculer les gains (Calculate Payoff)"| A
    B -->|"Calculer les gains (Calculate Payoff)"| C
```
...
```mermaid
flowchart TD
    A["Périphérique d'entrée (Input Device)"] --> B["Unité centrale de traitement (CPU)"]
    B --> C["Périphérique de sortie (Output Device)"]
    B <--> D["Mémoire (Memory)"]
    subgraph CPU ["À l'intérieur du CPU"]
        E["Unité de contrôle (Control Unit)"]
        F["Unité arithmétique et logique (ALU)"]
    end
    E <--> F
```
