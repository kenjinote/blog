...
```mermaid
flowchart LR
    A["Spieler A (Player A)"] -->|"Strategie wählen (Choose Strategy)"| B["Spielumgebung (Game Environment)"]
    C["Spieler B (Player B)"] -->|"Strategie wählen (Choose Strategy)"| B
    B -->|"Auszahlung berechnen (Calculate Payoff)"| A
    B -->|"Auszahlung berechnen (Calculate Payoff)"| C
```
...
```mermaid
flowchart TD
    A["Eingabegerät (Input Device)"] --> B["Zentraleinheit (CPU)"]
    B --> C["Ausgabegerät (Output Device)"]
    B <--> D["Speicher (Memory)"]
    subgraph CPU ["Innerhalb der CPU"]
        E["Steuerwerk (Control Unit)"]
        F["Rechenwerk (ALU)"]
    end
    E <--> F
```
