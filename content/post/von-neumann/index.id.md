...
```mermaid
flowchart LR
    A["Pemain A (Player A)"] -->|"Pilih Strategi (Choose Strategy)"| B["Lingkungan Permainan (Game Environment)"]
    C["Pemain B (Player B)"] -->|"Pilih Strategi (Choose Strategy)"| B
    B -->|"Hitung Hasil (Calculate Payoff)"| A
    B -->|"Hitung Hasil (Calculate Payoff)"| C
```
...
```mermaid
flowchart TD
    A["Perangkat Input (Input Device)"] --> B["Unit Pemroses Sentral (CPU)"]
    B --> C["Perangkat Output (Output Device)"]
    B <--> D["Memori (Memory)"]
    subgraph CPU ["Di dalam CPU"]
        E["Unit Kontrol (Control Unit)"]
        F["Unit Aritmatika Logika (ALU)"]
    end
    E <--> F
```
