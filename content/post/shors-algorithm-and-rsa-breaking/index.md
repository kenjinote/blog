...
3. **行列の生成と簡約** : 収集した関係式を元に巨大な疎行列（成分のほとんどが0の行列）を生成し、線形代数的な手法（ブ[ロック](https://kenji.blog/p/rdbms-transaction-acid-isolation-level-lock/)・ランチョス法など）を用いて解を求めます。
4. **平方根の計算** : 最後に代数体上での平方根を計算し、$ N $ の因数（素因数）を導き出します。

GNFSの計算量は、非漸近的に $ O(\exp((\sqrt[3]{\frac{64}{9}} + o(1)) (\log N)^{\frac{1}{3}} (\log \log N)^{\frac{2}{3}})) $ と評価されます。これは「準指数関数的（Sub-exponential）」な[時間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)と呼ばれます。指数関数時間よりは速いものの、多項式時間（Polynomial time）よりは遥かに遅い計算量です。
...
ここからが量子コンピュータの出番です。
量子コンピュータは、「0」と「1」の状態を同時に持つことができる「量子ビット（Qubit）」を使用します。[ショアのアルゴリズム](https://kenji.blog/p/quantum-computing-shors-algorithm/)では、入力を格納するレジスタ（第1レジスタ）と、計算結果を格納するレジスタ（第2レジスタ）の2つを用意します。
...
PQCは、[量子コンピュータ](https://kenji.blog/p/quantum-computing-shors-algorithm/)を用いても解読が困難（つまり、[ショアのアルゴリズム](https://kenji.blog/p/quantum-computing-shors-algorithm/)が適用できない）な数学的問題をベースにした新しい暗号アルゴリズムです。主なアプローチとして以下のようなものがあります。
