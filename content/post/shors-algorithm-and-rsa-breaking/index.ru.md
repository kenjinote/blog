---
title: 'Взломают ли квантовые компьютеры шифрование RSA? Объяснение алгоритма Шора'
...
description: 'Объясняется, как развитие квантовых компьютеров угрожает шифрованию RSA, которое поддерживает современную коммуникационную инфраструктуру. От механизма алгоритма Шора до текущего состояния аппаратного обеспечения эпохи NISQ — рассматривается будущее криптоанализа следующего поколения и безопасности.'
---

## Введение: Пересечение криптографии и квантовых компьютеров

В современном интернет-обществе основой защиты секретности связи является «криптография с открытым ключом». Наиболее представительным является шифрование [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/), разработанное в 1977 году Роном Ривестом, Ади Шамиром и Леонардом Адлеманом. От покупок в Интернете и просмотра веб-сайтов (HTTPS) до отправки и получения электронной почты, шифрование RSA служит сердцем интернет-инфраструктуры.
...
В этой статье подробно рассматривается механизм GNFS (Общий метод решета числового поля) как классический метод дешифрования, а также окончательный алгоритм дешифрования с использованием квантовых компьютеров — «Алгоритм Шора» ([Shor's Algorithm](https://kenji.blog/ru/p/quantum-computing-shors-algorithm/)). Мы доступно объясним такие концепции, как квантовое преобразование Фурье и поиск периода, и подробно рассмотрим текущее состояние аппаратного обеспечения в эпоху NISQ (Noisy Intermediate-Scale Quantum) и реальные препятствия для взлома [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)-2048.
...
и алгоритма [Евклид](https://kenji.blog/ru/p/euclid/)а мы можем легко найти факторы $ N $.
...
Будет ли [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/) взломан завтра? Ответ — однозначное «Нет», из-за аппаратных ограничений.
...
В ответ на эту угрозу NIST ускоряет стандартизацию ** Постквантовой криптографии (PQC: Post-Quantum [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy) **.
...
[RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/) не устареет завтра, но в свете угроз вроде "Store Now, Decrypt Later", масштабный переход криптографии к PQC уже начался. Мы являемся свидетелями смены парадигмы в информационной безопасности.
