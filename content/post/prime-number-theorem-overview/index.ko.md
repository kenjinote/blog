...
그럼에도 불구하고 거시적인 관점, 즉 '전체 수 중에서 소수가 어느 정도의 비율로 존재하는가' 하는 대국적인 움직임에는 놀랍도록 아름답고 매끄러운 법칙이 숨겨져 있다는 사실이 발견되었습니다. 그것이 본 기사에서 해설할 **소수 정리(Prime Number Theorem, PNT)**입니다.
...
이 놀라운 사실을 처음 깨달은 것은 당시 불과 15세였던 천재 카를 프리드리히 가우스(Carl Friedrich Gauss)였습니다. 1792년, 가우스는 로그표와 소수표를 열심히 조사하여 소수의 밀도가 자연로그에 반비례하여 감소해 가는 경향을 읽어냈습니다. 그는 다음과 같은 근사식을 추측했습니다.
...
소수 정리 증명을 향한 가장 큰 돌파구는 베른하르트 리만(Bernhard Riemann)에 의해 마련되었습니다. 1859년에 발표된 획기적인 논문 '주어진 수보다 작은 소수의 개수에 대하여'에서 리만은 소수의 분포와 **복소함수**의 움직임이 깊게 결부되어 있다는 것을 보여주었습니다.
...
그들 증명의 핵심은 '제타 함수 $\zeta(s)$는 복소평면상의 직선 $\operatorname{Re}(s) = 1$ 위에 영점을 갖지 않는다'는 것을 보여주는 것이었습니다. 복소해석학의 강력한 도구(코시의 적분 정리 등)를 이용함으로써 이 영점의 비존재로부터 소수 정리가 도출됩니다.
...
이 알고리즘은 페르마의 소정리를 확장한 것으로, 합성수임에도 불구하고 소수로 오판할 확률을 테스트 횟수 $k$를 늘림으로써 지수함수적으로 작게 만들 수 있습니다(오판 확률은 $4^{-k}$ 이하).
