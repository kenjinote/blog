...
- $r$은 음이 아닌 정수이며, <strong>랭크(계수)</strong>라고 불립니다.
...
BSD 추측의 응용으로서 매우 유명한 것이 <strong>합동수 문제(Congruent number problem)</strong>입니다. "어떤 자연수 $n$이, 모든 변의 길이가 유리수인 직각삼각형의 넓이가 될 수 있는가?"라는 문제입니다. 넓이가 될 수 있는 $n$을 합동수라고 부릅니다. 예를 들어 $n=5, 6, 7$은 합동수이지만, $n=1, 2, 3$은 합동수가 아닙니다.
