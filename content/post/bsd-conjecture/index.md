...
本記事では、BSD予想が何を意味しているのかを理解するために、楕円曲線の基礎から出発し、モーデルの定理、L関数の定義、そしてBSD予想の正確な主張（弱予想と強予想）まで、詳細かつ厳密に解説していきます。さらに、合同数問題との関連やガロアコホモロジーによる背景など、高度なトピックにも踏み込んでいきます。
...
無限遠点 $\mathcal{O}$ を零元（単位元）とすることで、楕円曲線 $E$ 上の点はアーベル群をなします。特に、有理数体 $\mathbb{Q}$ 上で定義された楕円曲線の有理点（座標 $x, y$ がともに有理数である点）全体の集合 $E(\mathbb{Q})$ は、この加法について部分群となります。
...
有理点を見つける問題は、古くからディオファントス方程式の主要な問題として研究されてきました。有理点の集合がどのような構造を持っているのか、その全貌を明らかにすることが最大の目標となります。

## 3. モーデルの定理とランク（階数）

1922年、ルイス・モーデル（Louis Mordell）は有理点群 $E(\mathbb{Q})$ の構造に関する決定的な定理を証明しました。のちにアンドレ・ヴェイユ（André Weil）がより一般の代数体とアーベル多様体へと拡張し、モーデル・ヴェイユの定理として知られています。

### 3.1. モーデルの定理 (Mordell's Theorem)

**定理（モーデル、1922年）**
楕円曲線 $E$ の有理点群 $E(\mathbb{Q})$ は有限生成アーベル群である。

代数学の有限生成アーベル群の基本定理によれば、$E(\mathbb{Q})$ は次のような同型を持ちます。
...
- $\mathbb{Z}^r$ は無限位数の点（何度足しても $\mathcal{O}$ にならない点）から生成される自由アーベル群です。
...
ハッセの定理（Hasse's bound）によれば、このずれは $|a_p| \le 2\sqrt{p}$ で抑えられることが知られています。これは、有限体上の楕円曲線に対するリーマン予想の類似の一種です。

### 4.2. L関数の定義

これらの局所的な情報 $a_p$ をすべての素数 $p$ について集め、一つの解析的な関数を構成します。これが **ハッセ・ヴェイユ L関数** （Hasse-Weil L-function） $L(E, s)$ です。
複素数 $s$ に対して、オイラー積を用いて次のように定義されます。
...
この無限積はハッセの境界を用いることで $\mathrm{Re}(s) > \frac{3}{2}$ の領域で絶対収束することが示されます。
...
この問題は、2001年に完全に証明された **モジュラリティ定理** （旧 谷山・志村予想）によって解決されました。アンドリュー・ワイルズ、リチャード・テイラー、クリストフ・ブロイル、ブライアン・コンラッド、フレッド・ダイアモンドらの壮大な業績により、「すべての有理数体上の楕円曲線はモジュラーである」ことが示されました。
...
もし有理点が多い（ランク $r$ が大きい）ならば、各素数 $p$ を法としても解の数 $N_p$ が大きくなる傾向があるはずです。すると $a_p = p + 1 - N_p$ は負の方向に大きくなり、オイラー積の項 $(1 - a_p p^{-1} + p^{-1})^{-1}$ は小さくなるため、$s=1$ における L関数の値は $0$ に近づくはずです。
...
4. **$c_p$ (玉河数)** : 悪い還元をもつ素数 $p$ に対する局所的な補正係数。局所体のガロア群の作用から計算されます。
...
### 5.3. 謎の群「Sha (Ш)」とガロアコホモロジー

公式の中で最も神秘的で難解な対象が、テイト・シャファレヴィッチ群 $\text{Sha}(E)$ （キリル文字の $\text{\textcyrillic{Sh}}$ で表される）です。

局所・大域原理（ハッセの原理）とは、「すべての方程式が有理数体（大域）で解を持つための必要十分条件は、すべての素数 $p$ について $p$ 進数体（局所）で解を持ち、かつ実数体でも解を持つことである」という原理です。二次形式に対してはこの原理が成り立ちます（ハッセ・ミンコフスキーの定理）。
しかし、楕円曲線（三次曲線）に対してはこの原理は成り立ちません。「局所的にはすべて解を持つのに、大域的には解を持たない」という現象が起こり得ます。

$\text{Sha}(E)$ は、この「局所・大域原理の失敗」をガロアコホモロジーを用いて測る群です。厳密には以下のように定義されます。

$$
\text{Sha}(E) = \ker \left( H^1(G_{\mathbb{Q}}, E) \to \prod_{v} H^1(G_{\mathbb{Q}_v}, E) \right)
$$

ここで $G_{\mathbb{Q}}$ は絶対ガロア群、積はすべての素点（有理素数と無限素点）にわたります。
...
- **コリヴァギンの定理 (Kolyvagin, 1989)** :
  オイラー系（Euler system）という強力なガロアコホモロジーの手法を構築し、解析的ランクが 0 または 1 の場合、代数的ランクと一致すること、さらにそのときに限りテイト・シャファレヴィッチ群 $\text{Sha}(E)$ が有限群になることを証明しました。
...
また、ランク 2 以上の場合におけるオイラー系のような有理点を構成するための系統的なメカニズムも見つかっておらず、現代数学の大きな壁として立ちはだかっています。
...
BSD予想が完全に解明された暁には、ディオファントス方程式の有理数解の理解に究極のブレイクスルーがもたらされるだけでなく、関数体上の類似（アルティン・テイト予想）や、数学の諸分野を統合する「ラングランズ・プログラム」などのより広範なモチヴィックL関数理論の根幹を支える確固たる土台となるでしょう。
