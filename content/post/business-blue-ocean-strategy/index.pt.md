...
```mermaid
flowchart TD
    A["Realização da Inovação de Valor"]
    A -- "Redução de Custos" --> B["Eliminar (Eliminate)"]
    A -- "Redução de Custos" --> C["Reduzir (Reduce)"]
    A -- "Aumento de Valor" --> D["Elevar (Raise)"]
    A -- "Aumento de Valor" --> E["Criar (Create)"]
```
...
```mermaid
flowchart LR
    A["Análise da Tela Estratégica"]
    A -- "Compreensão da Situação Atual" --> B["Plotagem dos Fatores Competitivos Existentes"]
    A -- "Diferenciação dos Concorrentes" --> C["Comparação de Curvas de Valor"]
    C -- "Ação ERRC" --> D["Criação de Nova Curva de Valor"]
    D -- "Pioneirismo em Novos Mercados" --> E["Descoberta do Oceano Azul"]
```
