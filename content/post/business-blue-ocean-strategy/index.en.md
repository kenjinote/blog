...
```mermaid
flowchart TD
    A["Realization of Value Innovation"]
    A -- "Cost Reduction" --> B["Eliminate"]
    A -- "Cost Reduction" --> C["Reduce"]
    A -- "Value Enhancement" --> D["Raise"]
    A -- "Value Enhancement" --> E["Create"]
```
...
```mermaid
flowchart LR
    A["Strategy Canvas Analysis"]
    A -- "Understand current status" --> B["Plotting existing competitive factors"]
    A -- "Differentiation from competitors" --> C["Comparison of value curves"]
    C -- "ERRC actions" --> D["Creation of new value curve"]
    D -- "New market development" --> E["Discovery of Blue Ocean"]
```
