...
```mermaid
flowchart TD
    A["Realisierung der Wertinnovation"]
    A -- "Kostensenkung" --> B["Eliminate (Eliminieren)"]
    A -- "Kostensenkung" --> C["Reduce (Reduzieren)"]
    A -- "Wertsteigerung" --> D["Raise (Steigern)"]
    A -- "Wertsteigerung" --> E["Create (Kreieren)"]
```
...
```mermaid
flowchart LR
    A["Analyse der Strategiekontur"]
    A -- "Erfassung des Ist-Zustands" --> B["Plotting bestehender Wettbewerbsfaktoren"]
    A -- "Differenzierung vom Wettbewerb" --> C["Vergleich der Wertkurven"]
    C -- "ERRC-Aktionen" --> D["Schaffung einer neuen Wertkurve"]
    D -- "Erschließung neuer Märkte" --> E["Entdeckung des Blue Ocean"]
```
