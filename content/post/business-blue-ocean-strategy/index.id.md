...
```mermaid
flowchart TD
    A["Mewujudkan Inovasi Nilai"]
    A -- "Pengurangan Biaya" --> B["Eliminate (Hapus)"]
    A -- "Pengurangan Biaya" --> C["Reduce (Kurangi)"]
    A -- "Peningkatan Nilai" --> D["Raise (Tingkatkan)"]
    A -- "Peningkatan Nilai" --> E["Create (Ciptakan)"]
```
...
```mermaid
flowchart LR
    A["Analisis Kanvas Strategi"]
    A -- "Memahami Kondisi Perusahaan Saat Ini" --> B["Plotting Faktor Persaingan yang Ada"]
    A -- "Diferensiasi dengan Pesaing" --> C["Perbandingan Kurva Nilai"]
    C -- "Aksi ERRC" --> D["Penciptaan Kurva Nilai Baru"]
    D -- "Pembukaan Pasar Baru" --> E["Penemuan Blue Ocean"]
```
