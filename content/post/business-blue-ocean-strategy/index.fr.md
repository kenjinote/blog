...
```mermaid
flowchart TD
    A["Réalisation de l'Innovation Valeur"]
    A -- "Réduction des coûts" --> B["Exclure (Eliminate)"]
    A -- "Réduction des coûts" --> C["Atténuer (Reduce)"]
    A -- "Amélioration de la valeur" --> D["Renforcer (Raise)"]
    A -- "Amélioration de la valeur" --> E["Créer (Create)"]
```
...
```mermaid
flowchart LR
    A["Analyse du canevas stratégique"]
    A -- "Comprendre la situation actuelle de l'entreprise" --> B["Traçage des facteurs concurrentiels existants"]
    A -- "Différenciation avec les concurrents" --> C["Comparaison des courbes de valeur"]
    C -- "Actions ERRC" --> D["Création de la nouvelle courbe de valeur"]
    D -- "Développement d'un nouveau marché" --> E["Découverte de l'océan bleu"]
```
