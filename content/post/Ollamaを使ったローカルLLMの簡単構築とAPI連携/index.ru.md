...
# Введение: Зачем нужна локальная [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/)?

С появлением [больших языковых моделей](/ru/p/large-language-models-llm-transformer-prompt-engineering/) (LLM) наша жизнь и методы разработки претерпели радикальные изменения. Мощные облачные ИИ-сервисы, такие как ChatGPT, Claude и [Gemini](/ru/p/google-one-gemini%E3%81%8C%E8%A7%A3%E7%B4%84%E3%81%A7%E3%81%8D%E3%81%AA%E3%81%84%E6%99%82%E3%81%AE%D1%81%D0%BF%D0%BE%D1%81%D0%BE%D0%B1-%D1%80%D0%B5%D1%88%D0%B5%D0%BD%D0%B8%D1%8F/), продолжают развиваться с каждым днем, предоставляя чрезвычайно продвинутые возможности логического вывода. Однако облачные LLM не всегда являются оптимальным выбором для всех сценариев использования. У облачных LLM есть следующие проблемы:
...
Ollama — это платформа для простого запуска и управления большими языковыми моделями с открытым исходным кодом (Llama 3, Phi-3, Mistral, Gemma и т. д.) в локальной среде. До сих пор для настройки локальной среды LLM требовались очень сложные процедуры: настройка среды Python, установка набора инструментов CUDA, разрешение зависимостей PyTorch, загрузка огромных файлов моделей с Hugging Face и преобразование форматов (например, из Safetensors в [GGUF](/ru/p/llama-cpp-quantization-gguf/)).

Ollama скрывает эти сложности, позволяя обращаться с LLM так же легко, как с [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/). С помощью одной команды вы можете скачать модель (`pull`), запустить ее (`run`) и поднять в качестве HTTP-сервера.

## Основная технология: Обертка для llama.cpp

В качестве бэкенда механизма вывода Ollama выступает **llama.cpp**, быстрая библиотека вывода [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/), написанная на C/C++. llama.cpp обладает способностью выполнять модели, максимально используя аппаратные возможности, будь то Apple Silicon (Metal), NVIDIA GPU (CUDA), AMD GPU (ROCm) или даже среды только с CPU.

Ollama включает в себя llama.cpp и использует архитектуру, в которой серверный процесс, написанный на языке [Go](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/), предоставляет [REST API](https://kenji.blog/ru/p/graphql-vs-rest-api-[overfetching](https://kenji.blog/ru/p/graphql-vs-rest-api-overfetching-type-safety/)-type-safety/), вызывая механизм вывода llama.cpp в фоновом режиме.
...
## Запуск с использованием [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)

Если вы не хотите загрязнять существующую среду или хотите интегрировать инструмент в контейнерную инфраструктуру, вы также можете использовать официальный образ Docker. Для использования GPU необходимо установить NVIDIA [Container](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/) Toolkit.
...
Самая большая привлекательность Ollama заключается в том, что управление моделями очень интуитивно понятно. Вы можете тестировать различные модели с тем же ощущением, что и при работе с образами [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/).
...
Здесь стоит немного остановиться на понятии квантования. Обычная [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) хранит параметры весов в 16-битном формате с плавающей запятой (FP16). Для модели с 8 миллиардами (8B) параметров только одни веса потребляют около 16 ГБ VRAM. Квантование — это технология сжатия их до 4-битных (Q4) или 8-битных (Q8) целых чисел.

Благодаря квантованию можно кардинально сократить требуемый объем памяти и пропускную способность памяти, при этом минимизируя ухудшение точности модели. Модели, распространяемые в Ollama, по умолчанию представлены в формате [GGUF](/ru/p/llama-cpp-quantization-gguf/) с примененным оптимальным квантованием (чаще всего 4-битным).
...
В Ollama вы можете использовать механизм под названием **Modelfile** для создания собственных пользовательских моделей путем внедрения системных промптов и настройки гиперпараметров в существующие модели. Эта концепция полностью аналогична [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)file в Docker.
...
Поскольку современные [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) дообучены (fine-tuned) для формата чата, при разработке приложений рекомендуется использовать `/api/chat`.
...
Для фронтенд-инженеров и fullstack-разработчиков возможность вызова локальных [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) из среды TypeScript/Node.js является огромным преимуществом. Мы будем использовать официальный NPM-пакет `ollama`.
...
Чтобы предоставить локальные [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) на уровне, пригодном для реального использования, необходим анализ задержки (latency) и пропускной способности (throughput). Ответы API Ollama содержат подробные метрики, касающиеся производительности.
...
$$
Добавив к этому память для контекста, мы видим, что при наличии примерно 5-6 ГБ VRAM можно полностью развернуть модель на GPU (Full Offload). Даже с графическими процессорами среднего класса с 8 ГБ VRAM (например, RTX 4060) последних лет вполне возможно запускать достаточно мощные [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/).
