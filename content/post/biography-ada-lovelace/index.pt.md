...
```mermaid
graph TD
    A["Pai: George Gordon Byron"] -- "Rica imaginação / Romantismo" --> C["Ada Lovelace"]
    B["Mãe: Anne Isabella Milbanke"] -- "Educação matemática rigorosa / Pensamento lógico" --> C
    C -- "Imaginação + Lógica" --> D["Ciência Poética (Poetical Science)"]
    D -- "Aplicação" --> E["Compreensão essencial e abstração dos computadores"]
```
...
```mermaid
graph TD
    A["Conceito da Máquina Analítica (Babbage)"] -- "Controle por cartão perfurado" --> B["Unidade de Entrada (Input)"]
    B -- "Dados e Instruções" --> C["Armazém (Store: Memória)"]
    B -- "Instruções de Cálculo" --> D["Moinho (Mill: CPU)"]
    C -- "Troca de Valores" --> D
    D -- "Resultados do Cálculo" --> E["Unidade de Saída (Output)"]
    
    F["Nota G de Ada Lovelace"] -- "Fornecimento de Algoritmo" --> B
    F -. "Procedimento de Cálculo dos Números de Bernoulli" .-> D
```
