...
```mermaid
graph TD
    A["Ayah: George Gordon Byron"] -- "Imajinasi Kaya / Romantisme" --> C["Ada Lovelace"]
    B["Ibu: Anne Isabella Milbanke"] -- "Pendidikan Matematika Ketat / Pemikiran Logis" --> C
    C -- "Imajinasi + Logika" --> D["Sains Puitis (Poetical Science)"]
    D -- "Aplikasi" --> E["Pemahaman Esensial dan Abstraksi Komputer"]
```
...
```mermaid
graph TD
    A["Konsep Mesin Analitik (Babbage)"] -- "Kontrol Kartu Punch" --> B["Bagian Input (Input)"]
    B -- "Data & Instruksi" --> C["Penyimpanan (Store: Memori)"]
    B -- "Instruksi Perhitungan" --> D["Tempat Perhitungan (Mill: CPU)"]
    C -- "Pertukaran Nilai" --> D
    D -- "Hasil Perhitungan" --> E["Bagian Output (Output)"]
    
    F["Catatan G Ada Lovelace"] -- "Menyediakan Algoritme" --> B
    F -. "Prosedur Perhitungan Bilangan Bernoulli" .-> D
```
