...
```mermaid
graph TD
    A["Père : George Gordon Byron"] -- "Riche imagination / Romantisme" --> C["Ada Lovelace"]
    B["Mère : Anne Isabella Milbanke"] -- "Éducation mathématique stricte / Pensée logique" --> C
    C -- "Imagination + Logique" --> D["Science poétique (Poetical Science)"]
    D -- "Application" --> E["Compréhension essentielle et abstraction de la machine à calculer"]
```
...
```mermaid
graph TD
    A["Conception de la machine analytique (Babbage)"] -- "Contrôle par cartes perforées" --> B["Unité d'entrée (Input)"]
    B -- "Données et Instructions" --> C["Mémoire (Store)"]
    B -- "Instructions de calcul" --> D["Unité de calcul (Mill : CPU)"]
    C -- "Échange de valeurs" --> D
    D -- "Résultats de calcul" --> E["Unité de sortie (Output)"]
    
    F["Note G d'Ada Lovelace"] -- "Fourniture de l'algorithme" --> B
    F -. "Procédure de calcul des nombres de Bernoulli" .-> D
```
