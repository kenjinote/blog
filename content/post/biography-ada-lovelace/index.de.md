...
```mermaid
graph TD
    A["Vater: George Gordon Byron"] -- "Reiche Vorstellungskraft / Romantik" --> C["Ada Lovelace"]
    B["Mutter: Anne Isabella Milbanke"] -- "Strenge Mathematikausbildung / Logisches Denken" --> C
    C -- "Vorstellungskraft + Logik" --> D["Poetische Wissenschaft (Poetical Science)"]
    D -- "Anwendung" --> E["Wesensverständnis und Abstraktion von Rechenmaschinen"]
```
...
```mermaid
graph TD
    A["Konzept der Analytical Engine (Babbage)"] -- "Lochkartensteuerung" --> B["Eingabe (Input)"]
    B -- "Daten / Befehle" --> C["Speicher (Store: Memory)"]
    B -- "Rechenanweisung" --> D["Rechenwerk (Mill: CPU)"]
    C -- "Austausch von Werten" --> D
    D -- "Berechnungsergebnis" --> E["Ausgabe (Output)"]
    
    F["Ada Lovelaces Note G"] -- "Algorithmusbereitstellung" --> B
    F -. "Berechnungsverfahren für Bernoulli-Zahlen" .-> D
```
