...
```mermaid
graph TD
    A["Cinta de entrada"] -- "Lectura/escritura de datos" --> B["Cabezal"]
    B -- "Transición de estado" --> C["Registro de estado"]
    C -- "Determinar próxima acción" --> D["Tabla de reglas de control"]
    D -- "Instrucción de acción" --> B
```
...
```mermaid
graph LR
    A["Juez humano (C)"] -- "Preguntas en texto" --> B["Humano (B)"]
    A -- "Preguntas en texto" --> C["Computadora (A)"]
    B -- "Respuestas" --> A
    C -- "Respuestas (fingiendo ser humano)" --> A
```
