...
```mermaid
graph TD
    A["Eingabeband"] -- "Daten lesen/schreiben" --> B["Kopf"]
    B -- "Zustandsübergang" --> C["Zustandsregister"]
    C -- "Nächste Aktion bestimmen" --> D["Steuerungsregeltabelle"]
    D -- "Aktionsanweisung" --> B
```
...
```mermaid
graph LR
    A["Menschlicher Prüfer (C)"] -- "Fragen per Text" --> B["Mensch (B)"]
    A -- "Fragen per Text" --> C["Computer (A)"]
    B -- "Antwort" --> A
    C -- "Antwort (als Mensch ausgeben)" --> A
```
