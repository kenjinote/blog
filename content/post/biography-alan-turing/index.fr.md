---
title: "Alan Turing : le père de l'IA et sa fin tragique"
description: "Plongez en profondeur dans la vie, les réalisations et l'héritage d'Alan Turing, le génie mathématicien qui a jeté les bases de l'informatique moderne et de l'intelligence artificielle, tout en étant victime de son époque avec une fin tragique."
...
```mermaid
graph TD
    A["Ruban d'entrée"] -- "Lecture et écriture de données" --> B["Tête"]
    B -- "Transition d'état" --> C["Registre d'état"]
    C -- "Détermine l'action suivante" --> D["Tableau des règles de contrôle"]
    D -- "Instruction d'action" --> B
```
...
```mermaid
graph LR
    A["Juge humain (C)"] -- "Questions textuelles" --> B["Humain (B)"]
    A -- "Questions textuelles" --> C["Ordinateur (A)"]
    B -- "Réponses" --> A
    C -- "Réponses (Se fait passer pour un humain)" --> A
```
