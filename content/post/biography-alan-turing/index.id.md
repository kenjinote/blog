...
```mermaid
graph TD
    A["Pita Input"] -- "Membaca/menulis data" --> B["Kepala (Head)"]
    B -- "Transisi status" --> C["Register Status"]
    C -- "Menentukan tindakan selanjutnya" --> D["Tabel Aturan Kendali"]
    D -- "Instruksi tindakan" --> B
```
...
```mermaid
graph LR
    A["Juri Manusia (C)"] -- "Bertanya lewat teks" --> B["Manusia (B)"]
    A -- "Bertanya lewat teks" --> C["Komputer (A)"]
    B -- "Jawaban" --> A
    C -- "Jawaban (Berpura-pura menjadi manusia)" --> A
```
