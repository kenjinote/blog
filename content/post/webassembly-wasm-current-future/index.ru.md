...
Веб-браузеры долгое время контролировались одним языком — JavaScript. Однако по мере того, как веб-приложения становились всё сложнее и от них требовалась производительность, сопоставимая с нативными приложениями, стали очевидны ограничения одного только JavaScript. Именно тогда и появился **WebAssembly ([Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/))**.

WebAssembly — это новый бинарный формат, который может выполняться в браузере со скоростью, близкой к нативному коду. Он компилируется из таких языков программирования, как C, C++ и [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/), и в настоящее время приносит инновации не только в веб-разработку, но и в широкий спектр областей, включая серверную часть, граничные вычисления (edge computing) и даже устройства IoT.
...
Языки вроде C и Rust обычно компилируются в машинный код, зависящий от ОС и архитектуры процессора. Однако в случае WebAssembly в качестве целевой архитектуры указывается специфичная для [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) архитектура, например, «wasm32».
...
Например, для выполнения простого сложения `$ 1 + 2 $` текстовое представление [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) (WAT) будет выглядеть так:
...
В C и [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) часто используются указатели для работы с памятью. Чтобы реализовать это, WebAssembly применяет концепцию **линейной памяти (Linear Memory)**.

Линейная память — это непрерывный массив байтов, к которому может обращаться экземпляр WebAssembly. Для JavaScript он выглядит как `ArrayBuffer` или `SharedArrayBuffer`. Указатели внутри [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) — это просто индексы (целые числа) этого массива.
...
Этот механизм не позволяет коду [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) напрямую обращаться к памяти операционной системы хоста, обеспечивая надёжную изолированную среду.
...
Сам по себе [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) не имеет возможности напрямую обращаться к DOM или Web API. Для этого необходимо использовать JavaScript.
Однако писать всё это вручную очень трудоёмко. Поэтому в экосистеме [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) предусмотрены такие инструменты, как **[wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/)-bindgen**.
...
При компиляции этого кода `wasm-bindgen` автоматически генерирует связующий код (glue code) на JavaScript, скрывая передачу строк в памяти и другие детали. Это создаёт ощущение разработки, как если бы вы вызывали API браузера напрямую из [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/).
...
1.  **Скорость парсинга**: Поскольку [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) является бинарным форматом, его декодирование происходит гораздо быстрее, чем парсинг исходного текстового кода JS для построения абстрактного синтаксического дерева (AST).
2.  **JIT-оптимизация**: Так как JS является языком с динамической типизацией, JIT-компилятору приходится выполнять вывод типов во время выполнения, и если вывод оказывается неверным, оптимизацию необходимо отменить (Deoptimization). [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) имеет статическую типизацию, и мощные оптимизации с помощью LLVM и других инструментов уже выполнены на этапе компиляции, поэтому браузер может сосредоточиться непосредственно на генерации машинного кода.
3.  **Избегание сборки мусора (GC)**: [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/), написанный на C или Rust, управляет памятью самостоятельно, поэтому не возникает неожиданных пауз (остановок) из-за работы сборщика мусора движка JS (※ о спецификации [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) GC будет рассказано ниже).

## 4.1 Бенчмарк: Последовательность Фибоначчи

Давайте сравним скорость работы JavaScript и Rust ([Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/)) на простом вычислении чисел Фибоначчи.
...
### Реализация на [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/)
```rust
#[no_mangle]
...
При расчёте для $n=40$ JavaScript (в движке V8) обычно выполняется довольно быстро благодаря JIT-оптимизациям, но [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/), сгенерированный из [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/), во многих случаях оказывается **примерно в 1.5 - 2 раза быстрее**. В областях, где активно используются матричные вычисления, обработка изображений, последовательный доступ к памяти и SIMD-инструкции, эта разница становится ещё более заметной.
...
Исторически C/C++ используются для переноса кода в веб дольше всего. **Emscripten** — это набор инструментов (toolchain), который использует LLVM для преобразования кода на C/C++ в [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/).
...
- **Небольшой размер среды выполнения (runtime)**: Поскольку в Rust нет GC и громоздкой среды выполнения, размер генерируемых бинарных файлов [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) можно сделать очень маленьким.
- **[wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/)-pack / [wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/)-bindgen**: Экосистема очень хорошо продумана, и создать проект [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/), а затем опубликовать его как npm-пакет можно всего за несколько строк команд.
...
Изначально Wasm был спроектирован для C и [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) с ручным управлением линейной памятью, но сейчас стандартизируется предложение **Wasm GC**, позволяющее эффективно компилировать в Wasm языки, требующие сборщика мусора, такие как [Java](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/), Kotlin, C# и Dart. Это привело к значительному повышению производительности таких технологий, как Flutter Web.

---

# 7. Мир за пределами браузера: WASI (WebAssembly System Interface)

Возможности WebAssembly не ограничиваются только браузером. Идея **«Что, если мы сможем использовать [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/) в качестве стандартного формата и вне браузера?»** привела к созданию **WASI (WebAssembly System Interface)**.
...
Если компонентная модель будет реализована, станет возможным бесшовный вызов функций из «модуля [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/), написанного на Python» в «модуль [Wasm](/ru/p/webassembly-wasm-from-cpp-and-rust/), написанного на [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/)». Это имеет потенциал стать основой для архитектуры микросервисов следующего поколения, не зависящей от платформы и языка.
