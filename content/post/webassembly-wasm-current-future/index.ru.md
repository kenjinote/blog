...
Однако писать всё это вручную очень трудоёмко. Поэтому в экосистеме [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) предусмотрены такие инструменты, как **wasm-bindgen**.
...
При расчёте для $n=40$ JavaScript (в движке V8) обычно выполняется довольно быстро благодаря JIT-оптимизациям, но Wasm, сгенерированный из [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/), во многих случаях оказывается **примерно в 1.5 - 2 раза быстрее**. В областях, где активно используются матричные вычисления, обработка изображений, последовательный доступ к памяти и SIMD-инструкции, эта разница становится ещё более заметной.
...
Если компонентная модель будет реализована, станет возможным бесшовный вызов функций из «модуля Wasm, написанного на Python» в «модуль Wasm, написанного на [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/)». Это имеет потенциал стать основой для архитектуры микросервисов следующего поколения, не зависящей от платформы и языка.
