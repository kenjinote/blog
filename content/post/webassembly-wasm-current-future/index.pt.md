...
Linguagens como C ou Rust normalmente são compiladas para código de máquina dependente do sistema operacional ou da arquitetura da CPU. No entanto, no caso do WebAssembly, especifica-se uma arquitetura específica para Wasm, como "wasm32", como a arquitetura alvo.
...
No C e no Rust, operações de memória usando ponteiros ocorrem frequentemente. Para permitir isso, o WebAssembly adotou o conceito de **Memória Linear (Linear Memory)**.
...
No entanto, escrever isso manualmente dá muito trabalho. É por isso que, no ecossistema Rust, existem ferramentas como o **wasm-bindgen**.
...
Enquanto o Wasm tradicional foi projetado para C e Rust com gestão manual da memória linear, a proposta do **Wasm GC** para compilar eficientemente linguagens que requerem Garbage Collection (GC) (como Java, Kotlin, C# e Dart) está em processo de padronização. Isso levou a uma melhoria drástica no desempenho do Flutter Web e afins.
...
## 7.2 Substituição e Coexistência com contêineres Docker
...
Atualmente, estão sendo ativamente desenvolvidos projetos (como Kwasm e Spin) para orquestrar módulos Wasm diretamente no Kubernetes em vez de contêineres Docker.
...
Se o Component Model for realizado, será possível chamar uma função sem atritos de um "módulo Wasm escrito em Python" para um "módulo Wasm escrito em Rust", por exemplo. Isso tem o potencial de ser a base da arquitetura de microsserviços da próxima geração, independentemente da plataforma e da linguagem.
