...
4.  **DOM Tree Construction (Построение дерева)** : Связывает созданные объекты в древовидную структуру данных на основе отношений вложенности тегов. Это и есть **DOM (Document Object Model)**.
...
После завершения построения DOM- и CSSOM-деревьев браузер объединяет их для построения **Render-дерева (Render Tree)** или **дерева стилей (Style Tree)**.
...
Цель этапа Paint — принять дерево макета (Layout Tree) в качестве входных данных, создать инструкции по раскраске пикселей на экране (Paint Records) и в конечном итоге выполнить растеризацию (Rasterization).
