...
실시간 처리에서 무음 부분에 대해서도 항상 추론을 실행하는 것은 계산 자원의 낭비입니다. VAD 알고리즘(단순한 에너지 기반 임계값 처리나 [WebRTC](https://kenji.blog/ko/p/webrtc-realtime-communication-p2p/) VAD 등)을 앞단에 삽입하여 **"발화가 시작되었을 때만 버퍼링을 시작하고 발화가 종료된(일정 시간 무음) 시점에 `whisper_full`을 킥(kick)한다"** 는 제어를 수행합니다.
