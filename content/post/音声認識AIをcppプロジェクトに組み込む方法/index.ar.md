...
داخليًا، لا يعالج Whisper بيانات الموجات الصوتية أحادية البعد (Raw Waveform) بشكل مباشر. يتم إدخالها إلى نموذج [Transformer](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) بعد تحويلها إلى **مخطط طيف ميل (Mel-Spectrogram)** ، وهو تمثيل للتردد أقرب إلى خصائص السمع البشري. يتضمن `whisper.cpp` عملية التحويل هذه داخل تطبيق C++، لكن فهم كيفية عمله مفيد للتعامل مع الضوضاء وتحسين المعالجة المسبقة.
...
## 3. هندسة النظام وتصميم مسار المعالجة ([Pipeline](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/))
...
في المعالجة بالوقت الفعلي، يعد تنفيذ الاستدلال باستمرار حتى على فترات الصمت إهدارًا لموارد الحوسبة. من خلال إدراج خوارزمية VAD (معالجة عتبة تعتمد على الطاقة البسيطة، أو [WebRTC](https://kenji.blog/ar/p/webrtc-realtime-communication-p2p/) VAD، وما إلى ذلك) في المرحلة السابقة، ننفذ تحكمًا حيث **"يبدأ التخزين المؤقت فقط عند بدء التحدث، ويتم تشغيل `whisper_full` عند انتهاء التحدث (فترة صمت معينة)"**.
