...
在实时处理时，不断对静音部分执行推理是对计算资源的浪费。通过在前端加入 VAD 算法（简单的基于能量的阈值处理，或 [WebRTC](https://kenji.blog/zh-cn/p/webrtc-realtime-communication-p2p/) VAD 等），可以实现如下控制：**“仅在开始说话时启动缓冲，在说话结束（持续一定时间的静音）时触发 `whisper_full`”**。
