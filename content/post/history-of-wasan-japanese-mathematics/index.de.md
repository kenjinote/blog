---
title: "Das Mathematik-Mysterium der Genies der Edo-Zeit: Die Geschichte von 'Wasan' (japanische Mathematik) und Sangaku"
description: "'Wasan' ist eine hochentwickelte Mathematik, die sich im isolierten Japan unter der Abschließung des Landes (Sakoku) eigenständig entwickelte. Wir beleuchten die Spuren von genialen Mathematikern wie Seki Takakazu und das Rätsel der 'Sangaku', den mathematischen Holztafeln, die an Schreinen und Tempeln gewidmet wurden."
...
```mermaid
graph TD
    A["Mathematik des alten China (z. B. Neun Kapitel der mathematischen Kunst)"] -- "Überlieferung & Assimilation" --> B["Frühes Wasan (Verbreitung des 'Jinkoki')"]
    B -- "Von praktischer Berechnung zu Spiel & Wissenschaft" --> C["Etablierung der Algebra durch Seki Takakazu (Tenzan-Jutsu)"]
    C -- "Weiterentwicklung & landesweite Vernetzung" --> D["Trend der Sangaku-Widmungen"]
    D -- "Entwicklung und Vertiefung des Enri (Infinitesimalrechnung)" --> E["Hochkomplexe Mathematik am Ende der Edo-Zeit"]
```
...
```mermaid
sequenceDiagram
    participant M["Mathematiker A (Fragesteller)"]
    participant S["Schrein/Tempel (Sangaku)"]
    participant N["Mathematiker B (Herausforderer)"]
    
    M->>S: "Widmet Problem und schöne Figuren (Idai)"
    S-->>N: "Entdeckt Sangaku beim Schreinbesuch und nimmt das Problem mit"
    N->>N: "Löst und berechnet das schwierige Problem über mehrere Monate"
    N->>S: "Widmet Lösung und ein eigenes Problem als neues Sangaku"
```
