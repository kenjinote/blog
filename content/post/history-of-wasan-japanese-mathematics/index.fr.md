---
title: "Le Mystère Mathématique Relevé par les Génies de l'Époque d'Edo : L'Histoire du « Wasan », Mathématiques Japonaises Uniques, et du Sangaku"
description: "Le « Wasan », des mathématiques avancées qui se sont développées de manière unique au Japon sous la politique d'isolement. Découvrez l'histoire des génies mathématiciens comme Seki Takakazu, et le mystère des « Sangaku » dédiés aux sanctuaires et aux temples."
...
```mermaid
graph TD
    A["Mathématiques de la Chine antique (Les Neuf Chapitres sur l'art mathématique, etc.)"] -- "Transmission et assimilation" --> B["Premiers Wasan (Diffusion du « Jinkōki »)"]
    B -- "Des calculs pratiques au jeu et à l'étude" --> C["Établissement de l'algèbre par Seki Takakazu (Tenzanjutsu)"]
    C -- "Sophistication et mise en réseau nationale" --> D["La mode de l'offrande de Sangaku"]
    D -- "Développement et approfondissement du principe du cercle (Enri - calcul infinitésimal)" --> E["Mathématiques de virtuosité de la fin de l'époque d'Edo"]
```
...
```mermaid
sequenceDiagram
    participant M["Mathématicien A (Poseur du problème)"]
    participant S["Sanctuaire/Temple (Sangaku)"]
    participant N["Mathématicien B (Challenger)"]
    
    M->>S: "Offre un problème et une belle figure (Idai)"
    S-->>N: "Découvre le Sangaku lors d'une visite et ramène le problème"
    N->>N: "Déchiffre et calcule le problème difficile pendant plusieurs mois"
    N->>S: "Offre la solution et son propre problème comme nouveau Sangaku"
```
