...
```mermaid
flowchart TD
    A["Todas las causas (100%)"] -- "Clasificación" --> B["Causas vitales (20%)"]
    A -- "Clasificación" --> C["Causas triviales (80%)"]
    B -- "Producen" --> D["Mayoría de los resultados (80%)"]
    C -- "Producen" --> E["Minoría de los resultados (20%)"]
```
...
```mermaid
flowchart LR
    A["Análisis del estado actual"] -- "Recolección y especificación de datos" --> B["Extracción del 20% importante"]
    B -- "Decisión" --> C["Concentración de recursos"]
    C -- "Ejecución" --> D["Obtención del 80% de los resultados"]
    D -- "Revisión periódica" --> A
```
