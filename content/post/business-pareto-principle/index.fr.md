...
```mermaid
flowchart TD
    A["Toutes les causes (100%)"] -- "Classification" --> B["Causes importantes (20%)"]
    A -- "Classification" --> C["Causes triviales (80%)"]
    B -- "Produisent" --> D["Majeure partie des résultats (80%)"]
    C -- "Produisent" --> E["Mineure partie des résultats (20%)"]
```
...
```mermaid
flowchart LR
    A["Analyse de la situation actuelle"] -- "Collecte et identification des données" --> B["Extraction des 20% importants"]
    B -- "Décision" --> C["Concentration des ressources"]
    C -- "Exécution" --> D["Obtention de 80% des résultats"]
    D -- "Révision périodique" --> A
```
