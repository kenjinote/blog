...
```mermaid
flowchart TD
    A["Semua penyebab (100%)"] -- "Klasifikasi" --> B["Penyebab penting (20%)"]
    A -- "Klasifikasi" --> C["Penyebab sepele (80%)"]
    B -- "Menghasilkan" --> D["Sebagian besar hasil (80%)"]
    C -- "Menghasilkan" --> E["Sebagian kecil hasil (20%)"]
```
...
```mermaid
flowchart LR
    A["Analisis situasi saat ini"] -- "Pengumpulan & identifikasi data" --> B["Ekstraksi 20% yang penting"]
    B -- "Keputusan" --> C["Pemusatan sumber daya"]
    C -- "Eksekusi" --> D["Pencapaian 80% hasil"]
    D -- "Tinjauan berkala" --> A
```
