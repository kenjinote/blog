...
```mermaid
graph TD
    subgraph "Thomas Edison (Direct Current: DC)"
        A["DC Power Plant"] -- "Transmits at low voltage" --> B["Short transmission grid"]
        B -- "Large power loss" --> C["Short-distance homes/factories"]
        style A fill:#ffcccc,stroke:#ff0000
    end

    subgraph "Nikola Tesla (Alternating Current: AC)"
        D["AC Power Plant"] -- "Step-up by transformer" --> E["Long-distance transmission grid"]
        E -- "Step-down by transformer" --> F["Long-distance homes/factories"]
...
```mermaid
graph TD
    A["Wardenclyffe Tower (Transmitting Station)"] -- "Energy propagation through the Earth" --> B["Receivers Worldwide"]
    A -- "Communication via the ionosphere" --> C["Global Information Network"]
    B -- "Power Supply" --> D["Factories, Lighting, Aircraft"]
    C -- "Information Transmission" --> E["Voice, Text, Images, Time Synchronization"]
