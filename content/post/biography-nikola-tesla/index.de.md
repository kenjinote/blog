...
```mermaid
graph TD
    subgraph "Thomas Edison (Gleichstrom: DC)"
        A["DC-Kraftwerk"] -- "Übertragung mit niedriger Spannung" --> B["Kurzes Stromnetz"]
        B -- "Großer Stromverlust" --> C["Haushalte und Fabriken in der Nähe"]
        style A fill:#ffcccc,stroke:#ff0000
    end

    subgraph "Nikola Tesla (Wechselstrom: AC)"
        D["AC-Kraftwerk"] -- "Spannungserhöhung durch Transformator" --> E["Langstreckenstromnetz"]
        E -- "Spannungsreduktion durch Transformator" --> F["Weit entfernte Haushalte und Fabriken"]
...
```mermaid
graph TD
    A["Wardenclyffe Tower (Sendestation)"] -- "Energieausbreitung durch die Erde" --> B["Empfänger weltweit"]
    A -- "Kommunikation über die Ionosphäre" --> C["Globales Informationsnetzwerk"]
    B -- "Stromversorgung" --> D["Fabriken, Beleuchtung, Flugzeuge"]
    C -- "Informationsübertragung" --> E["Sprache, Text, Bilder, Zeitsynchronisation"]
