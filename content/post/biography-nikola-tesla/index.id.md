...
```mermaid
graph TD
    subgraph "Thomas Edison (Arus Searah: DC)"
        A["Pembangkit Listrik DC"] -- "Transmisi dalam tegangan rendah" --> B["Jaringan Transmisi Pendek"]
        B -- "Kehilangan daya besar" --> C["Rumah dan Pabrik Jarak Dekat"]
        style A fill:#ffcccc,stroke:#ff0000
    end

    subgraph "Nikola Tesla (Arus Bolak-balik: AC)"
        D["Pembangkit Listrik AC"] -- "Dinaikkan oleh Transformator" --> E["Jaringan Transmisi Jarak Jauh"]
        E -- "Diturunkan oleh Transformator" --> F["Rumah dan Pabrik Jarak Jauh"]
...
```mermaid
graph TD
    A["Menara Wardenclyffe (Stasiun Pemancar)"] -- "Transmisi energi melalui Bumi" --> B["Penerima di seluruh dunia"]
    A -- "Komunikasi melalui ionosfer" --> C["Jaringan Informasi Global"]
    B -- "Pasokan Listrik" --> D["Pabrik, Pencahayaan, Pesawat"]
    C -- "Transmisi Informasi" --> E["Suara, Teks, Gambar, Sinkronisasi Waktu"]
