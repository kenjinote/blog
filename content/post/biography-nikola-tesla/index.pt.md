...
```mermaid
graph TD
    subgraph "Thomas Edison (Corrente Contínua: DC)"
        A["Usina DC"] -- "Transmissão em baixa tensão" --> B["Rede de transmissão curta"]
        B -- "Grande perda de energia" --> C["Casas e fábricas de curta distância"]
        style A fill:#ffcccc,stroke:#ff0000
    end

    subgraph "Nikola Tesla (Corrente Alternada: AC)"
        D["Usina AC"] -- "Alta tensão via transformador" --> E["Rede de transmissão de longa distância"]
        E -- "Baixa tensão via transformador" --> F["Casas e fábricas de longa distância"]
...
```mermaid
graph TD
    A["Torre Wardenclyffe (Estação transmissora)"] -- "Propagação de energia através da Terra" --> B["Receptores em todo o mundo"]
    A -- "Comunicação via ionosfera" --> C["Rede global de informação"]
    B -- "Fornecimento de energia" --> D["Fábricas, iluminação, aeronaves"]
    C -- "Transmissão de informação" --> E["Voz, texto, imagens, sincronização de tempo"]
