...
```mermaid
graph TD
    subgraph "Thomas Edison (Courant continu : CC)"
        A["Centrale électrique CC"] -- "Transmission à basse tension" --> B["Réseau de transmission court"]
        B -- "Forte perte de puissance" --> C["Maisons et usines à proximité"]
        style A fill:#ffcccc,stroke:#ff0000
    end

    subgraph "Nikola Tesla (Courant alternatif : CA)"
        D["Centrale électrique CA"] -- "Augmentation de la tension par transformateur" --> E["Réseau de transmission longue distance"]
        E -- "Réduction de la tension par transformateur" --> F["Maisons et usines éloignées"]
...
```mermaid
graph TD
    A["Tour de Wardenclyffe (Station émettrice)"] -- "Propagation de l'énergie à travers la Terre" --> B["Récepteurs dans le monde entier"]
    A -- "Communication via l'ionosphère" --> C["Réseau mondial d'informations"]
    B -- "Alimentation électrique" --> D["Usines, éclairages, avions"]
    C -- "Transmission d'informations" --> E["Audio, texte, images, synchronisation horaire"]
