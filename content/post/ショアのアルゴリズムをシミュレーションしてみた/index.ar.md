...
تعتمد معظم معايير الأمان في مجتمع الإنترنت الحديث على **تشفير المفتاح العام** (خاصة تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/)). عندما نرسل معلومات بطاقة الائتمان الخاصة بنا للتسوق عبر الإنترنت أو نتبادل البيانات شديدة السرية، فإن محتوى هذه الاتصالات محمي بقوة بواسطة تشفير RSA.

يعتمد أساس أمان تشفير RSA على حقيقة رياضية مفادها أن "**تحليل الأعداد الصحيحة الضخمة إلى عواملها الأولية يعد أمرًا بالغ الصعوبة بالنسبة للحواسيب الكلاسيكية (مثل أجهزة الكمبيوتر الشخصية والحواسيب الخارقة التي نستخدمها عادةً)**". ومع ذلك، فإن "** خوارزمية شور ([Shor's Algorithm](https://kenji.blog/ar/p/quantum-computing-shors-algorithm/))**" التي أعلن عنها بيتر شور (Peter [Shor](/ar/p/shors-algorithm-and-rsa-breaking/)) في عام 1994، قلبت هذا الافتراض رأسًا على عقب. لقد تم إثبات رياضيًا أنه إذا تم تشغيل خوارزمية شور على حاسوب كمومي واسع النطاق، فيمكنها حل مشاكل تحليل العوامل الأولية التي قد تستغرق من الحواسيب الكلاسيكية وقتًا أطول من عمر الكون، في غضون بضع دقائق إلى بضع ساعات.
...
لماذا يعد تحليل العوامل الأولية صعبًا؟ حتى لو استخدمنا "طريقة [غربال حقل الأعداد العام](/ar/p/beyond-gnfs-integer-factorization-algorithms/) (General Number Field Sieve, GNFS)"، والتي تُعرف بأنها أفضل خوارزمية لتحليل العوامل الأولية في الحواسيب الكلاسيكية، فإن تعقيدها الحسابي سيكون شبه أسي.
...
هذا يعني أن $N$ يشترك في قاسم مشترك إما مع $(a^{r/2} - 1)$ أو $(a^{r/2} + 1)$ (بشرط أن يحقق الشرط $a^{r/2} \not\equiv -1 \pmod N$). وبالتالي، باستخدام [خوارزمية إقليدس](https://kenji.blog/ar/p/euclidean-algorithm/):
...
على الرغم من سهولة تحليل $N=15$ على المحاكي، إلا أنه لا تزال هناك عقبات عديدة أمام الحواسيب الكمومية الفعلية لتحليل تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/)-2048 المستخدم في العالم الحقيقي (والذي يتكون من 617 رقمًا عشريًا).
...
استعدادًا لمجيء "Q-Day (اليوم الذي تكسر فيه الحواسيب الكمومية التشفير)"، يعكف علماء التشفير حول العالم، بقيادة المعهد الوطني للمعايير والتقنية (NIST) في الولايات المتحدة، على تطوير معايير **التشفير ما بعد الكمومي (Post-Quantum [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy, [PQC](/ar/p/post-quantum-cryptography/))**.

يعتمد [PQC](/ar/p/post-quantum-cryptography/) على مسائل رياضية جديدة (مثل مشاكل الشبكات، ومسائل كثيرات الحدود متعددة المتغيرات، والطرق المعتمدة على دوال التجزئة) والتي يُعتقد رياضيًا أنه لا يمكن حلها بكفاءة حتى باستخدام خوارزمية شور (أو حتى خوارزمية جروفر). لقد تم بالفعل اختيار خوارزميات مثل "CRYSTALS-Kyber" و "CRYSTALS-Dilithium" كمعايير قياسية، وبدأ دمجها تدريجيًا في بروتوكولات الاتصال الخاصة بـ iMessage من Apple ومختلف متصفحات الويب.

بالنسبة [للمهندسين](/ar/p/prompt-engineering-for-engineers/) الذين يديرون البنية التحتية لتكنولوجيا المعلومات، فإن دمج "المرونة التشفيرية ([Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)-Agility: التصميم الذي يسمح بالتبديل السريع لأساليب التشفير)" في الأنظمة للانتقال من تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) ومنحنيات إهليلجية الحالي إلى [PQC](/ar/p/post-quantum-cryptography/) سيكون مهمة حاسمة في المستقبل.
