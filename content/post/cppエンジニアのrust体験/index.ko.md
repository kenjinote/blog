...
더욱 강력한 것은 리소스를 참조하는 '차용(Borrowing)' 규칙입니다. [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)에서는 다음 규칙이 강제됩니다:
1. 임의의 타이밍에 "여러 개의 불변 참조(`&T`)" 또는 "단일 가변 참조(`&mut T`)" 중 **어느 한쪽만** 존재할 수 있다.
2. 참조는 원본 데이터의 스코프보다 오래 살아남아서는 안 된다(수명 제약).

C++에서는 같은 객체에 대해 여러 개의 가변적인(Mutable) 참조나 포인터를 쉽게 만들 수 있으며, 이것이 예기치 않은 상태 파괴(반복자 무효화 등)를 일으킵니다. [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)는 이 "에일리어싱(Aliasing) + 가변성(Mutability)" 조합을 언어 수준에서 금지함으로써 버그를 미연에 방지합니다.

---

# 2. 메모리 레이아웃과 스마트 포인터의 수학적 오버헤드

시스템 프로그래밍에서 메모리 레이아웃에 대한 정확한 이해는 필수적입니다. C++의 `std::shared_ptr`과 [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)의 `std::rc::Rc` / `std::sync::Arc`를 비교해 봅시다.
...
대조적으로, [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)는 용도에 따라 스마트 포인터를 엄격하게 분리하고 있습니다.
...
[Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)에서는 단일 스레드 전용인 `Rc<T>`를 사용하면 원자적 연산의 패널티를 완전히 피할 수 있습니다(제로 코스트 추상화). 그리고 후술할 스레드 안전성 메커니즘을 통해 `Rc<T>`를 실수로 다른 스레드에 전달하는 것은 타입 시스템에 의해 완전히 방지됩니다.
...
더 나아가 [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)에는 동시성의 안전성을 보장하는 두 가지 핵심 트레이트(Trait)가 존재합니다.
...
[Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)는 이 $P$를 극대화하기 위한 리팩터링을 타입 시스템에 의존하여 극도로 안전하게 수행할 수 있게 해줍니다.
...
[Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)에는 예외라는 개념이 존재하지 않습니다. 에러는 순수한 '값'으로 반환되며, `Result<T, E>`라는 열거형(대수적 데이터 타입)으로 표현됩니다.
...
[Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)는 고전적인 객체 지향의 "클래스 상속"을 버리고, 대신 "**트레이트(Traits)**"라는 개념을 채택했습니다(C++20의 Concept과 비슷하지만, 기능이 더 다양합니다).
...
[Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)에는 **Cargo** 라는 세계 최고 수준의 패키지 매니저 겸 빌드 시스템이 기본으로 탑재되어 있습니다.
...
## 2. 긴 컴파일 시간
C++도 템플릿의 중첩으로 인해 컴파일이 느려지지만, [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)의 컴파일 시간(특히 제로에서 시작하는 클린 빌드)도 결코 짧지 않습니다. LLVM의 강력한 최적화 패스, 매크로 확장, 제네릭스의 단형화(모노모피제이션)가 겹치기 때문에 대규모 프로젝트에서는 빌드 시간이 병목이 됩니다. 개발 중에는 `cargo check`를 자주 사용하는 등의 연구가 필수적입니다.

## 3. C++ 코드 베이스와의 상호 운용성
C 언어(FFI)와의 연동은 매우 매끄럽지만, 기존의 거대한 C++ 코드 베이스(클래스, 템플릿, 가상 함수를 다수 사용하는 것)와 [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)를 직접 연동하는 것은 매우 어렵습니다. 최근에는 `cxx`나 `autocxx` 같은 브리지 도구가 발전하고 있지만, 완전하고 원활한 전환에는 아직 높은 진입 장벽이 있습니다.

---

# 요약: 우리는 [Rust](https://kenji.blog/ko/p/webassembly-wasm-current-future/)로 마이그레이션해야 하는가?
