...
## [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) की ओनरशिप (Ownership) और बॉरो चेकर ([Borrow Checker](https://kenji.blog/hi/p/memory-management-garbage-collection/)) का पूर्ण बचाव

[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) इस "ओनरशिप" की अवधारणा को भाषा के मूल डिज़ाइन में शामिल करता है, और **बॉरो चेकर (Borrow Checker)** नामक कंपाइलर की सुविधा द्वारा सख्त स्टेटिक विश्लेषण करता है।
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में, जैसे ही किसी वेरिएबल की ओनरशिप स्थानांतरित होती है, मूल वेरिएबल को कंपाइलर द्वारा "असंसाधित (uninitialized)" अवस्था के बराबर माना जाता है, और आगे की पहुँच को पूरी तरह से अवरुद्ध कर दिया जाता है। इसके कारण, "Use-After-Free (मुक्त होने के बाद मेमोरी का उपयोग)" और "Dangling [Pointer](https://kenji.blog/hi/p/c-language-pointers-memory-management-stack-heap/) (डैंगलिंग पॉइंटर)" जैसे बग सैद्धांतिक रूप से कंपाइल नहीं हो सकते हैं।
...
इससे भी अधिक शक्तिशाली संसाधनों को संदर्भित करने वाले "बॉरोइंग (Borrowing)" के नियम हैं। [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में निम्नलिखित नियम लागू किए जाते हैं:
...
C++ में, आप एक ही ऑब्जेक्ट के लिए आसानी से कई म्यूटबल (परिवर्तनीय) संदर्भ या पॉइंटर्स बना सकते हैं, जिससे अप्रत्याशित अवस्था का विनाश (जैसे इटरेटर का अमान्यकरण) हो सकता है। [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) भाषा के स्तर पर इस "अलियासिंग (Aliasing) + म्यूटबिलिटी (Mutability)" के संयोजन को प्रतिबंधित करके बग को रोकता है।

---

# 2. मेमोरी लेआउट और स्मार्ट पॉइंटर्स का गणितीय ओवरहेड

सिस्टम प्रोग्रामिंग में, मेमोरी लेआउट की सटीक समझ आवश्यक है। आइए C++ के `std::shared_ptr` और [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) के `std::rc::Rc` / `std::sync::Arc` की तुलना करें।
...
इसके विपरीत, [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) स्मार्ट पॉइंटर्स को उपयोग के आधार पर सख्ती से अलग करता है।
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में, यदि आप सिंगल-थ्रेड के लिए विशेष रूप से `Rc<T>` का उपयोग करते हैं, तो आप एटॉमिक ऑपरेशंस के दंड (penalty) से पूरी तरह बच सकते हैं (ज़ीरो-कॉस्ट एब्स्ट्रैक्शन)। इसके अलावा, थ्रेड सुरक्षा तंत्र (जिसके बारे में बाद में बताया जाएगा) के कारण, गलती से `Rc<T>` को किसी अन्य थ्रेड में पास करने से टाइप सिस्टम द्वारा पूरी तरह से बचा जा सकता है।
...
## [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) का म्यूटेक्स डेटा को "ओन (own)" करता है

[Rust](https://kenji.blog/hi/p/programming-languages-history-paradigm-evolution/) में, `Mutex<T>` जेनरिक (Generics) का उपयोग करके सुरक्षित किए जाने वाले डेटा प्रकार `T` को **सम्मिलित (स्वयं का/own)** करता है। डेटा तक पहुँचने के लिए, आपको हमेशा `lock()` को कॉल करना होगा और एक गार्ड ऑब्जेक्ट प्राप्त करना होगा। लॉक प्राप्त किए बिना डेटा को छूना व्याकरणिक रूप से असंभव है।
...
इसके अलावा, [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में समवर्ती प्रसंस्करण (concurrent processing) की सुरक्षा सुनिश्चित करने के लिए दो कोर ट्रेट (Traits) हैं:
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) इस $P$ को अधिकतम करने के लिए रिफैक्टरिंग को टाइप सिस्टम की सहायता से अत्यधिक सुरक्षित रूप से करने की अनुमति देता है।
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में अपवाद की कोई अवधारणा नहीं है। त्रुटियों को शुद्ध "वैल्यू" के रूप में वापस किया जाता है और `Result<T, E>` नामक एन्यूमरेशन (बीजीय डेटा प्रकार) द्वारा दर्शाया जाता है।
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) क्लासिकल ऑब्जेक्ट-ओरिएंटेड "क्लास इनहेरिटेंस" को छोड़ देता है और इसके बजाय "**ट्रेट (Traits)**" की अवधारणा को अपनाता है (यह C++20 की Concept के समान है, लेकिन अधिक बहुमुखी है)।
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) के डायनेमिक डिस्पैच (`dyn Trait`) की सबसे बड़ी विशेषता यह है कि यह डेटा संरचना के भीतर vptr नहीं रखता है, बल्कि **फैट पॉइंटर (Fat [Pointer](https://kenji.blog/hi/p/c-language-pointers-memory-management-stack-heap/))** का उपयोग करता है। फैट पॉइंटर्स जोड़े (pair) में "डेटा का पॉइंटर" और "vtable का पॉइंटर" रखते हैं। यह बाहरी पुस्तकालयों में परिभाषित प्रकारों के लिए बाद में ट्रेट को लागू (विस्तारित) करना और डायनेमिक डिस्पैच करना बहुत आसान बनाता है।
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में **Cargo** नामक दुनिया का सबसे अच्छा पैकेज मैनेजर और बिल्ड सिस्टम मानक के रूप में शामिल है।
...
# 7. [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) सीखने के नुकसान और लर्निंग कर्व (Learning Curve)

अब तक मैंने [Rust](https://kenji.blog/hi/p/programming-languages-history-paradigm-evolution/) के फायदों के बारे में बात की है, लेकिन कुछ "दीवारें" और नुकसान भी हैं जिनका सामना C++ इंजीनियरों को Rust को व्यावहारिक उपयोग में लाते समय करना पड़ता है।

## 1. कठोर बॉरो चेकर ([Borrow Checker](https://kenji.blog/hi/p/memory-management-garbage-collection/)) के साथ संघर्ष
यदि आप C++ में डेटा स्ट्रक्चर्स (जैसे कि डबली-लिंक्ड लिस्ट्स, ग्राफ स्ट्रक्चर्स, सेल्फ-रेफरेंशियल स्ट्रक्चर्स आदि) जिन्हें आपने "किसी तरह रॉ पॉइंटर्स (raw pointers) के साथ जोड़ा था" को सीधे [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में लागू करने का प्रयास करते हैं, तो ओनरशिप और लाइफटाइम बाधाओं के कारण कंपाइलेशन पास नहीं होगा। बॉरो चेकर को संतुष्ट करने के लिए, आपको या तो `Rc<RefCell<T>>` जैसी जटिल रैपिंग करनी होगी, या एरिना एलोकेटर (arena allocator) या इंडेक्स-आधारित प्रबंधन का उपयोग करने के लिए डिज़ाइन पर मौलिक रूप से पुनर्विचार करना होगा।

## 2. लंबे समय तक कंपाइलेशन (Compilation Time)
C++ में टेम्पलेट्स की नेस्टिंग के कारण भी कंपाइलेशन धीमा हो जाता है, लेकिन [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) का कंपाइलेशन समय (विशेष रूप से खरोंच से क्लीन बिल्ड) भी कम नहीं है। LLVM के शक्तिशाली ऑप्टिमाइजेशन पास, मैक्रोज़ का विस्तार, और जेनरिक के मोनोमोर्फाइजेशन (Monomorphization) के ओवरलैप होने के कारण, बड़े पैमाने के प्रोजेक्ट्स में बिल्ड समय एक बाधा बन जाता है। विकास के दौरान `cargo check` का भारी उपयोग करना जैसी तकनीकें आवश्यक हैं।

## 3. C++ कोडबेस के साथ इंटरऑपरेबिलिटी (Interoperability)
C भाषा (FFI) के साथ एकीकरण बहुत सुचारू है, लेकिन [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) को सीधे मौजूदा विशाल C++ कोडबेस (जो क्लास, टेम्पलेट्स, वर्चुअल फ़ंक्शंस का भारी उपयोग करते हैं) के साथ एकीकृत करना बहुत मुश्किल है। हाल के वर्षों में, `cxx` और `autocxx` जैसे ब्रिजिंग टूल विकसित हुए हैं, लेकिन पूर्ण सीमलेस (seamless) माइग्रेशन के लिए अभी भी उच्च बाधाएँ हैं।

---

# निष्कर्ष: क्या हमें [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) की ओर रुख करना चाहिए?
...
हालाँकि, "नए शुरू किए जा रहे सिस्टम प्रोग्रामिंग प्रोजेक्ट्स" के लिए, मुझे लगता है कि **[Rust](https://kenji.blog/hi/p/programming-languages-history-paradigm-evolution/) को नहीं चुनने का कारण खोजना अब अधिक कठिन है** । एक बार कंपाइल हो जाने पर, अपरिभाषित व्यवहार (undefined behavior) और मेमोरी करप्शन के डर से मुक्ति, और उच्च प्रदर्शन के साथ सुरक्षित रूप से समवर्ती रूप से प्रोसेस करने में सक्षम होने की Rust की "निश्चितता" इंजीनियर के मानसिक मॉडल (mental model) में नाटकीय रूप से सुधार करती है।
