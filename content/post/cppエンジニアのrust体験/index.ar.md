...
في هندسة البرمجيات الحديثة، يقف كل من C++ و [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) كعملاقين في طليعة برمجة النظم. لسنوات عديدة، تربعت C++ كملك مطلق في مجالات مثل أنظمة التشغيل، والأجهزة المدمجة، ومحركات الألعاب، وأنظمة التداول عالي التردد (HFT)، حيث يتطلب الأمر استخراج أقصى درجات الأداء من العتاد. بصفتي مهندس C++ أول، بدأت مسيرتي من غابة المؤشرات الخام في حقبة C++98، وواصلت كتابة التعليمات البرمجية بالتزامن مع موجة التحديث التي جلبتها C++11 (إدخال المؤشرات الذكية، وتعبيرات لامدا، و `auto`)، وصولاً إلى التوسع الهائل في المواصفات مع إصدارات C++14/17/20.

ولكن في السنوات الأخيرة، شهدنا صعودًا دراماتيكيًا للغة [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) كحل للمشاكل الهيكلية التي تعاني منها C++ — وتحديدًا "الافتقار إلى أمان الذاكرة" والذي يؤدي إلى ثغرات أمنية (يُقال إن حوالي 70% من ثغرات CVE ناتجة عن أخطاء في الذاكرة) و "المواصفات المعقدة بلا حدود والسلوك غير المحدد (UB)". إن اعتمادها الرسمي في نواة Linux، ومشاريع الانتقال واسعة النطاق إلى [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) من قبل شركات التكنولوجيا العملاقة مثل Microsoft و Google و AWS، لا يعتبر مجرد صيحة عابرة، بل يمثل تحولًا جذريًا في نموذج برمجة النظم.
...
## الملكية (Ownership) في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) والدفاع المطلق لمدقق الاستعارة

تدمج [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) مفهوم "الملكية" في التصميم الأساسي للغة، وتُجري تحليلًا ثابتًا صارمًا من خلال ميزة في المترجم تُعرف باسم **مدقق الاستعارة ([Borrow Checker](https://kenji.blog/ar/p/memory-management-garbage-collection/))**.
...
في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، بمجرد انتقال ملكية المتغير، يعامله المترجم على أنه في حالة تعادل "غير مهيأ"، ويمنع تمامًا أي وصول لاحق إليه. نتيجة لذلك، فإن الأخطاء مثل "استخدام الذاكرة بعد تحريرها (Use-After-Free)" أو "المؤشر المتدلي (Dangling [Pointer](https://kenji.blog/ar/p/c-language-pointers-memory-management-stack-heap/))" نظريًا لا يمكنها اجتياز عملية الترجمة.
...
الأقوى من ذلك هو قواعد "الاستعارة (Borrowing)" التي تشير إلى الموارد. تفرض [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) القواعد التالية:
...
في C++، من السهل إنشاء مراجع أو مؤشرات متعددة قابلة للتغيير (Mutable) لنفس الكائن، مما قد يؤدي إلى تدمير غير متوقع للحالة (مثل إبطال المكرر - Iterator Invalidation). تمنع [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) هذا المزيج من "الأسماء المستعارة (Aliasing) + القابلية للتغيير (Mutability)" على مستوى اللغة، مما يمنع الأخطاء قبل حدوثها.

---

# 2. تخطيط الذاكرة والنفقات الرياضية للمؤشرات الذكية

في برمجة النظم، الفهم الدقيق لتخطيط الذاكرة أمر لا غنى عنه. دعونا نقارن بين `std::shared_ptr` في C++ و `std::rc::Rc` / `std::sync::Arc` في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/).
...
في المقابل، تفصل [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) بصرامة المؤشرات الذكية حسب الغرض:
...
في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، إذا استخدمت `Rc<T>` المخصص للخيط الواحد، يمكنك تجنب عقوبة العمليات الذرية بالكامل (تجريد بتكلفة صفرية - Zero-cost abstraction). ومن خلال آلية أمان الخيوط التي سيتم مناقشتها لاحقًا، يتم منع تمرير `Rc<T>` عن طريق الخطأ إلى خيط آخر تمامًا بواسطة نظام الأنواع.
...
## الكائنات المتبادلة (Mutex) في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) "تمتلك" البيانات

في [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/)، **تغلف (تمتلك)** `Mutex<T>` نوع البيانات المحمي `T` باستخدام الأنواع العامة (Generics). للوصول إلى البيانات، يجب عليك استدعاء `lock()` للحصول على كائن الحارس (Guard). من المستحيل نحويًا لمس البيانات دون الحصول على القفل.
...
علاوة على ذلك، تمتلك [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) سمتين أساسيتين (Traits) تضمنان أمان المعالجة المتزامنة:
...
تسمح [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) بإجراء عمليات إعادة الهيكلة (Refactoring) لتعظيم قيمة $P$ بأمان تام بالاعتماد على نظام الأنواع.
...
المعيار لمعالجة الأخطاء في C++ هو "الاستثناءات (Exceptions)". ومع ذلك، فإن الاستثناءات تجعل تدفق التحكم غير شفاف، وتسبب عقوبات في الأداء (فك المكدس - [Stack](https://kenji.blog/ar/p/c-language-pointers-memory-management-stack-heap/) Unwinding - وتضخم RTTI). في الأنظمة المدمجة ومحركات الألعاب، غالبًا ما يتم تعطيل الاستثناءات تمامًا (`-fno-exceptions`) واعتماد تصميم يعيد رموز الخطأ الكلاسيكية. في C++23، تم إدخال `std::expected`، لكن سيستغرق الأمر وقتًا حتى ينتشر في جميع أنحاء النظام البيئي.

لا يوجد مفهوم للاستثناءات في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/). يتم إرجاع الأخطاء كـ "قيم" بحتة، ويتم التعبير عنها بواسطة النوع المعداد (نوع بيانات جبري) `Result<T, E>`.
...
تخلت [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) عن "وراثة الفئات" الكلاسيكية الموجهة للكائنات، واعتمدت بدلاً من ذلك مفهوم "**السمات (Traits)**" (تشبه Concept في C++20، لكنها تتمتع بوظائف أكثر).
...
أهم ما يميز الإرسال الديناميكي في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) (`dyn Trait`) هو أنه لا يمتلك vptr داخل بنية البيانات، بل يستخدم **مؤشرًا سمينًا (Fat [Pointer](https://kenji.blog/ar/p/c-language-pointers-memory-management-stack-heap/))**. يحتفظ المؤشر السمين بزوج يتكون من "مؤشر إلى البيانات" و "مؤشر إلى vtable". هذا يجعل من السهل جدًا تنفيذ (أو توسيع) السمات للأنواع المعرفة في المكتبات الخارجية لاحقًا وإرسالها ديناميكيًا.
...
تأتي [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) مزودة بـ **Cargo** بشكل قياسي، وهو أحد أفضل مديري الحزم وأنظمة البناء في العالم.
...
# 7. عيوب ومنحنى التعلم عند تعلم [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)

لقد تحدثت حتى الآن عن مزايا [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/)، ولكن بالتأكيد توجد "جدران" وعيوب سيواجهها مهندس C++ عند وضع Rust موضع التنفيذ الفعلي.
...
## 2. طول وقت الترجمة
في حين أن بناء C++ يصبح بطيئًا أيضًا بسبب تداخل القوالب (Templates)، إلا أن وقت الترجمة في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) (خاصة عند البناء النظيف من الصفر - Clean Build) ليس قصيرًا على الإطلاق. وبسبب تزامن مسارات التحسين القوية لـ LLVM، وتوسيع وحدات الماكرو، وأحادية الشكل للأنواع العامة (Monomorphization)، يصبح وقت البناء عنق زجاجة في المشاريع الكبيرة. خلال مرحلة التطوير، من الضروري استخدام حيل مثل الاعتماد الكبير على `cargo check`.

## 3. قابلية التشغيل البيني مع قواعد شيفرة C++ الحالية
في حين أن التكامل مع لغة C (عبر FFI) سلس للغاية، إلا أنه من الصعب جدًا ربط [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) مباشرة بقواعد شيفرة C++ الضخمة الحالية (التي تعتمد بشكل كبير على الفئات، والقوالب، والدوال الافتراضية). في السنوات الأخيرة، تطورت أدوات الجسر مثل `cxx` و `autocxx`، ولكن لا تزال هناك عقبات كبيرة أمام تحقيق انتقال سلس ومثالي.

---

# الخلاصة: هل يجب علينا الانتقال إلى [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)؟
...
ولكن، بالنسبة لـ "مشاريع برمجة النظم الجديدة"، أجد الآن أنه **من الصعب إيجاد سبب يمنعنا من اختيار [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/)**. بمجرد اجتياز عملية الترجمة، يتم تحريرك من الخوف من السلوكيات غير المحددة (UB) وتدمير الذاكرة، و"اليقين" الذي تقدمه Rust بقدرتها على أداء المعالجة المتزامنة بأمان وبأداء عالٍ يحسن بشكل كبير من النموذج الذهني للمهندس.
