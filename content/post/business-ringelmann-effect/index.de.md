...
```mermaid
flowchart TD
    A["Größenwachstum der Gruppe"] -- "Verantwortungsdiffusion" --> B["Verringerung des individuellen Verantwortungsgefühls"]
    B -- "Unklarheit des Beitrags" --> C["Motivationseinbruch"]
    C -- "Entstehung von Trittbrettfahrern" --> D["Soziales Faulenzen (Ringelmann-Effekt)"]
    D -- "Verringerung der Gesamtproduktivität des Teams" --> E["Stagnation und Leistungsabfall der Organisation"]
```
...
```mermaid
flowchart TD
    A["Maßnahmen zur Verhinderung des Ringelmann-Effekts"] -- "Rollen klären" --> B["Verantwortungsbereich des Einzelnen festlegen"]
    A -- "Bewertungssystem optimieren" --> C["Prozesse und individuelle Beiträge visualisieren"]
    A -- "Teamgröße anpassen" --> D["Zwei-Pizzen-Regel (kleine Gruppen)"]
    A -- "Psychologische Sicherheit aufbauen" --> E["Gegenseitiges Vertrauen und Kommunikation stärken"]
    B -- "Eigenverantwortung steigern" --> F["Gesamtleistung des Teams maximieren"]
    C -- "Erwartung einer fairen Bewertung" --> F
    D -- "Gefühl des Untergehens beseitigen" --> F
    E -- "Zusammenarbeit fördern" --> F
```
