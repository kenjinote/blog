...
```mermaid
flowchart TD
    A["Augmentation de la taille du groupe"] -- "Diffusion de la responsabilité" --> B["Baisse du sens des responsabilités individuel"]
    B -- "Opacité de la contribution" --> C["Baisse de la motivation"]
    C -- "Apparition de passagers clandestins" --> D["Paresse sociale (Effet Ringelmann)"]
    D -- "Baisse de la productivité globale de l'équipe" --> E["Stagnation de l'organisation et détérioration des performances"]
```
...
```mermaid
flowchart TD
    A["Mesures de prévention de l'effet Ringelmann"] -- "Clarification des rôles" --> B["Identification du champ de responsabilité individuel"]
    A -- "Optimisation du système d'évaluation" --> C["Visualisation du processus et de la contribution individuelle"]
    A -- "Ajustement de la taille de l'équipe" --> D["Règle des deux pizzas (petites équipes)"]
    A -- "Construction de la sécurité psychologique" --> E["Renforcement de la confiance mutuelle et de la communication"]
    B -- "Amélioration de l'implication" --> F["Maximisation de la performance globale de l'équipe"]
    C -- "Attente d'une évaluation juste" --> F
    D -- "Élimination du sentiment d'enfouissement" --> F
    E -- "Promotion de la coopération" --> F
```
