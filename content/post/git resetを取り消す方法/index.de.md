...
Wenn Sie nach einem git commit versehentlich einen git reset ausführen, zeige ich Ihnen hier, wie Sie den git reset rückgängig machen können (wie Sie den [Zustand](https://kenji.blog/de/p/state-management-history-redux-context-recoil-zustand/) zum Zeitpunkt des git commits wiederherstellen).

1. Überprüfen Sie die Commit-ID vor dem Reset mit `git reflog`
2. Kehren Sie mit `git reset --hard HEAD@{Zahl}` in den [Zustand](https://kenji.blog/de/p/state-management-history-redux-context-recoil-zustand/) vor dem Reset zurück
