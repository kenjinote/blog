...
```mermaid
graph TD
    A["Moralisches Dilemma"] --> B{"1 Person opfern, um 5 zu retten?"}
    B -- "Hebel-Szenario" --> C["Viele antworten, es sei zulässig"]
    B -- "Fußgängerbrücken-Szenario" --> D["Viele antworten, es sei unzulässig"]
    C --> E["Utilitaristische Beurteilung (ergebnisorientiert)"]
    D --> F["Deontologische Beurteilung (handlungsorientiert)"]
    E -. "Widerspruch?" .- F
```
...
```mermaid
graph LR
    A["Opferstruktur (1 zu 5)"] --> B["Hebel (Zulässigkeit: hoch)"]
    A --> C["Loop (Zulässigkeit: mittel bis hoch)"]
    A --> D["Fußgängerbrücke (Zulässigkeit: niedrig)"]
    A --> E["Organtransplantation (Zulässigkeit: extrem niedrig)"]
    
    B --> F["Kein physischer Kontakt, keine instrumentelle Absicht"]
    C --> G["Kein physischer Kontakt, instrumentelle Absicht vorhanden"]
    D --> H["Physischer Kontakt vorhanden, instrumentelle Absicht vorhanden"]
    E --> I["Eindeutige Verletzung von Rechten, Verletzung beruflicher Pflichten"]
```
