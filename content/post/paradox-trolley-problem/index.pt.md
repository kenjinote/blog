...
```mermaid
graph TD
    A["Dilema moral"] --> B{"Sacrificar um para salvar cinco?"}
    B -- "Cenário da alavanca" --> C["Muitos respondem que é permitido"]
    B -- "Cenário da passarela" --> D["Muitos respondem que não é permitido"]
    C --> E["Julgamento utilitarista (foco nos resultados)"]
    D --> F["Julgamento deontológico (foco no ato em si)"]
    E -. "Contradição?" .- F
```
...
```mermaid
graph LR
    A["Estrutura do sacrifício (1 para 5)"] --> B["Alavanca (Permissão: Alta)"]
    A --> C["Loop (Permissão: Média a Alta)"]
    A --> D["Passarela (Permissão: Baixa)"]
    A --> E["Transplante de órgãos (Permissão: Muito Baixa)"]
    
    B --> F["Sem contato físico, sem intenção instrumental"]
    C --> G["Sem contato físico, com intenção instrumental"]
    D --> H["Com contato físico, com intenção instrumental"]
    E --> I["Clara violação de direitos e quebra de dever profissional"]
```
