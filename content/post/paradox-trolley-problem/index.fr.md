...
```mermaid
graph TD
    A["Dilemme moral"] --> B{"Sacrifier 1 personne pour en sauver 5 ?"}
    B -- "Scénario du levier" --> C["Beaucoup répondent que c'est acceptable"]
    B -- "Scénario de la passerelle" --> D["Beaucoup répondent que c'est inacceptable"]
    C --> E["Jugement utilitariste (axé sur les résultats)"]
    D --> F["Jugement déontologique (axé sur l'acte lui-même)"]
    E -. "Contradiction ?" .- F
```
...
```mermaid
graph LR
    A["Structure du sacrifice (1 contre 5)"] --> B["Levier (Acceptabilité : Haute)"]
    A --> C["Boucle (Acceptabilité : Moyenne à Haute)"]
    A --> D["Passerelle (Acceptabilité : Basse)"]
    A --> E["Transplantation d'organes (Acceptabilité : Très Basse)"]
    
    B --> F["Aucun contact physique / Aucune intention instrumentale"]
    C --> G["Aucun contact physique / Intention instrumentale"]
    D --> H["Contact physique / Intention instrumentale"]
    E --> I["Violation évidente des droits / Violation des obligations professionnelles"]
```
