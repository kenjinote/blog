...
```mermaid
graph TD
    A["Moral Dilemma"] --> B{"Sacrifice 1 to save 5?"}
    B -- "Lever Scenario" --> C["Many answer permissible"]
    B -- "Footbridge Scenario" --> D["Many answer impermissible"]
    C --> E["Utilitarian judgment (Focus on outcome)"]
    D --> F["Deontological judgment (Focus on the act itself)"]
    E -. "Contradiction?" .- F
```
...
```mermaid
graph LR
    A["Structure of Sacrifice (1 vs 5)"] --> B["Lever (Permissibility: High)"]
    A --> C["Loop (Permissibility: Medium to High)"]
    A --> D["Footbridge (Permissibility: Low)"]
    A --> E["Organ Transplant (Permissibility: Very Low)"]
    
    B --> F["No physical contact, No instrumental intention"]
    C --> G["No physical contact, Instrumental intention present"]
    D --> H["Physical contact present, Instrumental intention present"]
    E --> I["Clear violation of rights, Breach of professional duty"]
```
