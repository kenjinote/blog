...
```mermaid
graph TD
    A["Dilema moral"] --> B{"¿Sacrificar a 1 para salvar a 5?"}
    B -- "Escenario de la palanca" --> C["Muchos responden que es permisible"]
    B -- "Escenario del puente peatonal" --> D["Muchos responden que es inaceptable"]
    C --> E["Juicio utilitarista (enfoque en los resultados)"]
    D --> F["Juicio deontológico (enfoque en el acto en sí)"]
    E -. "¿Contradicción?" .- F
```
...
```mermaid
graph LR
    A["Estructura del sacrificio (1 contra 5)"] --> B["Palanca (Permisibilidad: alta)"]
    A --> C["Bucle (Permisibilidad: media-alta)"]
    A --> D["Puente peatonal (Permisibilidad: baja)"]
    A --> E["Trasplante de órganos (Permisibilidad: extremadamente baja)"]
    
    B --> F["Sin contacto físico / sin intención instrumental"]
    C --> G["Sin contacto físico / con intención instrumental"]
    D --> H["Con contacto físico / con intención instrumental"]
    E --> I["Clara violación de derechos / incumplimiento de deberes profesionales"]
```
