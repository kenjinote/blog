...
```mermaid
graph TD
    A["Dilema moral"] --> B{"Mengorbankan 1 orang untuk menyelamatkan 5 orang?"}
    B -- "Skenario tuas" --> C["Banyak orang menjawab diizinkan"]
    B -- "Skenario jembatan penyeberangan" --> D["Banyak orang menjawab tidak diizinkan"]
    C --> E["Penilaian utilitarian (fokus pada hasil)"]
    D --> F["Penilaian deontologis (fokus pada tindakan itu sendiri)"]
    E -. "Kontradiksi?" .- F
```
...
```mermaid
graph LR
    A["Komposisi pengorbanan (1 banding 5)"] --> B["Tuas (Toleransi: Tinggi)"]
    A --> C["Putaran (Toleransi: Sedang-Tinggi)"]
    A --> D["Jembatan (Toleransi: Rendah)"]
    A --> E["Transplantasi organ (Toleransi: Sangat Rendah)"]
    
    B --> F["Tanpa kontak fisik, tanpa niat instrumental"]
    C --> G["Tanpa kontak fisik, ada niat instrumental"]
    D --> H["Ada kontak fisik, ada niat instrumental"]
    E --> I["Pelanggaran hak yang jelas, pelanggaran tugas profesional"]
```
