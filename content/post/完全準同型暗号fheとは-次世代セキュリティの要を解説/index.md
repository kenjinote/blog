...
従来の[暗号化](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)技術（AESや[RSA](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)など）は、ストレージに保存されているデータ（Data at Rest）やネットワーク上を流れるデータ（Data in Transit）を保護することには長けています。しかし、サーバー側でデータに対して検索や機械学習などの **処理（計算）を行う際（Data in Use）には、一度暗号を復号して平文に戻す必要があります** 。もしこの復号されたタイミングでサーバーがハッキングされたり、内部の悪意ある管理者がデータを覗き見たりすれば、情報漏洩に直結します。
...
サーバーは[暗号化](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)されたデータ $E(x)$ を受け取りますが、秘密鍵を持たないためデータの中身を知ることは絶対にできません。しかし、FHEの性質を利用することで、暗号文に対して関数 $f$（例えば機械学習の推論モデル）を適用し、$E(f(x))$ を生成することができます。クライアントはこれを受け取り、自身の秘密鍵で復号することで、目的の結果 $y = f(x)$ を手に入れます。
...
*   **[RSA](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)暗号（乗算に対する準同型性）**
    RSA暗号は意図せずして乗算の準同型性を持っていました。平文 $m_1, m_2$、[公開鍵](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/) $(e, N)$ とすると：
    $$ E(m_1) = m_1^e \pmod N $$
    $$ E(m_2) = m_2^e \pmod N $$
    これらを掛け合わせると：
...
加算と乗算の両方を、 **回数無制限** で実行できる暗号方式です。[情報理論](https://kenji.blog/p/information-theory-shannon-entropy/)における[チューリング](https://kenji.blog/p/turing/)完全性と同様に、加算（XORに相当）と乗算（ANDに相当）を無限に組み合わせることができれば、原理上いかなる計算可能な関数・アルゴリズムも[暗号化](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)されたまま実行できることを意味します。
...
秘密鍵を多項式 $s(x)$ とし、ランダムな多項式 $a(x)$、小さなノイズ多項式 $e(x)$ とすると、[公開鍵](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)は以下のペアになります。

$$ (a(x), b(x)) \quad \text{where} \quad b(x) = -a(x) \cdot s(x) + e(x) \pmod q $$

[暗号化](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)の際には、この多項式の性質を利用して平文 $m(x)$ をエンコードし、暗号文を生成します。
...
直感的には「暗号文がノイズまみれになって壊れる前に、[暗号化](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)されたままの状態で『復号』して綺麗にし、新しい暗号文に入れ直す」という操作です。

1. ノイズが大きくなった暗号文 $C_{noisy}$ があるとします。
2. クライアントは、秘密鍵 $sk$ を「[公開鍵](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)で暗号化したもの」$E_{pk}(sk)$（これをブートストラッピングキーと呼ぶ）をあらかじめサーバーに渡しておきます。
...
これまでのFHEが「正確な整数計算」にこだわっていたのに対し、CKKSは **「浮動小数点数の近似計算」** を[暗号化](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)されたままサポートします。ニューラルネットワークの学習や推論など、小さな誤差が許容される実数計算において、圧倒的なパフォーマンスを発揮します。
...
ここでは、CKKSスキームを用いて、[暗号化](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)されたまま実数ベクトルを足し合わせ、掛け合わせる概念的なPythonコードの例を示します。
