...
## 1. [動的計画法](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)（[DP](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)）とは何か？

動的計画法（[Dynamic Programming](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)）は、複雑な問題をより小さな「部分問題」に分割し、それらの部分問題の解を記録・再利用することで、計算量を劇的に削減するアルゴリズムの設計手法です。
...
最短経路問題がわかりやすい例です。都市Aから都市Cへの最短経路が都市Bを経由する場合、「都市Aから都市Bまでの経路」もまた、AからBへの最短経路になっていなければなりません。もしAからBへの経路が最適（最短）でなければ、それを最適化することでAからCへの経路全体もさらに短くできるはずだからです。このように、部分的な最適解を組み合わせて全体の最適解を導き出せる性質が、[動的計画法](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)による状態遷移の基盤となります。
...
- **デメリット:** 
  ...
  - 再帰の深さが大きくなると[スタック](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)オーバーフローのリスクがある（特にPythonなどの言語では注意が必要）。
...
- **メリット:** 
  ...
  - 後述する「[空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)の最適化（配列の使い回し）」が容易。
- **デメリット:** 
...
## 3. 実践編1：[フィボナッチ](https://kenji.blog/p/fibonacci/)数列

まずは最も基本的でわかりやすい例として、[フィボナッチ](https://kenji.blog/p/fibonacci/)数列を取り上げます。
[フィボナッチ](https://kenji.blog/p/fibonacci/)数列は次のように定義されます：
...
小さい方から順番に配列を埋めていくアプローチです。[スタック](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)オーバーフローの心配がなく、極めて高速に動作します。
...
### 3-4. [空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)の最適化
...
## 4. 実践編2：0/1[ナップサック問題](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)（0/1 Knapsack Problem）

次はいよいよ本格的な最適化問題です。0/1ナップサック問題は、[動的計画法](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)の登竜門として知られています。
...
[DP](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)を解くための最重要ステップは「状態（[State](https://kenji.blog/p/iac-infrastructure-as-code-terraform/)）」を適切に定義することです。
...
### 4-4. [空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)の最適化（1次元配列化）

2次元配列 $dp[i][w]$ を更新する際、常に直前の行 $dp[i-1]$ しか参照していないことに気づきます。これは[フィボナッチ](https://kenji.blog/p/fibonacci/)数列の空間最適化と同じ原理です。
したがって、配列を1次元 $dp[w]$ に圧縮できます。ただし、更新時に注意が必要です。容量 $w$ を **大きい方から小さい方へ（後ろから前へ）** ループさせる必要があります。前から更新してしまうと、「$i-1$ 番目の状態」ではなく、同じステップ内で更新されたばかりの「$i$ 番目の状態」を参照してしまい、同じ品物を複数回選ぶことになってしまうからです（これは「個数制限なし[ナップサック問題](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)」の解法になってしまいます）。
...
```
これにより、[空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)が $O(nW)$ から $O(W)$ へと劇的に改善されます。実務や競技プログラミングにおいて必須のテクニックです。

---

## 5. 実践編3：最長共通部分列（LCS: Longest Common Subsequence）

文字列を扱う代表的な[DP](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)問題として、LCSを取り上げます。LCSは、ファイルの差分検出（diffツール）やDNA配列の類似度判定など、実社会で広く応用されているアルゴリズムです。
...
LCS問題でも、更新には直前の行（`dp[i-1]`）と現在の行（`dp[i]`）しか使用しないため、2行分（要素数 $2n$）の配列があれば計算可能です。これを「ローリング配列（Rolling Array）」と呼びます。[空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)を劇的に減らす手法として極めて有用です。

---

## 6. [動的計画法](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)をマスターするための思考プロセス

ここまで様々な問題を見てきましたが、未知の[DP](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)問題に直面したとき、どのように考えればよいのでしょうか？以下のステップを常に意識してください。

1. **この問題はDPで解けるか？（条件の確認）**
   再帰的に考えたとき、同じ状態が何度も現れるか（部分問題の重複）。最善の選択を組み合わせることで全体の最善が導けるか（部分構造最適性）。
2. **状態（[State](https://kenji.blog/p/iac-infrastructure-as-code-terraform/)）を定義する**
...
本記事では、[動的計画法](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)の基礎理論から、具体的な実装アプローチ、さらには代表的な最適化問題に至るまで、詳細に解説しました。
...
- [空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)の削減テクニック（配列の1次元化やローリング配列）は、実務レベルでパフォーマンスを要求される際に不可欠です。

動的計画法は、最初は難解に感じるかもしれません。しかし、様々な問題で「状態定義」と「遷移」を見つける訓練を繰り返すことで、徐々にパターンが見えてくるようになります。木[DP](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)、桁DP、ビットDP、区間DPなど、より高度な応用もありますが、その全てが今回学んだ「部分問題の重複」と「最適化」の基盤の上に成り立っています。
