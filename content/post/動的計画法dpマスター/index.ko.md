...
경쟁 프로그래밍부터 실무 알고리즘 설계까지 많은 상황에서 등장하며, 수많은 프로그래머에게 벽이 되는 것이 바로 **동적 계획법([Dynamic Programming](https://kenji.blog/ko/p/dynamic-programming-dp-introduction-knapsack-fibonacci/), 통칭 [DP](https://kenji.blog/ko/p/dynamic-programming-dp-introduction-knapsack-fibonacci/))** 입니다. "점화식을 세우지 못하겠다", "인덱스(첨자)에서 버그가 난다", "애초에 DP로 풀 수 있는 문제인지 판단하기 어렵다"…… 이런 고민을 안고 계신 분들이 많지 않을까요?
...
[DP](https://kenji.blog/ko/p/dynamic-programming-dp-introduction-knapsack-fibonacci/)로 문제를 풀기 위한 가장 중요한 단계는 '상태([State](https://kenji.blog/ko/p/iac-infrastructure-as-code-terraform/))'를 적절히 정의하는 것입니다.
...
문자열을 다루는 대표적인 [DP](https://kenji.blog/ko/p/dynamic-programming-dp-introduction-knapsack-fibonacci/) 문제로서 LCS를 살펴보겠습니다. LCS는 파일의 차이점 검출(diff 도구)이나 DNA 서열의 유사도 판정 등 실생활에서 널리 응용되고 있는 알고리즘입니다.
...
지금까지 여러 문제를 살펴보았는데, 미지의 [DP](https://kenji.blog/ko/p/dynamic-programming-dp-introduction-knapsack-fibonacci/) 문제에 직면했을 때 어떻게 생각하면 좋을까요? 다음 단계들을 항상 의식해 보세요.

1. **이 문제는 DP로 풀 수 있는가? (조건 확인)**
   재귀적으로 생각했을 때 같은 상태가 여러 번 나타나는지(부분 문제의 중복). 최선의 선택을 결합함으로써 전체의 최선을 이끌어낼 수 있는지(최적 부분 구조).
2. **상태([State](https://kenji.blog/ko/p/iac-infrastructure-as-code-terraform/))를 정의한다**
...
동적 계획법은 처음에는 난해하게 느껴질지도 모릅니다. 하지만 다양한 문제에서 '상태 정의'와 '전이'를 찾아내는 훈련을 반복함으로써 점차 패턴이 보이게 될 것입니다. 트리 [DP](https://kenji.blog/ko/p/dynamic-programming-dp-introduction-knapsack-fibonacci/), 자릿수 DP, 비트 DP, 구간 DP 등 더욱 고도화된 응용도 있지만, 그 모든 것이 이번에 배운 '부분 문제의 중복'과 '최적화'라는 기반 위에 성립되어 있습니다.
