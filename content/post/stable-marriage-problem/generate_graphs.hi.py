...
def main():
    plt.rcParams.update({"font.family": ['Nirmala UI', 'DejaVu Sans'], "font.size": 12, "axes.titlesize": 13,
    ...
    (ROOT / "calculation-results.hi.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "स्थिर" if not row["blocking_pairs"] else 'अस्थिर: {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("छह लोगों की रैंक का योग (कम रैंक अधिक पसंदीदा)")
    ax.set_title("रैंक का सबसे छोटा योग स्थिरता की गारंटी नहीं है", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.hi.svg")
    ...
    ax.text(2.5, 3.3, "A और X वर्तमान साथियों से अधिक एक-दूसरे को पसंद करते हैं", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "पक्ष L", ha="center")
    ax.text(4, 3.85, "पक्ष R", ha="center")
    ax.text(2.5, .4, "ठोस रेखा: वर्तमान जोड़ी    टूटी रेखा: अवरोधक जोड़ी", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="यह मिलान अस्थिर क्यों है")
    ax.axis("off")
    save(fig, "blocking-pair.hi.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "पक्ष L की औसत रैंक", "#0891b2"), (.18, means_r, "पक्ष R की औसत रैंक", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L प्रस्ताव देता है\nA–X, B–Y", "R प्रस्ताव देता है\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="औसत पसंद रैंक (कम बेहतर है)", title="दो-दो लोगों का अलग उदाहरण: दोनों परिणाम स्थिर हैं")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.hi.svg")
