...
この問いに、はっきりした基準を与えるのが**安定結婚問題**です。名前には結婚とありますが、数学的な核心は「希望を持つ二つのグループの間で、1対1の組を作ること」。この記事では性別や結婚生活の話から離れ、A・B・CとX・Y・Zという記号を使います。
...
安定結婚問題は、希望がぶつかる状況に対して「双方が乗り換えを望む2人を残さない」という基準を与えます。ゲール＝シャプレーのアルゴリズムは、申し込みと保留を繰り返すだけで、この基準を満たします。
...
全員の希望をそのままかなえられないからこそ、何を実現したいのかを言葉と数式で区別する価値があります。安定結婚問題は、最適化の答えを出す前に、まず「よい組み合わせ」の意味を考えるための、身近で奥深い入口なのです。
