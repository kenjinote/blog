...
이는 상한이지 매번 정확히 필요한 횟수는 아닙니다. 이 예는 $n=3$에서 다섯 번이면 끝납니다. 순위를 사전으로 저장해 비교를 일정한 시간에 처리하면 알고리즘의 시간 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)는 $O(n^2)$입니다. 입력 순위표 자체에도 $2n^2$개의 항목이 있습니다.
...
이 교육용 구현은 같은 인원수, 완전한 순위표, 동순위 없음을 전제로 하며 입력 검증이나 허용되지 않는 상대는 처리하지 않습니다. 검사 함수는 읽기 쉽게 `.index()`를 사용하므로 $O(n^3)$입니다. 앞의 $O(n^2)$는 사전으로 순위를 비교하는 매칭 본체의 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)이며, 추가 검사는 포함하지 않습니다.
