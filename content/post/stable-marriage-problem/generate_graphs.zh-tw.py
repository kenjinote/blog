...
def main():
    plt.rcParams.update({"font.family": ['Microsoft JhengHei', 'DejaVu Sans'], "font.size": 12, "axes.titlesize": 13,
    ...
    (ROOT / "calculation-results.zh-tw.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "穩定" if not row["blocking_pairs"] else '不穩定：{n}組'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("全部6人的偏好排名總和（越小越好）")
    ax.set_title("排名總和最小，不一定穩定", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.zh-tw.svg")
    ...
    ax.text(2.5, 3.3, "A 和 X 都更喜歡彼此，而不是目前的對象", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "L 方", ha="center")
    ax.text(4, 3.85, "R 方", ha="center")
    ax.text(2.5, .4, "實線：目前配對    虛線：阻擋配對", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="這個配對為什麼不穩定")
    ax.axis("off")
    save(fig, "blocking-pair.zh-tw.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "L 方平均排名", "#0891b2"), (.18, means_r, "R 方平均排名", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L 方提出提案\nA–X, B–Y", "R 方提出提案\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="平均偏好排名（越小越好）", title="另一個雙方各2人的例子：兩個結果都穩定")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.zh-tw.svg")
