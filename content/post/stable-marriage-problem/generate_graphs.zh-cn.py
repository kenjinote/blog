...
def main():
    plt.rcParams.update({"font.family": ['Microsoft YaHei', 'DejaVu Sans'], "font.size": 12, "axes.titlesize": 13,
    ...
    (ROOT / "calculation-results.zh-cn.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "稳定" if not row["blocking_pairs"] else '不稳定：{n}对'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("全部6人的偏好排名总和（越小越好）")
    ax.set_title("排名总和最小，不一定稳定", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.zh-cn.svg")
    ...
    ax.text(2.5, 3.3, "A 和 X 都更喜欢彼此，而不是当前对象", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "L 方", ha="center")
    ax.text(4, 3.85, "R 方", ha="center")
    ax.text(2.5, .4, "实线：当前配对    虚线：阻塞对", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="这个匹配为什么不稳定")
    ax.axis("off")
    save(fig, "blocking-pair.zh-cn.svg")
    ...
    ax.set_xticks([0,1], ["L 方提出申请\nA–X, B–Y", "R 方提出申请\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="平均偏好排名（越小越好）", title="另一个双方各2人的例子：两个结果都稳定")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.zh-cn.svg")
