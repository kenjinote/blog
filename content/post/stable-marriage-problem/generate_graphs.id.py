...
def main():
    ...
    (ROOT / "calculation-results.id.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "Stabil" if not row["blocking_pairs"] else 'Tidak stabil: {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("Jumlah peringkat keenam orang (lebih kecil lebih disukai)")
    ax.set_title("Jumlah peringkat terkecil belum tentu stabil", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.id.svg")
    ...
    ax.text(2.5, 3.3, "A dan X saling lebih menyukai daripada pasangan saat ini", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "Sisi L", ha="center")
    ax.text(4, 3.85, "Sisi R", ha="center")
    ax.text(2.5, .4, "Garis penuh: pasangan saat ini    Putus-putus: pasangan pemblokir", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="Mengapa pencocokan ini tidak stabil")
    ax.axis("off")
    save(fig, "blocking-pair.id.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "Rata-rata sisi L", "#0891b2"), (.18, means_r, "Rata-rata sisi R", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L melamar\nA–X, B–Y", "R melamar\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="Rata-rata peringkat pilihan (lebih kecil lebih baik)", title="Contoh lain, dua orang per sisi: keduanya stabil")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.id.svg")
