...
def main():
    plt.rcParams.update({"font.family": ['Segoe UI', 'DejaVu Sans'], "font.size": 12, "axes.titlesize": 13,
    ...
    (ROOT / "calculation-results.ar.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "مستقرة" if not row["blocking_pairs"] else 'غير مستقرة: {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("مجموع رتب المشاركين الستة (الأصغر أفضل)")
    ax.set_title("أصغر مجموع للرتب لا يضمن الاستقرار", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.ar.svg")
    ...
    ax.text(2.5, 3.3, "يفضل A وX بعضهما على شريكيهما الحاليين", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "الطرف L", ha="center")
    ax.text(4, 3.85, "الطرف R", ha="center")
    ax.text(2.5, .4, "خط متصل: الأزواج الحالية    متقطع: الزوج المعطل", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="لماذا هذه المطابقة غير مستقرة؟")
    ax.axis("off")
    save(fig, "blocking-pair.ar.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "متوسط رتب الطرف L", "#0891b2"), (.18, means_r, "متوسط رتب الطرف R", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["الطرف L يقدم العروض\nA–X, B–Y", "الطرف R يقدم العروض\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="متوسط رتبة التفضيل (الأصغر أفضل)", title="مثال آخر: اثنان في كل طرف، والنتيجتان مستقرتان")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.ar.svg")
