...
**[稳定婚姻问题](https://kenji.blog/zh-cn/p/stable-marriage-problem/)**为此提供了一个明确标准。虽然名字中有“婚姻”，数学上的核心却是两个具有偏好的群体之间的一对一匹配。本文使用 A、B、C 与 X、Y、Z，不预设性别，也不讨论真实婚姻关系。
...
这只是上界，并不意味着每次都需要 $n^2$ 次。本文在 $n=3$ 时只需五次。预先将接收方排名存入字典，以常数时间比较，算法的时间[复杂度](/zh-cn/p/time-space-complexity-big-o-notation-examples/)为 $O(n^2)$。输入的双方排名表本身就有 $2n^2$ 个条目。
