...
def main():
    ...
    (ROOT / "calculation-results.en.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "Stable" if not row["blocking_pairs"] else 'Unstable: {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("Sum of all six ranks (lower means more preferred)")
    ax.set_title("The smallest rank sum need not be stable", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.en.svg")
    ...
    ax.text(2.5, 3.3, "A and X prefer each other to their current partners", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "Side L", ha="center")
    ax.text(4, 3.85, "Side R", ha="center")
    ax.text(2.5, .4, "Solid: current pairs    Dashed: blocking pair", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="Why this matching is unstable")
    ax.axis("off")
    save(fig, "blocking-pair.en.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "Side L mean rank", "#0891b2"), (.18, means_r, "Side R mean rank", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L proposes\nA–X, B–Y", "R proposes\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="Mean preference rank (lower is better)", title="Separate two-by-two example: both outcomes are stable")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.en.svg")
