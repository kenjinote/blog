"""記事の例を全列挙し、SVG図とJSONを再生成する。依存: matplotlib。"""
...
def main():
    plt.rcParams.update({"font.family": ["Meiryo", "DejaVu Sans"], "font.size": 13,
    ...
    for i, row in enumerate(rows):
        note = "安定" if not row["blocking_pairs"] else f"不安定：{len(row['blocking_pairs'])}組"
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("全6人の希望順位の合計（小さいほど上位）")
    ax.set_title("順位の合計が最小でも、安定とは限らない", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.svg")
    ...
    ax.text(2.5, 3.3, "AとXは、互いに今の相手より好ましい", ha="center", color="#c2410c")
    ax.text(1, 3.85, "L側", ha="center")
    ax.text(4, 3.85, "R側", ha="center")
    ax.text(2.5, .4, "実線：現在の組　　破線：ブロッキングペア", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="この組み合わせが不安定な理由")
    ax.axis("off")
    save(fig, "blocking-pair.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "L側の平均順位", "#0891b2"), (.18, means_r, "R側の平均順位", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L側が申し込む\nA–X、B–Y", "R側が申し込む\nA–Y、B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="平均希望順位（小さいほど上位）", title="2人ずつの別例：どちらの結果も安定")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.svg")
