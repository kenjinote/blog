...
def main():
    ...
    (ROOT / "calculation-results.es.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "Estable" if not row["blocking_pairs"] else 'Inestable: {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("Suma de las seis posiciones (menor es mejor)")
    ax.set_title("La menor suma de posiciones no garantiza estabilidad", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.es.svg")
    ...
    ax.text(2.5, 3.3, "A y X se prefieren mutuamente a sus parejas actuales", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "Lado L", ha="center")
    ax.text(4, 3.85, "Lado R", ha="center")
    ax.text(2.5, .4, "Línea continua: parejas actuales    Discontinua: par bloqueante", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="Por qué este emparejamiento es inestable")
    ax.axis("off")
    save(fig, "blocking-pair.es.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "Media del lado L", "#0891b2"), (.18, means_r, "Media del lado R", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["Propone L\nA–X, B–Y", "Propone R\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="Posición media de preferencia (menor es mejor)", title="Otro ejemplo de dos contra dos: ambos resultados son estables")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.es.svg")
