...
**[穩定婚姻問題](https://kenji.blog/zh-tw/p/stable-marriage-problem/)**為此提供了一個明確標準。雖然名稱有「婚姻」，數學核心卻是兩個具有偏好的群體之間的一對一配對。本文使用 A、B、C 與 X、Y、Z，不預設性別，也不討論真實婚姻關係。
...
這只是上界，不代表每次都需要 $n^2$ 次。本例在 $n=3$ 時只需五次。預先將接收方排名存入字典，以常數時間比較，演算法的時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)為 $O(n^2)$。雙方輸入排序表本身就有 $2n^2$ 個項目。
