...
def main():
    plt.rcParams.update({"font.family": ['Malgun Gothic', 'DejaVu Sans'], "font.size": 12, "axes.titlesize": 13,
    ...
    (ROOT / "calculation-results.ko.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "안정" if not row["blocking_pairs"] else '불안정: {n}쌍'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("6명 전체의 희망 순위 합 (작을수록 선호)")
    ax.set_title("순위 합이 가장 작아도 안정적이지 않을 수 있다", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.ko.svg")
    ...
    ax.text(2.5, 3.3, "A와 X는 현재 상대보다 서로를 더 선호한다", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "L 그룹", ha="center")
    ax.text(4, 3.85, "R 그룹", ha="center")
    ax.text(2.5, .4, "실선: 현재 매칭    점선: 블로킹 쌍", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="이 매칭이 불안정한 이유")
    ax.axis("off")
    save(fig, "blocking-pair.ko.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "L 그룹 평균 순위", "#0891b2"), (.18, means_r, "R 그룹 평균 순위", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L 그룹이 제안\nA–X, B–Y", "R 그룹이 제안\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="평균 희망 순위 (작을수록 선호)", title="별도의 2명씩 사례: 두 결과 모두 안정적")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.ko.svg")
