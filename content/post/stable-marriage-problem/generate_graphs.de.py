...
def main():
    ...
    (ROOT / "calculation-results.de.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "Stabil" if not row["blocking_pairs"] else 'Instabil: {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("Summe aller sechs Ränge (kleiner bedeutet beliebter)")
    ax.set_title("Die kleinste Rangsumme muss nicht stabil sein", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.de.svg")
    ...
    ax.text(2.5, 3.3, "A und X ziehen einander ihren bisherigen Partnern vor", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "Seite L", ha="center")
    ax.text(4, 3.85, "Seite R", ha="center")
    ax.text(2.5, .4, "Durchgezogen: aktuelle Paare    Gestrichelt: blockierendes Paar", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="Warum diese Zuordnung instabil ist")
    ax.axis("off")
    save(fig, "blocking-pair.de.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "Mittlerer Rang für L", "#0891b2"), (.18, means_r, "Mittlerer Rang für R", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L stellt Anträge\nA–X, B–Y", "R stellt Anträge\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="Mittlerer Präferenzrang (kleiner ist besser)", title="Anderes Beispiel mit je zwei Personen: beide Ergebnisse stabil")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.de.svg")
