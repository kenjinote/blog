...
def main():
    ...
    (ROOT / "calculation-results.fr.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "Stable" if not row["blocking_pairs"] else 'Instable : {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("Somme des six rangs (plus petit = mieux classé)")
    ax.set_title("La plus petite somme des rangs ne garantit pas la stabilité", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.fr.svg")
    ...
    ax.text(2.5, 3.3, "A et X se préfèrent à leurs partenaires actuels", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "Groupe L", ha="center")
    ax.text(4, 3.85, "Groupe R", ha="center")
    ax.text(2.5, .4, "Trait plein : couples actuels    Pointillés : paire bloquante", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="Pourquoi cet appariement est instable")
    ax.axis("off")
    save(fig, "blocking-pair.fr.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "Rang moyen de L", "#0891b2"), (.18, means_r, "Rang moyen de R", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["L propose\nA–X, B–Y", "R propose\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="Rang moyen de préférence (plus petit = meilleur)", title="Autre exemple à deux contre deux : deux résultats stables")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.fr.svg")
