...
def main():
    ...
    (ROOT / "calculation-results.ru.json").write_text(json.dumps(data, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    ...
    for i, row in enumerate(rows):
        note = "Устойчиво" if not row["blocking_pairs"] else 'Неустойчиво: {n}'.format(n=len(row["blocking_pairs"]))
        ax.text(row["total"]+.2, i, f"{row['total']}  {note}", va="center", fontsize=11)
    ax.invert_yaxis()
    ax.set_xlim(0, 22)
    ax.set_xticks(range(0, 23, 2))
    ax.set_xlabel("Сумма рангов всех шести участников (меньше — лучше)")
    ax.set_title("Минимальная сумма рангов не гарантирует устойчивости", pad=20)
    ax.grid(axis="x", alpha=.2)
    ax.set_axisbelow(True)
    save(fig, "rank-comparison.ru.svg")
    ...
    ax.text(2.5, 3.3, "A и X предпочитают друг друга нынешним партнёрам", ha="center", color="#c2410c", fontsize=11)
    ax.text(1, 3.85, "Сторона L", ha="center")
    ax.text(4, 3.85, "Сторона R", ha="center")
    ax.text(2.5, .4, "Сплошные: нынешние пары    Пунктир: блокирующая пара", ha="center")
    ax.set(xlim=(.3, 4.7), ylim=(.1, 4.2), title="Почему это сочетание неустойчиво")
    ax.axis("off")
    save(fig, "blocking-pair.ru.svg")
    ...
    for shift, values, name, color in [(-.18, means_l, "Средний ранг для L", "#0891b2"), (.18, means_r, "Средний ранг для R", "#9333ea")]:
        bars = ax.bar([x+shift for x in range(2)], values, width=.34, label=name, color=color)
        ax.bar_label(bars, fmt="%.0f", padding=5)
    ax.set_xticks([0,1], ["Предлагает L\nA–X, B–Y", "Предлагает R\nA–Y, B–X"])
    ax.set(ylim=(0,2.7), yticks=[0,1,2], ylabel="Средний ранг партнёра (меньше — лучше)", title="Другой пример: по два участника, оба исхода устойчивы")
    ax.legend(loc="upper center", ncol=2, fontsize=11)
    save(fig, "proposer-comparison.ru.svg")
