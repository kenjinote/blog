...
In modern web development, JavaScript (and TypeScript) has long established its position as the sole programming language running on the browser. However, in recent years, there has been an increasing demand to execute more advanced computations directly in the browser, such as image processing, video encoding, 3D games, and physical simulations. This is where **[WebAssembly](https://kenji.blog/en/p/webassembly-wasm-current-future/) (commonly known as [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/))** comes into play.

In this article, starting from the basics of WebAssembly, we will explain the detailed steps and internal structures for outputting Wasm from two powerful system programming languages, C++ (using Emscripten) and [Rust](https://kenji.blog/en/p/webassembly-wasm-current-future/) (using `wasm-pack`), and integrating them with the JavaScript environment. Furthermore, we will delve deeply into managing memory boundaries, passing complex data such as strings and arrays, performance overheads, and the [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) binary format (`.wasm`).
...
[Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) is not meant to replace JavaScript. It is designed to work alongside JavaScript, taking advantage of each other's strengths by offloading computationally intensive tasks to Wasm.

## 3. Mathematical Challenge: Calculating the Mandelbrot Set

In this article, we will implement the drawing algorithm of the "Mandelbrot set", which places a heavy load on the CPU, using C++ and [Rust](https://kenji.blog/en/p/webassembly-wasm-current-future/).
...
Emscripten is an LLVM-based compiler toolchain and the de facto standard for compiling C/C++ code into [WebAssembly](https://kenji.blog/en/p/webassembly-wasm-current-future/). It provides a powerful runtime that emulates POSIX system calls with browser APIs (Web APIs).
...
On the JavaScript side, load the glue code (`mandelbrot.js`) generated by Emscripten, and call it using the [WebAssembly](https://kenji.blog/en/p/webassembly-wasm-current-future/) API as follows.
...
Import the generated package from JavaScript. Thanks to `wasm-bindgen`, [Rust](https://kenji.blog/en/p/webassembly-wasm-current-future/)'s `Vec<i32>` is automatically converted to JavaScript's `Int32Array` (hiding the pointer operations).
...
One of the most important concepts in [WebAssembly](https://kenji.blog/en/p/webassembly-wasm-current-future/) is "Linear Memory". [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) code cannot directly access the host's (browser's) memory space; instead, it is allocated a single isolated huge `ArrayBuffer`. This is the linear memory.
...
Integers and floating-point numbers (`i32`, `i64`, `f32`, `f64`) can be passed directly as values to [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) functions. However, complex types like strings, arrays, and structs cannot be passed directly through Wasm function signatures.

**For Emscripten**:
1. Call `Module._malloc` on the JS side to allocate a linear memory region on the [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) side.
...
**For wasm-bindgen ([Rust](https://kenji.blog/en/p/webassembly-wasm-current-future/))**:
The cumbersome memory management flow mentioned above is completely hidden within the automatically generated glue code (JS wrapper). When simply passing a `String` or `Array` from the JS side to a [Rust](https://kenji.blog/en/p/webassembly-wasm-current-future/) function, a series of processes such as buffer allocation (equivalent to `malloc`), copying, passing pointers, and freeing memory are automatically performed behind the scenes.

## 7. Performance Overhead and Optimization

[WebAssembly](https://kenji.blog/en/p/webassembly-wasm-current-future/) can be executed at near-native speed, but there is an overhead in the "communication crossing the boundary between JavaScript and WebAssembly (Interop)".

* **Call Overhead**: The switching cost for the JavaScript engine to call a [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) function. Although it is heavily optimized now, a design that calls very light functions tens of thousands of times per frame should be avoided.
...
For example, in game engines and physics engines, it is a common architecture to keep all states in [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/)'s linear memory, and JavaScript is only responsible for the trigger to "update" per frame and screen rendering (calling WebGL/WebGPU APIs).

## 8. Anatomy of the [WebAssembly](https://kenji.blog/en/p/webassembly-wasm-current-future/) Binary Format (.wasm)

Now let's look at the internal structure of the `.wasm` file output by the compiler. The [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) binary is composed of a set of logical blocks called "sections", prioritizing extensibility and parsing speed.
...
* **Import Section**: A list of functions and memory provided to [Wasm](https://kenji.blog/en/p/webassembly-wasm-current-future/) from the JavaScript environment. For example, if you call `console.log` from C++, it is declared here.
...
* **Data Section**: Static string literals and initialization data defined in C++ or [Rust](https://kenji.blog/en/p/webassembly-wasm-current-future/) code are loaded into linear memory from this section.
