...
在现代Web开发中，JavaScript（和TypeScript）长期确立了作为在浏览器上运行的唯一编程语言的地位。然而，近年来，直接在浏览器上执行更高级的计算需求不断增加，例如图像处理、视频编码、3D游戏和物理模拟等。因此， **[WebAssembly](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/) (简称 [Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/))** 应运而生。

本文将从WebAssembly的基础讲起，详细解说如何通过C++（使用Emscripten）和[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)（使用`wasm-pack`）这两种强大的系统编程语言导出[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)，并与JavaScript环境进行集成的具体步骤和内部结构。此外，我们还将深入探讨内存边界管理、字符串和数组等复杂数据的传递方式、性能开销以及Wasm的二进制格式（`.wasm`）。
...
[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)并不是要取代JavaScript。它被设计为与JavaScript协同工作，通过将计算密集型任务卸载给Wasm，从而发挥各自的优势。

## 3. 数学挑战：计算曼德博集合（Mandelbrot set）

在本文中，我们将使用对CPU负载较高的“曼德博集合”的绘制算法，分别用C++和[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)进行实现。
...
Emscripten是一个基于LLVM的编译器工具链，是将C/C++代码编译为[WebAssembly](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)的事实上的标准。它提供了一个强大的运行时，可以使用浏览器API（Web API）模拟POSIX系统调用。
...
在JavaScript端，加载Emscripten生成的胶水代码（`mandelbrot.js`），并按如下方式使用[WebAssembly](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/) API进行调用。
...
从JavaScript中导入生成的包。得益于`wasm-bindgen`，[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)的`Vec<i32>`会自动转换为JavaScript的`Int32Array`（隐藏了指针操作）。
...
[WebAssembly](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)中最重要的概念之一是“线性内存 (Linear Memory)”。[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)代码无法直接访问主机（浏览器）的内存空间，取而代之的是为其分配了一个隔离的、巨大的`ArrayBuffer`。这就是线性内存。
...
整数和浮点数（`i32`、`i64`、`f32`、`f64`）可以作为值直接传递给[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)函数。然而，字符串、数组和结构体等复杂类型不能直接通过Wasm的函数签名传递。

**在Emscripten中** ：
1. 在JS端调用`Module._malloc`，分配[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)端的线性内存区域。
2. JS通过`Module.HEAPU8.set()`等方法将数据写入分配好的内存地址（指针）。
3. 将指针传递给C++函数。
4. 计算完成后，在JS端从指针读取结果，最后调用`Module._free`。

**在wasm-bindgen ([Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)) 中** ：
上述繁琐的内存管理流程被完全隐藏在自动生成的胶水代码（JS封装器）中。只要从JS端简单地将`String`或`Array`传递给[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)函数，背后就会自动执行缓冲区分配（相当于`malloc`）、复制、指针传递和内存释放等一系列处理。

## 7. 性能开销与优化

尽管[WebAssembly](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)能够以接近原生的速度运行，但“跨越JavaScript与WebAssembly边界的通信（互操作Interop）”仍存在开销。

* **调用开销** ：这是JavaScript引擎调用[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)函数的切换成本。虽然目前已经进行了大幅优化，但仍应避免在每帧中数万次调用极小的函数这种设计。
...
例如，在游戏引擎或物理引擎中，通常的架构是将所有状态保存在[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)的线性内存中，JavaScript仅负责每帧发送“更新”触发器以及画面渲染（调用WebGL/WebGPU API）。

## 8. [WebAssembly](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/) 二进制格式 (.wasm) 剖析

现在，让我们看看编译器输出的`.wasm`文件的内部结构。[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)二进制文件注重扩展性和解析速度，由称为“段（Section）”的逻辑块集合组成。
...
* **Import Section (导入段)** ：JavaScript环境提供给[Wasm](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)的函数和内存的列表。例如，如果从C++调用 `console.log`，就会在这里声明。
* **Code Section (代码段)** ：存储实际的字节码指令（如 `i32.add`、`call`、`loop` 等）。因为是基于栈的虚拟机，所以采用将操作数压入栈然后调用运算指令的形式。
* **Data Section (数据段)** ：将在C++或[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)代码中定义的静态字符串字面量或初始化数据，从该段加载到线性内存中。
