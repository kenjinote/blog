...
आधुनिक वेब विकास में, JavaScript (और TypeScript) ने लंबे समय तक ब्राउज़र पर चलने वाली एकमात्र प्रोग्रामिंग भाषा के रूप में अपनी स्थिति स्थापित की है। हालाँकि, हाल के वर्षों में, ब्राउज़र पर अधिक उन्नत गणनाएँ, जैसे इमेज प्रोसेसिंग, वीडियो एनकोडिंग, 3D गेम और भौतिकी सिमुलेशन को सीधे ब्राउज़र पर निष्पादित करने की मांग बढ़ रही है। यहीं पर **[WebAssembly](https://kenji.blog/hi/p/webassembly-wasm-current-future/) (जिसे [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) भी कहा जाता है)** की भूमिका आती है।

इस लेख में, हम WebAssembly की मूल बातों से शुरू करेंगे और दो शक्तिशाली सिस्टम प्रोग्रामिंग भाषाओं - C++ (Emscripten का उपयोग करके) और [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) (`wasm-pack` का उपयोग करके) - से [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) को आउटपुट करने और इसे JavaScript वातावरण के साथ एकीकृत करने के विस्तृत चरणों और आंतरिक संरचना को समझाएंगे। इसके अलावा, हम मेमोरी बाउंड्री प्रबंधन, स्ट्रिंग्स और एरेज़ जैसे जटिल डेटा को पास करने के तरीके, प्रदर्शन ओवरहेड और Wasm बाइनरी फॉर्मेट (`.wasm`) के बारे में गहराई से जानेंगे।
...
[Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/), JavaScript को बदलने के लिए नहीं है। यह JavaScript के साथ काम करने के लिए डिज़ाइन किया गया है, और कम्प्यूटेशनली भारी कार्यों को Wasm पर ऑफलोड करके एक-दूसरे की ताकतों का लाभ उठाता है।

## 3. गणितीय चुनौती: मैंडेलब्रॉट सेट की गणना

इस लेख में, हम एक ड्राइंग एल्गोरिदम लागू करने के लिए C++ और [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) का उपयोग करेंगे जो CPU पर भारी भार डालता है: "मैंडेलब्रॉट सेट (Mandelbrot set)"।
...
Emscripten एक LLVM-आधारित कंपाइलर टूलचेन है और C/C++ कोड को [WebAssembly](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में संकलित करने के लिए वास्तविक मानक है। यह एक शक्तिशाली रनटाइम प्रदान करता है जो ब्राउज़र API (Web API) का उपयोग करके POSIX सिस्टम कॉल का अनुकरण (emulate) करता है।
...
JavaScript की ओर से, हम Emscripten द्वारा उत्पन्न ग्लू कोड (`mandelbrot.js`) को लोड करते हैं और इसे [WebAssembly](https://kenji.blog/hi/p/webassembly-wasm-current-future/) API का उपयोग करके इस प्रकार कॉल करते हैं:
...
उत्पन्न पैकेज को JavaScript से आयात करें। `wasm-bindgen` के लिए धन्यवाद, [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) का `Vec<i32>` स्वचालित रूप से JavaScript के `Int32Array` में परिवर्तित हो जाता है (पॉइंटर संचालन को छिपाना)।
...
[WebAssembly](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में सबसे महत्वपूर्ण अवधारणाओं में से एक "लीनियर मेमोरी (Linear Memory)" है। [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) कोड सीधे होस्ट (ब्राउज़र) मेमोरी स्पेस तक नहीं पहुंच सकता है; इसके बजाय, इसे एक पृथक, विशाल `ArrayBuffer` आवंटित किया जाता है। यही लीनियर मेमोरी है।
...
पूर्णांक (Integers) और फ्लोटिंग-पॉइंट संख्याएँ (`i32`, `i64`, `f32`, `f64`) को [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) फ़ंक्शंस में सीधे मानों के रूप में पास किया जा सकता है। हालाँकि, जटिल प्रकार जैसे स्ट्रिंग, एरे और संरचनाओं को सीधे Wasm फ़ंक्शन सिग्नेचर के रूप में पास नहीं किया जा सकता है।

**Emscripten के मामले में**:
1. JS साइड पर `Module._malloc` को कॉल करें ताकि [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) साइड पर लीनियर मेमोरी क्षेत्र आवंटित किया जा सके।
...
[WebAssembly](https://kenji.blog/hi/p/webassembly-wasm-current-future/) को नेटिव-जैसी गति से निष्पादित किया जा सकता है, लेकिन "JavaScript और WebAssembly (Interop) के बीच संचार" में ओवरहेड होता है।

* **कॉलिंग ओवरहेड (Calling Overhead)**: JavaScript इंजन के लिए [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) फ़ंक्शंस को कॉल करने की स्विचिंग लागत। हालाँकि अब इसे बहुत अधिक अनुकूलित किया गया है, ऐसे डिज़ाइनों से बचा जाना चाहिए जहाँ एक बहुत ही हल्के फ़ंक्शन को प्रति फ्रेम दसियों हज़ार बार कॉल किया जाता है।
...
उदाहरण के लिए, गेम इंजन और भौतिकी इंजन में, यह आर्किटेक्चर आम है जहाँ सभी अवस्थाओं (states) को [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) की लीनियर मेमोरी के भीतर रखा जाता है, और JavaScript केवल प्रत्येक फ्रेम को "अपडेट" ट्रिगर करने और स्क्रीन रेंडरिंग (WebGL/WebGPU API कॉलिंग) के लिए ज़िम्मेदार होता है।

## 8. [WebAssembly](https://kenji.blog/hi/p/webassembly-wasm-current-future/) बाइनरी फॉर्मेट (.wasm) की शारीरिक रचना (Anatomy)

अब, आइए कंपाइलर द्वारा आउटपुट की गई `.wasm` फ़ाइल की आंतरिक संरचना पर एक नज़र डालें। [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) बाइनरी विस्तारशीलता (extensibility) और पार्सिंग गति को महत्व देने के लिए "सेक्शन (sections)" नामक तार्किक ब्लॉकों के संग्रह से बनी होती है।
...
* **इम्पोर्ट सेक्शन (Import Section)**: JavaScript वातावरण द्वारा [Wasm](https://kenji.blog/hi/p/webassembly-wasm-current-future/) को प्रदान किए गए फ़ंक्शंस और मेमोरी की सूची। उदाहरण के लिए, यदि आप C++ से `console.log` को कॉल करते हैं, तो इसे यहाँ घोषित किया जाएगा।
...
* **डेटा सेक्शन (Data Section)**: C++ या [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) कोड में परिभाषित स्थिर स्ट्रिंग लिटरल और इनिशियलाइज़ेशन डेटा इस सेक्शन से लीनियर मेमोरी में लोड किए जाते हैं।
