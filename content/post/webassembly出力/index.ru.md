...
В современной веб-разработке JavaScript (и TypeScript) долгое время удерживал позицию единственного языка программирования, работающего в браузере. Однако в последние годы растет потребность в выполнении более сложных вычислений, таких как обработка изображений, кодирование видео, 3D-игры и физические симуляции, непосредственно в самом браузере. Именно здесь на сцену выходит **[WebAssembly](https://kenji.blog/ru/p/webassembly-wasm-current-future/) (сокращенно [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/))**.

В этой статье мы начнем с основ WebAssembly и подробно объясним внутреннюю структуру и шаги для компиляции Wasm из двух мощных языков системного программирования: C++ (с использованием Emscripten) и [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) (с использованием `wasm-pack`), а также их интеграцию со средой JavaScript. Кроме того, мы глубоко погрузимся в управление границами памяти, передачу сложных данных (таких как строки и массивы), накладные расходы на производительность и бинарный формат [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/) (`.wasm`).
...
[Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/) не заменяет JavaScript. Он разработан для работы вместе с JavaScript, позволяя переносить ресурсоемкие задачи на Wasm и тем самым использовать сильные стороны каждой из технологий.

## 3. Математическая задача: Вычисление множества Мандельброта

В этой статье мы используем алгоритм отрисовки «множества Мандельброта (Mandelbrot set)», создающий высокую нагрузку на CPU, и реализуем его на C++ и [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/).
...
Emscripten — это набор инструментов компилятора на базе LLVM, де-факто являющийся стандартом для компиляции кода C/C++ в [WebAssembly](https://kenji.blog/ru/p/webassembly-wasm-current-future/). Он предоставляет мощную среду выполнения, которая эмулирует системные вызовы POSIX с помощью браузерных API (Web API).
...
На стороне JavaScript мы загружаем связующий код (`mandelbrot.js`), сгенерированный Emscripten, и используем API [WebAssembly](https://kenji.blog/ru/p/webassembly-wasm-current-future/) для вызова функции, как показано ниже:
...
[Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) предлагает первоклассную поддержку WebAssembly, а инструменты `wasm-bindgen` и `wasm-pack` обеспечивают глубокую интеграцию между JavaScript и Rust. В то время как Emscripten использует подход «привнесения огромной среды выполнения C/C++ в браузер», [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) с `wasm-pack` использует подход «генерации только необходимого минимума привязок (связующего JS-кода)».
...
Импортируйте сгенерированный пакет из JavaScript. Благодаря `wasm-bindgen`, тип [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) `Vec<i32>` автоматически конвертируется в JavaScript `Int32Array` (скрывая операции с указателями).
...
Одна из самых важных концепций в [WebAssembly](https://kenji.blog/ru/p/webassembly-wasm-current-future/) — это «линейная память (Linear Memory)». Код [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/) не может напрямую обращаться к адресному пространству хоста (браузера); вместо этого ему выделяется один огромный изолированный `ArrayBuffer`. Это и есть линейная память.
...
Целые числа и числа с плавающей запятой (`i32`, `i64`, `f32`, `f64`) можно напрямую передавать в функции [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/) как значения. Однако сложные типы, такие как строки, массивы или структуры, не могут быть напрямую переданы в качестве сигнатуры функции Wasm.

**В случае Emscripten**:
1. Вызовите `Module._malloc` на стороне JS, чтобы выделить область линейной памяти [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/).
...
**В случае wasm-bindgen ([Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/))**:
Сложный процесс управления памятью, описанный выше, полностью скрыт в автоматически сгенерированном связующем коде (обертке JS). Если передать простой объект `String` или `Array` со стороны JS в функцию [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/), за кулисами автоматически выполняются выделение буфера (эквивалент `malloc`), копирование, передача указателей и освобождение памяти.

## 7. Накладные расходы производительности и оптимизация

[WebAssembly](https://kenji.blog/ru/p/webassembly-wasm-current-future/) может выполняться со скоростью, близкой к нативной, но существует накладной расход на «пересечение границы между JavaScript и WebAssembly (Interop)».

* **Накладные расходы вызова**: Затраты на переключение, когда движок JavaScript вызывает функцию [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/). Хотя сейчас это значительно оптимизировано, следует избегать архитектуры, в которой очень легкие функции вызываются десятки тысяч раз за кадр.
...
Например, в игровых движках и движках физических расчетов распространена архитектура, в которой все состояния хранятся в линейной памяти [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/), а JavaScript отвечает только за триггер обновления («обновить на каждом кадре») и за отрисовку экрана (вызов API WebGL/WebGPU).

## 8. Анатомия бинарного формата [WebAssembly](https://kenji.blog/ru/p/webassembly-wasm-current-future/) (.wasm)

Здесь мы рассмотрим внутреннюю структуру файла `.wasm`, генерируемого компилятором. Бинарный файл [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/) состоит из набора логических блоков, называемых «секциями» (sections), с упором на расширяемость и скорость парсинга.
...
* **Import Section (Секция импорта)**: Список функций и памяти, предоставляемых среде [Wasm](https://kenji.blog/ru/p/webassembly-wasm-current-future/) из JavaScript. Например, если вы вызываете `console.log` из C++, это объявляется здесь.
...
* **Data Section (Секция данных)**: Статические строковые литералы и данные инициализации, определенные в коде C++ или [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/), загружаются из этой секции в линейную память.
