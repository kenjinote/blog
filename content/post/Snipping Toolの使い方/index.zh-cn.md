...
## 什么是 Snipping Tool
Snipping Tool 是 Windows 自带的屏幕截图工具。您可以截取屏幕的一部分并保存，或者捕获整个屏幕，还可以进行录屏。

## Snipping Tool 启动方法

主要可以通过以下 3 种方法启动。

- 按 `Win键`，输入 `Snipping Tool` 并按 `Enter键`。
- 按 `Win键` + `Shift` + `S键`。
