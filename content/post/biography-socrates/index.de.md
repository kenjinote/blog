...
```mermaid
graph TD
    Oracle["Delphic Oracle (Orakel von Delphi)"] -->|Niemand ist weiser als Sokrates| Soc["Socrates (Sokrates)"]
    
    Soc -->|Sokratische Methode / Mäeutik| Citizens["Athenian Citizens (Athener Bürger)"]
    Citizens -->|Verärgert über die Aufdeckung von Unwissenheit| Accusers["Meletus, Anytus, Lycon (Ankläger)"]
    
    Accusers -->|Gottlosigkeit und Verderbnis der Jugend| Trial["Trial and Death (Prozess und Tod)"]
    Soc --> Trial
    
    Soc -->|Meister-Schüler-Beziehung| Plato["Plato (Platon)"]
    Soc -->|Meister-Schüler-Beziehung| Xenophon["Xenophon (Xenophon)"]
    Soc -->|Meister-Schüler-Beziehung| Antisthenes["Antisthenes (Antisthenes)"]
    
    Plato -->|Ideenlehre / Akademie| Aristotle["Aristotle (Aristoteles)"]
    Aristotle -->|Systematisierung der westlichen Philosophie| WesternPhil["Western Philosophy (Westliche Philosophie)"]
```
