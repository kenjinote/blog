...
Terraform — это инструмент IaC с открытым исходным кодом, разработанный компанией HashiCorp на языке [Go](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/). Он позволяет конфигурировать и управлять любыми API в виде кода, от облачной инфраструктуры до настроек SaaS.
...
Например, коэффициент доступности системы в архитектуре Multi-AZ ([Availability](https://kenji.blog/ru/p/cap-theorem-distributed-systems-tradeoff/) Zone) можно выразить математической моделью.
...
2. **Жесткое кодирование учетных данных** : Прямое указание ключей доступа и секретных ключей в коде. Используйте переменные окружения или роли IAM (например, интеграцию [OIDC](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/)).
