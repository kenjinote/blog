...
在机器学习的语境下，为了在多元回归分析中稳定且高速地计算求出最优参数的“[最小二乘法](https://kenji.blog/p/method-of-least-squares/)”，常常会用到QR分解。直接求解正规方程（$A^T A \mathbf{x} = A^T \mathbf{b}$）的方法，在实际应用中因为矩阵 $A^T A$ 的条件数容易恶化，对数值误差极为敏感，因此常规的做法是将其分解为 $A=QR$，然后通过回代求解 $R \mathbf{x} = Q^T \mathbf{b}$。
