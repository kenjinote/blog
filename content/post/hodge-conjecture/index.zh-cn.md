...
维度为 k 的代数闭链，通过庞加莱对偶性 ([Poincaré](https://kenji.blog/zh-cn/p/poincare/) Duality)，确定了 X 的 2k 次上同调群中的一个元素。重要的是，由代数子流形确定的上同调类，在霍奇分解中只出现在特定的成分里。具体来说，余维度（总维度减去子流形的维度）为 p 的代数子流形确定的上同调类，属于 H^{p,p}(X) 这个成分。
...
最简单易懂的例子之一是1维的复流形，即**黎曼面** ([Riemann](https://kenji.blog/zh-cn/p/riemann/) Surface)。其中，亏格（孔的数量）为1的环面（甜甜圈形状），在代数几何中被称为**椭圆曲线** (Elliptic Curve)。
