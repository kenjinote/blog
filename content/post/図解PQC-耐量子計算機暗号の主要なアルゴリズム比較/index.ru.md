...
2. **Цифровые подписи ([Digital Signature](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)s)**: Механизм для проверки отсутствия фальсификации данных и предотвращения подмены отправителя (аутентичность) при обновлениях программного обеспечения или в цифровых сертификатах.
...
2. **Криптография на основе хешей (Hash-based [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)**:
...
3. **Криптография на основе кодов (Code-based [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)**:
...
4. **Многомерная полиномиальная криптография (Multivariate Polynomial [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)**:
...
5. **Криптография на основе изогений (Isogeny-based [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)**:
...
В эру PQC стандартом станет не прямой подход, как в [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/), где «клиент создает симметричный ключ, шифрует его открытым ключом сервера и отправляет», а концепция инкапсуляции, называемая KEM.
...
Главными преимуществами Kyber являются его потрясающая **скорость обработки ** и ** умеренный размер ключа**. Для Kyber768 размер открытого ключа составляет 1184 байта, а размер шифротекста — 1088 байт. И хотя они больше по сравнению с [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)-3072 (размер ключа около 384 байт), они помещаются в MTU (Maximum Transmission Unit) современных интернет-сетей без фрагментации пакетов и практически не влияют на сетевые задержки.
...
Реалистичным и рекомендуемым подходом для смягчения риска этой неопределенности является « **Гибридная криптография (Hybrid [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)** ».
...
graph TD
    ...
    B --> C
    C -->|"#quot;Вход: (симметричный ключ X25519)#quot;#124;#124; (симметричный ключ Kyber)"| D["Защищенный ключ связи (AES-256 / ChaCha20)"]
...
---
*References:*
* *NIST Post-Quantum [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy Standardization Program*
* *FIPS 203: Module-Lattice-Based Key-Encapsulation Mechanism Standard*
* *FIPS 204: Module-Lattice-Based [Digital Signature](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/) Standard*
