...
It is extremely dangerous to think that "there is no problem because the full-scale completion of quantum computers is still decades away." This is because an attack method called **Store Now, Decrypt Later (SNDL)** is already a real threat. This is an attack where malicious states or hacker organizations save a massive amount of currently encrypted communication data (such as TLS traffic) in storage and decrypt all of it the moment a powerful quantum computer becomes available in the future. [State](https://kenji.blog/en/p/iac-infrastructure-as-code-terraform/) secrets, infrastructure information, and medical data that need long-term protection are already exposed to this threat.
