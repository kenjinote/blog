...
```mermaid
graph LR
    subgraph "Movement during discharge (while using a smartphone)"
        Negative["Negative electrode (Graphite)"] -->|"Electrons (e-) pass through the conducting wire"| Device(("Smartphone"))
        Device -->|"Electrons (e-)"| Positive["Positive electrode (Metal oxide)"]
        Negative -.->|"Lithium ions (Li+) pass through the electrolyte"| Positive
    end
```
