...
```mermaid
graph LR
    subgraph "Movimiento durante la descarga (usando el teléfono inteligente)"
        Negative["Electrodo negativo (Grafito)"] -- "Los electrones (e-) pasan a través del cable" --> Device(("Teléfono inteligente"))
        Device -- "Electrones (e-)" --> Positive["Electrodo positivo (Óxido metálico)"]
        Negative -. "Los iones de litio (Li+) pasan a través del electrolito" .-> Positive
    end
```
