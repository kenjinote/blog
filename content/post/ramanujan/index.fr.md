...
sequenceDiagram
    ...
    participant C as "Université de Cambridge"
    
    R->>H: "Envoie une lettre contenant des théorèmes"
    Note over H: "Lit la lettre et est stupéfait"
    H->>R: "Reconnaît le talent et envoie une invitation"
    R->>C: "Quitte l'Inde et voyage en Angleterre"
    Note over R,C: "Début de la recherche conjointe"
```
...
```mermaid
graph TD
    A["Intuition de Ramanujan"] --> B["Théorie Analytique des Nombres"]
    A --> C["Séries Infinies et Fractions Continues"]
    A --> D["Fausses Fonctions Thêta"]
    
    B --> E["Formule de Partition"]
    B --> F["Nombre Premier de Ramanujan"]
    
    C --> G["Calcul Rapide de Pi"]
    
    D --> H["Physique Moderne et Théorie des Cordes"]
```
