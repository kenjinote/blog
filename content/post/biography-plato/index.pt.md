---
title: "A Vida e Filosofia de Platão: O Grande Pensador que Lançou as Bases do Pensamento Ocidental"
...
```mermaid
graph TD
    S["Sócrates (Professor)"] -->|Método de Diálogo / Investigação Ética| P["Platão"]
    P -->|Teoria das Ideias / Rei Filósofo| A["Aristóteles (Aluno)"]
    P -->|Fundação| AC["Academia (Escola)"]
    AC -->|Investigação Académica| A
    P -.->|Influência na Posteridade| NP["Neoplatonismo (Plotino, etc.)"]
    NP -.->|Influência Ideológica| CT["Teologia Cristã (Agostinho, etc.)"]
    P -.->|Influência Ideológica| MP["Filosofia Moderna (Descartes, Kant, etc.)"]
```
