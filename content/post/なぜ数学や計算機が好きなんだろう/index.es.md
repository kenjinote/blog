---




title: '¿Por qué me gustan las matemáticas y las computadoras? Pensando en su encanto y las razones de su atractivo'
slug: "なぜMatemáticasや計算機が好きなんだろう"
date: 2022-09-23T01:56:52+09:00
tags: ["Miscelánea"]
draft: false
image: "img.webp"
categories: ["Matemáticas, Criptografía, Cuántica"]
description: 'Consideraciones sobre por qué me atraen las matemáticas y las computadoras. Hablamos de su profundo encanto: son justas para todos, las reglas son claras, la respuesta derivada es siempre correcta y siempre hay problemas sin resolver con la dificultad adecuada.'
---





## He pensado en por qué me gustan las matemáticas y las computadoras

### Igualdad para todos
Siempre devuelve el mismo resultado para la misma entrada, sin importar la persona, la posición o el lugar.
### Reglas claras
Las reglas están claramente establecidas, y no hay absurdos ni injusticias.
### Las respuestas y teorías deducidas son eternamente correctas
No hay que preocuparse de que una respuesta ya calculada sea modificada más tarde.
### Tienen una dificultad adecuada
Siempre hay problemas sin resolver y siempre se puede asumir el desafío de resolverlos.
