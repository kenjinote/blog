...
這與現代[物件導向程式設計](/zh-tw/p/object-oriented-programming-oop-solid-principles/)中的「單一職責原則（SRP）」完全吻合。即一個類別或模組應該只有一個引起它變化的原因。
...
將巨大的單體應用拆分為一組可獨立部署的小型服務集合的[微服務架構](/zh-tw/p/microservices-architecture-bff-api-gateway/)。這可以說是Unix哲學的放大版：將「做好一件事」的程式群透過HTTP或gRPC等通用協定（現代版的管道）連接起來。
...
函數式程式設計中的函數組合（將一個函數的輸出作為另一個函數的輸入）與Unix管道的概念在數學上具有相似性。大數據處理中的Apache Kafka等串流處理，也是將文本流概念應用到[分散式系統](/zh-tw/p/cap-theorem-distributed-systems-tradeoff/)中的產物。
...
在我們今後設計新系統時，將一次又一次地回歸到Ken Thompson等人留下的簡單而美麗的哲學中。無論是編寫小型腳本，還是構建全球規模的[分散式系統](/zh-tw/p/cap-theorem-distributed-systems-tradeoff/)，Unix哲學都將永遠是指引我們走向正確方向的指南針。
