---
title: 'Взлом шифрования RSA? Подробное математическое объяснение GNFS и алгоритма Шора'
...
description: 'Проблема факторизации поддерживает безопасность RSA-шифрования. Мы детально и с помощью формул и концептуальных диаграмм объясним математические механизмы самого сильного классического алгоритма «Общего метода решета числового поля (GNFS)» и то, почему квантовый «алгоритм Шора» способен его победить.'
---

Информационная безопасность в современном интернет-обществе защищена криптографией с открытым ключом, такой как [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/). Основа безопасности RSA опирается на тот факт, что **"факторизация огромных составных чисел вычислительно крайне сложна"** .
