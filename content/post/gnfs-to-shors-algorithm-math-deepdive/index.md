...
現代のインターネット社会における情報セキュリティは、[RSA](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)暗号をはじめとする[公開鍵](https://kenji.blog/p/modern-cryptography-public-key-hash-signature/)暗号方式によって守られています。RSA暗号の安全性の根拠は、 **「巨大な合成数の素因数分解は計算量的に極めて困難である」** という事実に依存しています。
...
## 1. 古典計算における素因数分解のアプローチ：[フェルマー](https://kenji.blog/p/fermat/)の素因数分解法からの発展
...
GNFSが準指数関数的な時間を必要とするのに対し、1994年にピーター・ショアが発表した **「Shorのアルゴリズム」 ** は、[量子コンピュータ](https://kenji.blog/p/quantum-computing-shors-algorithm/)を用いることでこの問題を ** 「多項式時間」** で解くことができます。
