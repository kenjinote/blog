...
サーガには、中央のコントローラーがステップを指示する「オーケストレーション型」と、各サービスが自律的にイベントを購読して動作する「コレオ[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)ィ型」があります。Kafkaのようなイベントバスを用いたEDAでは、コレオグラフィ型サーガが非常に自然に実装できます。
