...
現代のソフトウェア開発において、コードの品質を維持しながら迅速に機能を追加していくことは至上命題です。特にC++のような複雑でパフォーマンスが要求される言語においては、[メモリ管理](https://kenji.blog/p/memory-management-garbage-collection/)のミスや未定義動作（Undefined Behavior）が致命的なバグにつながりやすく、テストの重要性は他の言語以上に高いと言えます。
...
そこで登場するのが **依存性の注入（[Dependency Injection](https://kenji.blog/p/design-patterns-modern-practices/): DI）** と、 **GoogleMock** を使ったインターフェースのモック化です。
