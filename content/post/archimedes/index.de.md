...
```mermaid
flowchart TD
    A["Kugel"] -->|"Volumen und Oberfläche"| B["Zylinder"]
    B -->|"Verhältnis"| C["Kugel ist 2/3 des Zylinders"]
    A -->|"Eingeschrieben in"| B
    %% Dies ist ein Diagramm, das die Beziehung zwischen einer Kugel und einem Zylinder zeigt
```
...
```mermaid
flowchart LR
    A["Objekt ins Wasser tauchen"] --> B["Wasser verdrängen"]
    B --> C["Gewicht des verdrängten Wassers = Auftrieb"]
    C --> D["Objekt wird leichter"]
    %% Prozess des Archimedischen Prinzips
```
