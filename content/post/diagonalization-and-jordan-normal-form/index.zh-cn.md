...
在这里，$D$ 的对角元素是 $A$ 的 **[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)** $\lambda_i$，而 $P$ 的各个列向量是对应的 **特征向量** $\mathbf{v}_i$。
...
**步骤1：计算[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)**
...
因此，[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)为 $\lambda = 2$ (代数重数为2) 和 $\lambda = 9$。
...
作为特征方程之解的[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)，具有 **代数重数** （作为方程解的重根数量）和 **几何重数** （对应特征空间的维数，即线性无关特征向量的数量）。在数学上，以下关系总是成立：
...
这个矩阵的[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)是 $\lambda = 1$ (代数重数为2)，但在求特征空间时：
...
若尔当标准型是由在对角线上排列的、被称为 **若尔当块** 的矩阵块所组成，其对角线上是[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)，而正上方的一个元素是 $1$。
...
与对角化非常相似的一种矩阵分解方法是 **[奇异值分解](/zh-cn/p/singular-value-decomposition/) (Singular Value Decomposition: [SVD](/zh-cn/p/singular-value-decomposition/))**。这两种方法在目的和适用范围上有所不同。

对角化 $A = PDP^{-1}$ 仅适用于方阵，在计算矩阵的“重复应用（求幂）”或“指数函数”时极为有用。

另一方面，[奇异值分解](/zh-cn/p/singular-value-decomposition/) $A = U \Sigma V^*$ 可应用于任意的 $m \times n$ 矩阵。这里，$U$ 和 $V$ 都是酉矩阵，$\Sigma$ 则是对角线上排列着非负实数（奇异值）的矩阵。[奇异值分解](/zh-cn/p/singular-value-decomposition/)将矩阵所表示的变换分解为“旋转”、“缩放”、“旋转”三个步骤，广泛用于数据压缩和伪逆矩阵的计算。
...
例如，作为谷歌PageRank算法基础的转移概率矩阵研究等，在分析现实世界复杂的网络和随机过程时，矩阵[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)和对角化的概念是不可或缺的。

## 在量子力学中对角化的意义

在物理学，尤其是量子力学中，矩阵的对角化与“观测”的概念密切相关。在量子力学中，物理量（例如能量、动量等）表现为埃尔米特矩阵。埃尔米特矩阵的一个重要性质是：“必定拥有实数[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)，并且可通过酉矩阵对角化”。
...
在现代，使用计算机进行这些计算是很普遍的。以下是一个使用 Python 计算矩阵[特征值](/zh-cn/p/eigenvalues-and-eigenvectors/)和正交变换的示例。
