...
透過將[費馬小定理](/zh-tw/p/fermats-little-theorem/)推廣的「尤拉定理」，在數學上證明了這樣的解密必定能還原回原本的明文。要讓攻擊者從 $n$ 反推出 $p$ 和 $q$（即質因數分解），即使使用現代的超級電腦，在現實時間內也是不可能的。
...
然而，技術的進步不會停止。目前，潛藏著能高速解開作為RSA密碼根基的質因數分解可能性的「量子電腦」，其開發正在持續進行中。一旦彼得·秀爾（Peter Shor）提出的「秀爾演算法（[Shor's algorithm](/zh-tw/p/shors-algorithm-and-rsa-breaking/)）」得以實現，據說現今所有的公開金鑰密碼都將被破解。
