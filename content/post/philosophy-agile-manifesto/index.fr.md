...
```mermaid
graph TD
    A["Méthodes traditionnelles (Cascade)"]
    B["Planification stricte et séparation des phases"]
    C["Coûts d'adaptation au changement énormes"]
    D["Manifeste Agile (2001)"]
    E["Privilégier les individus et leurs interactions"]
    F["Privilégier les logiciels opérationnels"]
    G["Privilégier la collaboration avec les clients"]
    H["Privilégier l'adaptation au changement"]

    A -- "Caractéristiques" --> B
    B -- "Résultat" --> C
    C -- "Demande de changement de paradigme" --> D
    D -- "Valeur fondamentale 1" --> E
    D -- "Valeur fondamentale 2" --> F
    D -- "Valeur fondamentale 3" --> G
    D -- "Valeur fondamentale 4" --> H
```
...
```mermaid
graph LR
    A["Malentendus sur l'Agile"]
    B["Planification/Documentation inutiles"]
    C["Scrum = Agile"]
    D["Simple accélération"]

    E["Vérités sur l'Agile"]
    F["Planification continue et documentation appropriée"]
    G["Pratique d'un état d'esprit et d'une philosophie"]
    H["Capacité d'adaptation et maximisation de la valeur"]
