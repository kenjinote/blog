...
```mermaid
graph TD
    A["Konventionelle Methode (Wasserfall)"]
    B["Strikte Planung und Trennung der Phasen"]
    C["Enorme Kosten für Reaktionen auf Änderungen"]
    D["Agiles Manifest (2001)"]
    E["Fokus auf Individuen und Interaktionen"]
    F["Fokus auf funktionierende Software"]
    G["Fokus auf Zusammenarbeit mit dem Kunden"]
    H["Fokus auf Reagieren auf Veränderung"]

    A -- "Eigenschaften" --> B
    B -- "Ergebnis" --> C
    C -- "Forderung nach Paradigmenwechsel" --> D
    D -- "Wert 1" --> E
    D -- "Wert 2" --> F
    D -- "Wert 3" --> G
    D -- "Wert 4" --> H
...
```mermaid
graph LR
    A["Missverständnisse über Agile"]
    B["Pläne/Dokumente nicht nötig"]
    C["Scrum = Agile"]
    D["Reine Beschleunigung"]

    E["Die Wahrheit über Agile"]
    F["Kontinuierliche Planung und angemessene Dokumentation"]
    G["Praxis von Mindset und Philosophie"]
    H["Anpassungsfähigkeit an Änderungen und Wertmaximierung"]

    A -- "Missverständnis 1" --> B
    A -- "Missverständnis 2" --> C
    A -- "Missverständnis 3" --> D
