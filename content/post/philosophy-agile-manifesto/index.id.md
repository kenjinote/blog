...
```mermaid
graph TD
    A["Metode Konvensional (Waterfall)"]
    B["Perencanaan Kaku dan Pemisahan Fase"]
    C["Biaya Adaptasi Perubahan Sangat Besar"]
    D["Agile Manifesto (2001)"]
    E["Mengutamakan Individu dan Interaksi"]
    F["Mengutamakan Perangkat Lunak Berfungsi"]
    G["Mengutamakan Kolaborasi Pelanggan"]
    H["Mengutamakan Tanggap terhadap Perubahan"]

    A -- "Ciri-ciri" --> B
    B -- "Hasil" --> C
    C -- "Tuntutan Perubahan Paradigma" --> D
    D -- "Nilai Utama 1" --> E
    D -- "Nilai Utama 2" --> F
    D -- "Nilai Utama 3" --> G
    D -- "Nilai Utama 4" --> H
...
```mermaid
graph LR
    A["Kesalahpahaman Agile"]
    B["Tidak Butuh Rencana/Dokumen"]
    C["Agile = Scrum"]
    D["Sekadar Kecepatan"]

    E["Kebenaran Agile"]
    F["Perencanaan Berkelanjutan & Dokumen Cukup"]
    G["Praktik Pola Pikir dan Filosofi"]
    H["Adaptasi Perubahan & Maksimalisasi Nilai"]
