---
title: "O Carisma do Esteticismo: A Vida e a Filosofia de Oscar Wilde"
...
```mermaid
graph TD
    A["Oscar Wilde (1854-1900)"] -->|Influência| B["Esteticismo (Aestheticism)"]
    A -->|Obra-prima| C["O Retrato de Dorian Gray"]
    A -->|Sucesso teatral| D["A Importância de Ser Prudente"]
    
    A -->|Relação homoafetiva| E["Lord Alfred Douglas (Bosie)"]
    E -->|Conflito| F["Marquês de Queensberry"]
    F -->|Processo por difamação| A
    A -->|Prisão e condenação| G["Cárcere de Reading (1895-1897)"]
    
    G -->|Escrito na prisão| H["De Profundis"]
    H -->|Exílio após soltura| I["Morte em Paris (1900)"]
    
    I -->|Reavaliação histórica| J["Status de Ícone LGBTQ+"]
    C -->|Legado literário| K["Estética Decadentista e Simbolista"]
    D -->|Comédia de costumes| L["Impacto no Teatro Moderno"]
```
