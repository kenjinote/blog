...
隨機矩陣是指元素由隨機變數構成的矩陣。乍看之下，它不過是數字的隨機排列，但當矩陣的尺寸趨於無限大時，其[特徵值](/p/eigenvalues-and-eigenvectors/)的分佈會呈現出令人驚嘆的優美而普適的規律。從微觀世界的原子核、質數分佈之謎、金融市場的價格波動，乃至最前沿深度學習模型的訓練動力學，這一規律潛藏在截然不同的系統背後。
...
令人驚訝的是，這個極度簡化的隨機矩陣的[特徵值](/p/eigenvalues-and-eigenvectors/)間距分佈，與實際鈾原子核能階間距分佈驚人地一致。維格那進一步發現，在矩陣尺寸 $N$ 趨於無限大的極限下，[特徵值](/p/eigenvalues-and-eigenvectors/)的整體密度分佈會呈現半圓的形狀。這就是著名的「維格那半圓律（Wigner's semicircle law）」。
...
考慮一個尺寸為 $N \times N$ 的實對稱矩陣 $X$，其元素 $X_{ij}$ 是相互獨立、均值為0、方差為1的隨機變數。我們要求出縮放矩陣 $W = \frac{1}{\sqrt{N}}X$ 在極限（$N \to \infty$）時的[特徵值](/p/eigenvalues-and-eigenvectors/)分佈。

## 基於動差法的推導

為了分析[特徵值](/p/eigenvalues-and-eigenvectors/)的經驗分佈函數，我們計算分佈的第 $k$ 階動差 $m_k$。由於矩陣的跡（對角元素的和）等於[特徵值](/p/eigenvalues-and-eigenvectors/)之和，故：
...
長度為 $2m$ 的此類路徑（戴克路徑）的總數，由組合數學中著名的「[卡塔蘭數](/p/catalan-numbers/)（Catalan numbers）」 $C_m$ 給出：
...
有一天，在普林斯頓高等研究院的下午茶時間，蒙哥馬利向物理學家弗里曼·戴森談起了這個結果。戴森大為震驚。因為這個公式，與戴森自己推導出的 GUE（高斯酉系綜）的[特徵值](/p/eigenvalues-and-eigenvectors/)間距分佈完全一致。
...
這裡就是隨機矩陣理論大顯身手的地方。1999年，Bouchaud等和Laloux等分別獨立地將隨機矩陣理論應用到了金融市場的協方差矩陣中。他們將完全隨機的時間序列數據所得出的協方差矩陣[特徵值](/p/eigenvalues-and-eigenvectors/)分佈（馬爾琴科-巴斯圖爾分佈），與實際市場數據協方差矩陣的[特徵值](/p/eigenvalues-and-eigenvectors/)分佈進行了比較。

結果發現，市場數據的絕大部分[特徵值](/p/eigenvalues-and-eigenvectors/)（90%以上）都落在隨機矩陣理論預測的理論邊界內。這意味著這些僅僅是「雜訊」。另一方面，只有極少數遠超邊界的大[特徵值](/p/eigenvalues-and-eigenvectors/)，才包含了反映市場真實相關結構（如市場因子或行業因子）的有意義的資訊。

基於這一發現，人們開發出了一種透過過濾（將其歸零或替換為均值）對應雜訊的[特徵值](/p/eigenvalues-and-eigenvectors/)，來「清理」協方差矩陣的方法。這極大地提高了投資組合的表現和穩定性，目前已被許多量化基金作為標準技術使用。
...
當用隨機值初始化權重矩陣時，它本身就是一個隨機矩陣。利用隨機矩陣理論，可以嚴格分析信號方差在穿過各層時的演變過程，以及反向傳播中梯度的行為。例如，透過分析非線性激活函數對隨機矩陣譜（[特徵值](/p/eigenvalues-and-eigenvectors/)分佈）的影響，為Xavier初始化和He初始化等現代標準初始化方法提供了理論上的正當性。

## 海森（Hessian）矩陣的特徵值分佈

在理解訓練過程動力學時，分析表示損失函數曲率的海森矩陣是必不可少的。擁有數千萬乃至數千億參數的[LLM](/p/large-language-models-llm-transformer-prompt-engineering/)（大型語言模型）的海森矩陣極其龐大，直接研究其性質非常困難，但利用隨機矩陣理論，我們可以近似並預測其[特徵值](/p/eigenvalues-and-eigenvectors/)分佈。

研究表明，深度神經網路海森矩陣的[特徵值](/p/eigenvalues-and-eigenvectors/)分佈，是由一個主體（bulk，大量接近零的[特徵值](/p/eigenvalues-and-eigenvectors/)）和極少數巨大的離群值（outlier）組成的。主體部分可建模為包含雜訊的隨機矩陣（例如，資訊較少的方向），而離群值則表示與任務直接相關的重要訓練方向。對這種譜結構的理解，為改善優化演算法（如SGD、Adam等）的收斂性、以及優化學習率排程提供了極其有價值的見解。
...
執行這段程式碼，你會看到隨機生成的矩陣的[特徵值](/p/eigenvalues-and-eigenvectors/)呈現出優美的半圓形狀分佈。儘管單個矩陣的元素完全是隨機的，但整體卻能湧現出如此規律的法則，這正是隨機矩陣理論最大的魅力所在。

---

# 結語

在本文中，我們追溯了從核物理學起步的隨機矩陣理論，延伸到純數學、金融工程，甚至現代AI技術的宏大故事。看似毫不相干的複雜系統，在極限狀態下卻能用「隨機矩陣的[特徵值](/p/eigenvalues-and-eigenvectors/)」這一共同語言進行對話，揭示了自然界與數學那神秘而深邃的本質。
