...
中国剩余定理（Chinese Remainder Theorem，简称CRT）是数论中最重要且优美的定理之一。其起源可以追溯到3世纪至5世纪左右编纂的古代中国数学著作《孙子算经》。这个从古代朴素的算术问题开始的定理，在经过几千年后的现代，在我们日常使用的互联网安全通信所依赖的 **RSA加密** 等公钥加密技术中，发挥着不可或缺的作用。
...
3. 由于 $\gcd(M_i, m_i) = 1$，所以在模 $m_i$ 下存在 $M_i$ 的乘法逆元 $y_i$。即，使用扩展欧几里得算法等方法求出满足 $M_i y_i \equiv 1 \pmod{m_i}$ 的 $y_i$。
4. 最终的解 $x$ 由下式给出：
