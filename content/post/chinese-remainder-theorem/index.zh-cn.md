...
中国剩余定理（[Chinese Remainder Theorem](https://kenji.blog/p/chinese-remainder-theorem/)，简称CRT）是数论中最重要且优美的定理之一。其起源可以追溯到3世纪至5世纪左右编纂的古代中国数学著作《孙子算经》。这个从古代朴素的算术问题开始的定理，在经过几千年后的现代，在我们日常使用的互联网安全通信所依赖的 **RSA加密** 等公钥加密技术中，发挥着不可或缺的作用。
...
3. 由于 $\gcd(M_i, m_i) = 1$，所以在模 $m_i$ 下存在 $M_i$ 的乘法逆元 $y_i$。即，使用扩展[欧几里得算法](https://kenji.blog/p/euclidean-algorithm/)等方法求出满足 $M_i y_i \equiv 1 \pmod{m_i}$ 的 $y_i$。
4. 最终的解 $x$ 由下式给出：
...
当模数的位长减半（例如1024位）时，幂运算的成本大约会变为1/8。即使执行两次，总成本也只是大约1/4。利用RSA-CRT，能够使解密和签名生成的运算 **提速约4倍** 。对于智能手机和IC卡等计算资源有限的设备，这种加速是至关重要的。
...
整数情形下的“互素整数”，在多项式环中对应于“没有公共根（最大公约多项式为常数）的多项式”。这种多项式版本的CRT构成了拉格朗日插值法（Lagrange interpolation）的理论基础，并完全等同于唯一确定穿过给定多点的最低次多项式的算法。此外，这也是一种纠错码—— **里德-所罗门码** 的数学基石。
