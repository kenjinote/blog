...
3. 由於 $\gcd(M_i, m_i) = 1$，所以在模 $m_i$ 下存在 $M_i$ 的乘法逆元 $y_i$。即，使用擴展[歐幾里得](https://kenji.blog/zh-tw/p/euclid/)演算法等方法求出滿足 $M_i y_i \equiv 1 \pmod{m_i}$ 的 $y_i$。
4. 最終的解 $x$ 由下式給出：
...
整數情形下的「互質整數」，在多項式環中對應於「沒有公共根（最大公因式為常數）的多項式」。這種多項式版本的CRT構成了拉格朗日插值法（[Lagrange](https://kenji.blog/zh-tw/p/lagrange/) interpolation）的理論基礎，並完全等同於唯一確定穿過給定多點的最低次多項式的演算法。此外，這也是一種錯誤更正碼—— **里德-所羅門碼** 的數學基石。
