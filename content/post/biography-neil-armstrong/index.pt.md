...
description: "\"Este é um pequeno passo para o homem, um salto gigante para a humanidade\" (That's one small step for man, one giant leap for mankind)."
---
...
```mermaid
flowchart TD
    A["Nascimento (1930, Ohio)"] --> B["Aviador Naval (Guerra da Coreia)"]
    B --> C["Piloto de Testes (X-15, etc.)"]
    C --> D["Astronauta da NASA (1962)"]
    D --> E["Gemini 8 (Primeiro acoplamento orbital)"]
    E --> F["Apollo 11 (1969, Primeiro pouso na Lua)"]
    F --> G["Professor na Universidade de Cincinnati (Formação da nova geração)"]
    G --> H["Membro da comissão de investigação do acidente do Challenger"]
    H --> I["Legado para o futuro (O Herói Silencioso)"]
```
