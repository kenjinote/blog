...
## 1. 引言：什么是[卡塔兰数](https://kenji.blog/p/catalan-numbers/)？

在数学和计算机科学的世界中，我们经常会看到一种美妙的现象：多个看似完全不同的问题，实际上在背后拥有完全相同的结构。其中一个著名的例子就是 **[卡塔兰数](https://kenji.blog/p/catalan-numbers/)** (Catalan numbers)。

[卡塔兰数](https://kenji.blog/p/catalan-numbers/)以比利时数学家欧仁·查理·卡塔兰的名字命名，该数列如下所示：

$$ C_0 = 1, \quad C_1 = 1, \quad C_2 = 2, \quad C_3 = 5, \quad C_4 = 14, \quad C_5 = 42, \quad C_6 = 132, \quad C_7 = 429, \quad \dots $$

这个数列作为各种组合问题的解频繁出现。在本文中，我们将介绍四个涉及[卡塔兰数](https://kenji.blog/p/catalan-numbers/)的著名例子（合法括号序列、二叉树、多边形三角剖分和迪克路径）。我们将剖析它们背后的递归结构，以了解为什么它们对应着完全相同的数列。此外，我们还将详细介绍使用动态规划 (DP) 的计算算法以及使用母函数的数学推导。

## 2. [卡塔兰数](https://kenji.blog/p/catalan-numbers/)出现的四个具体例子

### 例子1：合法括号序列 (Valid Parentheses)

在编程中，确保括号正确匹配至关重要。使用 $n$ 对括号 `()` 所能组成的“合法括号序列”的数量正是[卡塔兰数](https://kenji.blog/p/catalan-numbers/) $C_n$。
...
接下来，我们来看看大家熟悉的数据结构：二叉树。包含 $n$ 个内部节点的二叉树的形状数量也是[卡塔兰数](https://kenji.blog/p/catalan-numbers/) $C_n$。
...
[卡塔兰数](https://kenji.blog/p/catalan-numbers/)也出现在几何学中。通过在顶点之间绘制不相交的对角线，将凸 $(n+2)$ 边形分割成 $n$ 个三角形的方法数正是 $C_n$。

例如，当 $n = 3$ 时，我们考虑将一个五边形（$3+2=5$）进行三角剖分。通过绘制对角线形成3个三角形的方法恰好有 5 种。在这里，我们再次看到了数字 $C_3 = 5$。

### 例子4：迪克路径 (Dyck Paths)

[卡塔兰数](https://kenji.blog/p/catalan-numbers/)同样出现在网格路径问题中。在一个 $n \times n$ 的网格上，考虑从左下角 $(0, 0)$ 到右上角 $(n, n)$ 的最短路径，每次只能向右或向上移动一个单位。那些永远不会穿过对角线 $y = x$（即始终满足 $y \le x$）的路径数量为 $C_n$。这些被称为 **迪克路径** (Dyck path)。

如果我们用 `R` 表示向右移动，用 `U` 表示向上移动，该条件要求在路径的任何前缀中，`U` 的数量永远不超过 `R` 的数量。这与合法括号序列中 `(` 和 `)` 之间的关系完全等价。

## 3. 为什么它们是相同的？（背后的结构）

为什么这些看似无关的问题都会得出相同的[卡塔兰数](https://kenji.blog/p/catalan-numbers/)列？答案在于它们都共享着 **完全相同的递归结构**。

[卡塔兰数](https://kenji.blog/p/catalan-numbers/) $C_n$ 由以下递推关系定义：
...
[卡塔兰数](https://kenji.blog/p/catalan-numbers/)可以使用组合学符号通过一个非常简单的 **闭合公式** (Closed-form formula) 来表示：
...
### 4.2. 母函数 ([Generating Functions](https://kenji.blog/p/generating-functions/)) 方法

令[卡塔兰数](https://kenji.blog/p/catalan-numbers/)的母函数为 $C(x) = \sum_{n=0}^\infty C_n x^n$。
...
## 5. [卡塔兰数](https://kenji.blog/p/catalan-numbers/)的计算算法

当通过编程计算[卡塔兰数](https://kenji.blog/p/catalan-numbers/)时，主要有三种方法。
...
[卡塔兰数](https://kenji.blog/p/catalan-numbers/)列 $C_n$ 是一个迷人的数列，它统一地出现在众多看似不同的问题中，例如合法括号序列、二叉树形状、多边形三角剖分和迪克路径。这些问题得出相同数量的原因在于它们都体现了一个共同的递归结构：**“将整体分割为两个子问题并将它们组合”**。
