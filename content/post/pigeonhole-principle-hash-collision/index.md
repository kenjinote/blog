...
例えば、ビットコインなどのブロックチェーン技術でも使われている SHA-256 の出力は256ビットです。したがって、取り得るハッシュ値の種類は $2^{256}$ 通り（約 $1.15 \times 10^{77}$ ）となります。これは観測可能な宇宙に存在する原子の総数に迫るほど膨大な数ですが、あくまで **有限の数** です。
...
暗号技術以外の分野でも、鳩の巣原理とハッシュ衝突は重要なテーマです。プログラミングで頻繁に使用される 「 **ハッシュテーブル（連想配列や辞書型）** 」 がその代表例です。
