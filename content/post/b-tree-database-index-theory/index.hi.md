...
## 1. डेटाबेस इंडेक्स और B-[Tree](https://kenji.blog/hi/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/) का मिलन
...
मेमोरी में खोज के लिए, **बाइनरी सर्च ट्री** ([Binary Search](https://kenji.blog/hi/p/search-algorithms-linear-binary-hash-table-principles/) Tree: BST) और **रेड-ब्लैक ट्री** (Red-Black Tree) जैसे संतुलित बाइनरी सर्च ट्री $ O(\log N) $ की जटिलता के साथ तेज़ खोज को संभव बनाते हैं। हालाँकि, अगर इसे सीधे डिस्क पर डेटाबेस में लागू किया जाए, तो गंभीर समस्याएँ उत्पन्न होती हैं।
...
वास्तविक [RDBMS](https://kenji.blog/hi/p/rdbms-transaction-acid-isolation-level-lock/) (जैसे MySQL का InnoDB या PostgreSQL) में, B-[Tree](https://kenji.blog/hi/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/) के उन्नत संस्करण, **B+Tree** का उपयोग किया जाता है।
...
2. **लीफ नोड्स एक-दूसरे से लिंक्ड लिस्ट ([पॉइंटर्स](/hi/p/c-language-pointers-memory-management-stack-heap/)) के माध्यम से जुड़े होते हैं** । यह अनुक्रमिक (sequential) एक्सेस और रेंज क्वेरी (Range Query) को बहुत तेज़ बनाता है।

### 4.2 B+Tree को अपनाने का कारण

आंतरिक नोड्स से वास्तविक डेटा के [पॉइंटर्स](/hi/p/c-language-pointers-memory-management-stack-heap/) को हटाकर, एक आंतरिक नोड (पेज) में अधिक कुंजियों (keys) को पैक करना संभव हो गया है। इससे फैन-आउट (Fan-out) और भी बढ़ जाता है, जिससे पेड़ की ऊँचाई $ h $ कम रहती है और डिस्क I/O की संख्या कम हो जाती है।

इसके अलावा, SQL में अक्सर उपयोग की जाने वाली रेंज खोज जैसे `WHERE id BETWEEN 10 AND 100` में, B-[Tree](https://kenji.blog/hi/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/) को पेड़ को बार-बार ट्रैवर्स करना पड़ता है, लेकिन **B+Tree** के साथ, एक बार शुरुआत का लीफ नोड मिल जाने के बाद, आप केवल लीफ नोड के लिंक का पालन करके डेटा को लगातार पढ़ सकते हैं।
...
```
*(चित्र: B+[Tree](https://kenji.blog/hi/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/) की संरचना। लीफ नोड्स श्रृंखला (chain) के रूप में जुड़े हुए हैं)*
...
जैसा कि इस कार्यान्वयन से देखा जा सकता है, B-[Tree](https://kenji.blog/hi/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/) में इंसर्शन के दौरान आवश्यकतानुसार नोड्स को नीचे से ऊपर की ओर विभाजित (Split) किया जाता है, जिससे पेड़ पूरी तरह से संतुलित (Balanced) रहता है। इसके कारण, डेटा किसी भी क्रम में डाला जाए, खोज प्रदर्शन में कोई गिरावट नहीं आती है।
