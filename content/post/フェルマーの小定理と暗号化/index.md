...
RSA暗号をはじめとする多くの暗号アルゴリズムの安全性や正当性は、17世紀のフランスの数学者ピエール・ド・フェルマー（Pierre de Fermat）が発見した非常に美しく、かつ強力な定理に大きく依存しています。それが **フェルマーの小定理（Fermat's Little Theorem）** です。さらに、これを一般化したレオンハルト・[オイラー](https://kenji.blog/p/euler/)（Leonhard Euler）の定理も、暗号理論において決定的な役割を果たしています。
...
フェルマーの小定理は「素数 $p$」に関する定理ですが、これを「任意の正の整数 $n$」に一般化したのが、レオンハルト・[オイラー](https://kenji.blog/p/euler/)です。RSA暗号を理解するには、この拡張が不可欠です。
...
[オイラー](https://kenji.blog/p/euler/)は、フェルマーの小定理を次のように一般化しました。

> **[オイラー](https://kenji.blog/p/euler/)の定理 (Euler's Theorem)**
> 正の整数 $n$ と、それと互いに素な整数 $a$ に対して、次が成り立つ。
> $$ a^{\phi(n)} \equiv 1 \pmod n $$

もし $n$ が素数 $p$ であれば $\phi(p) = p - 1$ なので、これはフェルマーの小定理そのもの（$a^{p-1} \equiv 1 \pmod p$）になります。つまり、フェルマーの小定理は[オイラー](https://kenji.blog/p/euler/)の定理の特殊なケースに過ぎません。
...
フェルマーの小定理（および[オイラー](https://kenji.blog/p/euler/)の定理）の最も偉大な応用先が、1977年にRivest, Shamir, Adlemanの3人によって開発された **RSA暗号** です。
...
ここで、フェルマーの小定理（[オイラー](https://kenji.blog/p/euler/)の定理）が真価を発揮します。なぜ $C^d \pmod N$ が $M$ に戻るのでしょうか？
...
しかし、約300年後の1970年代、コンピュータネットワークの黎明期において、安全な通信プロトコルを確立するために不可欠な暗号技術として、フェルマーの定理は劇的な復活を遂げました。フェルマーの小定理に基づく素数判定技術と、[オイラー](https://kenji.blog/p/euler/)の定理に基づくRSA暗号は、現代のインターネットインフラを文字通り支えています。
