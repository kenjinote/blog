...
```mermaid
graph TD
    A["Ungarn unter Nazi-Besatzung (Ursprung der Überlebensstrategie)"] --> B["London School of Economics (LSE)"]
    B --> C["Philosophie von Karl Popper (Fallibilismus)"]
    C --> D["Theorie der Reflexivität (Reflexivity)"]
    D --> E["Gründung des Quantum Fund"]
    E --> F["Leerverkauf des Pfund 1992 (Schwarzer Mittwoch)"]
    C --> G["Gründung der Open Society Foundations (Philanthropie)"]
    F --> H["Weltweite Unterstützung der Demokratisierung und Sozialreformen"]
    G --> H
```
