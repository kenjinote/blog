...
在常规的快速排序中，总是采取固定策略，例如选择数组末尾的元素作为基准值。但是，在这种情况下，如果给定的是原本就已经排序好的数组，最坏[复杂度](/zh-cn/p/time-space-complexity-big-o-notation-examples/)会达到 $O(n^2)$ 。

在 **随机化快速排序** 中，会从数组中随机选择基准值。通过这样做，可以从数学上保证，对于任何输入数据，平均[复杂度](/zh-cn/p/time-space-complexity-big-o-notation-examples/)都能达到 $O(n \log n)$ 。输出的排序结果本身则始终是完全正确的。
