...
这是一种用于高速判定某个巨大数是否为素数的算法。在 RSA 密码等生成密钥时，需要数百位的素数，如果用确定性的试除法（依次除以 $2, 3, 5, \dots$ 的方法）来进行，那么直到宇宙寿命耗尽也无法结束。

在此，我们利用称为 **米勒-拉宾素性检验法** 的蒙特卡罗方法。
对于想要判定的数 $n$ ，随机选择一个基数 $a$ ，测试它是否满足基于[费马小定理](https://kenji.blog/p/fermats-little-theorem/)扩展的特定条件式。
...
作为排序算法代表的快速排序中，随机选择基准值（Pivot）的方法是拉斯维加斯方法的典型例子。
...
## 3. [蒙特卡罗方法与拉斯维加斯方法](https://kenji.blog/p/monte-carlo-and-las-vegas-algorithms/)的比较
