...
Это алгоритм, который быстро определяет, является ли огромное число простым. При генерации ключей в криптографии RSA и т.д. требуются простые числа длиной в сотни цифр, но если делать это методом пробного деления (последовательным делением на $2, 3, 5, \dots$), это не закончится и до конца жизни Вселенной.
