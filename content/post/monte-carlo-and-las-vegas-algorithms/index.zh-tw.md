...
在一般的快速排序中，通常會採取固定策略，例如總是選擇陣列最後一個元素作為基準點。但是，在這種情況下，如果給定一個原本就已經排序好的陣列，最壞計算[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)就會變成 $O(n^2)$。

在 **隨機快速排序** 中，會從陣列中隨機選擇基準點。這樣一來，對於任何輸入資料，其平均計算[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)在數學上都被保證為 $O(n \log n)$。輸出的排序結果本身也總是完全正確的。
