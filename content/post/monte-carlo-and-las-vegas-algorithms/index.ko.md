...
거대한 수가 소수인지 여부를 고속으로 판정하는 알고리즘입니다. [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 암호 등에서 키를 생성할 때 수백 자리의 소수가 필요해지는데, 이를 결정론적인 시험 나눗셈법($2, 3, 5, \dots$ 로 순서대로 나누어 가는 방법)으로 수행하면 우주의 수명이 다해도 끝나지 않습니다.

여기서 ** 밀러-라빈 소수 판정법 ** 이라는 몬테카를로법을 이용합니다.
판정하고 싶은 수 $n$ 에 대해 무작위로 기수 $a$ 를 선택하고, [페르마의 소정리](https://kenji.blog/ko/p/fermats-little-theorem/)의 확장에 기반한 특정 조건식을 만족하는지를 테스트합니다.
...
[정렬 알고리즘](https://kenji.blog/ko/p/sorting-algorithms/)의 대표격인 퀵 정렬에서 피벗(기준값)을 고르는 방법을 랜덤하게 하는 방법이 라스베이거스법의 전형적인 예시입니다.
...
## 3. [몬테카를로법과 라스베이거스법](https://kenji.blog/ko/p/monte-carlo-and-las-vegas-algorithms/)의 비교
