...
यह सुविधा जो "स्थानांतरित चर तक पहुंच को संकलन समय पर अक्षम कर देती है", उन कारणों में से एक है कि [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) C++ के `std::unique_ptr` से अधिक सुरक्षित क्यों है।
...
[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में उधार दो प्रकार के होते हैं:
- **अपरिवर्तनीय संदर्भ (Immutable Reference)**: `&T` (C++ के `const T&` के समान)
- **परिवर्तनीय संदर्भ (Mutable Reference)**: `&mut T` (C++ के `T&` के समान)

### बरो चेकर ([Borrow Checker](https://kenji.blog/hi/p/memory-management-garbage-collection/)) के कठोर नियम

[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) के कंपाइलर में एक अंतर्निहित "बरो चेकर" होता है जो संदर्भों की वैधता की पुष्टि करता है। बरो चेकर निम्नलिखित सख्त नियमों को लागू करता है:
...
इस नियम के माध्यम से, **डेटा रेस (Data Race) संकलन समय (compile time) पर पूरी तरह से समाप्त हो जाता है** । डेटा रेस तब होता है जब: ① दो या दो से अधिक पॉइंटर्स एक ही डेटा तक एक साथ पहुंचते हैं, ② कम से कम एक लिख रहा होता है (write operation), और ③ कोई सिंक्रनाइज़ेशन तंत्र नहीं होता है। [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) संकलन समय पर स्थिति ② को नष्ट करके डेटा रेस को रोकता है।
...
इस तरह, चूँकि [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) कंपाइलर स्तर पर "किसी मान को पढ़ते समय (अपरिवर्तनीय उधार) उसे बदलना (परिवर्तनीय उधार)" सख्त मना करता है, यूज़-आफ्टर-फ्री और इटरेटर अमान्यकरण जैसे घातक बग संकलन समय पर निश्चित रूप से पकड़े जाते हैं।
...
## 6. [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में साझा स्वामित्व: `Rc` और `Arc`

[Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) साझा स्वामित्व (Shared Ownership) भी प्रदान करता है जो C++ के `std::shared_ptr` के बराबर है, लेकिन प्रकारों को सिंगल-थ्रेडेड और मल्टी-थ्रेडेड उपयोग के लिए स्पष्ट रूप से अलग किया गया है।
...
दूसरी ओर, [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) में, आप अकेले `Arc<T>` के साथ **आंतरिक डेटा को संशोधित नहीं कर सकते** हैं। यदि संशोधन की आवश्यकता है, तो इसे म्यूटेक्स `Mutex<T>` के साथ जोड़ा जाना चाहिए।
...
उल्लेखनीय बात यह है कि [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) का `Mutex<T>` केवल एक लॉकिंग तंत्र नहीं है, बल्कि **"सुरक्षित किए जाने वाले डेटा को एक प्रकार (type) के रूप में समाहित करता है"** । यह संकलन स्तर पर "लॉक प्राप्त करना भूल जाना और डेटा तक पहुंचना" जैसी गलतियों को पूरी तरह से रोकता है। जब तक आप लॉक (`lock()`) प्राप्त नहीं करते हैं, तब तक आप आंतरिक डेटा तक पहुंचने के अधिकार (संदर्भ) प्राप्त नहीं कर सकते हैं।
