...
ميزة "جعل الوصول إلى المتغيرات بعد النقل مستحيلاً في وقت الترجمة" هي أحد الأسباب التي تجعل [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) أكثر أمانًا من `std::unique_ptr` في C++.
...
هناك نوعان من الاستعارة في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/):
...
### القانون الصارم لمدقق الاستعارة ([Borrow Checker](https://kenji.blog/ar/p/memory-management-garbage-collection/))

يحتوي مترجم [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) على "مدقق الاستعارة" الذي يتحقق من صحة المراجع. يفرض مدقق الاستعارة القواعد الصارمة التالية:
...
من خلال هذه القاعدة، يتم **منع سباق البيانات (Data Race) تمامًا في وقت الترجمة**. يحدث سباق البيانات عندما: (1) يصل مؤشران أو أكثر إلى نفس البيانات في وقت واحد، (2) يقوم واحد على الأقل بالكتابة، و (3) لا توجد آلية للمزامنة. تمنع [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) سباق البيانات مسبقًا عن طريق تدمير الشرط (2) في وقت الترجمة.
...
وبهذه الطريقة، تمنع [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) على مستوى المترجم "تعديل القيمة (استعارة قابلة للتغيير) أثناء قراءتها (استعارة غير قابلة للتغيير)"، مما يضمن اكتشاف الأخطاء القاتلة مثل الاستخدام بعد التحرير (Use-After-Free) وإبطال المكرر في وقت الترجمة بشكل مؤكد.
...
## 6. الملكية المشتركة في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/): `Rc` و `Arc`

توفر [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) أيضًا ملكية مشتركة تعادل `std::shared_ptr` في C++، ولكن هناك تمييز واضح في الأنواع للاستخدام في خيط واحد (Single Thread) أو عبر خيوط متعددة (Multi-thread).
...
من ناحية أخرى في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، لا يمكن لـ `Arc<T>` **تغيير البيانات الداخلية بمفرده**. إذا كانت التعديلات ضرورية، يجب دمجه مع كائن مزامنة `Mutex<T>`.
...
والجدير بالذكر أن `Mutex<T>` في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) ليس مجرد آلية قفل فحسب، بل **"يغلف البيانات التي يجب حمايتها كنوع"**. يمنع هذا التصميم بالكامل خطأ "الوصول إلى البيانات ونسيان القفل" على مستوى الترجمة. النظام مبني بحيث لا يمكنك الحصول على حق الوصول (مرجع) للبيانات الداخلية دون الحصول على القفل أولاً (`lock()`).
