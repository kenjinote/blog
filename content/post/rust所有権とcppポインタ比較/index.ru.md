...
В современном системном программировании баланс между производительностью и безопасностью памяти является вечной проблемой. C++ долгие годы господствовал в этой области как король, но в последние годы [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) начинает угрожать его позициям. Главная особенность [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) заключается в концепциях «владения» (Ownership) и «заимствования» (Borrowing), которые гарантируют безопасность памяти во время компиляции без использования сборщика мусора (GC).

В этой статье мы подробно сравним указатели C++ (сырые указатели, `std::unique_ptr`, `std::shared_ptr`) с моделью владения [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/), и с помощью примеров кода и диаграмм детально разберем, как компилятор [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) (borrow checker) предотвращает такие ошибки, как использование после освобождения (Use-After-Free) и состояние гонки данных (Data Race).

## 1. Основы управления памятью: Стек и Куча

Чтобы понять основы управления памятью, давайте сначала вспомним, как программа использует память. Области памяти в основном делятся на «стек» ([Stack](https://kenji.blog/ru/p/c-language-pointers-memory-management-stack-heap/)) и «кучу» ([Heap](https://kenji.blog/ru/p/c-language-pointers-memory-management-stack-heap/)).
...
### Эпоха сырых указателей (Raw [Pointer](https://kenji.blog/ru/p/c-language-pointers-memory-management-stack-heap/)s) и их проблемы
...
- **Висячий указатель (Dangling [Pointer](https://kenji.blog/ru/p/c-language-pointers-memory-management-stack-heap/))**: Обращение к указателю после освобождения памяти (после `delete`).
...
## 3. Владение (Ownership) в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/): Смена парадигмы

[Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) берет концепцию `std::unique_ptr` из C++ за основу спецификации языка и обладает еще более строгой «моделью владения».

### Три правила владения

Система владения в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) основана на трех предельно простых правилах:

1. **Каждое значение в [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) имеет переменную, которая называется его владельцем (owner).**
...
Именно эта функция, «делающая переменную недоступной на этапе компиляции после перемещения», является одной из причин, почему [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) безопаснее, чем `std::unique_ptr` в C++.
...
sequenceDiagram
    ...
    S1--x Heap: "Теряет владение (Инвалидируется)"
...
В [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) существует два типа заимствования:
...
В компилятор [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) встроен «borrow checker» (проверщик заимствований), который проверяет корректность ссылок. Borrow checker принудительно применяет следующие строгие правила:
...
### Защита на этапе компиляции в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/)

Давайте напишем точно такую же логику на [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/).
...
Таким образом, в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) на уровне компилятора запрещено «изменять значение (делать изменяемое заимствование) во время его чтения (неизменяемого заимствования)». Благодаря этому фатальные баги, такие как Use-After-Free или инвалидация итератора, гарантированно перехватываются на этапе компиляции.

```mermaid
graph LR
    A["Переменная v (Владелец)"] --> B["Массив в куче [1, 2, 3]"]
    C["Ссылка 'first' (&v[0])"] -.->|"Неизменяемое заимствование"| B
    A -->|"X #quot;Изменяемое заимствование запрещено!#quot;"| D["v.push(4)"]
    
    style C stroke:#00FF00,stroke-width:2px
    style D stroke:#FF0000,stroke-width:2px
```

## 6. Совместное владение в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/): `Rc` и `Arc`

В [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) также предусмотрено совместное владение, эквивалентное `std::shared_ptr` в C++, но оно четко разделено на типы для однопоточного и многопоточного использования.
...
С другой стороны, в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) **невозможно изменить внутренние данные** с помощью одного лишь `Arc<T>`. Если необходимо внести изменения, его нужно комбинировать с мьютексом `Mutex<T>`.
...
Особо стоит отметить, что `Mutex<T>` в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) — это не просто механизм блокировки, он **«инкапсулирует защищаемые данные как тип»**. Это позволяет на уровне компиляции полностью предотвратить такую ошибку, как «доступ к данным без предварительной блокировки». Без получения блокировки (`lock()`) просто невозможно получить права доступа (ссылку) к внутренним данным.
...
В то же время [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/), встраивая в компилятор правила владения (Ownership) и заимствования (Borrowing), обнаруживает подобные ошибки **на этапе компиляции**, а не во время выполнения. Твердая гарантия того, что «если код компилируется, то он безопасен для памяти», является главной причиной стремительного роста популярности [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) в системном программировании.
