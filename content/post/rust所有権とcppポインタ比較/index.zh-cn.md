...
这个“在编译时禁止访问移动后的变量”的特性，正是[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)比C++的`std::unique_ptr`更安全的原因之一。
...
[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)中的借用分为两种：
...
[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)编译器内置了“借用检查器”来验证引用的有效性。借用检查器强制执行以下严格规则：
...
就像这样，[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)在编译器级别禁止“在读取值（不可变借用）的过程中对其进行修改（可变借用）”，因此能够确保在编译时捕获Use-After-Free和迭代器失效等致命Bug。

```mermaid
graph LR
    A["变量 v（所有者）"] --> B["堆数组 [1, 2, 3]"]
    C["引用 'first' (&v[0])"] -.->|"不可变借用"| B
    A -->|"X #quot;可变借用被拒绝！#quot;"| D["v.push(4)"]
    
    style C stroke:#00FF00,stroke-width:2px
    style D stroke:#FF0000,stroke-width:2px
```

## 6. [Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)中的共享所有权：`Rc` 与 `Arc`

[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)也提供了相当于C++中`std::shared_ptr`的共享所有权，但根据单线程和多线程用途划分了明确的类型。
...
而在[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)中，仅靠`Arc<T>`**无法修改其内部的数据** 。如果需要修改，必须将其与互斥锁 `Mutex<T>` 结合使用。
...
值得特别注意的是，[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/)的`Mutex<T>`不仅是一个锁机制，更重要的一点是它 **“将受保护的数据包含在了类型内部”** 。通过这种方式，可以在编译级别彻底防止“忘记获取锁就访问数据”的错误。如果不获取锁（`lock()`），就无法获得内部数据的访问权（引用）。
