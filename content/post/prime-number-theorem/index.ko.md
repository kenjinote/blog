...
수학 분야에서 가장 아름다운 결과 중 하나가 **소수 정리** (Prime Number Theorem, PNT)입니다. 소수라는, 언뜻 보면 불규칙하고 무작위로 나타나는 수들이, 거시적으로 보면 놀라울 정도로 매끄러운 규칙성을 가지고 있음을 보여줍니다.
...
소수 정리의 역사는 18세기 후반으로 거슬러 올라갑니다. 불과 15세의 천재 수학자 카를 프리드리히 가우스는 소수표를 바라보다가, 소수의 출현 빈도가 로그 함수와 관련되어 있다는 것을 깨달았습니다. 같은 시기에 아드리앵마리 르장드르도 독립적으로 비슷한 가설을 세웠습니다.

하지만 그들은 이것을 엄밀하게 증명하는 데에는 이르지 못했습니다.

증명의 큰 진전은 1859년 베른하르트 리만의 획기적인 논문 '주어진 수보다 작은 소수의 개수에 관하여'에 의해 이루어졌습니다. 리만은 복소함수인 **제타 함수** $\zeta(s)$ 를 사용하여, 소수의 분포를 복소평면상의 문제로 변환한다는 전혀 새로운 접근법을 제시했습니다.
