---
title: "Nikolai Lobachevsky: 'Copernicus dari Geometri' yang Membuka Pintu Geometri Non-Euclidean"
...
Pada tahun 1807, ia memasuki Universitas Kazan yang baru didirikan sebagai siswa beasiswa. Awalnya, ia bercita-cita untuk belajar kedokteran, tetapi ia bertemu Martin Bartels (juga guru dari [Carl Friedrich Gauss](/id/p/gauss/)), seorang ahli matematika yang luar biasa, dan terpesona oleh pesona matematika yang mendalam. Di bawah bimbingan Bartels, Lobachevsky mengembangkan bakatnya yang luar biasa, memperoleh gelar master pada usia 21 tahun. Setelah itu, ia menaiki tangga akademik dengan kecepatan yang tidak biasa, menjadi profesor luar biasa pada usia 24 tahun.
...
```mermaid
graph TD
    A["Geometri Euclidean"] -->|Kebenaran mutlak selama 2000 tahun| B["Upaya untuk membuktikan postulat sejajar"]
    B -->|Tembok ketidakmungkinan pembuktian| C["Pergeseran paradigma Lobachevsky"]
    C -->|Banyak garis sejajar bisa ada| D["Konstruksi geometri non-Euclidean (geometri hiperbolik)"]
    D --> E["Kelahiran konsep kelengkungan ruang"]
    E --> F["Perkembangan ke Geometri Riemannian"]
    F --> G["Teori Relativitas Umum Einstein"]
    G --> H["Kosmologi modern dan pemahaman lubang hitam"]
```
...
```mermaid
graph LR
    subgraph "Diagram Korelasi Pengaruh"
        direction LR
        L["Lobachevsky"]
        E["Euclid"]
        G["Gauss"]
        ...
        L -->|Menyangkal postulat sejajar| E
        L ---|Melakukan penemuan yang sama secara independen| B
        G -->|Menghargai secara pribadi tetapi tidak mempublikasikan| L
        L -->|Memberikan dasar teoretis| R
        R -->|Memperluas geometri ruang| A
    end
```
