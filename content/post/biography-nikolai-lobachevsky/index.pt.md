---
title: "Nikolai Lobachevsky: O 'Copérnico da Geometria' que Abriu as Portas da Geometria Não-Euclidiana"
...
```mermaid
graph TD
    A["Geometria Euclidiana"] -->|Verdade absoluta por 2000 anos| B["Tentativas de provar o postulado das paralelas"]
    B -->|Barreira da impossibilidade de prova| C["Mudança de paradigma de Lobachevsky"]
    C -->|Múltiplas paralelas podem existir| D["Construção da geometria não-euclidiana (geometria hiperbólica)"]
    D --> E["Nascimento do conceito de curvatura do espaço"]
    E --> F["Desenvolvimento para a Geometria Riemanniana"]
    F --> G["Teoria da Relatividade Geral de Einstein"]
    G --> H["Cosmologia moderna e compreensão dos buracos negros"]
```
...
```mermaid
graph LR
    subgraph "Diagrama de Correlação de Influência"
        direction LR
        L["Lobachevsky"]
        E["Euclides"]
        G["Gauss"]
        ...
        L -->|Nega o postulado das paralelas| E
        L ---|Fez a mesma descoberta independentemente| B
        G -->|Aprecia em privado mas não publica| L
        L -->|Fornece a base teórica| R
        R -->|Expande a geometria do espaço| A
    end
```
