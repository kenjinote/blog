...
```mermaid
graph TD
    A["Géométrie euclidienne"] -->|Vérité absolue pendant 2000 ans| B["Tentative de preuve du postulat des parallèles"]
    B -->|Mur de l'impossibilité de prouver| C["Changement de perspective de Lobatchevski"]
    C -->|Il peut exister plusieurs droites parallèles| D["Construction de la géométrie non euclidienne (géométrie hyperbolique)"]
    D --> E["Naissance du concept de courbure de l'espace"]
    E --> F["Développement en géométrie riemannienne"]
    F --> G["Théorie de la relativité générale d'Einstein"]
    G --> H["Compréhension de la cosmologie moderne et des trous noirs"]
```
...
```mermaid
graph LR
    subgraph "Diagramme des parties prenantes et des influences"
        direction LR
        L["Lobatchevski"]
        E["Euclide"]
        G["Gauss"]
        ...
        L -->|Rejette le postulat des parallèles| E
        L ---|Découverte similaire indépendante| B
        G -->|Appréciation privée mais aucune publication| L
        L -->|Fournit les bases de la théorie| R
        R -->|Élargit la géométrie de l'espace| A
    end
```
