---
title: "Nikolai Lobatschewski: Der 'Kopernikus der Geometrie', der die Tür zur nicht-euklidischen Geometrie öffnete"
...
```mermaid
graph TD
    A["Euklidische Geometrie"] -->|Absolute Wahrheit für 2000 Jahre| B["Versuche, das Parallelenpostulat zu beweisen"]
    B -->|Mauer der Unbeweisbarkeit| C["Lobatschewskis Paradigmenwechsel"]
    C -->|Mehrere Parallelen können existieren| D["Konstruktion der nicht-euklidischen Geometrie (hyperbolische Geometrie)"]
    D --> E["Geburt des Konzepts der Raumkrümmung"]
    E --> F["Entwicklung zur Riemannschen Geometrie"]
    F --> G["Einsteins Allgemeine Relativitätstheorie"]
    G --> H["Moderne Kosmologie und Verständnis von Schwarzen Löchern"]
```
...
```mermaid
graph LR
    subgraph "Einfluss-Korrelationsdiagramm"
        direction LR
        L["Lobatschewski"]
        E["Euklid"]
        G["Gauss"]
        ...
        L -->|Lehnt das Parallelenpostulat ab| E
        L ---|Machte unabhängig dieselbe Entdeckung| B
        G -->|Schätzt es privat, veröffentlicht es aber nicht| L
        L -->|Liefert theoretische Grundlage| R
        R -->|Erweitert die Geometrie des Raums| A
    end
```
