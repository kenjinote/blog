...
社会の分断を論じる上で、まず避けて通れないのが「ネットワーク理論（[Graph Theory](https://kenji.blog/p/graph-theory-dijkstra-a-star/)）」を用いたコミュニティの構造解析です。ソーシャルメディア上の人間関係は、ユーザーを「ノード（頂点）」、ユーザー間のフォローや相互作用を「エッジ（辺）」とする巨大な[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)としてモデル化することができます。
