...
def main():
    ...
    if font is None:
        raise RuntimeError("Installez une police compatible avec la langue du graphique.")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="Le dernier type demande à lui seul 10 tirages en moyenne", xlabel="Types déjà obtenus (10 au total)",
           ylabel="Nombre moyen de tirages avant un nouveau type", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.fr.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.fr.svg")
    plt.close(fig)
    ...
    for m, offset in [(30, (8, -40)), (51, (-105, -60)), (66, (10, -35))]:
        ...
        ax.annotate('{m} tirages : {p}'.format(m=m, p=number(cdf[m] * 100, 1) + "%"), (m, cdf[m]), xytext=offset,
                    textcoords="offset points", fontsize=13,
                    arrowprops={"arrowstyle": "-", "color": "#65758b"})
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='Moyenne : {v} tirages'.format(v=number(mean, 2)))
    ax.set(title="Après 30 tirages, environ 63 % des collections sont complètes", xlabel="Nombre de tirages m (10 types)",
           ylabel="Probabilité de terminer la collection en m tirages", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.fr.svg", dpi=160)
    finish_svg(OUT / "completion-probability.fr.svg")
    plt.close(fig)
    ...
    ax.bar(x, empirical, color=cyan, alpha=0.8, width=0.78, label="Simulation (10 000 collections)")
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="Probabilité théorique dans les mêmes classes")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["100 ou plus"], rotation=55, ha="right", fontsize=10)
    ax.set(title="Certaines collections se terminent vite, d’autres beaucoup moins", xlabel="Tirages nécessaires (classes de 5 ; dernière : 100 ou plus)",
           ylabel="Proportion dans chaque classe", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.fr.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.fr.svg")
    plt.close(fig)

    report = {
        "model": "Tirages indépendants et équiprobables avec remise",
    ...
    (OUT / "calculation-results.fr.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
