...
def main():
    ...
    if font is None:
        raise RuntimeError("Установите шрифт с поддержкой языка графика.")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="Только последний вид требует в среднем 10 попыток", xlabel="Уже собранные виды (всего 10)",
           ylabel="Среднее число попыток до следующего нового вида", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.ru.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.ru.svg")
    plt.close(fig)
    ...
    for m, offset in [(30, (8, -40)), (51, (-105, -60)), (66, (10, -35))]:
        ...
        ax.annotate('{m} попыток: {p}'.format(m=m, p=number(cdf[m] * 100, 1) + "%"), (m, cdf[m]), xytext=offset,
                    textcoords="offset points", fontsize=13,
                    arrowprops={"arrowstyle": "-", "color": "#65758b"})
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='Среднее: {v} попыток'.format(v=number(mean, 2)))
    ax.set(title="После 30 попыток собрано лишь около 63% коллекций", xlabel="Число попыток m (10 видов)",
           ylabel="Вероятность собрать коллекцию за m попыток", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.ru.svg", dpi=160)
    finish_svg(OUT / "completion-probability.ru.svg")
    plt.close(fig)
    ...
    ax.bar(x, empirical, color=cyan, alpha=0.8, width=0.78, label="Моделирование (10 000 коллекций)")
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="Теоретическая вероятность в тех же интервалах")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["от 100"], rotation=55, ha="right", fontsize=10)
    ax.set(title="Одни коллекции собираются быстро, другие — гораздо дольше", xlabel="Попытки до завершения (интервалы по 5; последний: от 100)",
           ylabel="Доля в каждом интервале", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.ru.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.ru.svg")
    plt.close(fig)

    report = {
        "model": "Независимые равновероятные выборки с возвращением",
    ...
    (OUT / "calculation-results.ru.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
