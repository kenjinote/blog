...
def main():
    ...
    if font is None:
        raise RuntimeError("Pasang fon yang mendukung bahasa grafik.")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="Jenis terakhir saja memerlukan rata-rata 10 pengambilan", xlabel="Jenis yang sudah terkumpul (total 10)",
           ylabel="Rata-rata pengambilan hingga jenis baru berikutnya", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.id.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.id.svg")
    plt.close(fig)
    ...
    for m, offset in [(30, (8, -40)), (51, (-105, -60)), (66, (10, -35))]:
        ...
        ax.annotate('{m} kali: {p}'.format(m=m, p=number(cdf[m] * 100, 1) + "%"), (m, cdf[m]), xytext=offset,
                    textcoords="offset points", fontsize=13,
                    arrowprops={"arrowstyle": "-", "color": "#65758b"})
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='Rata-rata: {v} kali'.format(v=number(mean, 2)))
    ax.set(title="Setelah 30 pengambilan, hanya sekitar 63% yang lengkap", xlabel="Jumlah pengambilan m (10 jenis)",
           ylabel="Peluang koleksi lengkap dalam m pengambilan", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.id.svg", dpi=160)
    finish_svg(OUT / "completion-probability.id.svg")
    plt.close(fig)
    ...
    ax.bar(x, empirical, color=cyan, alpha=0.8, width=0.78, label="Simulasi (10.000 koleksi)")
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="Peluang teoretis pada interval yang sama")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["100 ke atas"], rotation=55, ha="right", fontsize=10)
    ax.set(title="Ada yang cepat lengkap, ada yang harus menunggu lama", xlabel="Pengambilan hingga lengkap (interval 5; terakhir: 100 atau lebih)",
           ylabel="Proporsi pada setiap interval", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.id.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.id.svg")
    plt.close(fig)

    report = {
        "model": "Pengambilan independen dengan peluang sama dan pengembalian",
    ...
    (OUT / "calculation-results.id.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
