...
def main():
    ...
    if font is None:
        raise RuntimeError("ثبّت خطًا يدعم لغة الرسم البياني.")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="النوع الأخير وحده يحتاج إلى 10 سحبات في المتوسط", xlabel="عدد الأنواع المجموعة (من أصل 10)",
           ylabel="متوسط السحبات حتى ظهور نوع جديد", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.ar.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.ar.svg")
    plt.close(fig)
    ...
    for m, offset in [(30, (8, -40)), (51, (-105, -60)), (66, (10, -35))]:
        ...
        ax.annotate('{m} سحبة: {p}'.format(m=m, p=number(cdf[m] * 100, 1) + "%"), (m, cdf[m]), xytext=offset,
                    textcoords="offset points", fontsize=13,
                    arrowprops={"arrowstyle": "-", "color": "#65758b"})
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='المتوسط: {v} سحبة'.format(v=number(mean, 2)))
    ax.set(title="بعد 30 سحبة، تكتمل نحو 63% فقط من المجموعات", xlabel="عدد السحبات (10 أنواع)",
           ylabel="احتمال اكتمال المجموعة حتى عدد السحبات المحدد", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.ar.svg", dpi=160)
    finish_svg(OUT / "completion-probability.ar.svg")
    plt.close(fig)
    ...
    ax.bar(x, empirical, color=cyan, alpha=0.8, width=0.78, label="محاكاة 10,000 مجموعة")
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="الاحتمال النظري للفئات نفسها")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["100 فأكثر"], rotation=55, ha="right", fontsize=10)
    ax.set(title="مجموعات تكتمل بسرعة وأخرى تستغرق وقتًا أطول", xlabel="السحبات حتى الاكتمال (فئات من 5؛ الأخيرة: 100 فأكثر)",
           ylabel="النسبة في كل فئة", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.ar.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.ar.svg")
    plt.close(fig)

    report = {
        "model": "سحبات مستقلة متساوية الاحتمال مع الإرجاع",
    ...
    (OUT / "calculation-results.ar.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
