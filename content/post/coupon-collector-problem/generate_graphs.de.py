...
def main():
    ...
    if font is None:
        raise RuntimeError("Installiere eine Schriftart, die die Diagrammsprache unterstützt.")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="Allein der letzte Typ braucht im Mittel 10 Ziehungen", xlabel="Bereits gesammelte Typen (insgesamt 10)",
           ylabel="Mittlere Ziehungen bis zum nächsten neuen Typ", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.de.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.de.svg")
    plt.close(fig)
    ...
    for m, offset in [(30, (8, -40)), (51, (-105, -60)), (66, (10, -35))]:
        ...
        ax.annotate('{m} Ziehungen: {p}'.format(m=m, p=number(cdf[m] * 100, 1) + "%"), (m, cdf[m]), xytext=offset,
                    textcoords="offset points", fontsize=13,
                    arrowprops={"arrowstyle": "-", "color": "#65758b"})
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='Mittelwert: {v} Ziehungen'.format(v=number(mean, 2)))
    ax.set(title="Nach 30 Ziehungen sind nur etwa 63 % vollständig", xlabel="Anzahl der Ziehungen m (10 Typen)",
           ylabel="Wahrscheinlichkeit für Vollständigkeit nach m Ziehungen", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.de.svg", dpi=160)
    finish_svg(OUT / "completion-probability.de.svg")
    plt.close(fig)
    ...
    ax.bar(x, empirical, color=cyan, alpha=0.8, width=0.78, label="Simulation (10.000 Sammlungen)")
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="Theoretische Wahrscheinlichkeit je Gruppe")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["ab 100"], rotation=55, ha="right", fontsize=10)
    ax.set(title="Manche Sammlungen sind schnell vollständig, andere erst spät", xlabel="Ziehungen bis zur Vollständigkeit (5er-Gruppen; zuletzt: ab 100)",
           ylabel="Anteil in jeder Gruppe", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.de.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.de.svg")
    plt.close(fig)

    report = {
        "model": "Unabhängige gleichverteilte Ziehungen mit Zurücklegen",
    ...
    (OUT / "calculation-results.de.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
