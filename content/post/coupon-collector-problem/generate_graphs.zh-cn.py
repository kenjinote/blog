...
def main():
    ...
    if font is None:
        raise RuntimeError("请安装支持图表语言的字体。")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="仅收集最后一种，平均就要抽取10次", xlabel="已收集的种类数（共10种）",
           ylabel="获得下一种新卡片所需的平均抽取次数", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.zh-cn.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.zh-cn.svg")
    plt.close(fig)
    ...
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='平均次数：{v}次'.format(v=number(mean, 2)))
    ax.set(title="抽取30次，集齐概率也只有约63%", xlabel="抽取次数 m（共10种）",
           ylabel="在m次以内集齐所有种类的概率", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.zh-cn.svg", dpi=160)
    finish_svg(OUT / "completion-probability.zh-cn.svg")
    plt.close(fig)
    ...
    ax.bar(x, empirical, color=cyan, alpha=0.8, width=0.78, label="模拟实验（1万次）")
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="相同分组的理论概率")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["100及以上"], rotation=55, ha="right", fontsize=10)
    ax.set(title="有时很快集齐，有时却要等待很久", xlabel="集齐所需次数（每组5次，最后一组为100次及以上）",
           ylabel="各组所占比例", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.zh-cn.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.zh-cn.svg")
    plt.close(fig)
    ...
    (OUT / "calculation-results.zh-cn.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
