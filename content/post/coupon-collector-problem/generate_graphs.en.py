...
def main():
    ...
    if font is None:
        raise RuntimeError("Install a font supporting the chart language.")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="The last type alone takes 10 draws on average", xlabel="Types already collected (10 in total)",
           ylabel="Mean draws until the next new type", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.en.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.en.svg")
    plt.close(fig)
    ...
    for m, offset in [(30, (8, -40)), (51, (-105, -60)), (66, (10, -35))]:
        ...
        ax.annotate('{m} draws: {p}'.format(m=m, p=number(cdf[m] * 100, 1) + "%"), (m, cdf[m]), xytext=offset,
                    textcoords="offset points", fontsize=13,
                    arrowprops={"arrowstyle": "-", "color": "#65758b"})
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='Mean: {v} draws'.format(v=number(mean, 2)))
    ax.set(title="After 30 draws, only about 63% have finished", xlabel="Number of draws m (10 types)",
           ylabel="Probability of completing the collection by draw m", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.en.svg", dpi=160)
    finish_svg(OUT / "completion-probability.en.svg")
    plt.close(fig)
    ...
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="Theoretical probability in the same bins")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["100+"], rotation=55, ha="right", fontsize=10)
    ax.set(title="Some collections finish quickly; others take much longer", xlabel="Draws to completion (bins of 5; last bin: 100 or more)",
           ylabel="Proportion in each bin", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.en.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.en.svg")
    plt.close(fig)
    ...
    (OUT / "calculation-results.en.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
