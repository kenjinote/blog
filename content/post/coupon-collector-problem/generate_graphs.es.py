...
def main():
    ...
    if font is None:
        raise RuntimeError("Instala una fuente compatible con el idioma del gráfico.")
    plt.rcParams.update({
        "font.family": font,
        "font.size": 13,
        "axes.titlesize": 17,
    ...
    ax.set(title="Solo el último tipo requiere una media de 10 extracciones", xlabel="Tipos ya reunidos (10 en total)",
           ylabel="Extracciones medias hasta el siguiente tipo nuevo", ylim=(0, 11.7), xticks=range(N))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    fig.savefig(OUT / "stage-waiting.es.svg", dpi=160)
    finish_svg(OUT / "stage-waiting.es.svg")
    plt.close(fig)
    ...
    for m, offset in [(30, (8, -40)), (51, (-105, -60)), (66, (10, -35))]:
        ...
        ax.annotate('{m} extracciones: {p}'.format(m=m, p=number(cdf[m] * 100, 1) + "%"), (m, cdf[m]), xytext=offset,
                    textcoords="offset points", fontsize=13,
                    arrowprops={"arrowstyle": "-", "color": "#65758b"})
    ax.axvline(mean, color="#69778b", linestyle="--", linewidth=1.2,
               label='Media: {v} extracciones'.format(v=number(mean, 2)))
    ax.set(title="Tras 30 extracciones, solo se completa alrededor del 63 %", xlabel="Número de extracciones m (10 tipos)",
           ylabel="Probabilidad de completar la colección en m extracciones", xlim=(0, 100), ylim=(0, 1.06))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.grid(alpha=0.18)
    ax.legend(loc="lower right", frameon=False)
    fig.savefig(OUT / "completion-probability.es.svg", dpi=160)
    finish_svg(OUT / "completion-probability.es.svg")
    plt.close(fig)
    ...
    ax.bar(x, empirical, color=cyan, alpha=0.8, width=0.78, label="Simulación (10 000 colecciones)")
    ax.plot(x, exact, color=orange, marker="o", markersize=4, linewidth=1.6,
            label="Probabilidad teórica en los mismos intervalos")
    ax.set_xticks(x, [f"{lo}–{lo+4}" for lo in lows[:-1]] + ["100 o más"], rotation=55, ha="right", fontsize=10)
    ax.set(title="Algunas colecciones se completan pronto; otras tardan más", xlabel="Extracciones hasta completar (intervalos de 5; último: 100 o más)",
           ylabel="Proporción en cada intervalo", ylim=(0, max(exact + empirical) * 1.22))
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _: number(value * 100, 0) + "%"))
    ax.set_axisbelow(True)
    ax.grid(axis="y", alpha=0.18)
    ax.legend(frameon=False)
    fig.savefig(OUT / "simulation-distribution.es.svg", dpi=160)
    finish_svg(OUT / "simulation-distribution.es.svg")
    plt.close(fig)

    report = {
        "model": "Extracciones independientes y equiprobables con reemplazo",
    ...
    (OUT / "calculation-results.es.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
