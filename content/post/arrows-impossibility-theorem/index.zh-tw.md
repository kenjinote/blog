...
3. **不僅測量順序，還測量「強度」** ：
   阿羅定理的前提是只統計「喜歡 A 勝過 B」的順序。近年來，也有研究探討透過引入偏好的「強度」或「容忍度」來迴避悖論的制度，例如為每個選項評分的「評分投票法（Range Voting）」或「[認可](https://kenji.blog/zh-tw/p/oauth2-oidc-authentication-authorization-difference/)投票法（Approval Voting）」。
