...
各コミットは、その時点でのプロジェクト全体のスナップショットへの[ポインタ](/p/c-language-pointers-memory-management-stack-heap/)（ツリー）と、一つ以上の「親コミット」への[ポインタ](/p/c-language-pointers-memory-management-stack-heap/)を持つ。この単純なデータ構造の連鎖によって、Gitは複雑なブランチの分岐やマージの履歴を、数学的に矛盾のないグラフとして表現する。
...
しかしGitにおいて、ブランチは単なる「特定のコミットを指し示す動的な[ポインタ](/p/c-language-pointers-memory-management-stack-heap/)（ファイル内の40文字のハッシュ値）」に過ぎない。ブランチを作成するコストは文字通りゼロに近い。
...
リーナス・トーバルズは、Gitを設計するにあたり「データの破壊や改ざんを絶対に許さない」という強い意志を持っていた。Gitのハッシュモデルは、中央の権威（サーバー）に頼ることなく、データそのものに信頼を内在させるという、[ブロックチェーン](/p/blockchain-technology-smart-contract-distributed-ledger/)にも通じる非中央集権の究極の形を体現している。
