...
각 커밋은 그 시점의 프로젝트 전체의 스냅샷을 가리키는 [포인터](/ko/p/c-language-pointers-memory-management-stack-heap/)(트리)와 하나 이상의 '부모 커밋'을 가리키는 [포인터](/ko/p/c-language-pointers-memory-management-stack-heap/)를 가진다. 이 단순한 데이터 구조의 연쇄를 통해 Git은 복잡한 브랜치의 분기와 병합의 이력을 수학적으로 모순이 없는 그래프로 표현한다.
...
하지만 Git에서 브랜치는 단순한 '특정 커밋을 가리키는 동적인 [포인터](/ko/p/c-language-pointers-memory-management-stack-heap/)(파일 내의 40자 해시값)'에 불과하다. 브랜치를 생성하는 비용은 말 그대로 제로에 가깝다.
...
리누스 토발즈는 Git을 설계함에 있어 '데이터의 파괴나 위조를 절대 용서하지 않겠다'는 강한 의지를 갖고 있었다. Git의 해시 모델은 중앙의 권위(서버)에 의존하지 않고 데이터 자체에 신뢰를 내재시킨다는, [블록체인](/ko/p/blockchain-technology-smart-contract-distributed-ledger/)과도 상통하는 탈중앙화의 궁극적인 형태를 구현하고 있다.
