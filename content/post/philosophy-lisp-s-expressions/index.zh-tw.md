...
麥卡錫從阿隆佐·邱奇 (Alonzo Church) 的「λ 演算 ([Lambda Calculus](/zh-tw/p/lambda-calculus-functional-programming/))」中獲得靈感，建構了能描述純粹數學函數語言的理論基礎。由此誕生的便是 Lisp，它透過一種極其簡單被稱為串列 (List) 的資料結構來表達程式的結構。
...
同像性所帶來的最大恩惠，就是 Lisp 的**巨集 (Macro)** 系統。它與 C 語言的文字替換巨集有著根本上的不同。Lisp 的巨集是<strong>「在編譯期執行的 Lisp 程式」</strong>。
...
其中一個原因，就在於它極高的自由度。有些人將此稱為<strong>「Lisp 的詛咒 (The Lisp Curse)」</strong>。
