...
```mermaid
graph TD
    A["Designphilosophie der Betriebssysteme"]
    A -- "Integration der Benutzererfahrung" --> B["Mac (Apple)"]
    A -- "Überwältigende Verbreitung und Kompatibilität" --> C["Windows (Microsoft)"]
    A -- "Freiheit und Open Source" --> D["Linux (Community)"]
    
    B -- "Geschlossenes Ökosystem" --> E["Hohe Designqualität und Stabilität"]
    C -- "Offene Hardware" --> F["De-facto-Standard"]
    D -- "Offene Software" --> G["Herrscher der Server-Infrastruktur"]
```
...
```mermaid
graph LR
    A["Art und Weise von Software"]
    A -- "Zentralisierte Verwaltung durch Unternehmen" --> B["Proprietär (Win/Mac)"]
    A -- "Dezentralisierte Entwicklung durch Community" --> C["Open Source (Linux)"]
    
    B -- "Vorteile" --> D["Klare Verantwortlichkeiten und konsistente Erfahrung"]
    B -- "Nachteile" --> E["Vendor-Lock-in und Intransparenz"]
    
    C -- "Vorteile" --> F["Transparenz, freie Modifikation und Innovation"]
    C -- "Nachteile" --> G["Unsicherheit des Supports und Fragmentierung"]
```
