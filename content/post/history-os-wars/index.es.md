...
```mermaid
graph TD
    A["Filosofía de diseño del OS"]
    A -- "Integración de la experiencia del usuario" --> B["Mac (Apple)"]
    A -- "Abordaje abrumador y compatibilidad" --> C["Windows (Microsoft)"]
    A -- "Libertad y código abierto" --> D["Linux (Comunidad)"]
    
    B -- "Ecosistema cerrado" --> E["Alto diseño y estabilidad"]
    C -- "Hardware abierto" --> F["Estándar de facto"]
    D -- "Software abierto" --> G["Dominio de la infraestructura de servidores"]
```
...
```mermaid
graph LR
    A["Naturaleza del software"]
    A -- "Control centralizado por una empresa" --> B["Propietario (Win/Mac)"]
    A -- "Desarrollo descentralizado por la comunidad" --> C["Código abierto (Linux)"]
    
    B -- "Ventajas" --> D["Responsabilidades claras y experiencia consistente"]
    B -- "Desventajas" --> E["Dependencia del proveedor (Vendor lock-in) y opacidad"]
    
    C -- "Ventajas" --> F["Transparencia, modificación libre e innovación"]
    C -- "Desventajas" --> G["Incertidumbre en el soporte y fragmentación"]
```
