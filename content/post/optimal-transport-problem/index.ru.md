...
Задача оптимального транспорта ([Optimal Transport Problem](https://kenji.blog/p/optimal-transport-problem/)) — это математическая задача, которая задает вопрос, **«как переместить материал с минимальными усилиями»** при перемещении вещества (например, кучи песка) из одного места в другое (например, в яму).
