---
title: 'ローコード・ノーコード開発の歴史（1980年代～現在）｜HyperCardから最新ツールまで'
...
* **FileMaker（Claris/Apple, 1985）**
  [グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)ィカルなデータベースソフト。表形式のデータ表示に加え、自由なレイアウト編集とスクリプト機能を備えたリレーショナルDBツールです。現在は「Claris FileMaker Pro」として提供され、フルライセンス版は594ドル（2024年時点）。
...
* **Visual Basic（Microsoft, 1991）**
  Windowsアプリ開発に特化したIDE。ボタンやラベルなどのGUI部品をドラッグ＆ドロップで配置し、[イベント駆動](https://kenji.blog/p/event-driven-architecture-async/)型のアプリを簡単に構築できました。1998年のVB6.0を最後に、.NET時代へと移行しています。
...
* **Scratch（MIT, 2007）**
  子ども向けに開発されたプログラミング学習ツール。色付きブ[ロック](https://kenji.blog/p/rdbms-transaction-acid-isolation-level-lock/)を組み合わせることでゲームやアニメーションを作成でき、教育現場で広く利用されています。無償提供。
