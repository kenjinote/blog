...
```mermaid
graph TD
    A["Studium an der Universität Berlin (1915)"] -->|"Starker Einfluss von Frobenius"| B["Neigung zur Zahlentheorie und reinen Mathematik"]
    B -->|"Erster Weltkrieg und Verweigerung des Militärdienstes"| C["Tortur der psychiatrischen Einweisung"]
    C -->|"Rettung durch Landau"| D["Wechsel an die Universität Göttingen (1917)"]
    D -->|"Interaktion mit Hilbert und Landau"| E["Volles Aufblühen als Mathematiker"]
```
...
```mermaid
graph LR
    A["Elliptische Modulformen (g=1)"] -->|"Erweiterung zur multivariablen Funktionentheorie"| B["Siegelsche Modulformen (g>=2)"]
    B -->|"Analyse der Darstellungen quadratischer Formen"| C["Siegel-Weil-Formel"]
    C -->|"Automorphe Darstellungstheorie"| D["Entwicklung zum Langlands-Programm"]
```
