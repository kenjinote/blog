...
# 4. خطوة التنفيذ 1: `/slack/install` والمعلمة `state` للحماية من [CSRF](https://kenji.blog/ar/p/web-security-basics-cors-csp/)
...
رمز الوصول (`xoxb-...`) هو حرفياً "المفتاح الرئيسي" لمساحة عمل Slack. يجب ألا تحفظه بنص صريح في قاعدة البيانات (مثل MySQL، PostgreSQL، [MongoDB](https://kenji.blog/ar/p/nosql-database-selection-kvs-document-graph-wide-column/)). في حالة حدوث كارثة مثل تسرب قاعدة البيانات عبر حقن SQL مثلاً، سيتم اختراق مساحات عمل Slack لجميع العملاء.
