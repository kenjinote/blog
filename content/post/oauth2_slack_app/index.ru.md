...
2. **Client (Клиент)**
   - Приложение, которое пытается получить доступ к серверу ресурсов с разрешения владельца ресурса. В нашем примере это «приложение на Node.js, которое вы разрабатываете (Slack App)». Хотя оно и называется «клиентом», в контексте [OAuth](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) даже серверные веб-приложения называются «клиентами».
3. **[Authorization](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) Server (Сервер авторизации)**
...
Поток [OAuth](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) в двух словах — это **«последовательность действий, при которой Клиент получает согласие Владельца ресурса, получает маркер доступа от Сервера авторизации и с его помощью получает или изменяет данные на Сервере ресурсов»**.

---

# 2. Полный разбор [Authorization](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) Code Grant (Предоставление кода авторизации)

В [OAuth 2.0](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) существует несколько потоков (типов предоставления / grant types), но самым рекомендуемым и широко используемым для сред, где можно безопасно хранить секретный ключ (Client Secret) — например, в серверных веб-приложениях — является **Authorization Code Grant (Предоставление кода авторизации)**.
...
4. Перейдите на вкладку «[OAuth](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) & Permissions» и добавьте URL обратного вызова в «Redirect URLs». Для локальной разработки укажите следующее:
...
# 4. Шаг реализации 1: `/slack/install` и параметр `state` для защиты от [CSRF](https://kenji.blog/ru/p/web-security-basics-cors-csp/)
...
Браузер пользователя немедленно перейдет по указанному `Location`, и появится экран Slack (Consent Screen) со знакомым сообщением «My First [OAuth](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) App запрашивает доступ к вашему рабочему пространству».
...
Маркер доступа (`xoxb-...`) — это буквальный «ключ» к рабочему пространству Slack. Его нельзя хранить в базе данных (MySQL, PostgreSQL, [MongoDB](https://kenji.blog/ru/p/nosql-database-selection-kvs-document-graph-wide-column/) и т.д.) в виде открытого (plain) текста. В случае утечки базы данных, например, из-за SQL-инъекции, произойдет катастрофа: аккаунты Slack всех клиентов будут скомпрометированы.

Обязательно шифруйте токены на уровне приложения с использованием сильных алгоритмов симметричного шифрования, таких как **AES-256-GCM**, перед их сохранением в БД. Главный ключ для шифрования/дешифрования должен строго управляться с использованием безопасных сервисов управления ключами, таких как AWS KMS (Key Management [Service](https://kenji.blog/ru/p/kubernetes-k8s-architecture-pod-service-ingress/)) или GCP Cloud KMS.

## 2. Ротация токенов (Token Rotation)
Использование долгоживущих токенов связано с рисками. В современных реализациях [OAuth](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) рекомендуется внедрять механизм перевыпуска нового токена доступа каждые несколько часов (Token Rotation) с использованием «токена обновления (Refresh Token)». В Slack API также можно включить ротацию токенов в настройках.

---

# Заключение

В этой статье мы подробно разобрали поток [Authorization](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) Code Grant в [OAuth 2.0](https://kenji.blog/ru/p/oauth2-oidc-authentication-authorization-difference/) с примерами реализации на Node.js для интеграции со Slack App.
