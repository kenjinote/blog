...
Преимущество Event Loop заключается в отсутствии необходимости управления блокировками для разделяемого состояния. Однако, если выполнять ресурсоемкие (CPU-bound) задачи в Call Stack, весь Event Loop будет заблокирован, и система может остановиться (блокировка Event Loop). Следует ограничиваться легкими задачами со сложностью от $ O(1) $ до $ O(N) $.

---

## 3. Модель акторов и передача сообщений (Rust / Erlang / Akka)
...
В модели акторов базовой единицей обработки является "актор". Каждый актор имеет независимое состояние (State) и поведение (Behavior) и не разделяет свое состояние напрямую с другими акторами. Связь между акторами осуществляется исключительно посредством **асинхронной передачи сообщений**.
...
### 3.2 Пример реализации актора на Rust

В Rust, языке системного программирования, можно создавать модели акторов с использованием мощных асинхронных крейтов, таких как `tokio` и `actix`. Здесь показана реализация простого паттерна актора с использованием канала `mpsc` (Multi-Producer, Single-Consumer).
...
В EDA изменения состояния внутри системы представляются как "события", которые асинхронно доставляются через шины событий или брокеры сообщений (Apache Kafka, RabbitMQ, AWS EventBridge и т.д.).
...
Событийно-ориентированные и асинхронные архитектуры мощны, но требуют принятия концепции **согласованности в конечном счете (Eventual Consistency)**. Поскольку данные не отражаются во всех системах мгновенно (сильная согласованность), необходимы решения на стороне UI/UX (например, оптимистичное обновление пользовательского интерфейса).
