...
머신러닝에서의 차원 축소 (PCA), 구글의 검색 엔진을 지탱한 PageRank 알고리즘, 건물의 내진 설계부터 양자역학의 슈뢰딩거 방정식에 이르기까지 고윳값과 고유벡터는 어디에나 등장합니다.
...
고윳값과 고유벡터를 이해하기 위해서는 먼저 '행렬이란 무엇인가'에 대한 관점을 바꿔야 합니다. 행렬은 단순한 숫자들의 배열이 아닙니다. 공간 내의 **변환기 (Transformation)** 입니다.
...
그러면 구체적인 $2 \times 2$ 행렬을 사용하여 손으로 고윳값과 고유벡터를 구해보겠습니다. 이는 선형대수학 시험 등에서도 자주 출제되는 단계입니다.
...
이제 행렬 $A$ 의 고윳값과 고유벡터를 모두 구했습니다.

## Python을 이용한 고윳값과 고유벡터 계산
...
고윳값과 고유벡터의 가장 중요한 응용 중 하나가 **행렬의 대각화** 입니다. 대각화란 복잡한 행렬 $A$ 를 계산이 쉬운 대각행렬 $D$ 를 사용하여 다음과 같이 분해하는 것입니다.
...
PCA에서는 데이터의 공분산 행렬의 고윳값과 고유벡터를 계산합니다.
...
얼핏 보면 고윳값과 고유벡터는 추상적인 수학 퍼즐처럼 보일지도 모릅니다. 하지만 기하학적으로 볼 때 이는 '행렬에 의한 복잡한 변환 속에서 결코 변하지 않는 본질적인 축'을 추출하는 작업이며, 그 응용 범위는 컴퓨터 과학, 데이터 사이언스, 이론 물리학, 기계 공학까지 다방면에 걸쳐 있습니다.
