...
```mermaid
flowchart TD
    A["Superordinateur"] -- "Envoi de données sensorielles (signaux électriques)" --> B["Cerveau dans une cuve"]
    B -- "Sortie de commandes motrices (signaux électriques)" --> A
    B -. "Croit que c'est la réalité" .-> C["Monde simulé"]
```
...
```mermaid
flowchart LR
    A["Trilemme de Bostrom"] --> B["Proposition 1 : L'humanité s'éteindra avant de maîtriser la technologie de simulation"]
    A --> C["Proposition 2 : Les civilisations avancées ne sont pas intéressées par l'exécution de simulations du passé"]
    A --> D["Proposition 3 : Nous vivons presque certainement dans une simulation"]
```
