...
```mermaid
flowchart TD
    A["Supercomputer"] -- "Senden von Sinnesdaten (elektrische Signale)" --> B["Gehirn im Tank"]
    B -- "Ausgabe von motorischen Befehlen (elektrische Signale)" --> A
    B -. "Glaubt, es sei die Realität" .-> C["Simulierte Welt"]
```
...
```mermaid
flowchart LR
    A["Bostroms Trilemma"] --> B["Proposition 1: Die Menschheit stirbt aus, bevor sie die Simulationstechnologie etabliert"]
    A --> C["Proposition 2: Fortgeschrittene Zivilisationen sind nicht daran interessiert, historische Simulationen durchzuführen"]
    A --> D["Proposition 3: Wir leben mit an Sicherheit grenzender Wahrscheinlichkeit in einer Simulation"]
```
