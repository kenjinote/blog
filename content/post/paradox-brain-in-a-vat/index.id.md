...
```mermaid
flowchart TD
    A["Superkomputer"] -- "Pengiriman data sensorik (Sinyal listrik)" --> B["Otak di dalam tong"]
    B -- "Keluaran perintah motorik (Sinyal listrik)" --> A
    B -. "Percaya bahwa itu realitas" .-> C["Dunia yang disimulasikan"]
```
...
```mermaid
flowchart LR
    A["Trilema Bostrom"] --> B["Proposisi 1: Umat manusia punah sebelum menguasai teknologi simulasi"]
    A --> C["Proposisi 2: Peradaban maju tidak tertarik menjalankan simulasi masa lalu"]
    A --> D["Proposisi 3: Kita hampir pasti hidup di dalam sebuah simulasi"]
```
