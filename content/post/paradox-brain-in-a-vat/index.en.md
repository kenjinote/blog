...
```mermaid
flowchart TD
    A["Supercomputer"] -- "Send sensory data (electrical signals)" --> B["Brain in the vat"]
    B -- "Output motor commands (electrical signals)" --> A
    B -. "Believes it is reality" .-> C["Simulated world"]
```
...
```mermaid
flowchart LR
    A["Bostrom's Trilemma"] --> B["Proposition 1: Humanity will go extinct before establishing simulation technology"]
    A --> C["Proposition 2: Advanced civilizations have no interest in running ancestor simulations"]
    A --> D["Proposition 3: We are almost certainly living in a simulation"]
```
