...
```mermaid
flowchart TD
    A["Supercomputadora"] -- "Envío de datos sensoriales (señales eléctricas)" --> B["Cerebro en la cubeta"]
    B -- "Salida de comandos motores (señales eléctricas)" --> A
    B -. "Cree que es la realidad" .-> C["Mundo simulado"]
```
...
```mermaid
flowchart LR
    A["Trilema de Bostrom"] --> B["Proposición 1: La humanidad se extinguirá antes de establecer la tecnología de simulación"]
    A --> C["Proposición 2: Una civilización avanzada no estará interesada en ejecutar simulaciones del pasado"]
    A --> D["Proposición 3: Es casi seguro que vivimos en una simulación"]
```
