...
```mermaid
flowchart TD
    A["Supercomputador"] -- "Transmissão de dados sensoriais (sinais elétricos)" --> B["Cérebro na cuba"]
    B -- "Saída de comandos motores (sinais elétricos)" --> A
    B -. "Acredita ser real" .-> C["Mundo simulado"]
```
...
```mermaid
flowchart LR
    A["Trilema de Bostrom"] --> B["Proposição 1: A humanidade irá extinguir-se antes de estabelecer a tecnologia de simulação"]
    A --> C["Proposição 2: Uma civilização avançada não tem interesse em executar simulações do seu passado"]
    A --> D["Proposição 3: Quase de certeza que vivemos numa simulação"]
```
