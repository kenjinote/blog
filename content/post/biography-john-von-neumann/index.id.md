...
```mermaid
flowchart TD
    Start["Mulai Permainan"] -- "Pilihan Pemain A" --> ChoiceA["Strategi A atau Strategi B"]
    ChoiceA -- "Pilih Strategi A" --> ResultA["Hitung Kerugian Maksimum A"]
    ChoiceA -- "Pilih Strategi B" --> ResultB["Hitung Kerugian Maksimum B"]
    ResultA -- "Bandingkan" --> MinMax["Tentukan Strategi dengan Kerugian Lebih Kecil"]
    ResultB -- "Bandingkan" --> MinMax
    MinMax -- "Eksekusi" --> End["Tentukan Strategi Optimal"]
```
...
```mermaid
flowchart TD
    In["Perangkat Input (Input)"] --> CPU["Unit Pemrosesan Pusat (CPU)"]
    CPU <--> Mem["Perangkat Penyimpanan (Memory/Program & Data)"]
    CPU --> Out["Perangkat Output (Output)"]
    subgraph CPU_Block["CPU"]
        CU["Unit Kontrol (Control Unit)"]
        ALU["Unit Logika Aritmatika (ALU)"]
