...
```mermaid
flowchart TD
    Start["Début du Jeu"] -- "Choix du Joueur A" --> ChoiceA["Stratégie A ou Stratégie B"]
    ChoiceA -- "Choisir la Stratégie A" --> ResultA["Calculer la Perte Max de A"]
    ChoiceA -- "Choisir la Stratégie B" --> ResultB["Calculer la Perte Max de B"]
    ResultA -- "Comparer" --> MinMax["Finaliser la Stratégie avec la Moindre Perte"]
    ResultB -- "Comparer" --> MinMax
    MinMax -- "Exécuter" --> End["Déterminer la Stratégie Optimale"]
```
...
```mermaid
flowchart TD
    In["Périphérique d'Entrée (Input)"] --> CPU["Unité Centrale de Traitement (CPU)"]
    CPU <--> Mem["Périphérique de Stockage (Memory/Program & Data)"]
    CPU --> Out["Périphérique de Sortie (Output)"]
    subgraph CPU_Block["CPU"]
        CU["Unité de Contrôle (Control Unit)"]
        ALU["Unité Arithmétique et Logique (ALU)"]
        Reg["Registres (Registers)"]
    end
    CU <--> ALU
    ALU <--> Reg
```
