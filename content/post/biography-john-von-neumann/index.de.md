...
```mermaid
flowchart TD
    Start["Spielbeginn"] -- "Wahl von Spieler A" --> ChoiceA["Strategie A oder Strategie B"]
    ChoiceA -- "Strategie A wählen" --> ResultA["Berechne maximalen Verlust von A"]
    ChoiceA -- "Strategie B wählen" --> ResultB["Berechne maximalen Verlust von B"]
    ResultA -- "Vergleichen" --> MinMax["Strategie mit geringerem Verlust festlegen"]
    ResultB -- "Vergleichen" --> MinMax
    MinMax -- "Ausführen" --> End["Optimale Strategie bestimmen"]
```
...
```mermaid
flowchart TD
    In["Eingabegerät (Input)"] --> CPU["Zentraleingabeeinheit (CPU)"]
    CPU <--> Mem["Speichergerät (Memory/Program & Data)"]
    CPU --> Out["Ausgabegerät (Output)"]
    subgraph CPU_Block["CPU"]
        CU["Steuerwerk (Control Unit)"]
        ALU["Arithmetisch-logische Einheit (ALU)"]
