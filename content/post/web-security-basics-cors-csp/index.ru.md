...
### 1.1.3 Конкретные примеры кода XSS и меры защиты
...
Основа предотвращения XSS — обезвреживание (экранирование) пользовательского ввода, чтобы он не интерпретировался как HTML. В частности, 5 специальных символов `<`, `>`, `&`, `"`, `'` преобразуются в HTML-сущности.
...
В настоящее время современные фронтенд-фреймворки, такие как React и Vue.js, по умолчанию выполняют экранирование, поэтому определенный уровень защиты от XSS обеспечивается, даже если разработчик об этом не задумывается. Однако при использовании `dangerouslySetInnerHTML` (React) или `v-html` (Vue.js) все равно требуется осторожность.

---

## 1.2 Межсайтовая подделка запросов (CSRF)
...
В соответствии со спецификацией браузера, запросы к определенному домену автоматически сопровождаются Cookie, связанными с этим доменом. CSRF злоупотребляет этим механизмом.
...
Установка атрибута **SameSite**, описанного ниже, для Cookie позволяет контролировать, чтобы Cookie не прикреплялись к запросам с других сайтов, что является очень эффективной мерой против CSRF.
...
**Политика защиты контента (Content Security Policy: CSP)** — это мощный механизм глубокой защиты (defense-in-depth) для фундаментального предотвращения XSS и атак с внедрением данных. Он строго определяет происхождение (источники) ресурсов (скриптов, изображений, таблиц стилей и т.д.), которые может загружать веб-страница, в качестве белого списка на стороне сервера.
...
### 2.2.2 Искоренение XSS путем запрета встроенных скриптов

Главная особенность CSP заключается в том, что по умолчанию **запрещается выполнение встроенных скриптов ( `<script>...</script>` ) и использование `eval()`**. В результате, даже если злоумышленник внедрит вредоносный скрипт в HTML (Stored XSS или Reflected XSS), браузер заблокирует его выполнение как нарушение CSP.
...
В CSP есть функция, которая позволяет браузеру отправлять отчет на указанную конечную точку при нарушении политики. Это позволяет администраторам замечать попытки неизвестных XSS и ошибки конфигурации.
...
## 2.3 Защита от CSRF с помощью SameSite Cookie
...
| Действие пользователя (на сайте-ловушке) | HTTP-метод | Тип запроса | Отправка Cookie | Влияние на CSRF |
| :--- | :--- | :--- | :--- | :--- |
...
| Отправка формы (`<form>`) | POST | Навигация верхнего уровня | **Блокируется** | **Предотвращает атаку CSRF** |
...
Таким образом, простая установка `SameSite=Lax` (или функционирование в качестве значения браузера по умолчанию) делает классические атаки CSRF с использованием метода POST неэффективными. Однако для полной защиты рекомендуется использовать его в сочетании с традиционными CSRF-токенами.
...
*   **XSS и CSRF**: Классические, но все еще фатальные уязвимости. Базовой является защита с помощью соответствующего экранирования и токенов.
