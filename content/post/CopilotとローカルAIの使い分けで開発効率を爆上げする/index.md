...
企業の機密情報（シークレットキー、独自のアルゴリズム、未公開のアーキテクチャ）を扱う際のセキュリティリスク、APIの遅延（レイテンシ）、さらにはネットワークに接続できない[オフライン](https://kenji.blog/p/pwa-progressive-web-apps-service-worker/)環境での作業など、クラウドベースのAIにはいくつかの課題が存在します。そこで近年急速に注目を集めているのが、Llama 3やCodeLlama、Mistralといった **ローカルで動作するオープンモデル（ローカルAI）** の活用です。
...
その際、開発者のローカルPCは単なるエディタを動かす画面ではなく、推論エンジンの最前線（エッジAI）としての役割を強く帯びることになります。[NVIDIA](/p/history-of-nvidia/)やAppleが開発者向けマシンのメモリ（VRAM / 統合メモリ）を増強し続けているのは、この未来を見据えてのことです。
...
*   **ローカルAI (Ollama, LM Studio)**: 機密性の高いコードの処理、[オフライン](https://kenji.blog/p/pwa-progressive-web-apps-service-worker/)環境、ネットワークレイテンシを排除した超高速なインライン補完、およびAPIコストの削減に使用する。
