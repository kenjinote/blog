...
Here, let's strictly formulate **Zipf's Law** mathematically. We arrange the elements in a dataset (e.g., words) in descending order of their frequency.

Let the rank of the most frequent element be $r = 1$, and the second be $r = 2$. If the frequency for an element of rank $r$ is $f(r)$, Zipf's Law is expressed as follows:
...
Here, $N$ is the variety of elements (such as vocabulary size). The series in the denominator converges to the Riemann zeta function $\zeta(\alpha)$ in the limit $\alpha > 1$. Therefore, **Zipf's Law** is sometimes referred to as the zeta distribution.
...
This means that when plotted on a log-log graph, it becomes a straight line with a slope of $-\alpha$. The easiest way to check if a dataset follows **Zipf's Law** is to draw a log-log graph and see if it forms a straight line. If it is a straight line, it can be said that a **Power Law** exists behind that phenomenon.

# 4. Amazing Real-World Examples

**Zipf's Law** goes beyond the mere bounds of linguistics and applies to a surprisingly wide variety of phenomena. Here, let's take a detailed look at examples from 5 different fields.
...
Not only in linguistics, but **Zipf's Law** is also observed in geography and urban engineering. When ordering the population of cities in a certain country, the relationship shows that the second largest city has half the population of the first, and the third has a third.
...
# 5. Why Does Zipf's Law Occur? (Generation Mechanism)
...
As a compromise between these two conflicting "efforts", a distribution of a few polysemous frequent words and many unambiguous rare words, namely **Zipf's Law**, is explained to naturally arise.

## 5.3. Random Typing Model (Monkeys Hitting Typewriters)

Surprisingly, mathematicians like Benoit Mandelbrot have shown that distributions similar to **Zipf's Law** can arise even from completely random processes.
For instance, suppose monkeys hit typewriter keys (26 alphabet letters and a blank space) completely at random to create "words". Let $p$ be the probability of a space occurring; the shorter the word, the higher the probability of it being generated. Ordering these by rank yields a power law distribution just like natural language. This suggests the possibility that **Zipf's Law** originates not just from complex human intellectual activity, but from the statistical properties of the system itself.

# 6. Simulation and Python Code

Let's actually use Python to write code that verifies **Zipf's Law** from text data. The following code counts word frequencies using randomly generated text or an existing corpus, and plots them on a log-log graph.
...
For data scientists and engineers, understanding whether a dataset follows a normal distribution (bell curve) or a power law like **Zipf's Law** (having a long tail) makes a critical difference in system design and model building. Please keep **Zipf's Law** in mind as a powerful lens for deciphering the hidden order of the world.
