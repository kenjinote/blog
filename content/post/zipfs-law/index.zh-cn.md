...
这个惊人的法则正是 **齐普夫定律** （[Zipf's Law](https://kenji.blog/zh-cn/p/zipfs-law/)）。该定律是一个经验法则，指出在特定的数据集中，元素的出现频率与其排名成反比。最常出现的元素的频率大约是第二常出现元素的2倍，是第三常出现元素的约3倍。
...
如此这般，仅仅几十个高频词汇占据了整个文本近一半的比例，而剩下的几十万个单词却几乎从未出现。这种“长尾（Long Tail）”现象，在搜索引擎的索引构建以及[大型语言模型](/zh-cn/p/large-language-models-llm-transformer-prompt-engineering/)（[LLM](https://kenji.blog/zh-cn/p/large-language-models-llm-transformer-prompt-engineering/)）的词汇设计中极其重要。在自然语言处理领域，由于频繁出现的单词（停用词）信息量太低，通常会使用 TF-IDF 等方法来降低其权重。
...
企业的营业额、员工数量乃至个人的收入分布，也都遵循 **幂律** 。关于收入分布的法则因意大利经济学家维尔弗雷多·帕累托而得名，被称为 **[帕累托法则](/zh-cn/p/business-pareto-principle/)** （Pareto Principle）。它也作为“80/20法则”广为人知，即“80% 的财富被 20% 的人所拥有”。在数学上， **齐普夫定律** 和 **[帕累托法则](/zh-cn/p/business-pareto-principle/)** 不过是从不同角度（排名或规模）看待同一个现象罢了。
...
在本文中，我们详细讲解了 **齐普夫定律** （[Zipf's Law](https://kenji.blog/zh-cn/p/zipfs-law/)），从其定义到数学背景，再到多样的实例以及生成机制。
