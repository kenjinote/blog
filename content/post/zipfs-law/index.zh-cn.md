...
如此这般，仅仅几十个高频词汇占据了整个文本近一半的比例，而剩下的几十万个单词却几乎从未出现。这种“长尾（Long Tail）”现象，在搜索引擎的索引构建以及大型语言模型（[LLM](https://kenji.blog/zh-cn/p/large-language-models-llm-transformer-prompt-engineering/)）的词汇设计中极其重要。在自然语言处理领域，由于频繁出现的单词（停用词）信息量太低，通常会使用 TF-IDF 等方法来降低其权重。
