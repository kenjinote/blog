...
## 2. Comparison of Hosting [Service](https://kenji.blog/en/p/kubernetes-k8s-architecture-pod-service-ingress/) Architectures
...
Cloudflare Pages offers two methods: linking a GitHub repository to build on Cloudflare's infrastructure, or using "Direct Upload" for static files built via [GitHub Actions](https://kenji.blog/en/p/cicd-pipeline-github-actions-best-practices/). If you want to manage Hugo versions more strictly and integrate with other jobs (such as testing and image optimization), building on GitHub Actions and using the Direct Upload method is recommended.
...
Adding the `defer` attribute allows the script to load asynchronously without blocking HTML parsing, letting it run after the DOM is built. This minimizes its impact on initial display speed ([LCP](https://kenji.blog/en/p/web-vitals-frontend-performance-optimization-lcp-fid-cls/): Largest Contentful [Paint](https://kenji.blog/en/p/browser-rendering-mechanism-dom-paint/) and FCP: First Contentful Paint).
...
1. **Blazing Fast Builds**: Leverage Hugo's speed to minimize [CI/CD](https://kenji.blog/en/p/cicd-pipeline-github-actions-best-practices/) pipeline ([GitHub Actions](https://kenji.blog/en/p/cicd-pipeline-github-actions-best-practices/)) execution times.
