...
Ключевой технологией, лежащей в основе Биткойна, является «[блокчейн](/ru/p/blockchain-technology-smart-contract-distributed-ledger/)» ([Blockchain](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/)). [Блокчейн](/ru/p/blockchain-technology-smart-contract-distributed-ledger/) — это одна из форм технологии распределенного реестра ([Distributed Ledger](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/) Technology: DLT), в которой данные объединяются в блоки, криптографически связанные друг с другом наподобие цепи.
...
Каждый блок условно состоит из «заголовка блока» (Block Header) и «данных транзакций» ([Transaction](https://kenji.blog/ru/p/rdbms-transaction-acid-isolation-level-lock/) Data).
...
5. **Цель сложности (Difficulty Target, Bits)**: Значение, указывающее на сложность доказательства выполнения работы ([PoW](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/)).
...
[Блокчейн](/ru/p/blockchain-technology-smart-contract-distributed-ledger/) использует структуру данных под названием **дерево Меркла (Merkle [Tree](https://kenji.blog/ru/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/))** для эффективного обнаружения подделки данных при сохранении небольшого размера блока. Дерево Меркла — это тип двоичного дерева, в котором листовые узлы содержат хэш-значения каждой транзакции, а родительские узлы представляют собой хэшированные объединения хэш-значений своих дочерних узлов.
...
### Криптография с открытым ключом ([Public Key](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/) [Cryptography](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)) и цифровые подписи
...
Когда Алиса отправляет биткойны Бобу, она использует свой закрытый ключ для создания **цифровой подписи ([Digital Signature](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/))** к данным транзакции. Участники сети могут использовать открытый ключ Алисы, чтобы проверить подлинность этой подписи (действительно ли она была создана Алисой с использованием ее закрытого ключа).

### Криптография на эллиптических кривых (Elliptic Curve Cryptography: ECC) и secp256k1

Вместо [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)-криптографии для генерации открытого ключа Биткойна и создания цифровых подписей применяется **криптография на эллиптических кривых ([ECC](/ru/p/elliptic-curve-cryptography-math-cpp/))** . Преимущество [ECC](/ru/p/elliptic-curve-cryptography-math-cpp/) в том, что она обеспечивает тот же уровень безопасности при гораздо меньшей длине ключа по сравнению с RSA.
...
## 5. Алгоритмы консенсуса и доказательство выполнения работы ([PoW](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/))
...
### Задача византийских генералов ([Byzantine Generals](https://kenji.blog/ru/p/byzantine-generals-problem-consensus/) Problem)
...
Биткойн практически решил эту проблему, объединив **доказательство выполнения работы ([PoW](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/))** с **правилом самой длинной цепи (Longest Chain Rule)** .
...
В отличие от системы банковских счетов (Account-based model), транзакции в Биткойне используют модель **UTXO (Unspent [Transaction](https://kenji.blog/ru/p/rdbms-transaction-acid-isolation-level-lock/) Output: Неизрасходованный выход транзакции)** .
...
В Lightning Network участники открывают «платежные каналы» (Payment Channels) вне блокчейна (off-chain). Внутри канала, пока обе стороны согласны, средства могут переводиться мгновенно, почти бесплатно и сколько угодно раз без записи транзакций в [блокчейн](/ru/p/blockchain-technology-smart-contract-distributed-ledger/). В [блокчейн](/ru/p/blockchain-technology-smart-contract-distributed-ledger/) (Уровень 1) транзакция записывается только при окончательном расчете остатков.

### Сравнение с Proof of Stake ([PoS](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/))

Еще одной серьезной проблемой [PoW](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/) является колоссальное потребление электроэнергии при майнинге. В качестве решения этой экологической проблемы такие сети, как Ethereum, перешли на другой алгоритм консенсуса — **доказательство доли владения (Proof of Stake: PoS)** .
...
За рассмотренными в предыдущих главах алгоритмами SHA-256 и криптографией на эллиптических кривых ([ECC](/ru/p/elliptic-curve-cryptography-math-cpp/)) стоят две парадигмы безопасности: теоретико-информационная и вычислительная. Современные криптовалюты, включая Биткойн, в основном полагаются на вычислительную безопасность (Computational Security).
...
1. **Защита хэш-функцией**: Биткойн-адрес — это не сам открытый ключ, а результат применения к нему хэш-функций SHA-256 и RIPEMD-160. Даже с помощью квантовых компьютеров обратить хэш-функцию (используя [алгоритм Гровера](/ru/p/grovers-algorithm-quantum-search/), сложность составит $\mathcal{O}(\sqrt{N})$) по-прежнему будет невероятно сложно. Следовательно, пока вы не совершите транзакцию и не раскроете свой открытый ключ сети, содержимое адреса остается в безопасности даже от квантовых атак.
2. **Переход на постквантовую криптографию (Post-Quantum [Cryptography](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/): PQC)**: Обсуждается вариант, при котором до того, как квантовые компьютеры станут практичными, протокол Биткойна пройдет через хард-форк для перехода на новые алгоритмы подписи, устойчивые к квантовому взлому. Среди кандидатов — криптография на решетках ([Lattice-based cryptography](/ru/p/lattice-based-cryptography-math-intuition/)) или криптография на многочленах от многих переменных (Multivariate polynomial cryptography), которые стандартизируются NIST (Национальным институтом стандартов и технологий США).

## 9. Топология сети и детали протокола [P2P](https://kenji.blog/ru/p/webrtc-realtime-communication-p2p/)
...
- **Узел майнинга (Mining Node)**: Выполняет вычисления [PoW](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/) для генерации новых блоков. В настоящее время эту роль играют гигантские «майнинговые пулы», объединяющие специализированное оборудование для майнинга, называемое ASIC (Application Specific Integrated Circuit).
...
Причина, по которой майнеры готовы тратить огромные средства на электроэнергию и оборудование для добычи блоков, заключается в экономическом вознаграждении. Когда майнер успешно генерирует новый блок, он получает вновь выпущенные биткойны через специальную транзакцию, называемую **Coinbase [Transaction](https://kenji.blog/ru/p/rdbms-transaction-acid-isolation-level-lock/)**.
