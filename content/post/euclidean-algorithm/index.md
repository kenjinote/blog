...
> **ラメの定理**
> 2つの自然数 $a, b$ （$a > b$）に対してユークリッドの互除法を適用したときの割り算の回数は、 $b$ の十進法での桁数の $5$ 倍以下である。
...
拡張ユークリッドの互除法は、単なる数学パズルではなく、現代のインターネット社会を支える不可欠な技術です。
代表的な例が **RSA暗号** です。RSA暗号の鍵生成プロセスでは、ある数 $e$ と、オイラー関数 $\phi(N)$ に対して、 $e d \equiv 1 \pmod{\phi(N)}$ を満たす秘密鍵 $d$ （モジュラ逆元）を求める必要があります。
これは $ed + k\phi(N) = 1$ という形に変形できるため、まさに拡張ユークリッドの互除法を用いて高速に $d$ を計算することができます。
