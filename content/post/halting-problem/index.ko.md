...
하지만 컴퓨터 과학 분야에서는 그러한 꿈의 툴은 ** '절대로 만들 수 없다' ** 는 것이 수학적으로 증명되어 있습니다. 이것이 유명한 ** '정지 문제(Halting Problem)' ** 입니다.

본 기사에서는 1936년 앨런 튜링(Alan Turing)에 의해 증명된 이 문제에 대해 직관적인 구체적 예시, 수식(KaTeX), 그리고 도해(Mermaid)를 사용하여 알기 쉽게 해설합니다.
...
앨런 튜링이 이 문제를 제기하고 증명한 것은 1936년, 아직 현대와 같은 전자 계산기(컴퓨터)가 존재하지 않던 시대였습니다. 그는 '계산이란 무엇인가?'를 수학적으로 엄밀하게 정의하기 위해 ** '튜링 머신(Turing Machine)' ** 이라는 가상의 기계를 고안했습니다.
...
정지 문제 증명의 근저에 있는 '자기 언급의 역설'은 튜링보다 조금 앞서 1931년 쿠르트 괴델(Kurt Gödel)이 발표한 ** '불완전성 정리(Incompleteness Theorems)' ** 와 깊은 연관을 맺고 있습니다.
