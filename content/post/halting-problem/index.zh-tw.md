...
然而，在計算機科學的領域中，這種夢幻般的工具已經在數學上被證明是 **「絕對無法打造」** 的。這就是著名的 **「停止性問題（Halting Problem）」** 。

本文將針對 1936 年由艾倫·圖靈（Alan Turing）所證明的這個問題，運用直觀的具體例子、數學公式（KaTeX）以及圖解（Mermaid），來進行淺顯易懂的解說。
...
艾倫·圖靈提出並證明這個問題是在 1936 年，那是一個還沒有像現代這種電子計算機（電腦）存在的時代。他為了在數學上嚴密定義「計算是什麼？」，構想出了一種稱為 **「圖靈機（Turing Machine）」** 的虛擬機器。
...
停止性問題證明根底的「自我指涉悖論」，與圖靈之前，在 1931 年由庫爾特·哥德爾（Kurt Gödel）發表的 **「不完備定理（Incompleteness Theorems）」** 有著深層的關聯。
