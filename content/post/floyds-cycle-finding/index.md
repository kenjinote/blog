...
このアルゴリズムは、速度の異なる2つの[ポインタ](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)（擬似的に「ウサギ」と「カメ」と呼ばれることが多い）を用いることから、 **ウサギとカメのアルゴリズム** （Tortoise and Hare Algorithm）としても広く知られています。

本記事では、このアルゴリズムの仕組みから数学的な背景、そして C++ と [Rust](https://kenji.blog/p/webassembly-wasm-current-future/) による具体的な実装例までを詳しく解説します。

## 循環検出とは何か？

単方向連結リスト（Singly Linked List）や状態遷移[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)において、あるノードから辿っていったときに、以前に訪れたノードに再び到達してしまう構造を **循環（サイクル）** と呼びます。
...
具体的には、以下の2つの[ポインタ](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)を使います。
...
以下のようなサイクルを持つ[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)で考えます。
...
ステップごとの[ポインタ](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)の移動は以下のようになります。
（※ カメ = $T$、ウサギ = $H$）
...
それでは、上記の理論を C++ と [Rust](https://kenji.blog/p/webassembly-wasm-current-future/) で実装してみましょう。
...
### [Rust](https://kenji.blog/p/webassembly-wasm-current-future/) による実装

[Rust](https://kenji.blog/p/programming-languages-history-paradigm-evolution/) の場合は、所有権と借用のルールにより連結リストの実装が複雑になりがちですが、配列（または `Vec`）上のインデックス参照問題としてモデル化するのが競技プログラミングなどでは一般的です。
ここでは「次への[ポインタ](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)」の代わりに、「次のインデックス」を保持する配列を用いた実装例を示します。
...
- **[時間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)**: $O(N)$
  ウサギはサイクルに入るまで最大 $N$ ステップ移動し、サイクルに入った後はカメに追いつくまでに最大でサイクル長 $C$ ステップ移動します。$C \le N$ であるため、全体のステップ数は線形時間に収まります。
- **[空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/)**: $O(1)$
  ハッシュセットなどで訪問済みのノードを記憶する必要がなく、単に2つの[ポインタ](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)変数を維持するだけで済むため、追加のメモリ使用量は定数空間となります。
...
2. **重複する数字の検出（Find the Duplicate Number）**:
   ...
   具体的には、配列 `nums` が与えられたとき、状態遷移を `next_node = nums[current_node]` と定義します。重複する値が存在するということは、複数の異なるインデックスから同じ値（つまり同じ次のノード）への遷移が存在することを意味し、これがサイクルの入り口を形成します。したがって、ウサギとカメのアルゴリズムをそのまま適用することで、[時間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/) $O(N)$、[空間計算量](https://kenji.blog/p/time-space-complexity-big-o-notation-examples/) $O(1)$ で重複する値（サイクルの始点）を特定できます。
...
速度の違う2つの[ポインタ](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)を走らせるというシンプルな発想でありながら、$O(N)$ の時間と $O(1)$ の空間でサイクルの検出と始点の特定を可能にするエレガントな手法です。
数学的な裏付けを理解することで、なぜ衝突後に1つのポインタを先頭に戻して同じ速度で進めると始点を見つけられるのかが明確になったと思います。

データ構造の実装や競技プログラミングにおいて、このアルゴリズムは非常に強力な武器となります。ぜひ、実際に手を動かして C++ や [Rust](https://kenji.blog/p/webassembly-wasm-current-future/) で実装してみてください。
