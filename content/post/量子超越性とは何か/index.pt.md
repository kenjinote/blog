...
Embora o Google e a IBM adotem arquiteturas e abordagens diferentes, eles estão envolvidos em uma competição acirrada de desenvolvimento em direção ao seu objetivo final: a FTQC (Fault-Tolerant [Quantum Computing](https://kenji.blog/pt/p/quantum-computing-shors-algorithm/)).
...
A implementação de protocolos para executar portas não Clifford, como a destilação de estados mágicos (Magic [State](https://kenji.blog/pt/p/iac-infrastructure-as-code-terraform/) Distillation), de maneira tolerante a falhas será um grande obstáculo tecnológico no futuro. Diz-se que a execução do algoritmo de Shor na prática para decifrar a criptografia [RSA](https://kenji.blog/pt/p/modern-cryptography-public-key-hash-signature/) de 2048 bits exigirá milhares de qubits lógicos com uma taxa de erro de $10^{-8}$ ou inferior, o que se traduz em milhões a dezenas de milhões de qubits físicos, o que indica que o caminho ainda é longo.
