...
- **Bypass**: Ничего не блокируется, и предупреждения не отображаются. Часто используется для временного выполнения скриптов (например, в [CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/) конвейерах).
...
- $\alpha$ : Накладные расходы сети и протокола (обычно 0.1 - 0.2 для протоколов [TCP](https://kenji.blog/ru/p/http3-quic-protocol-tcp-udp/)/IP или SMB)
