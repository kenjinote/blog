...
3. **Adaptarse a la desaprobación de funciones de transacciones**: La funcionalidad de transacciones en el registro (TxR) utilizando el Kernel [Transaction](https://kenji.blog/es/p/rdbms-transaction-acid-isolation-level-lock/) Manager (KTM), introducida en Windows Vista, está obsoleta (Deprecated) a partir de Windows 10. Es necesario que el lado de la aplicación implemente sus propios mecanismos de copia de seguridad y retroceso (rollback) (por ejemplo, leyendo el valor original antes del cambio y manteniéndolo en memoria).
