...
```mermaid
flowchart TD
    A["In Hochsprache geschriebener Code"] -- "Kompilierungsprozess" --> B["Compiler wie A-0"]
    B -- "Übersetzung" --> C["Maschinencode aus 0 und 1"]
    C -- "Ausführung" --> D["Computer-Hardware"]
    A -- "Fehlerüberprüfung" --> A
```
...
```mermaid
flowchart TD
    A["Entwicklung der englischbasierten FLOW-MATIC"] -- "Vererbung der Designphilosophie" --> B["Gründung des CODASYL-Ausschusses"]
    B -- "Standardisierungsprozess" --> C["Geburt der COBOL-Sprache"]
    C -- "Weite Verbreitung" --> D["Kernsysteme in Finanzen und Verwaltung"]
    D -- "Wartung und Modernisierung" --> E["Fortbestand als Teil moderner IT-Infrastruktur"]
```
