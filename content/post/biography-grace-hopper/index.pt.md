...
```mermaid
flowchart TD
    A["Código de linguagem de alto nível compreensível por humanos"] -- "Processo de compilação" --> B["Compilador (A-0, etc.)"]
    B -- "Tradução" --> C["Linguagem de máquina (código binário de 0 e 1)"]
    C -- "Execução" --> D["Hardware do computador"]
    A -- "Verificação de erros" --> A
```
...
```mermaid
flowchart TD
    A["Desenvolvimento do FLOW-MATIC (baseado em inglês)"] -- "Herança da filosofia de design" --> B["Formação do comitê CODASYL"]
    B -- "Processo de padronização" --> C["Nascimento da linguagem COBOL"]
    C -- "Ampla disseminação" --> D["Sistemas centrais, como financeiros e administrativos"]
    D -- "Manutenção e modernização" --> E["Sobrevivência como parte da infraestrutura de TI moderna"]
```
