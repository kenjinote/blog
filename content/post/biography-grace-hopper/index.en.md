...
```mermaid
flowchart TD
    A["High-level language code understandable by humans"] -- "Compilation process" --> B["Compiler such as A-0"]
    B -- "Translation" --> C["Machine language i.e. 0 and 1 binary code"]
    C -- "Execution" --> D["Computer hardware"]
    A -- "Error checking" --> A
```
...
```mermaid
flowchart TD
    A["Development of FLOW-MATIC based on English"] -- "Inheritance of design philosophy" --> B["Launch of CODASYL committee"]
    B -- "Standardization process" --> C["Birth of COBOL language"]
    C -- "Widespread adoption" --> D["Core systems for finance and administration"]
    D -- "Maintenance and modernization" --> E["Survival as part of modern IT infrastructure"]
```
