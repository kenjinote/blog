...
```mermaid
flowchart TD
    A["Kode bahasa tingkat tinggi yang dapat dipahami manusia"] -- "Proses kompilasi" --> B["Kompiler (seperti A-0)"]
    B -- "Terjemahan" --> C["Bahasa mesin (kode biner 0 dan 1)"]
    C -- "Eksekusi" --> D["Perangkat keras komputer"]
    A -- "Pemeriksaan kesalahan" --> A
```
...
```mermaid
flowchart TD
    A["Pengembangan FLOW-MATIC (berbasis bahasa Inggris)"] -- "Pewarisan filosofi desain" --> B["Pembentukan komite CODASYL"]
    B -- "Proses standardisasi" --> C["Kelahiran bahasa COBOL"]
    C -- "Adopsi luas" --> D["Sistem inti seperti keuangan dan administrasi"]
    D -- "Pemeliharaan dan modernisasi" --> E["Bertahan sebagai bagian dari infrastruktur IT modern"]
```
