...
[オイラー](https://kenji.blog/p/euler/)のトーティエント関数（$\phi$ 関数）は、ある自然数 $N$ に対して、 **「1から $N$ までの自然数のうち、$N$ と互いに素（最大公約数が1）である数の個数」** を返す関数です。
...
レオンハルト・[オイラー](https://kenji.blog/p/euler/)は、この $\phi(N)$ を使って次のような美しい定理を証明しました。

**[オイラー](https://kenji.blog/p/euler/)の定理：**
整数 $a$ と $N$ が互いに素であるとき、以下の合同式が成り立つ。
$$ a^{\phi(N)} \equiv 1 \pmod N $$

これは、「ある数 $a$ を $\phi(N)$ 回掛け合わせて $N$ で割ると、余りが必ず $1$ になる」という驚くべき性質です。（$N$ が素数 $p$ の場合は $a^{p-1} \equiv 1 \pmod p$ となり、[フェルマーの小定理](https://kenji.blog/p/fermats-little-theorem/)と呼ばれます）。

この[オイラー](https://kenji.blog/p/euler/)の定理を変形してみましょう。両辺に $a$ をもう一回掛けます。
...
ここで、メッセージ $M$ と $N$ が互いに素であると仮定すると、 **[オイラー](https://kenji.blog/p/euler/)の定理** より $M^{\phi(N)} \equiv 1 \pmod N$ となります。
