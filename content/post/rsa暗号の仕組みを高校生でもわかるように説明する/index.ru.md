...
[Леонард Эйлер](https://kenji.blog/p/euler/), используя функцию $\phi(N)$, доказал следующую красивую теорему.
...
## 8. Как найти закрытый ключ $d$: Расширенный алгоритм [Евклид](https://kenji.blog/p/euclid/)а

В примере с ручными вычислениями мы нашли $d=103$, подобрав $k$ интуитивно, но если числа состоят из сотен цифр, этот метод не сработает. В реальных программах используется алгоритм, называемый **"расширенным алгоритмом [Евклид](https://kenji.blog/p/euclid/)а"**.

Решить $7d \equiv 1 \pmod{120}$ — это то же самое, что найти целые числа $d$ и $y$, удовлетворяющие уравнению $7d + 120y = 1$. Выполняя алгоритм [Евклид](https://kenji.blog/p/euclid/)а в обратном порядке, это можно вычислить механически.
