...
إحدى التقنيات التي تدعم أمن مجتمع الإنترنت من جذوره هي "تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/)". العديد من الاتصالات التي نستخدمها يوميًا دون تفكير، مثل مدفوعات بطاقات الائتمان في التسوق عبر الإنترنت، والتواصل مع الأصدقاء على شبكات التواصل الاجتماعي، وإرسال واستقبال المعلومات السرية للشركات، محمية بواسطة تشفير RSA أو التقنيات التي خلفته.
...
## 2. الأساس الرياضي الذي يدعم تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) 1: الأعداد الأولية والتحليل إلى العوامل الأولية
...
من هنا تبدأ الرياضيات السحرية التي تشكل جوهر تشفير RSA. ستظهر "مبرهنة أويلر"، وهي تعميم لـ "[مبرهنة فيرما الصغرى](https://kenji.blog/ar/p/fermats-little-theorem/)".
...
هذه الخاصية تعمل كـ "باب خلفي سري (Trapdoor)" لتشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/). فالشخص الذي يعرف $p$ و $q$ (منشئ المفتاح) يمكنه حساب $\phi(N)$ في لحظة، لكن الطرف الثالث الذي يعرف $N$ فقط لا يمكنه حساب $\phi(N)$ ما لم يقم بتحليل $N$ إلى عوامله الأولية.

### 4.2 مبرهنة أويلر

أثبت [ليونهارد أويلر](https://kenji.blog/ar/p/euler/) هذه المبرهنة الجميلة باستخدام $\phi(N)$:

**مبرهنة أويلر:**
عندما يكون العددان الصحيحان $a$ و $N$ أوليين نسبيًا، يتحقق التطابق التالي:
$$ a^{\phi(N)} \equiv 1 \pmod N $$

هذه خاصية مذهلة تعني أن "حاصل ضرب أي عدد $a$ في نفسه $\phi(N)$ مرة ثم قسمته على $N$، سيعطي باقيًا يساوي $1$ دائمًا". (إذا كان $N$ عددًا أوليًا $p$، فإنها تصبح $a^{p-1} \equiv 1 \pmod p$، وتُعرف باسم [مبرهنة فيرما الصغرى](https://kenji.blog/ar/p/fermats-little-theorem/)).
...
هذه المعادلة بالذات هي المبدأ الأساسي الذي يجعل سحر تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) **"العودة إلى الحالة الأصلية بعد التشفير وفك التشفير"** ممكنًا.
...
## 7. مثال تطبيقي: دعونا نجرب تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) بالحساب اليدوي باستخدام أعداد أولية صغيرة
...
## 8. كيفية إيجاد المفتاح السري $d$: [خوارزمية [إقليدس](https://kenji.blog/ar/p/euclid/)](https://kenji.blog/p/euclidean-algorithm/) الممتدة

في مثال الحساب اليدوي، اعتمدنا على التخمين للبحث عن $k$ وإيجاد $d=103$، لكن هذه الطريقة مستحيلة عندما تتكون الأعداد من مئات الأرقام. في البرامج الحقيقية، تُستخدم خوارزمية تسمى **"[خوارزمية [إقليدس](https://kenji.blog/ar/p/euclid/)](https://kenji.blog/p/euclidean-algorithm/) الممتدة"**.

حل التطابق $7d \equiv 1 \pmod{120}$ هو نفسه إيجاد الأعداد الصحيحة $d, y$ التي تحقق المعادلة $7d + 120y = 1$. من خلال الحساب العكسي ل[خوارزمية [إقليدس](https://kenji.blog/ar/p/euclid/)](https://kenji.blog/p/euclidean-algorithm/)، يمكن إيجاد ذلك ميكانيكيًا.
...
## 9. الوجه الآخر لتشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/): التوقيع الرقمي
...
## 10. تجربة تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) من خلال البرمجة
...
لقد قمنا بتوضيح آلية عمل تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) بناءً على المعرفة برياضيات المرحلة الثانوية.
