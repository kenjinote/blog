...
## 1. Введение: Почему [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) нужно квантование?

Эволюция [больших языковых моделей](/ru/p/large-language-models-llm-transformer-prompt-engineering/) (LLM: [Large Language Models](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/)) в последние годы поразительна, но за ней скрываются серьезные проблемы: "истощение вычислительных ресурсов" и "узкое место пропускной способности памяти". Например, если загрузить модель с 70 миллиардами параметров (70B), такую как Llama 3, в память с использованием стандартных 16-битных чисел с плавающей запятой (FP16), только параметры займут около 140 ГБ VRAM/RAM. Если к этому добавить контекст во время логического вывода (KV кэш), модель не будет работать без кластеризации нескольких высококлассных GPU для дата-центров (NVIDIA A100 80GB или H100 80GB).
...
**GGUF**, представленный в августе 2023 года, является универсальным форматом, разработанным для решения этих проблем. Его главная особенность - принятие ** структуры метаданных на основе пар ключ-значение ([Key-Value](https://kenji.blog/ru/p/nosql-database-selection-kvs-document-graph-wide-column/))**.
...
1. **Гибкость:** Все настройки, такие как гиперпараметры модели, конфигурация RoPE (Rotary Positional Embedding), данные словаря токенизатора и т.д., хранятся в виде именованных пар [Key-Value](https://kenji.blog/ru/p/nosql-database-selection-kvs-document-graph-wide-column/). Неизвестные ключи игнорируются, что упрощает добавление новых функций.
...
Простая загрузка модели GGUF в память не делает логический вывод быстрым. Большая часть логического вывода в [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) - это «умножение матрицы на вектор (Matrix-Vector Multiplication, или GEMV, или Matrix-Matrix, GEMM)». Ключ к успеху - это то, как ускорить операцию умножения и сложения между квантованными весами и активациями (входными данными), хранящимися в FP16 (или FP32).
...
При логическом выводе [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/), когда длина контекста (количество токенов промпта) увеличивается, потребление памяти взрывообразно растет не только из-за весов модели, но и из-за **KV-кэша**, который сохраняет прошлые состояния Attention.
...
1. **Гибкость GGUF:** Структура метаданных типа "ключ-значение" создала надежную экосистему, которая может идти в ногу с быстрой эволюцией [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) (появление новых архитектур моделей) без разрушительных изменений.
