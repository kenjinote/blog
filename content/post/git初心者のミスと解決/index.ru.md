...
### 2.1 Объектная модель: Blob, [Tree](https://kenji.blog/ru/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/), Commit
...
2. **[Tree](https://kenji.blog/ru/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/) (Дерево)**
...
В Git встроен инструмент, который находит коммит с багом с помощью математического бинарного поиска ([Binary Search](https://kenji.blog/ru/p/search-algorithms-linear-binary-hash-table-principles/)). Вычислительная сложность составляет $\mathcal{O}(\log N)$, поэтому даже при 1000 коммитах вы сможете найти нужный примерно за 10 проверок.
...
В следующий раз, когда вы подумаете «Я всё сломал!», не спешите закрывать [терминал](/ru/p/terminal-efficiency-powershell-bash-shortcuts/). Сделайте глубокий вдох и введите `git status`. Git обязательно подскажет вам путь к восстановлению.
