...
В настоящее время самым популярным форматом является **[GGUF](/ru/p/llama-cpp-quantization-gguf/) (GPT-Generated Unified Format)**, разработанный Георгием Гергановым (автором llama.cpp). [GGUF](/ru/p/llama-cpp-quantization-gguf/) — это бинарный формат для эффективного инференса как на CPU, так и на GPU, и он отличается превосходной совместимостью с архитектурой объединенной памяти (Unified Memory) на Mac (Apple Silicon).
...
Хотя Llama 3 использует стандартную архитектуру [Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/), по сравнению с предыдущим поколением (Llama 2) в нее было внесено множество технических улучшений. Особо стоит отметить следующие моменты:
...
Llama 3 имеет самую мощную поддержку сообщества, и её сильной стороной является то, что любые форматы квантования, такие как [GGUF](/ru/p/llama-cpp-quantization-gguf/), AWQ, EXL2, доступны немедленно.
...
«Mixtral 8x7B» стала первой [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) с открытым исходным кодом, которая полноценно внедрила архитектуру **MoE (Mixture of Experts)** и добилась огромного успеха.
...
Серия «Gemma» — это открытые модели, разработанные Google с использованием технологий их самой передовой модели «[Gemini](/ru/p/google-one-gemini%E3%81%8C%E8%A7%A3%E7%B4%84%E3%81%A7%E3%81%8D%E3%81%AA%E3%81%84%E6%99%82%E3%81%AE%D1%81%D0%BF%D0%BE%D1%81%D0%BE%D0%B1-%D1%80%D0%B5%D1%88%D0%B5%D0%BD%D0%B8%D1%8F/)». Gemma 2, как её второе поколение, привнесла значительные изменения в архитектуру.

### Уникальный дизайн архитектуры

Gemma 2 применяет несколько уникальных проектных решений, отличающих её от других [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/).
...
Скорость инференса локальных [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) сильно зависит от «пропускной способности памяти» (Memory Bandwidth) GPU. Это связано с тем, что на этапе генерации (декодирования) необходимо считывать все веса модели из памяти для генерации каждого токена. Процесс ограничен не вычислениями (Compute-bound), а памятью (Memory-bound).
...
### 1. Ollama
В настоящее время это самый простой и популярный инструмент. Подобно [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/), он загружает и запускает модели одной командой. Поддерживает Mac, Windows и Linux.
Просто откройте терминал и введите следующую команду, чтобы запустить Llama 3:

```bash
ollama run llama3
```
Кроме того, поскольку Ollama работает в фоновом режиме как [REST API](https://kenji.blog/ru/p/graphql-vs-rest-api-[overfetching](https://kenji.blog/ru/p/graphql-vs-rest-api-overfetching-type-safety/)-type-safety/)-сервер, её чрезвычайно легко интегрировать со скриптами Python или внешними приложениями.
...
### 3. llama.cpp
Это библиотека на C/C++, которая положила начало буму локальных [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) и является основой для всего. Она предназначена для инженеров, желающих настроить производительность до максимума, или хакеров, которые хотят интегрировать её в свои собственные скрипты. Она выжимает максимум из потенциала любого оборудования, от Apple Metal, NVIDIA CUDA, AMD ROCm до наборов инструкций Intel AVX.
...
Скорость эволюции [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) с открытым исходным кодом поражает: каждые несколько месяцев объявляются о прорывах, которые переворачивают прежний здравый смысл. В будущем, благодаря дальнейшему совершенствованию технологий квантования и появлению новых архитектур, может наступить день, когда локальные среды превзойдут облачный ИИ.
