...
```mermaid
graph TD
    A["Ensinamentos de seu pai Melville"] --> B["Conhecer a essência em vez de apenas o nome"]
    A --> C["Ceticismo em relação à autoridade"]
    B --> D["Curiosidade científica de Feynman"]
    C --> D
```
...
```mermaid
graph LR
    A["Elétron"] -- "Emite fóton virtual" --> B["Ponto de interação"]
    C["Fóton"] -- "Interação eletromagnética" --> B
    B -- "Espalhamento, criação de pares, etc." --> D["Novo estado"]
```
