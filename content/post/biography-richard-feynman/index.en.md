...
```mermaid
graph TD
    A["Teachings of his father Melville"] --> B["Knowing the essence, not the name"]
    A --> C["Skepticism towards authority"]
    B --> D["Feynman's scientific curiosity"]
    C --> D
```
...
```mermaid
graph LR
    A["Electron"] -- "Emits virtual photon" --> B["Interaction point"]
    C["Photon"] -- "Electromagnetic interaction" --> B
    B -- "Scattering, pair production, etc." --> D["New state"]
```
