...
```mermaid
graph TD
    A["Enseñanzas de su padre Melville"] --> B["Conocer la esencia en lugar de solo el nombre"]
    A --> C["Escepticismo hacia la autoridad"]
    B --> D["El espíritu de investigación científica de Feynman"]
    C --> D
```
...
```mermaid
graph LR
    A["Electrón"] -- "Emite un fotón virtual" --> B["Punto de interacción"]
    C["Fotón"] -- "Interacción electromagnética" --> B
    B -- "Dispersión, creación de pares, etc." --> D["Nuevo estado"]
```
