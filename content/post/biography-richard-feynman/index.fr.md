...
```mermaid
graph TD
    A["Enseignements du père Melville"] --> B["Connaître l'essence, pas seulement le nom"]
    A --> C["Scepticisme envers l'autorité"]
    B --> D["Curiosité scientifique de Feynman"]
    C --> D
```
...
```mermaid
graph LR
    A["Électron"] -- "Émet un photon virtuel" --> B["Point d'interaction"]
    C["Photon"] -- "Interaction électromagnétique" --> B
    B -- "Diffusion, création de paires, etc." --> D["Nouvel état"]
```
