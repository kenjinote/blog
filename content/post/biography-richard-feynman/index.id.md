...
```mermaid
graph TD
    A["Ajaran ayah Melville"] --> B["Bukan mengetahui nama, tapi memahami esensi"]
    A --> C["Skeptisisme terhadap otoritas"]
    B --> D["Rasa ingin tahu ilmiah Feynman"]
    C --> D
```
...
```mermaid
graph LR
    A["Elektron"] -- "Memancarkan foton virtual" --> B["Titik interaksi"]
    C["Foton"] -- "Interaksi elektromagnetik" --> B
    B -- "Hamburan, produksi pasangan, dll." --> D["Keadaan baru"]
```
