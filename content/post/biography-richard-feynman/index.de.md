...
```mermaid
graph TD
    A["Die Lehren von Vater Melville"] --> B["Das Wesen kennen, nicht den Namen"]
    A --> C["Skepsis gegenüber Autoritäten"]
    B --> D["Feynmans wissenschaftliche Neugier"]
    C --> D
```
...
```mermaid
graph LR
    A["Elektron"] -- "Gibt ein virtuelles Photon ab" --> B["Wechselwirkungspunkt"]
    C["Photon"] -- "Elektromagnetische Wechselwirkung" --> B
    B -- "Streuung, Paarbildung usw." --> D["Neuer Zustand"]
```
