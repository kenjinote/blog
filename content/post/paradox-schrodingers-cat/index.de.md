...
```mermaid
flowchart TD
    A["Radioaktives Material"] -- "Zerfällt mit 50% Wahrscheinlichkeit" --> B["Geigerzähler schlägt aus"]
    A -- "Zerfällt nicht mit 50% Wahrscheinlichkeit" --> C["Geigerzähler schlägt nicht aus"]
    B -- "Hammer fällt" --> D["Giftgas wird freigesetzt und die Katze stirbt"]
    C -- "Hammer fällt nicht" --> E["Kein Giftgas wird freigesetzt und die Katze lebt"]
    D -- "Vor der Beobachtung" --> F["Überlagerung von Leben und Tod"]
    E -- "Vor der Beobachtung" --> F
```
...
```mermaid
flowchart TD
    A["Vor dem Öffnen der Box"] -- "Box öffnen" --> B["Verzweigung des Universums"]
    B -- "Weltlinie A" --> C["Ich beobachte eine lebende Katze"]
    B -- "Weltlinie B" --> D["Ich beobachte eine tote Katze"]
    C -- "Können nicht miteinander interagieren" --> D
```
