...
```mermaid
flowchart TD
    A["Radioactive Substance"] -- "50% chance of decay" --> B["Geiger Counter Detects"]
    A -- "50% chance of no decay" --> C["Geiger Counter Does Not Detect"]
    B -- "Hammer falls" --> D["Poison gas is released and the cat dies"]
    C -- "Hammer does not fall" --> E["Poison gas is not released and the cat lives"]
    D -- "Before observation" --> F["Superposition of life and death states"]
    E -- "Before observation" --> F
```
...
```mermaid
flowchart TD
    A["Before opening the box"] -- "Open the box" --> B["Branching of the universe"]
    B -- "Worldline A" --> C["Me observing a live cat"]
    B -- "Worldline B" --> D["Me observing a dead cat"]
    C -- "Cannot interfere with each other" --> D
```
