...
```mermaid
flowchart TD
    A["Matière radioactive"] -- "50 % de chances de se désintégrer" --> B["Le compteur Geiger détecte"]
    A -- "50 % de chances de ne pas se désintégrer" --> C["Le compteur Geiger ne détecte pas"]
    B -- "Le marteau tombe" --> D["Du gaz toxique est produit et le chat meurt"]
    C -- "Le marteau ne tombe pas" --> E["Aucun gaz toxique n'est produit et le chat survit"]
    D -- "Avant observation" --> F["État superposé de vie et de mort"]
    E -- "Avant observation" --> F
```
...
```mermaid
flowchart TD
    A["Avant d'ouvrir la boîte"] -- "Ouvrir la boîte" --> B["Bifurcation de l'univers"]
    B -- "Ligne d'univers A" --> C["Moi observant le chat vivant"]
    B -- "Ligne d'univers B" --> D["Moi observant le chat mort"]
    C -- "Incapables d'interférer l'un avec l'autre" --> D
```
