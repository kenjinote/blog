...
```mermaid
flowchart TD
    A["Material radioativo"] -- "50% de probabilidade de decair" --> B["Contador Geiger deteta"]
    A -- "50% de probabilidade de não decair" --> C["Contador Geiger não deteta"]
    B -- "O martelo cai" --> D["Gás venenoso é gerado e o gato morre"]
    C -- "O martelo não cai" --> E["Gás venenoso não é gerado e o gato vive"]
    D -- "Antes da observação" --> F["Estado de superposição de vida e morte"]
    E -- "Antes da observação" --> F
```
...
```mermaid
flowchart TD
    A["Antes de abrir a caixa"] -- "Abrir a caixa" --> B["Ramificação do universo"]
    B -- "Linha de mundo A" --> C["Eu a observar o gato vivo"]
    B -- "Linha de mundo B" --> D["Eu a observar o gato morto"]
    C -- "Não podem interferir um com o outro" --> D
```
