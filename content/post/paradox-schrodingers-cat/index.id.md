...
```mermaid
flowchart TD
    A["Zat radioaktif"] -- "Peluang 50% meluruh" --> B["Pencacah Geiger mendeteksi"]
    A -- "Peluang 50% tidak meluruh" --> C["Pencacah Geiger tidak mendeteksi"]
    B -- "Palu jatuh" --> D["Gas beracun muncul dan kucing mati"]
    C -- "Palu tidak jatuh" --> E["Gas beracun tidak muncul dan kucing hidup"]
    D -- "Sebelum pengamatan" --> F["Keadaan superposisi hidup dan mati"]
    E -- "Sebelum pengamatan" --> F
```
...
```mermaid
flowchart TD
    A["Sebelum membuka kotak"] -- "Membuka kotak" --> B["Percabangan alam semesta"]
    B -- "Garis dunia A" --> C["Saya mengamati kucing hidup"]
    B -- "Garis dunia B" --> D["Saya mengamati kucing mati"]
    C -- "Tidak dapat saling berinteraksi" --> D
```
