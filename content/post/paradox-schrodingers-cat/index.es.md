...
```mermaid
flowchart TD
    A["Material radiactivo"] -- "50% de probabilidad de decaimiento" --> B["El contador Geiger lo detecta"]
    A -- "50% de probabilidad de no decaimiento" --> C["El contador Geiger no lo detecta"]
    B -- "Cae el martillo" --> D["Se genera gas venenoso y el gato muere"]
    C -- "El martillo no cae" --> E["No se genera gas venenoso y el gato vive"]
    D -- "Antes de la observación" --> F["Estado de superposición de vida y muerte"]
    E -- "Antes de la observación" --> F
```
...
```mermaid
flowchart TD
    A["Antes de abrir la caja"] -- "Abrir la caja" --> B["Bifurcación del universo"]
    B -- "Línea de universo A" --> C["Yo observando al gato vivo"]
    B -- "Línea de universo B" --> D["Yo observando al gato muerto"]
    C -- "Incapaces de interferir entre sí" --> D
```
