...
## 4. Рождение React и [Flux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/): Революция однонаправленного потока данных
...
Архитектура [Flux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) имеет строгие правила:
...
## 5. Гегемония и ограничения [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/)
...
2.  **Состояние доступно только для чтения ([State](https://kenji.blog/ru/p/iac-infrastructure-as-code-terraform/) is read-only)**:
...
Благодаря этому сочетанию «иммутабельности» (неизменяемости) и «чистых функций» [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) реализовал мощную отладку с возможностью путешествия во времени (откат к прошлым состояниям) и горячую перезагрузку (hot reloading). Это был огромный прорыв с точки зрения опыта разработчика (DX).
...
Голоса о том, что «[Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) — это избыточно», становились все громче, и начались поиски новых подходов к управлению состоянием.

## 6. Движение «без Redux» с помощью [Context API](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) и Hooks
...
Более того, комбинируя хук `useReducer`, стало возможным реализовать управление состоянием, подобное [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/), используя только встроенные возможности React.
...
Благодаря этому широко распространилось мнение, что «[Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) не нужен для простого глобального состояния». Однако в этом подходе скрывалась фатальная ловушка с точки зрения производительности.

### 6.2 Проблема производительности [Context API](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) (Лишние ререндеры)
...
- **Состояние сервера (Server [State](https://kenji.blog/ru/p/iac-infrastructure-as-code-terraform/))**: Принадлежит серверу. Получается асинхронно. Может использоваться и изменяться несколькими людьми, поэтому всегда есть вероятность, что оно устареет (Stale). Требует управления кэшем, фоновых обновлений и обработки повторных попыток (retry).
...
### 7.1 Появление React Query, SWR, [Apollo Client](https://kenji.blog/ru/p/graphql-vs-rest-api-overfetching-type-safety/)

Подход, при котором управление состоянием сервера отделяется от [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) или Context и передается специализированным библиотекам, стал мейнстримом. Появились **React Query (ныне TanStack Query)** и **SWR**.
...
В результате данные, которыми нужно управлять в глобальном хранилище типа [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/), резко сократились только до «чистых клиентских состояний», и нагрузка по управлению состоянием была значительно снижена.

## 8. Атомарное управление состоянием: [Recoil](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) и [Jotai](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/)
...
Подход **атомарного управления состоянием (Atomic [State](https://kenji.blog/ru/p/iac-infrastructure-as-code-terraform/) Management)** был создан для решения проблем производительности [Context API](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) и модели рендеринга React (сверху вниз).

В 2020 году команда Facebook представила **[Recoil](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/)**, под влиянием которого появились такие библиотеки, как **[Jotai](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/)**.

### 8.1 Управление состоянием "снизу вверх"

В то время как [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) использует подход «выделения нужных частей из единого огромного дерева состояния (сверху вниз)», Recoil и Jotai используют подход «создания минимальных единиц состояния (Атомов - Atom) и их комбинирования для внедрения в дерево компонентов (снизу вверх)».
...
Атом — это независимая единица состояния. Компоненты подписываются (Subscribe) только на необходимые им Атомы. При обновлении Атома ререндерятся только те компоненты, которые на него подписаны. Это полностью решает проблему ненужных ререндеров, от которой страдал [Context API](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/).
...
Такие инструменты, как [Jotai](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/), можно использовать почти с тем же ощущением, что и `useState` в React, поэтому у них низкая кривая обучения, и при этом они обеспечивают высокую производительность, что делает их очень популярным выбором в современных React-приложениях.

## 9. Прокси и мутабельность: [Zustand](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) и Valtio
...
### 9.1 Zustand: Максимально упрощенный [Flux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/)

Zustand, как и [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/), использует единое хранилище (архитектура Flux), но он устраняет сложные концепции, такие как Reducer и Provider, и предоставляет чрезвычайно простой API на основе Hooks.
...
[Zustand](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) утвердился в качестве своего рода «современной версии [Redux](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/)», сочетающей в себе надежность Redux с простотой Hooks.
...
В основе Signals лежит теория «реактивного программирования», которая моделирует зависимости между состояниями и вычислениями как **направленный ациклический граф (Directed Acyclic [Graph](https://kenji.blog/ru/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/): DAG)** и использует топологическую сортировку графа для эффективного определения порядка обновления.
...
- **Разработка на React малого и среднего масштаба**: Часто оптимальными решениями становятся [Jotai](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/) или [Zustand](https://kenji.blog/ru/p/state-management-history-redux-context-recoil-zustand/).
