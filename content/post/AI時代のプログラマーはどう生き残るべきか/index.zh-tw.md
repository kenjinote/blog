...
1.  **徹底重新學習技術的「基礎」**: 框架的用法交給 AI 就好。但是，對作業系統的運作原理、網路協定（TCP/IP, HTTP/3）、資料庫的內部結構（[B-Tree](https://kenji.blog/zh-tw/p/b-tree-database-index-theory/), 交易隔離等級）、資料結構與演算法的深入理解是絕對必要的。為了判斷 AI 的輸出是否正確，堅實的計算機科學基礎不可或缺。
