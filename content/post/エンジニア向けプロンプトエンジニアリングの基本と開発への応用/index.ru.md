...
Мир разработки программного обеспечения находится в самом центре беспрецедентного сдвига парадигмы, вызванного стремительной эволюцией больших языковых моделей ([LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/)). Без преувеличения можно сказать, что мы переходим от «Software 2.0» (разработка с использованием нейронных сетей), предложенного Андреем Карпаты, к «Software 3.0» (разработка на основе промптов на естественном языке).
...
Для того чтобы оптимизировать промпты и стабильно получать желаемые результаты, необходимо иметь математическое и структурное понимание «содержимого черного ящика»: того, как LLM внутренне обрабатывают и генерируют текст и код. Большинство современных LLM представляют собой авторегрессионные (auto-regressive) языковые модели на основе архитектуры [Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/).
...
Ядром архитектуры [Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) является механизм самовнимания (Self-Attention). Благодаря ему модель может вычислять зависимости между токенами, находящимися далеко друг от друга в последовательности.
...
Системный промпт определяет **глобальные ограничения, персону (роль) и основные правила поведения** для [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/). Если проводить аналогию с проектированием программного обеспечения, он выполняет роль «переменных окружения» или «базового класса» приложения, или же «[Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)file» для контейнера.
...
**Zero-Shot Prompting (промптинг без примеров)** — это метод, при котором модели дается только инструкция к задаче без каких-либо примеров, и ожидается ответ. Для общих запросов, таких как «напиши быструю сортировку на Python», современные продвинутые [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) отлично справляются и в режиме Zero-Shot.
...
Настоящим прорывом в способностях рассуждения [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) стала **Chain-of-Thought (CoT: Цепочка мыслей)**. В задачах, требующих сложной логики (например, реализация сложного алгоритма, отслеживание запутанного бага, составление регулярных выражений и т.д.), если заставить LLM сразу выдать финальный код, часто возникают логические пробелы или ошибки (галлюцинации).
...
### 3.3 [Tree](https://kenji.blog/ru/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/) of Thoughts (ToT)
...
## 4. Агентские рабочие процессы (Agentic [Workflow](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/)) и ReAct (Reasoning and Acting)

Применение [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) стремительно эволюционирует от простого текстового ввода-вывода к области **ИИ-агентов (AI Agents)**, которые могут автономно планировать и выполнять задачи, взаимодействуя с внешней средой. Ключевой парадигмой в архитектуре таких агентов является **ReAct (Рассуждение и Действие)**.
...
Инженеры передают [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) системный промпт вместе с «определением доступных инструментов (схема JSON)». LLM анализирует контекст промпта и, если решает, что необходимо использовать инструмент, вместо обычного текста выводит «имя вызываемой функции» и «JSON с ее аргументами». Приложение выполняет эту функцию и возвращает результат обратно в LLM, таким образом формируя цикл.

**Пример применения в разработке (автономный агент отладки):**
При создании агента, который исследует причину и генерирует патч при падении тестов в [CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/)-конвейере, [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) можно предоставить следующие инструменты:
...
[LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) автономно рассуждает и действует следующим образом:
...
- **Observation**: (Приложение считывает содержимое файла и возвращает его в [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/))
...
Внедрите [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) в CI-конвейер для автоматического проведения код-ревью при создании Pull Request (PR). Цель состоит в том, чтобы он указывал на несоответствия бизнес-логики и архитектурные антипаттерны, которые не могут быть обнаружены инструментами линтинга (Lint) или статического анализа.
...
### 7.1 [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/)-as-a-Judge (Оценка LLM с помощью LLM)
...
Это позволяет автоматически обнаруживать регрессию (ухудшение производительности) при изменении промпта прямо в [CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/)-конвейере. Промпт-инжиниринг эволюционирует от кустарного «копания в промптах» к управляемой данными, воспроизводимой «инженерии».
...
Когда-то, перейдя от языка ассемблера к языку C, а затем к языкам высокого уровня со сборкой мусора, мы освободились от бремени управления памятью и смогли сосредоточиться на построении более сложной бизнес-логики. [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) и промпт-инжиниринг — это следующая за ними волна абстракции.
