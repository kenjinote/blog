...
GitHub Copilot, Cursor, या विभिन्न [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) API का उपयोग करने वाले AI सहायक उपकरणों के प्रसार के साथ, एक इंजीनियर का मुख्य कार्य "शून्य से कोड लिखने" से बदलकर "AI को वांछित कोड उत्पन्न करने के लिए निर्देशों को डिज़ाइन करने और उत्पन्न कोड की समीक्षा करने और एकीकृत करने" में बदल गया है।

इस नई विकास पद्धति में सबसे महत्वपूर्ण कौशल **प्रॉम्प्ट इंजीनियरिंग ([Prompt Engineering](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/))** है। प्रॉम्प्ट इंजीनियरिंग को अक्सर गैर-इंजीनियरों के लिए "AI के साथ अच्छी तरह से बातचीत करने" जैसे बज़वर्ड के रूप में वर्णित किया जाता है, लेकिन इसका सार ** गैर-नियतात्मक (Non-deterministic) कंप्यूटिंग सिस्टम के लिए प्रोग्रामिंग भाषा का एक नया रूप** है।
...
प्रॉम्प्ट को अनुकूलित करने और लगातार वांछित आउटपुट प्राप्त करने के लिए, "ब्लैक बॉक्स के अंदर" को गणितीय और संरचनात्मक रूप से समझना आवश्यक है कि LLM आंतरिक रूप से टेक्स्ट और कोड को कैसे प्रोसेस और उत्पन्न करते हैं। अधिकांश आधुनिक LLM ट्रांसफार्मर ([Transformer](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/)) आर्किटेक्चर का उपयोग करने वाले ऑटो-रिग्रेसिव (Auto-regressive) भाषा मॉडल हैं।
...
सिस्टम प्रॉम्प्ट **वैश्विक बाधाओं, व्यक्तित्व (भूमिका), और व्यवहार के मूलभूत नियमों** को परिभाषित करता है जो [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) पर लागू होते हैं। सॉफ्टवेयर डिज़ाइन के संदर्भ में, यह किसी एप्लिकेशन के "पर्यावरण चर (environment variables)", "बेस क्लास", या कंटेनर के "[Docker](https://kenji.blog/hi/p/docker-container-namespace-[cgroups](https://kenji.blog/hi/p/docker-container-namespace-cgroups-layers/)-layers/)file" के समान भूमिका निभाता है।
...
LLMs की तर्क क्षमता (reasoning ability) के संबंध में एक बड़ी सफलता **चेन-ऑफ-थॉट (CoT: विचारों की श्रृंखला)** थी। उन कार्यों के लिए जिनमें जटिल तर्क की आवश्यकता होती है (जैसे, जटिल एल्गोरिदम लागू करना, कठिन बग्स को ट्रैक करना, रेगुलर एक्सप्रेशन बनाना, आदि), यदि आप [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) से तुरंत अंतिम कोड आउटपुट करने के लिए कहते हैं, तो तार्किक छलांग (logical leaps) और त्रुटियां (मतिभ्रम) होने की संभावना होती है।
...
## 4. एजेंटीक वर्कफ़्लो (Agentic [Workflow](https://kenji.blog/hi/p/cicd-pipeline-github-actions-best-practices/)) और ReAct (Reasoning and Acting)
...
पारंपरिक [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) "उत्तर देने से पहले सोच (CoT)" सकते थे, लेकिन वे अपने ज्ञान के अंतराल की भरपाई के लिए "कार्य (act)" नहीं कर सकते थे। ReAct फ्रेमवर्क LLM को "सोच (Thought)" और "कार्रवाई (Action)" को बारी-बारी से दोहराकर इस सीमा को पार करने की अनुमति देता है।
...
इंजीनियर [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) को सिस्टम प्रॉम्प्ट के साथ "उपलब्ध टूल्स की परिभाषा (JSON स्कीमा)" प्रदान करते हैं। LLM प्रॉम्प्ट संदर्भ का विश्लेषण करता है, और यदि यह निर्धारित करता है कि किसी टूल का उपयोग किया जाना चाहिए, तो यह सामान्य टेक्स्ट के बजाय "कॉल किए जाने वाले फ़ंक्शन का नाम" और "इसके तर्कों का JSON" आउटपुट करता है। एप्लिकेशन साइड उस फ़ंक्शन को निष्पादित करता है और परिणाम वापस LLM को देता है, जिससे एक लूप बनता है।

**विकास में अनुप्रयोग का उदाहरण (स्वायत्त डिबगिंग एजेंट):**
एक एजेंट का निर्माण करते समय जो कारण की जांच करता है और [CI/CD](https://kenji.blog/hi/p/cicd-pipeline-github-actions-best-practices/) पाइपलाइन में परीक्षण विफल होने पर एक पैच उत्पन्न करता है, [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) को निम्नलिखित उपकरण प्रदान किए जाते हैं:
...
[LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) स्वायत्त रूप से इस प्रकार तर्क करता है और कार्य करता है:
...
- **अवलोकन (Observation)**: (एप्लिकेशन फ़ाइल की सामग्री पढ़ता है और उसे [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) को वापस करता है)
...
[LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) की सबसे बड़ी कमजोरियों में से एक यह है कि वे "निजी जानकारी (private information)" या "नवीनतम जानकारी (latest information)" नहीं जानते हैं जो उनके पूर्व-प्रशिक्षण (pre-training) डेटा में शामिल नहीं है। यदि आप उन्हें अपने कंपनी के आंतरिक रिपॉजिटरी या मालिकाना API विनिर्देशों (proprietary API specifications) के बारे में पूछते हैं, तो LLM शांति से झूठ (मतिभ्रम) बोलेंगे या केवल सामान्य उत्तर प्रदान करेंगे।
...
### 7.1 [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/)-एज़-ए-जज (LLM-as-a-Judge)
...
यह आपको [CI/CD](https://kenji.blog/hi/p/cicd-pipeline-github-actions-best-practices/) पाइपलाइन में प्रॉम्प्ट को संशोधित करते समय होने वाले प्रतिगमन (regression - प्रदर्शन में गिरावट) का स्वचालित रूप से पता लगाने की अनुमति देता है। प्रॉम्प्ट इंजीनियरिंग कारीगर (artisanal) "प्रॉम्प्ट-ट्वीकिंग" से डेटा-संचालित, प्रतिलिपि प्रस्तुत करने योग्य "इंजीनियरिंग" में विकसित हुई है।
...
1. **आर्किटेक्चर को समझना**: [LLM](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) की संभाव्य प्रकृति (probabilistic nature - ऑटो-रिग्रेसिव, अटेंशन, तापमान) को समझें और सिस्टम की गैर-नियतात्मकता (non-determinism) को नियंत्रित करें।
...
---
*Generated using [Prompt Engineering](https://kenji.blog/hi/p/large-language-models-llm-transformer-prompt-engineering/) Techniques.*
