...
يشهد عالم تطوير البرمجيات تحولاً جذرياً غير مسبوق بسبب التطور السريع للنماذج اللغوية الكبيرة ([LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/)). ليس من المبالغة القول إننا ننتقل الآن من "البرمجيات 2.0" (التطوير باستخدام الشبكات العصبية) التي اقترحها Andrejs Karpathy، إلى "البرمجيات 3.0" (التطوير الموجه بالتلقين باستخدام اللغة الطبيعية).
...
المهارة الأكثر أهمية في منهجية التطوير الجديدة هذه هي **هندسة التلقين ([Prompt Engineering](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/))**. غالباً ما يُتحدث عن هندسة التلقين ككلمة طنانة لغير المهندسين تحت مسمى "الدردشة الجيدة مع الذكاء الاصطناعي"، ولكن جوهرها هو ** شكل جديد من لغات البرمجة للأنظمة الحسابية غير الحتمية (Non-deterministic)**.
...
من أجل تحسين التلقين (Prompt) والحصول على المخرجات المقصودة بشكل مستقر، من الضروري فهم "ما بداخل الصندوق الأسود" بشكل رياضي وهيكلي، وكيف تقوم نماذج LLM بمعالجة النصوص والأكواد وتوليدها داخلياً. معظم نماذج LLM الحديثة هي نماذج لغوية ذاتية الانحدار (Auto-regressive) تستخدم بنية المحولات ([Transformer](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/)).
...
جوهر بنية [Transformer](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) هو آلية الانتباه الذاتي (Self-Attention). يسمح هذا للنموذج بحساب التبعيات بين الرموز المتباعدة داخل التسلسل.
...
**Zero-Shot Prompting** هو أسلوب يُطلب فيه من النموذج الإجابة من خلال إعطائه تعليمات المهمة فقط، دون تقديم أي أمثلة. بالنسبة للطلبات العامة مثل "اكتب خوارزمية الترتيب السريع ([Quicksort](https://kenji.blog/ar/p/sorting-algorithms/)) بلغة Python"، تعمل نماذج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) المتقدمة الحالية بشكل جيد كفاية باستخدام Zero-Shot.
...
كان **Chain-of-Thought (CoT: سلسلة الأفكار)** بمثابة اختراق كبير فيما يتعلق بقدرات الاستدلال لنماذج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/). في المهام التي تتطلب منطقاً معقداً (مثل: تنفيذ خوارزميات معقدة، تتبع أخطاء برمجية غامضة، بناء تعبيرات نمطية، إلخ)، إذا تركت نموذج LLM يُخرج الكود النهائي فجأة، فمن المرجح أن تحدث قفزات منطقية أو أخطاء (هلوسة).
...
## 4. سير العمل الوكيلي (Agentic [Workflow](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/)) و ReAct (الاستدلال والتصرف)

يتطور تطبيق نماذج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) بسرعة من مجرد إدخال وإخراج للنصوص الفردية، إلى مجال **وكلاء الذكاء الاصطناعي (AI Agents)** الذين يخططون بشكل مستقل وينفذون المهام أثناء التفاعل مع البيئة الخارجية. النموذج الأساسي الذي يشكل جوهر هذه البنية الوكيلة هو **ReAct (Reasoning and Acting)**.
...
يمرر المهندسون لنموذج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) "تعريفاً لمجموعة الأدوات المتاحة (مخطط JSON)" جنباً إلى جنب مع التلقين النظامي. يحلل LLM سياق التلقين، وإذا قرر أنه يجب استخدام أداة، فإنه يُخرج "اسم الدالة المراد استدعاؤها" و "متغيرات JSON لتلك الدالة" بدلاً من النص العادي. يتم تنفيذ الدالة على جانب التطبيق، وتُعاد النتيجة إلى LLM، مما يُشكل حلقة مغلقة.

**أمثلة على التطبيق في التطوير (وكيل تصحيح أخطاء مستقل):**
عند بناء وكيل (Agent) يحقق في سبب فشل الاختبارات في مسار [CI/CD](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/) وينشئ تصحيحاً (Patch)، يتم تزويد [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) بالأدوات التالية:
...
يستنتج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) ويتصرف بشكل مستقل على النحو التالي:
...
- **Observation**: (يقرأ التطبيق محتويات الملف ويعيدها إلى [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/))
...
أحد أكبر نقاط الضعف في نماذج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) هو أنها لا تعرف "المعلومات الخاصة" أو "أحدث المعلومات" التي لم يتم تضمينها في بيانات التدريب المسبق. إذا سألتها عن مستودع خاص داخلي في الشركة أو عن مواصفات API مخصصة، فإن LLM سيهلوس بكل هدوء أو سيقدم إجابات عامة فقط.
...
يتم دمج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) في مسار التكامل المستمر (CI)، ويُطلب منه إجراء مراجعة تلقائية للكود عند إنشاء طلب سحب (Pull Request - PR). الغرض من ذلك هو الإشارة إلى التناقضات في منطق الأعمال أو الأنماط المضادة (Anti-patterns) في التصميم والتي لا يمكن لأدوات Lint أو أدوات التحليل الثابت اكتشافها.
...
## 7. تقييم التلقينات وعمليات تشغيل [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) (LLMOps و Eval)
...
يتيح هذا إمكانية الاكتشاف التلقائي للتراجع في الأداء (Regression) عند تعديل التلقين في مسار [CI/CD](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/). لقد تطورت هندسة التلقين من "العبث بالتلقينات" كحرفة يدوية، إلى "هندسة (Engineering)" قابلة للتكرار وتعتمد على البيانات.
...
في الماضي، من خلال الانتقال من لغة التجميع (Assembly) إلى لغة C، ثم إلى لغات المستوى العالي المزودة بميزة جمع القمامة ([Garbage Collection](https://kenji.blog/ar/p/memory-management-garbage-collection/))، تحررنا من متاعب إدارة الذاكرة وأصبح بإمكاننا التركيز على بناء منطق أعمال أكثر تعقيداً. تعد نماذج [LLM](https://kenji.blog/ar/p/large-language-models-llm-transformer-prompt-engineering/) وهندسة التلقين الموجة التالية من التجريد التي تلي ذلك.
