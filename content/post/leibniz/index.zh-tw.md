...
除了作為理論數學家，萊布尼茨還是一位實用的發明家，在機械計算器的歷史上留下了自己的名字。他改進了[布萊茲·帕斯卡](https://kenji.blog/zh-tw/p/pascal/)發明的只能進行加減運算的計算器（Pascaline），發明了一種使用「萊布尼茨輪（步進圓柱體）」的計算器（步進計算器），能夠進行乘法和除法運算。
