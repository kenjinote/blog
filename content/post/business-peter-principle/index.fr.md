...
```mermaid
flowchart TD
    A["Nouvel employé"] -- "Promu car compétent" --> B["Employé de niveau intermédiaire (en charge des opérations)"]
    B -- "Promu car compétent" --> C["Chef d'équipe"]
    C -- "Manque de compétences managériales exposé" --> D["Atteint le niveau d'incompétence (arrêt des promotions)"]
    
    style D fill:#f99,stroke:#333,stroke-width:2px
    
    E["Le travail réel de l'organisation"] -- "Est soutenu par" --> B
    E -- "Est soutenu par" --> C
```
...
```mermaid
flowchart LR
    A["Jeune employé"] --> B["Employé de niveau intermédiaire"]
    
    B -- "Aptitude au management" --> C["Parcours managérial (Manager)"]
    C --> D["Cadre dirigeant / Direction"]
    
    B -- "Spécialisation technique" --> E["Parcours d'expertise (Spécialiste)"]
    E --> F["Fellow / Executive Engineer"]
    
    style C fill:#d4edda,stroke:#28a745
    style E fill:#cce5ff,stroke:#007bff
    
    C -. "Rémunération et autorité équivalentes" .- E
```
