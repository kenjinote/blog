...
```mermaid
flowchart TD
    A["Neue Mitarbeiter"] -- "Kompetent, daher befördert" --> B["Mittlere Mitarbeiter (ausführende Ebene)"]
    B -- "Kompetent, daher befördert" --> C["Teamleiter"]
    C -- "Mangelnde Managementfähigkeiten werden offensichtlich" --> D["Erreicht Inkompetenzniveau (Beförderung gestoppt)"]
    
    style D fill:#f99,stroke:#333,stroke-width:2px
    
    E["Die tatsächliche Arbeit der Organisation"] -- "Wird getragen von" --> B
    E -- "Wird getragen von" --> C
```
...
```mermaid
flowchart LR
    A["Junge Mitarbeiter"] --> B["Mittlere Mitarbeiter"]
    
    B -- "Eignung für Management" --> C["Management-Weg (Manager)"]
    C --> D["Vorstandsmitglieder / Führungskräfte"]
    
    B -- "Spezialisierung auf Fachwissen" --> E["Spezialisten-Weg (Fachkraft)"]
    E --> F["Fellow / Executive Engineer"]
    
    style C fill:#d4edda,stroke:#28a745
    style E fill:#cce5ff,stroke:#007bff
    
    C -. "Gleichwertige Vergütung und Befugnisse" .- E
```
