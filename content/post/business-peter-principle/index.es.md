...
```mermaid
flowchart TD
    A["Nuevo empleado"] -- "Ascendido por ser competente" --> B["Empleado de nivel medio (Personal de primera línea)"]
    B -- "Ascendido por ser competente" --> C["Líder de equipo"]
    C -- "Se expone la falta de habilidades de gestión" --> D["Alcanza nivel de incompetencia (Ascenso detenido)"]
    
    style D fill:#f99,stroke:#333,stroke-width:2px
    
    E["Trabajo real de la organización"] -- "Es sostenido por" --> B
    E -- "Es sostenido por" --> C
```
...
```mermaid
flowchart LR
    A["Empleado joven"] --> B["Empleado de nivel medio"]
    
    B -- "Aptitud para la gestión" --> C["Ruta gerencial (Gerente)"]
    C --> D["Ejecutivo / Alta dirección"]
    
    B -- "Especializado en habilidades técnicas" --> E["Ruta profesional (Especialista)"]
    E --> F["Fellow / Ingeniero Ejecutivo"]
    
    style C fill:#d4edda,stroke:#28a745
    style E fill:#cce5ff,stroke:#007bff
    
    C -. "Recompensa y autoridad equivalentes" .- E
```
