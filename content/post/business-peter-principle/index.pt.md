...
```mermaid
flowchart TD
    A["Novo funcionário"] -- "Promovido por ser competente" --> B["Funcionário de nível médio (Prático)"]
    B -- "Promovido por ser competente" --> C["Líder de equipe"]
    C -- "A falta de habilidade de gestão é exposta" --> D["Atinge o nível de incompetência (Promoção interrompida)"]
    
    style D fill:#f99,stroke:#333,stroke-width:2px
    
    E["Trabalho real da organização"] -- "É sustentado por" --> B
    E -- "É sustentado por" --> C
```
...
```mermaid
flowchart LR
    A["Funcionário júnior"] --> B["Funcionário de nível médio"]
    
    B -- "Tem aptidão para gestão" --> C["Rota de gestão (Gerente)"]
    C --> D["Executivos/Diretoria"]
    
    B -- "Especializado em tecnologia" --> E["Rota de especialista (Profissional)"]
    E --> F["Fellow / Engenheiro Executivo"]
    
    style C fill:#d4edda,stroke:#28a745
    style E fill:#cce5ff,stroke:#007bff
    
    C -. "Remuneração e autoridade equivalentes" .- E
```
