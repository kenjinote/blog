...
graph TD
    ...
    subgraph "Hubungan Intim"
        B["Klik Dukungan (Sekitar 5 orang)"]
        C["Kelompok Simpati (Sekitar 15 orang)"]
    end
    
    subgraph "Angka Dunbar"
        D["Kelompok Afinitas (Sekitar 50 orang)"]
        E["Jaringan Aktif (Sekitar 150 orang)"]
    end
    
    subgraph "Batas Kognitif"
        F["Kenalan (Sekitar 500 orang)"]
        G["Orang yang Dikenal Wajahnya (Sekitar 1500 orang)"]
    end
    
    A -- "Kepercayaan absolut" --> B
    B -- "Berbagi kesedihan yang mendalam" --> C
    C -- "Interaksi yang sering" --> D
    D -- "Batas hubungan yang stabil" --> E
    E -- "Mengingat wajah dan nama" --> F
    F -- "Pernah melihatnya" --> G
```
...
```mermaid
graph TD
    A["Startup yang berkembang pesat"]
    
    A -- "Jumlah karyawan meningkat" --> B["Batas 150 orang (Tembok Dunbar)"]
    
    B -- "Tanpa penanggulangan" --> C["Silo organisasi, birokrasi, disfungsi komunikasi"]
    B -- "Penanggulangan dengan kesadaran akan Angka Dunbar" --> D["Desain ulang organisasi"]
    
    D -- "Membagi ke unit kecil yang otonom" --> E["Organisasi Agile (Model Spotify, dll.)"]
    D -- "Pengenalan hierarki dan proses" --> F["Transisi ke perusahaan menengah (Manajemen berbasis aturan)"]
    
    E -- "Pemeliharaan budaya" --> G["Mempertahankan engagement dan kecepatan tinggi"]
    F -- "Peningkatan efisiensi" --> G
```
