...
graph TD
    ...
    subgraph "Relaciones íntimas"
        B["Grupo de apoyo (aprox. 5 personas)"]
        C["Grupo de simpatía (aprox. 15 personas)"]
    end
    
    subgraph "Número de Dunbar"
        D["Grupo de afinidad (aprox. 50 personas)"]
        E["Red activa (aprox. 150 personas)"]
    end
    
    subgraph "Límite cognitivo"
        F["Conocidos (aprox. 500 personas)"]
        G["Caras conocidas (aprox. 1500 personas)"]
    end
    
    A -- "Confianza absoluta" --> B
    B -- "Comparten dolor profundo" --> C
    C -- "Interacción frecuente" --> D
    D -- "Límite de relaciones estables" --> E
    E -- "La cara coincide con el nombre" --> F
    F -- "Los has visto" --> G
```
...
```mermaid
graph TD
    A["Startup de rápido crecimiento"]
    
    A -- "Aumento de empleados" --> B["Límite de 150 personas (Muro de Dunbar)"]
    
    B -- "Sin contramedidas" --> C["Silos en la organización, burocracia, fallas de comunicación"]
    B -- "Medidas conscientes del número de Dunbar" --> D["Rediseño de la organización"]
    
    D -- "División en unidades pequeñas autónomas" --> E["Organización ágil (Modelo Spotify, etc.)"]
    D -- "Introducción de jerarquías y procesos" --> F["Transición a mediana empresa (gestión por reglas)"]
    
    E -- "Mantenimiento de la cultura" --> G["Mantenimiento de alta velocidad y compromiso"]
    F -- "Mejora de la eficiencia" --> G
```
