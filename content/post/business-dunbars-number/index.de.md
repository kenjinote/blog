...
graph TD
    ...
    subgraph "Intime Beziehungen"
        B["Support-Clique, ca. 5 Personen"]
        C["Sympathie-Gruppe, ca. 15 Personen"]
    end
    
    subgraph "Dunbar-Zahl"
        D["Affinitäts-Gruppe, ca. 50 Personen"]
        E["Aktives Netzwerk, ca. 150 Personen"]
    end
    
    subgraph "Kognitive Grenze"
        F["Bekannte, ca. 500 Personen"]
        G["Flüchtige Bekannte, ca. 1500 Personen"]
    end
    
    A -- "Absolutes Vertrauen" --> B
    B -- "Teilen tiefer Trauer" --> C
    C -- "Häufige Interaktion" --> D
    D -- "Grenze stabiler Beziehungen" --> E
    E -- "Gesicht und Name stimmen überein" --> F
    F -- "Schon mal gesehen" --> G
```
...
```mermaid
graph TD
    A["Schnell wachsendes Startup"]
    
    A -- "Mitarbeiterwachstum" --> B["Die 150-Personen-Grenze, Dunbar-Grenze"]
    
    B -- "Ohne Gegenmaßnahmen" --> C["Silo-Bildung, Bürokratie, Kommunikationsversagen"]
    B -- "Maßnahmen im Bewusstsein der Dunbar-Zahl" --> D["Organisatorische Neugestaltung"]
    
    D -- "Aufteilung in kleine autonome Einheiten" --> E["Agile Organisation, z.B. Spotify-Modell"]
    D -- "Einführung von Hierarchien und Prozessen" --> F["Übergang zu mittelständischem Unternehmen, Regelbasierte Verwaltung"]
    
    E -- "Bewahrung der Kultur" --> G["Aufrechterhaltung von hohem Engagement und Geschwindigkeit"]
    F -- "Effizienzsteigerung" --> G
```
