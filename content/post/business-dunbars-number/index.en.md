...
graph TD
    ...
    subgraph "Intimate Relationships"
        B["Support Clique (approx. 5 people)"]
        C["Sympathy Group (approx. 15 people)"]
    end
    
    subgraph "Dunbar's Number"
        D["Affinity Group (approx. 50 people)"]
        E["Active Network (approx. 150 people)"]
    end
    
    subgraph "Cognitive Limits"
        F["Acquaintances (approx. 500 people)"]
        G["Familiar Faces (approx. 1500 people)"]
    end
    
    A -- "Absolute trust" --> B
    B -- "Share deep sorrow" --> C
    C -- "Frequent interaction" --> D
    D -- "Limit of stable relationships" --> E
    E -- "Face and name match" --> F
    F -- "Have seen before" --> G
```
...
graph TD
    ...
    A -- "Increase in employees" --> B["Limit of 150 people (Dunbar's wall)"]
    
    B -- "No countermeasures" --> C["Organizational silos, bureaucracy, communication failure"]
    B -- "Countermeasures mindful of Dunbar's number" --> D["Organizational redesign"]
    
    D -- "Division into autonomous small units" --> E["Agile organization (Spotify model, etc.)"]
    D -- "Introduction of hierarchy and processes" --> F["Transition to mid-sized company (management by rules)"]
    
    E -- "Maintenance of culture" --> G["Maintenance of high engagement and speed"]
    F -- "Efficiency improvement" --> G
```
