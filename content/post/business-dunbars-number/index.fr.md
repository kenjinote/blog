...
graph TD
    ...
    subgraph "Relations intimes"
        B["Cercle de soutien - environ 5 personnes"]
        C["Groupe de sympathie - environ 15 personnes"]
    end
    
    subgraph "Nombre de Dunbar"
        D["Groupe d'affinite - environ 50 personnes"]
        E["Reseau actif - environ 150 personnes"]
    end
    
    subgraph "Limites cognitives"
        F["Connaissances - environ 500 personnes"]
        G["Visages connus - environ 1500 personnes"]
    end
    
    A -- "Confiance absolue" --> B
    B -- "Partage de profonds chagrins" --> C
    C -- "Interactions frequentes" --> D
    D -- "Limite des relations stables" --> E
    E -- "Association visage et nom" --> F
    F -- "Deja vu" --> G
```
...
```mermaid
graph TD
    A["Startup en croissance rapide"]
    
    A -- "Augmentation du nombre d'employes" --> B["Point limite de 150 personnes - Mur de Dunbar"]
    
    B -- "Aucune mesure prise" --> C["Fonctionnement en silos, bureaucratie, echec de communication"]
    B -- "Mesures tenant compte du nombre de Dunbar" --> D["Reconception organisationnelle"]
    
    D -- "Division en petites unites autonomes" --> E["Organisation agile - Modele Spotify, etc."]
    D -- "Introduction de hierarchie et processus" --> F["Transition vers une ETI - gestion par les regles"]
    
    E -- "Maintien de la culture" --> G["Maintien d'un engagement eleve et de la rapidite"]
    F -- "Optimisation de l'efficacite" --> G
```
