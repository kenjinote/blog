---
title: "O que é o Número de Dunbar? Uma explicação completa sobre os limites cognitivos das relações humanas e sua aplicação no design organizacional"
date: "2026-09-24T19:44:38+09:00"
description: "O Número de Dunbar afirma que \"o limite de relações que os humanos conseguem manter de forma estável é de 150 pessoas\". Neste artigo, exploramos a fundo desde as bases biológicas dessa regra proposta pelo psicólogo evolucionista Robin Dunbar, até as relações na era das redes sociais e sua aplicação no design organizacional quando as empresas crescem."
...
graph TD
    ...
    subgraph "Relações íntimas"
        B["Grupo de apoio (aprox. 5 pessoas)"]
        C["Grupo de simpatia (aprox. 15 pessoas)"]
    end
    
    subgraph "Número de Dunbar"
        D["Grupo de afinidade (aprox. 50 pessoas)"]
        E["Rede ativa (aprox. 150 pessoas)"]
    end
    
    subgraph "Limite cognitivo"
        F["Conhecidos (aprox. 500 pessoas)"]
        G["Rostos familiares (aprox. 1500 pessoas)"]
    end
    
    A -- "Confiança absoluta" --> B
    B -- "Compartilham profunda tristeza" --> C
    C -- "Interação frequente" --> D
    D -- "Limite das relações estáveis" --> E
    E -- "Associa rosto e nome" --> F
    F -- "Já vi antes" --> G
```
...
```mermaid
graph TD
    A["Startup em rápido crescimento"]
    
    A -- "Aumento de funcionários" --> B["Limite de 150 pessoas (Barreira de Dunbar)"]
    
    B -- "Sem medidas" --> C["Silos organizacionais, burocracia, falhas na comunicação"]
    B -- "Medidas conscientes do Número de Dunbar" --> D["Redesenho organizacional"]
    
    D -- "Divisão em pequenas unidades autônomas" --> E["Organização ágil (Modelo Spotify, etc.)"]
    D -- "Introdução de hierarquias e processos" --> F["Transição para média empresa (Gestão baseada em regras)"]
    
    E -- "Manutenção da cultura" --> G["Manutenção de alto engajamento e velocidade"]
    F -- "Ganhos de eficiência" --> G
```
