...
لأتمتة العملية في مسار [CI/CD](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/)، أو لإعادة حزم مجموعة من الملفات يدوياً من مثبت موجود، ستحتاج إلى أدوات سطر الأوامر. إذا كانت بيئة Windows SDK مثبتة، يمكنك الوصول إلى الأدوات التالية من موجه أوامر المطور.
...
يستخدم التوقيع لـ MSIX عادةً مزيجاً من تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) و SHA-256 (خوارزمية التجزئة الآمنة 256 بت).
...
حيث $n$ هو معامل RSA (حاصل ضرب عددين أوليين ضخمين). يتم تضمين هذه الشهادة (بصيغة X.509) التي تحتوي على التوقيع $\sigma$ و "المفتاح العام ([Public Key](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/))" $e$ الخاص بالناشر كجزء من حزمة MSIX (في `AppxSignature.p7x`).
...
- **التحقق من عارض الأحداث (Event Viewer)**: للبحث في سبب أكثر تفصيلاً للخطأ، من المهم جداً فتح عارض أحداث Windows والتحقق من السجلات ضمن "سجلات التطبيقات والخدمات (Applications and [Service](https://kenji.blog/ar/p/kubernetes-k8s-architecture-pod-service-ingress/)s Logs)" -> "Microsoft" -> "Windows" -> "AppxPackagingOM" أو "AppXDeployment-Server".
