...
本記事では、競技プログラミングで頻出となる主要なグラフアルゴリズム（トポロジカル[ソート](https://kenji.blog/p/sorting-algorithms/)、ダイクストラ法、ベルマンフォード法、ワーシャルフロイド法、クラスカル法、プリム法、強連結成分分解）について、その理論的な背景、数式を用いた計算量評価、そしてモダンなC++（C++17/20）による高度に最適化された実装例を交えて完全に網羅します。約10,000文字の大ボリュームでお届けする、まさに「完全攻略」ガイドです。
