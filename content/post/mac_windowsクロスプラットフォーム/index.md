...
多くの[プログラミング言語](https://kenji.blog/p/programming-languages-history-paradigm-evolution/)は、Windows上でも `/` をファイル区切りとしてよしなに解釈してくれます（Win32 API自体が `/` をサポートしている部分もあるため）。しかし、コマンドライン引数としてパスを渡す場合や、システムコールを直接叩く場合、文字列としてパスを比較・パースする場合には致命的なエラーを引き起こします。
