...
Однако, если вы закоммитите этот код и запустите сборку на сервере [CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/) (обычно это Linux, например Ubuntu), вы получите ошибку компиляции «файл не найден», так как файловая система ext4 в Linux чувствительна к регистру.
...
При работе с нативным кодом (языками, компилируемыми непосредственно в машинный код), такими как C++ или [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/), необходимо преодолевать не только специфичные для ОС API, но и различия в системах сборки и компиляторах.
...
## 8. Кроссплатформенная проверка в [CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/) (матричная сборка)
...
Используйте современные инструменты CI, такие как [GitHub Actions](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/) или GitLab CI, и настройте матричную сборку (Matrix Build) для **параллельного запуска сборок и тестов во всех средах (Windows, macOS, Linux)** при каждом создании Pull Request.
...
Визуализация этого процесса [CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/) выглядит следующим образом:
...
5.  **Переменные окружения и оболочка**: Используйте инструменты абстракции, такие как `cross-env`, или унифицируйте среду выполнения, например, с помощью WSL/[Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/).
...
8.  **[CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/)**: Внедрите матричные сборки, чтобы автоматизировать чистые сборки и тестирование во всех целевых ОС и исключить человеческий фактор.
