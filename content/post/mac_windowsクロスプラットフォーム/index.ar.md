...
ومع ذلك، إذا قمت بالالتزام (commit) بهذا الكود وتشغيل البناء على خادم [CI/CD](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/) (عادةً Linux مثل Ubuntu)، فسيؤدي ذلك إلى خطأ في التجميع يوضح "لم يتم العثور على الملف" لأن نظام ملفات ext4 الخاص بـ Linux حساس لحالة الأحرف.
...
في حالة ext4 الذي يميز بين الأحرف الكبيرة والصغيرة، تتم إدارة الإدخالات في الدليل باستخدام هياكل مثل جداول التجزئة ([Hash Table](https://kenji.blog/ar/p/search-algorithms-linear-binary-hash-table-principles/)s) أو أشجار B ([B-Tree](https://kenji.blog/ar/p/b-tree-database-index-theory/)s). بافتراض أن عدد الملفات في الدليل هو $N$، وطول اسم الملف هو $L$، فإن تعقيد الوقت للبحث الثنائي البسيط أو البحث في الشجرة سيكون على النحو التالي:
...
*   **macOS / Linux**: `bash` أو `zsh` هما السائدان. يقومان بمعالجة خطوط الأنابيب ([Pipeline](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/)) القائمة على النصوص.
...
عند التعامل مع التعليمات البرمجية الأصلية (اللغات التي يتم تجميعها مباشرة في كود الآلة) مثل C++ أو [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، من الضروري التغلب على اختلافات نظام البناء والمترجم، وليس فقط واجهات برمجة التطبيقات الخاصة بنظام التشغيل.
...
## 8. التحقق من المنصات المشتركة في [CI/CD](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/) (بناء المصفوفة - Matrix Build)
...
استفد من أدوات CI الحديثة مثل [GitHub Actions](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/) و GitLab CI، وقم بإعداد بناء المصفوفة (Matrix Build) **لإجراء البناء والاختبار بالتوازي في جميع بيئات Windows و macOS و Linux** في كل مرة يتم فيها إنشاء طلب سحب (Pull Request).
...
إذا قمنا بتصور تدفق [CI/CD](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/) هذا، فسيبدو كالتالي:
...
5.  **متغيرات البيئة والصدفة**: استخدم أدوات التجريد مثل `cross-env`، أو وحّد بيئة التنفيذ إلى WSL/[Docker](https://kenji.blog/ar/p/docker-container-namespace-[cgroups](https://kenji.blog/ar/p/docker-container-namespace-cgroups-layers/)-layers/) وما إلى ذلك.
...
8.  **[CI/CD](https://kenji.blog/ar/p/cicd-pipeline-github-actions-best-practices/)**: قم بتقديم بناء المصفوفة (Matrix Build) لأتمتة البناء والاختبار النظيف على جميع أنظمة التشغيل المستهدفة، والقضاء على الاعتماد على الأفراد.
