...
A primera vista, un problema tan difícil de matemáticas puras relacionado con la distribución de los números primos puede parecer ajeno a nuestra vida diaria. Sin embargo, la seguridad de Internet que sustenta la infraestructura de la sociedad moderna, en particular **las tecnologías criptográficas modernas como la criptografía [RSA](https://kenji.blog/es/p/modern-cryptography-public-key-hash-signature/) y la criptografía de curva elíptica ([ECC](/es/p/elliptic-curve-cryptography-math-cpp/))**, dependen profundamente de las propiedades de los números primos gigantes.

En este artículo, emprenderemos un viaje matemático desde la distribución de los números primos, pasando por el [Teorema de los Números Primos](/es/p/prime-number-theorem/), la función zeta de [Riemann](https://kenji.blog/es/p/riemann/), hasta llegar al núcleo de la Hipótesis de Riemann. Profundizaremos en detalle sobre cómo se relaciona con la criptografía moderna y qué sucedería en el mundo si se demostrara la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/).
...
Gracias a este teorema, sabemos que, a nivel macroscópico, los números primos tienen una distribución muy suave y predecible. Sin embargo, a nivel microscópico, siempre existe un "error" o "fluctuación" entre $\pi(x)$ y $\text{Li}(x)$. La verdadera naturaleza de esta fluctuación es exactamente el mayor misterio que la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) intenta desentrañar.

---

# 3. La función zeta de [Riemann](https://kenji.blog/es/p/riemann/) y el producto de Euler

El arma más poderosa para analizar la distribución de los números primos es la **función zeta de [Riemann](https://kenji.blog/es/p/riemann/) (Riemann Zeta Function)**. Originalmente fue una serie infinita definida por [Leonhard Euler](https://kenji.blog/es/p/euler/) para números reales $s > 1$.
...
La genialidad de [Riemann](https://kenji.blog/es/p/riemann/) radica en haber extendido la variable $s$ de $\zeta(s)$, que Euler había considerado únicamente para números reales, a **números complejos $s = \sigma + it$ ($\sigma$ es la parte real, $t$ es la parte imaginaria)**.

La serie infinita original converge solo para $\sigma > 1$, pero [Riemann](https://kenji.blog/es/p/riemann/) utilizó una técnica llamada "continuación analítica" (Analytic Continuation) para extender la definición de manera que $\zeta(s)$ tenga sentido en todo el plano complejo, excluyendo el polo en $s = 1$.

Además, dedujo una hermosa ecuación funcional (Functional equation) que satisface la función zeta.

$$ \zeta(s) = 2^s \pi^{s-1} \sin\left(\frac{\pi s}{2}\right) \Gamma(1-s) \zeta(1-s) $$

Donde $\Gamma(x)$ es [la función gamma](/es/p/gamma-function/). A través de esta ecuación, podemos conocer las propiedades del semiplano izquierdo a partir de las propiedades del semiplano derecho.
...
# 5. El núcleo de la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) y la fórmula explícita

[Riemann](https://kenji.blog/es/p/riemann/) calculó unos pocos ceros y formuló una conjetura sorprendente. Esta es la **Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/)**.

> **Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) ([Riemann](https://kenji.blog/es/p/riemann/) Hypothesis)**
> Todos los ceros no triviales de la función zeta de [Riemann](https://kenji.blog/es/p/riemann/) $\zeta(s)$ se encuentran en la línea recta donde la parte real es igual a $1/2$ ($\text{Re}(s) = 1/2$).
...
¿Por qué es tan importante la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/)? Porque los ceros de la función zeta determinan **completamente** la distribución de los números primos.

[Riemann](https://kenji.blog/es/p/riemann/) y el posterior matemático von Mangoldt derivaron una "fórmula explícita" (Explicit formula) que describe con precisión la distribución de los números primos. Utilizando la función de Chebyshev $\psi(x)$, se expresa de la siguiente manera:

$$ \psi(x) = x - \sum_{\rho} \frac{x^\rho}{\rho} - \ln(2\pi) - \frac{1}{2}\ln(1 - x^{-2}) $$

Aquí, $\rho$ es la suma sobre todos los ceros no triviales de la función zeta.
El término principal es $x$ (que corresponde al [Teorema de los Números Primos](/es/p/prime-number-theorem/)), y al sumar y restar los términos ondulatorios que dependen de los ceros $\rho$, se restaura la distribución precisa en forma de escalera de los números primos. Se puede decir que los ceros no triviales representan la "frecuencia (onda)" de la distribución de los números primos.

Si la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) es correcta y la parte real de todos los ceros no triviales $\rho$ es exactamente $1/2$, entonces el término de error del [Teorema de los Números Primos](/es/p/prime-number-theorem/) se mantendrá dentro del rango mínimo teóricamente concebible.

$$ |\pi(x) - \text{Li}(x)| \le \frac{1}{8\pi} \sqrt{x} \ln x \quad \text{for} \quad x \ge 2657 $$

En otras palabras, **si la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) es verdadera, se demuestra que los números primos están distribuidos de la manera más "regular y hermosa" que podamos imaginar**.
...
La seguridad de todas las comunicaciones, como los pagos con tarjeta de crédito en Internet, la transmisión de contraseñas y las firmas digitales de [blockchain](/es/p/blockchain-technology-smart-contract-distributed-ledger/), depende de los "números primos".
...
Sin embargo, si asumimos como cierta la **"Hipótesis Generalizada de [Riemann](https://kenji.blog/es/p/riemann/)" (Generalized Riemann Hypothesis, GRH)**, que extiende la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) a las funciones L de Dirichlet, la historia cambia drásticamente.
...
En otras palabras, la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) (y sus generalizaciones) desempeña el papel de otorgar una garantía directa a la creación de los cimientos de la criptografía: "si es posible generar números primos gigantes a gran velocidad y con absoluta confianza".
...
Para evaluar rigurosamente con qué frecuencia aparecen los números lisos, se requiere una profunda comprensión de la distribución de los números primos, y es aquí donde también se emplean técnicas de teoría analítica de números que están directamente vinculadas a la función zeta y a la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/). Si se demuestra la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) y se determina completamente el error en la distribución de los números primos, también será posible identificar de manera más precisa los límites de rendimiento de los algoritmos de factorización en números primos.
...
Existe una probabilidad extremadamente alta de que, en el proceso de demostrar la Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/), se descubran **"nuevas herramientas matemáticas" y "métodos analíticos desconocidos"**. Si observamos la historia, cuando se demostró el Último Teorema de Fermat o la [Conjetura de Poincaré](https://kenji.blog/es/p/poincare-conjecture/), las nuevas teorías desarrolladas en el proceso hicieron avanzar enormemente a todas las matemáticas.
...
La incansable búsqueda sobre los números primos que continúa desde la antigua Grecia fue sublimada en una hermosa sinfonía sobre el plano complejo (los ceros de la función zeta) por un genio llamado [Riemann](https://kenji.blog/es/p/riemann/). Y sorprendentemente, ese cristal de matemáticas puras y puristas se está aplicando, siglos después, como el escudo más fuerte para garantizar la seguridad de la sociedad de Internet.

La Hipótesis de [Riemann](https://kenji.blog/es/p/riemann/) es una entidad que simboliza simultáneamente la "belleza abstracta" que poseen las matemáticas y su "asombrosa aplicabilidad al mundo físico y a la sociedad real".
