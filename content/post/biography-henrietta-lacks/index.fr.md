...
```mermaid
graph TD
    A["Henrietta Lacks"] -- "Biopsie de tumeur (sans consentement)" --> B["Hôpital Johns Hopkins"]
    B -- "Fourniture d'échantillon de tissu" --> C["Laboratoire du Dr George Gey"]
    C -- "Culture dans un milieu spécial" --> D["Naissance des cellules HeLa (cellules immortalisées)"]
    D -- "Distribution gratuite" --> E["Institutions de recherche dans le monde entier"]
    E -- "Utilisation pour la recherche médicale" --> F["Vaccin contre la polio, recherche sur le cancer, génie génétique"]
```
...
```mermaid
graph LR
    A["Contributions des cellules HeLa"] --> B["Recherche sur les virus"]
    A --> C["Génétique et biologie cellulaire"]
    A --> D["Médecine et développement de médicaments"]
    
    B --> B1["Tests du vaccin contre la polio"]
    B --> B2["Recherche sur le VIH et le papillomavirus"]
    
    C --> C1["Détermination du nombre de chromosomes humains (46)"]
    C --> C2["Découverte de la télomérase"]
    C --> C3["Fondement de la technologie de clonage"]
    
    D --> D1["Tests de médicaments anticancéreux"]
    D --> D2["Développement de la technologie de FIV"]
    D --> D3["Recherche sur le vaccin COVID-19"]
```
