...
```mermaid
graph TD
    A["Henrietta Lacks"] -- "Tumorbiopsie (ohne Einwilligung)" --> B["Johns Hopkins Hospital"]
    B -- "Bereitstellung einer Gewebeprobe" --> C["Labor von Dr. George Gey"]
    C -- "Kultivierung in Spezialmedium" --> D["Geburt von HeLa-Zellen (unsterbliche Zellen)"]
    D -- "Kostenlose Verteilung" --> E["Forschungsinstitute weltweit"]
    E -- "Nutzung für medizinische Forschung" --> F["Polio-Impfstoff, Krebsforschung, Gentechnik"]
```
...
```mermaid
graph LR
    A["Beiträge von HeLa-Zellen"] --> B["Virusforschung"]
    A --> C["Genetik & Zellbiologie"]
    A --> D["Medizin & Medikamentenentwicklung"]
    
    B --> B1["Polio-Impfstofftests"]
    B --> B2["HIV-, Papillomavirus-Forschung"]
    
    C --> C1["Bestimmung der menschlichen Chromosomenzahl (46)"]
    C --> C2["Entdeckung von Telomerase"]
    C --> C3["Grundlage der Klontechnik"]
    
    D --> D1["Tests von Krebsmedikamenten"]
    D --> D2["Entwicklung der IVF-Technologie"]
    D --> D3["Erforschung von COVID-19-Impfstoffen"]
```
