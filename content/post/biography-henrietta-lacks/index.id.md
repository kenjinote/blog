...
```mermaid
graph TD
    A["Henrietta Lacks"] -- "Biopsi tumor (tanpa persetujuan)" --> B["Rumah Sakit Johns Hopkins"]
    B -- "Penyediaan sampel jaringan" --> C["Laboratorium Dr. George Gey"]
    C -- "Budidaya dalam media khusus" --> D["Kelahiran sel HeLa (sel abadi)"]
    D -- "Distribusi gratis" --> E["Lembaga penelitian di seluruh dunia"]
    E -- "Pemanfaatan untuk penelitian medis" --> F["Vaksin polio, penelitian kanker, rekayasa genetika"]
```
...
```mermaid
graph LR
    A["Kontribusi Sel HeLa"] --> B["Penelitian Virus"]
    A --> C["Genetika & Biologi Sel"]
    A --> D["Kedokteran & Pengembangan Obat"]
    
    B --> B1["Pengujian vaksin polio"]
    B --> B2["Penelitian HIV, papillomavirus"]
    
    C --> C1["Penentuan jumlah kromosom manusia (46)"]
    C --> C2["Penemuan telomerase"]
    C --> C3["Dasar teknologi kloning"]
    
    D --> D1["Pengujian obat antikanker"]
    D --> D2["Pengembangan teknologi IVF"]
    D --> D3["Penelitian vaksin COVID-19"]
```
