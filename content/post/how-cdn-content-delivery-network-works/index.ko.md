...
그 이면에는 **CDN(Content Delivery Network, 콘텐츠 전송 네트워크)**이라는 강력한 인프라스트럭처가 존재하고 있습니다. 본 기사에서는 현대 인터넷에서 필수 불가결해진 CDN의 원리와, 이를 뒷받침하는 핵심 기술(캐시, 에지 서버, 애니캐스트 라우팅)에 대해 상세히 해설합니다. 인프라 엔지니어나 웹 개발자뿐만 아니라 인터넷의 이면에 관심이 있는 모든 분들을 위한 기술 해설입니다.
...
CDN 제공업체(Cloudflare, Akamai, Fastly, AWS CloudFront 등)는 전 세계의 주요 인터넷 교환 노드(IX: Internet Exchange)나 데이터 센터에 수천에서 수만 대의 에지 서버를 설치해 두고 있습니다. 이러한 거점을 **PoP(Point of Presence)**라고 부릅니다.
...
'가장 가까운 에지 서버에 접속하게 한다'고 간단히 말했지만, 인터넷 상에서 사용자를 가장 가까운 서버로 자동 유도하려면 고도의 네트워크 기술이 필요합니다. 이때 사용되는 것이 **애니캐스트(Anycast)**입니다.
