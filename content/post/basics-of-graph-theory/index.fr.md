...
*   **Parcours en Profondeur (DFS)** : Explore aussi profondément que possible, et lorsqu'il arrive dans une impasse, revient au point de ramification précédent pour explorer un autre chemin. C'est comme résoudre un labyrinthe en suivant les murs. Utilisé pour détecter les cycles dans un graphe ou pour le tri topologique. Il est implémenté à l'aide d'une Pile ([Stack](https://kenji.blog/fr/p/c-language-pointers-memory-management-stack-heap/)) ou d'appels de fonctions récursives.
...
Les modèles d'apprentissage automatique traditionnels, tels que les CNN utilisés dans la reconnaissance d'images ou les [Transformer](https://kenji.blog/fr/p/large-language-models-llm-transformer-prompt-engineering/)s utilisés dans le traitement du langage naturel, ont été conçus pour gérer des données régulières comme des tableaux de pixels en grille ou des séquences de mots unidimensionnelles. Cependant, gérer des données de graphes irrégulières et complexes comme les connexions complexes des SNS ou les structures de liaisons atomiques constituant les molécules était extrêmement difficile.
