...
該当するハンドル（File, Section, Mutex, Eventなど）を保持しているプロセスが特定できたら、対象プロセスを右クリックして `Close Handle` を強制実行することで、プロセスをキルせずにファイルの[ロック](https://kenji.blog/p/rdbms-transaction-acid-isolation-level-lock/)を解除できます（ただし、アプリの動作が不安定になるリスクには注意が必要です）。
