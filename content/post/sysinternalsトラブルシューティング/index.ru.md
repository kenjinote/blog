...
*   **[Service](https://kenji.blog/ru/p/kubernetes-k8s-architecture-pod-service-ingress/)s / Drivers**: Драйверы, загружаемые в режиме ядра. Здесь можно отключить подозрительные файлы `.sys`, вызывающие 100% загрузку ЦП, о чем упоминалось ранее.
...
TCPView отслеживает все конечные точки [TCP](https://kenji.blog/ru/p/http3-quic-protocol-tcp-udp/) и [UDP](https://kenji.blog/ru/p/http3-quic-protocol-tcp-udp/) в реальном времени и отображает список, показывающий, какой процесс взаимодействует с каким удаленным адресом и портом.
...
При обнаружении подозрительного соединения вы можете принудительно разорвать [TCP](https://kenji.blog/ru/p/http3-quic-protocol-tcp-udp/)-сессию (выдать пакет RST), отправив `Close Connection` прямо из TCPView, или принудительно завершить процесс с помощью `End Process`.
