---
title: '【PQC】تهديد الحواسيب الكمية والنظرة الشاملة لتشفير ما بعد الكم (PQC) للجيل القادم'
...
description: 'يواجه تشفير المفتاح العام خطر اختراقه في المستقبل مع الاستخدام العملي للحواسيب الكمية. تشرح هذه المقالة بالتفصيل آلية وأهمية تقنية أمان الجيل القادم "تشفير ما بعد الكم (PQC)" الجاري تطويره لمواجهة هذا التهديد (مشكلة Y2Q).'
---

## مقدمة: "التهديد" الذي تشكله الحواسيب الكمومية على تقنيات التشفير

في الوقت الحاضر، يتم حماية الغالبية العظمى من الاتصالات اليومية التي نجريها عبر الإنترنت - بدءًا من المدفوعات المصرفية عبر الإنترنت، وتصفح الويب (HTTPS)، وتبادل الرسائل في التطبيقات، وصولاً إلى معاملات البلوكشين والأصول المشفرة - بواسطة تقنية تُعرف باسم "تشفير المفتاح العام". وعلى وجه التحديد، تشكل خوارزميات مثل تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) وتشفير المنحنى الإهليلجي ([ECC](/ar/p/elliptic-curve-cryptography-math-cpp/)) الأساس الذي تقوم عليه موثوقية المجتمع الرقمي الحديث.
...
غالبًا ما يتم الخلط بين هذه التكنولوجيا و "التشفير الكمومي" أو "توزيع المفتاح الكمومي (QKD)"، إلا أنهما نهجان مختلفان تمامًا. فالتشفير الكمومي (QKD) هو تقنية تعتمد على الأجهزة تستخدم القوانين الفيزيائية لميكانيكا الكم (مثل خاصية تغير الحالة عند المراقبة) لجعل التنصت على [مسار](/ar/p/windows-%E3%81%A7%D9%85%D8%B3%D8%A7%D8%B1%E3%81%AE%E9%80%9A%E3%81%A3%E3%81%9F%D9%85%D9%84%D9%81-%D8%AA%D9%86%D9%81%D9%8A%D8%B0%D9%8A%E3%81%AE%E5%A0%B4%E6%89%80%E3%82%92%E8%A6%8B%E3%81%A4%E3%81%91%E3%82%8B%E6%96%B9%E6%B3%95/) الاتصال مستحيلاً ماديًا. وهو يتطلب أليافًا بصرية خاصة أو معدات خاصة، ويواجه تحديات مثل تكلفة التنفيذ وقيود المسافة.

من ناحية أخرى، **تُعد PQC تقنية تشفير تعتمد على البرامج وتستند بالكامل إلى "الرياضيات"** . وبالتالي، يمكن دمجها كتحديث برمجي في البنية التحتية الحالية للإنترنت، والخوادم، والهواتف الذكية، والمتصفحات، وغيرها، مما يجعل قابليتها للتطبيق في العالم الحقيقي عالية جدًا. وتعتبر شركات تكنولوجيا المعلومات والوكالات الحكومية في جميع أنحاء العالم الآن أن استبدال (ترحيل) خوارزميات [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) و [ECC](/ar/p/elliptic-curve-cryptography-math-cpp/) المستخدمة حاليًا بتقنية PQC هذه يمثل أولوية قصوى.
...
### 1. التشفير القائم على الشبكة (Lattice-based [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)
...
- سرعة المعالجة عالية جدًا (تكون أحيانًا أسرع من [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) أو [ECC](/ar/p/elliptic-curve-cryptography-math-cpp/)).
...
### 2. التشفير القائم على التجزئة (Hash-based [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)
...
**نظرة عامة على الآلية:** 
يبدأ الأمر بمخطط توقيع يُستخدم لمرة واحدة يُسمى "توقيع لامبورت (Lamport Signature)"، والذي لا يمكن استخدامه إلا مرة واحدة. من خلال تجميع هذه التوقيعات في بنية بيانات شجرية تسمى "شجرة ميركل (Merkle [Tree](https://kenji.blog/ar/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/))"، فإنها تتيح توقيعات متعددة باستخدام زوج مفاتيح واحد.
...
### 3. التشفير متعدد المتغيرات (Multivariate [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)
...
- سرعة التحقق من التوقيع سريعة للغاية. مناسب لأجهزة [إنترنت الأشياء](/ar/p/technology-iot/) (IoT) ذات الموارد المحدودة.
...
### 4. التشفير القائم على الكود (Code-based [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)

التشفير القائم على الكود هو تطبيق لنظرية "أكواد تصحيح الأخطاء" المستخدمة لتصحيح الأخطاء في قنوات الاتصال في مجال التشفير. يعتبر "تشفير ماك إيليس (McEliece [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)" المقترح في عام 1978 هو الأكثر شهرة، وهو أحد أقدم التقنيات في مجال PQC.
...
بينما أنهت NIST عملية التوحيد القياسي الأولية، فإنها تواصل البحث عن خوارزميات إضافية. ونظرًا لأن المعايير تميل بشكل كبير نحو "التشفير القائم على الشبكة"، يعتبر ضمان **"تنوع التشفير ([Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/) Diversity)"** أمرًا بالغ الأهمية. ويجري تقييم التشفير القائم على الكود وغيرها كمعايير احتياطية لتبادل المفاتيح، مما سيعزز من قوة أساس PQC في المستقبل.

---

## سيناريوهات وتحديات الانتقال إلى PQC: أهمية "رشاقة التشفير"

مع إصدار المعايير الرسمية من قبل NIST، ستعمل الوكالات الحكومية والمؤسسات المالية وشركات التكنولوجيا في جميع أنحاء العالم على تسريع انتقالها (الهجرة) من [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/)/[ECC](/ar/p/elliptic-curve-cryptography-math-cpp/) الحالي إلى PQC. كما توصي المبادئ التوجيهية الصادرة عن وكالة الأمن القومي الأمريكية (NSA) وغيرها بالانتهاء من الانتقال في وقت مبكر.
...
### تحقيق رشاقة التشفير ([Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)-Agility)

إن أهم شيء يجب على الشركات ومطوري الأنظمة إدراكه للمضي قدمًا هو ضمان **"رشاقة التشفير ([Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)-Agility)"** . من الضروري تصميم بنية مرنة تسمح بتبديل خوارزميات التشفير أو تحديثها بسرعة دون إيقاف النظام في حالة اكتشاف عيوب في الخوارزمية أو ظهور معايير جديدة.

يعد إنشاء قائمة مواد التشفير (CBOM: [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy Bill of Materials) لفهم "أين" و "أي تشفير" و "لأي غرض" يتم استخدامه داخل الأنظمة الداخلية بدقة، بمثابة خطوة أولى مهمة نحو الانتقال إلى PQC.
