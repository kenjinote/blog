...
```mermaid
graph TD
    A["Schiff des Theseus mit Originalteilen"] -- "Alterung der Teile" --> B["Teilweiser Austausch"]
    B -- "Kontinuierlicher Austausch" --> C["Schiff des Theseus mit komplett neuen Teilen"]
    A -- "Frage der Identität" --> C
```
...
```mermaid
graph TD
    A["Originales Schiff"] -- "Alte Teile entfernen" --> B["Restauriertes Schiff (neue Teile)"]
    A -- "Alte Teile sammeln" --> C["Schiff aus alten Teilen zusammengebaut"]
    B -- "Identisch mit A?" --> D["Divergenz der Identität"]
    C -- "Identisch mit A?" --> D
```
