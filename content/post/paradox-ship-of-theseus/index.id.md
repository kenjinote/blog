...
```mermaid
graph TD
    A["Kapal Theseus dengan bagian asli"] -- "Kerusakan bagian" --> B["Penggantian sebagian"]
    B -- "Penggantian terus-menerus" --> C["Kapal Theseus dengan semua bagian baru"]
    A -- "Pertanyaan tentang identitas" --> C
```
...
```mermaid
graph TD
    A["Kapal asli"] -- "Melepas bagian lama" --> B["Kapal yang direstorasi (bagian baru)"]
    A -- "Mengumpulkan bagian lama" --> C["Kapal yang dirakit dari bagian lama"]
    B -- "Apakah identik dengan A?" --> D["Divergensi identitas"]
    C -- "Apakah identik dengan A?" --> D
```
