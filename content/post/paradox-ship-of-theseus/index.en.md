...
```mermaid
graph TD
    A["Original parts of the Ship of Theseus"] -- "Parts aging and deteriorating" --> B["Partially replaced"]
    B -- "Continuous replacement" --> C["Ship of Theseus with all new parts"]
    A -- "Question of identity" --> C
```
...
```mermaid
graph TD
    A["Original ship"] -- "Remove old parts" --> B["Restored ship (new parts)"]
    A -- "Gather old parts" --> C["Ship assembled with old parts"]
    B -- "Is it identical to A?" --> D["Divergence of identity"]
    C -- "Is it identical to A?" --> D
```
