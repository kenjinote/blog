...
2. **Чанкинг (разделение текста)**: Разделение длинных текстов на осмысленные фрагменты (чанки) для того, чтобы они поместились в ограничение на ввод [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) (контекстное окно) и для повышения точности поиска.
...
1. **Языковая модель ([LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/))**
...
2. **Модель встраивания (Embedding)**
   - Модель: `intfloat/multilingual-e5-large` или `BAAI/bge-m3`. При локальном запуске обычно их скачивают с Hugging Face и запускают с помощью Sentence-[Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/)s.
...
Ниже представлен полный Python-скрипт для загрузки PDF-файла, его векторизации и ответов на вопросы с помощью локальной [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/).
...
3. **Отбор**: Только верхние 3–5 с наивысшими баллами передаются в промпт [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) в качестве окончательного контекста.
