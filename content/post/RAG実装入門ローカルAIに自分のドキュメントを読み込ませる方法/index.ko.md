...
벡터 데이터베이스가 관련 문서를 검색할 때 가장 일반적으로 사용되는 거리 지표가 **코사인 유사도(Cosine Similarity)** 입니다. [유클리드](https://kenji.blog/p/euclid/) 거리(공간적인 절대 거리)와는 달리, 코사인 유사도는 '두 벡터가 이루는 각'에 주목합니다. 문장의 길이(벡터의 노름)에 영향을 덜 받기 때문에 텍스트의 유사도 계산에 매우 적합합니다.
