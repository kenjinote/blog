...
當向量資料庫在檢索相關文件時，最常使用的距離指標是 **餘弦相似度（Cosine Similarity）** 。與[歐幾里得](https://kenji.blog/zh-tw/p/euclid/)距離（空間上的絕對距離）不同，餘弦相似度關注的是「兩個向量之間的夾角」。因為它較不易受文章長度（向量的範數）影響，所以非常適合用於文字的相似度計算。
