---
title: "Napoleão Bonaparte: Filho da Revolução ou Ditador?"
...
```mermaid
graph TD
    A["Ascensão Militar (1793-1799)"] --> B["Nomeação como Primeiro Cônsul (1799)"]
    B --> C["Promoção de Reformas Internas"]
    C --> D["Criação do Banco de França (1800)"]
    C --> E["Assinatura da Concordata (1801)"]
    C --> F["Criação da Ordem da Legião de Honra (1802)"]
    C --> G["Promulgação do Código Napoleônico (1804)"]
    B --> H["Transição para o Império (1804)"]
    H --> I["Batalha de Austerlitz (1805)"]
    H --> J["Bloqueio Continental (1806)"]
```
...
```mermaid
graph LR
    N["Napoleão Bonaparte"] -->|Primeira Esposa| J["Josefina"]
    N -->|Segunda Esposa| ML["Maria Luísa (Casa de Habsburgo)"]
    N -->|Arqui-inimigo| W["Duque de Wellington (Inglaterra)"]
    N -->|Arqui-inimigo| N2["Almirante Nelson (Inglaterra)"]
    N -->|Aliança e Traição| A["Alexandre I (Rússia)"]
    N -->|Ministro das Relações Exteriores| T["Talleyrand"]
    N -->|Ministro da Polícia| F["Fouché"]
```
