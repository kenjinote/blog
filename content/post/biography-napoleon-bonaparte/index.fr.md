---
title: "Napoléon Bonaparte : Enfant de la Révolution ou dictateur ?"
...
```mermaid
graph TD
    A["Ascension militaire (1793-1799)"] --> B["Nomination comme Premier Consul (1799)"]
    B --> C["Promotion des réformes intérieures"]
    C --> D["Création de la Banque de France (1800)"]
    C --> E["Signature du Concordat (1801)"]
    C --> F["Création de la Légion d'honneur (1802)"]
    C --> G["Promulgation du Code Napoléon (1804)"]
    B --> H["Transition vers l'Empire (1804)"]
    H --> I["Bataille d'Austerlitz (1805)"]
    H --> J["Décret du Blocus continental (1806)"]
```
...
```mermaid
graph LR
    N["Napoléon Bonaparte"] -->|Première épouse| J["Joséphine"]
    N -->|Seconde épouse| ML["Marie-Louise (Maison de Habsbourg)"]
    N -->|Ennemi juré| W["Duc de Wellington (Grande-Bretagne)"]
    N -->|Ennemi juré| N2["Amiral Nelson (Grande-Bretagne)"]
    N -->|Alliance et trahison| A["Alexandre Ier (Russie)"]
    N -->|Ministre des Affaires étrangères| T["Talleyrand"]
    N -->|Ministre de la Police| F["Fouché"]
```
