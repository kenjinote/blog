---
title: "Napoleon Bonaparte: Kind der Revolution oder Diktator?"
...
```mermaid
graph TD
    A["Militärischer Aufstieg (1793-1799)"] --> B["Ernennung zum Ersten Konsul (1799)"]
    B --> C["Förderung innenpolitischer Reformen"]
    C --> D["Gründung der Bank von Frankreich (1800)"]
    C --> E["Abschluss des Konkordats (1801)"]
    C --> F["Gründung des Ordens der Ehrenlegion (1802)"]
    C --> G["Verkündung des Code civil (1804)"]
    B --> H["Übergang zum Kaiserreich (1804)"]
    H --> I["Schlacht bei Austerlitz (1805)"]
    H --> J["Kontinentalsperre (1806)"]
```
...
```mermaid
graph LR
    N["Napoleon Bonaparte"] -->|Erste Ehefrau| J["Joséphine"]
    N -->|Zweite Ehefrau| ML["Marie-Louise (Habsburg)"]
    N -->|Erzfeind| W["Herzog von Wellington (Großbritannien)"]
    N -->|Erzfeind| N2["Admiral Nelson (Großbritannien)"]
    N -->|Allianz und Verrat| A["Alexander I. (Russland)"]
    N -->|Außenminister| T["Talleyrand"]
    N -->|Polizeiminister| F["Fouché"]
```
