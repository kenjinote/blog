---
title: 'Ускорение разработки в Salesforce! Подборка рекомендуемых расширений Chrome'
slug: "SalesforceおすすめChrome拡張"
date: 2022-05-02T10:39:36+09:00
tags: ["Salesforce", "Расширения Chrome"]
draft: false
image: "img.webp"
categories: ["ИТ и технологии"]
description: 'Представляем рекомендуемые расширения Chrome, которые кардинально повышают эффективность разработки и работы с Salesforce. Мы отобрали полезные инструменты для инженеров, такие как DevTools, удобный для поиска настраиваемых полей и построения SOQL, а также расширения для поиска по коду.'
---

### Salesforce DevTools

![img_1.png](img_1.webp)

![img_2.png](img_2.webp)

Вы можете искать пользовательские поля или выводить поля пользовательских объектов все сразу.

Также удобно собирать SOQL на месте.

[Salesforce DevTools](https://chrome.google.com/webstore/detail/salesforce-devtools/ehgmhinnhggigkogkbhnbodhbfjgncjf)

### Salesforce advanced Code searcher

![img.png](img.webp)

Это удобно при поиске кода Salesforce.

[Salesforce advanced Code searcher](https://chrome.google.com/webstore/detail/salesforce-advanced-code/lnkgcmpjkkkeffambkllliefdpjdklmi)
