---



title: '¡Mejora el desarrollo en Salesforce! Resumen de las mejores extensiones de Chrome'
slug: "SalesforceおすすめChrome拡張"
date: 2022-05-02T10:39:36+09:00
tags: ["Salesforce", "Extensiones de Chrome"]
draft: false
image: "img.webp"
categories: ["TI y Tecnología"]
description: 'Presentamos extensiones de Chrome recomendadas que mejoran drásticamente la eficiencia en el desarrollo y operación de Salesforce. Seleccionamos herramientas imprescindibles para ingenieros, como DevTools para buscar campos personalizados y armar SOQL fácilmente, así como extensiones útiles para búsquedas de código.'
---




### Salesforce DevTools

![img_1.png](img_1.webp)

![img_2.png](img_2.webp)

Puedes buscar campos personalizados o generar en lote los campos de objetos personalizados.

También es útil para construir SOQL sobre la marcha.

[Salesforce DevTools](https://chrome.google.com/webstore/detail/salesforce-devtools/ehgmhinnhggigkogkbhnbodhbfjgncjf)

### Salesforce advanced Code searcher

![img.png](img.webp)

Es útil para buscar código en Salesforce.

[Salesforce advanced Code searcher](https://chrome.google.com/webstore/detail/salesforce-advanced-code/lnkgcmpjkkkeffambkllliefdpjdklmi)
