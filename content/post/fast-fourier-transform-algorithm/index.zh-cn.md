...
1965年，由詹姆斯·库利（James Cooley）和约翰·图基（John Tukey）重新发现的算法（实际上据说卡尔·弗里德里希·高斯在1805年就已经发现了类似的方法），是现代最常用的FFT算法。这里我们将推导当数据量 $N$ 是2的幂（$N = 2^m$）时的基-2时间抽取（Decimation-in-Time, DIT）FFT。
