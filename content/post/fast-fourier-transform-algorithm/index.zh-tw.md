...
1965 年，由詹姆斯·庫利（James Cooley）與約翰·圖基（John Tukey）重新發現的演算法（事實上[卡爾·弗里德里希·高斯](/zh-tw/p/gauss/)早在 1805 年就發現了類似的手法），是現代最被廣泛使用的 FFT 演算法。在此，我們將推導當資料數 $N$ 為 2 的次方（$N = 2^m$）時，基數為 2 的時間抽取（Decimation-in-Time, DIT）FFT。
