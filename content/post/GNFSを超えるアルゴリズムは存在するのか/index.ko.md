...
현대 사회에서 인터넷 통신의 안전성은 공개키 암호인 [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 암호의 안전성에 크게 의존하고 있습니다. 그리고 RSA 암호의 안전성은 '거대한 합성수의 소인수분해의 어려움'이라는 수학적 가정에 기반을 두고 있습니다. 만약 극도로 효율적인 소인수분해 알고리즘이 발견된다면, 전 세계의 통신 인프라는 뿌리째 흔들리게 될 것입니다.
...
소인수분해 알고리즘의 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)를 평가할 때, 표준적인 다항식 시간 표기($O(n^k)$ 등)가 아니라 입력 $n$의 자릿수에 대한 준지수 시간을 표현하기 위해 **L 표기법(L-notation)** 이 사용됩니다. L 표기법은 다음과 같이 정의됩니다.
...
- **연분수법(CFRAC)이나 다중 다항식 이차 체(MPQS)**: $\alpha = 1/2$ 클래스에 속하며, 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)는 $L_n[1/2, 1]$ 정도입니다.
- **일반 수체 체(GNFS)**: $\alpha = 1/3$을 달성하여, 현재 알려진 고전 알고리즘 중 가장 빠른 $L_n[1/3, (64/9)^{1/3}]$이라는 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)를 자랑합니다.
...
현재까지 일반 정수에 대한 소인수분해에서 점근적 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)가 $L_n[1/3, c]$를 밑도는 고전 알고리즘은 발견되지 않았습니다. 그러나 이론적, 실천적 한계를 돌파하기 위한 몇 가지 시도나 파생 알고리즘이 존재합니다.
...
여러 개의 대수체를 이용함으로써 각 체질 단계에서 '어느 한 대수체에서 매끄러워질' 확률을 비약적으로 높일 수 있습니다. Coppersmith는 이 접근법을 통해 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) $L_n[1/3, c]$의 상수 $c$를 약간 감소시키는 데 성공했습니다.
구체적으로, GNFS의 상수가 $c = (64/9)^{1/3} \approx 1.923$인 반면, MNFS를 최적화하면 $c \approx 1.902$ 정도까지 계산량을 줄일 수 있음이 이론적으로 증명되었습니다.
그러나 실제 적용에 있어서는 여러 체를 관리하는 오버헤드가 크기 때문에, 실용적 규모의 [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 모듈러스에 대한 결정적인 돌파구에는 이르지 못했습니다.
...
만약 $L_n[1/4]$ 혹은 고전적인 다항식 시간 알고리즘이 존재한다고 가정한다면, 그것은 GNFS와 같은 '매끄러움 기반'의 접근법과는 전혀 다른, 현재 인류가 생각지도 못한 새로운 수학적 구조(예를 들어, [타원곡선 암호](/ko/p/elliptic-curve-cryptography-math-cpp/)에 대한 Schoof의 알고리즘과 같은 더 고차원적인 대수기하학적 접근 등)에 의존해야 할 것입니다. 그러나 현재로서는 그러한 징후가 보이지 않습니다.

---

## 5. 양자 컴퓨터에 의한 돌파구: 쇼어의 알고리즘

고전 컴퓨터가 $L_n[1/3]$의 벽에 직면해 있는 반면, 계산 모델 자체를 근본적으로 바꿈으로써 이 벽을 산산조각 낸 것이 1994년 Peter Shor가 발표한 **쇼어의 알고리즘([Shor's Algorithm](https://kenji.blog/ko/p/quantum-computing-shors-algorithm/))** 입니다.
...
계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 측면에서 볼 때, 쇼어의 알고리즘의 실행 시간은 **양자 다항식 시간**, 구체적으로 다음과 같습니다.
$$ O((\log n)^3) $$
...
이 두 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 클래스의 차이는 현실 세계의 암호 보안에서 결정적인 의미를 갖습니다.

예를 들어 [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/)-2048(2048비트의 합성수)을 소인수분해하는 경우를 생각해 봅시다.
...
- **[Shor's Algorithm](https://kenji.blog/ko/p/quantum-computing-shors-algorithm/) (양자)**: $O((\log n)^3)$ 알고리즘에서는 $2048^3 \approx 8.5 \times 10^9$번 정도의 논리 게이트 조작으로 끝납니다. 이는 적절한 하드웨어(수백만 개의 물리 양자 비트와 오류 정정 능력을 갖춘 범용 양자 컴퓨터)가 존재한다면 단 몇 시간에서 며칠 만에 계산이 완료됨을 의미합니다.
...
2. **고전 컴퓨터 상에서의 [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 안전성은 여전히 견고하다**: GNFS의 계산량은 여전히 막대하며, RSA-2048이나 RSA-4096은 고전 컴퓨터의 공격에 대해 앞으로 수십 년간 안전성을 유지할 것입니다.
3. **진정한 위협은 양자 알고리즘이다**: 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)의 벽을 넘은 것은 양자역학의 원리에 기반한 쇼어의 알고리즘입니다. 이로 인해 세계는 양자 내성 암호(PQC: Post-Quantum [Crypto](https://kenji.blog/ko/p/cryptocurrency-and-bitcoin/)graphy)로의 전환을 강요받고 있습니다. 격자 기반 암호나 해시 기반 암호 등 양자 컴퓨터로도 해독이 어려운(다항식 시간으로 풀 수 없는) 새로운 수학적 문제로의 전환이 현재 암호학의 최전선이 되고 있습니다.

일반 수체 체(GNFS)는 인류가 고전 수학과 알고리즘 설계의 극한까지 도전하여 도달한 '최고 도달점' 중 하나입니다. GNFS의 심오한 수학적 구조를 이해하는 것은 단순히 암호 해독의 역사를 배우는 것을 넘어, 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 이론이나 대수적 정수론의 아름다움을 접하는 지적 탐구의 여정이기도 합니다. 양자 컴퓨터가 실용화되는 그날까지 GNFS는 최강의 소인수분해 알고리즘으로서의 왕좌를 계속 지킬 것입니다.
