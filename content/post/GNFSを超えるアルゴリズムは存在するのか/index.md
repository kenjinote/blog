...
3. **真の脅威は量子アルゴリズムである**: 計算複雑性の壁を越えたのは量子力学の原理に基づくショアのアルゴリズムです。これにより、世界は耐量子計算機暗号（PQC: Post-Quantum [Crypto](https://kenji.blog/p/cryptocurrency-and-bitcoin/)graphy）への移行を余儀なくされています。格子暗号やハッシュベース暗号など、量子コンピュータでも解読が困難（多項式時間で解けない）とされる新しい数学的問題への移行が、現在の暗号学の最前線となっています。
