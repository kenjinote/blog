...
# Что такое [Гамма-функция](https://kenji.blog/ru/p/gamma-function/)?

При изучении математики мы иногда сталкиваемся с вопросом: "Можно ли расширить дискретное понятие до непрерывного?" Одним из самых красивых и важных примеров этого является **[Гамма-функция](https://kenji.blog/ru/p/gamma-function/)**.

[Гамма-функция](https://kenji.blog/ru/p/gamma-function/) расширяет "факториал" ($n!$), определенный для натуральных чисел, на положительные действительные числа и даже на всю комплексную плоскость. Открытая великим математиком 18-го века Леонардом Эйлером, эта функция появляется почти во всех областях: от математического анализа и теории вероятностей до статистики и физики.
...
[Гамма-функция](https://kenji.blog/ru/p/gamma-function/) $\Gamma(z)$ обычно определяется следующим интегралом (интеграл Эйлера второго рода):
...
Это уравнение является главной причиной того, что [Гамма-функция](https://kenji.blog/ru/p/gamma-function/) является расширением факториала. Если $z$ является натуральным числом $n$, мы можем вычислить его следующим образом, используя $\Gamma(1) = 1$:
...
Повторяя эту операцию, [Гамма-функция](https://kenji.blog/ru/p/gamma-function/) становится мероморфной функцией, определенной на всей комплексной плоскости, за исключением $z = 0, -1, -2, \dots$ (всех неположительных целых чисел). [Гамма-функция](https://kenji.blog/ru/p/gamma-function/) расходится в неположительных целых числах, и в каждой из этих точек существует **Полюс**.
...
[Гамма-функция](https://kenji.blog/ru/p/gamma-function/) тесно связана с еще одной важной специальной функцией — **Бета-функцией**. Бета-функция $B(x, y)$ определяется следующим образом:
...
[Гамма-функция](https://kenji.blog/ru/p/gamma-function/) — это не просто плод математического любопытства. Она играет практическую роль во многих областях, таких как:
...
2. **Физика**: В размерной регуляризации в квантовой механике и квантовой теории поля [Гамма-функция](https://kenji.blog/ru/p/gamma-function/) играет роль в контроле расходимостей.
...
Поиск, который начался с простого вопроса о расширении факториала на действительные числа, выявил великолепную структуру, пронизывающую всю математику. [Гамма-функция](https://kenji.blog/ru/p/gamma-function/) — это поистине шедевр Эйлера, перекидывающий мост между дискретным и непрерывным мирами.
