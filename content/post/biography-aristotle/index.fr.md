...
```mermaid
flowchart TD
    %% Généalogie de la Grèce antique
    S["Socrate (Quête de l'éthique)"] --> P["Platon (Théorie des Idées)"]
    P --> A["Aristote (Empirisme et systématisation)"]
    
    %% Influence directe d'Aristote
    A --> Alex["Alexandre le Grand (Conquérant macédonien)"]
    A --> Lyceum["Lycée (École péripatéticienne)"]
    
    %% Propagation et développement pour la postérité
    Lyceum --> Islam["Philosophie islamique (Ibn Rushd, etc.)"]
    Islam --> MiddleAges["Scolastique médiévale (Thomas d'Aquin)"]
    
    %% Lien vers l'ère moderne
    MiddleAges --> Modern["Science et philosophie modernes (Héritage de la logique et de la taxonomie)"]
