---
title: "Aristóteles, o Pai de Todas as Ciências: O Supremo Buscador do Conhecimento Deixado pela Grécia Antiga"
...
```mermaid
flowchart TD
    %% Genealogia da Grécia Antiga
    S["Sócrates (Busca da Ética)"] --> P["Platão (Teoria das Ideias)"]
    P --> A["Aristóteles (Empirismo e Sistematização)"]
    
    %% Influência Direta de Aristóteles
    A --> Alex["Alexandre, o Grande (Conquistador da Macedônia)"]
    A --> Lyceum["Liceu (Escola Peripatética)"]
    
    %% Propagação e Desenvolvimento para as Gerações Posteriores
    Lyceum --> Islam["Filosofia Islâmica (Ibn Rushd, etc.)"]
    Islam --> MiddleAges["Escolasticismo Medieval (Tomás de Aquino)"]
    
    %% Conexão com os Tempos Modernos
    MiddleAges --> Modern["Ciência e Filosofia Modernas (Herança da Lógica e Classificação)"]
