...
description: "HTTPの仕組みと歴史、Webを支えるステートレスプロトコルについて解説します。"
...
Hypertext Transfer Protocol (HTTP) は、Webの基盤となる通信プロトコルです。

## ステートレスな設計

HTTPは状態を持たない（ステートレス）プロトコルです。各リクエストは独立して処理されます。
...
HTTP/2 や HTTP/3 では、多重化によりラウンドトリップタイム(RTT)の影響を軽減しています。ページのロード時間は次のようにモデル化できます。

$$ T_{load} = T_{DNS} + T_{TCP} + T_{TLS} + \sum_{i=1}^{N} \left( \frac{S_i}{B} + RTT \right) $$

多重化により、後半の $\sum$ 部分が並列化され、劇的に時間が短縮されます。
