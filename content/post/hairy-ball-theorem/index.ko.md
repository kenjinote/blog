...
위상수학(토폴로지)이라는 수학 분야에는 직관적으로 재미있고 강력한 정리가 많이 존재합니다. 그중에서도 특히 유명한 것이 **털 난 공 정리** (Hairy Ball Theorem)입니다. 이 정리는 "털이 난 공을, 가마를 하나도 만들지 않고 예쁘게 빗질할 수는 없다"라는, 매우 시각적이고 알기 쉬운 말로 표현됩니다.
...
털 난 공 정리는 1885년에 앙리 푸앵카레(Henri Poincaré)에 의해 처음 언급되었고, 1912년에 라위천 에흐베르튀스 얀 브라우어르(Luitzen Egbertus Jan Brouwer)에 의해 엄밀하게 증명되었습니다.
...
다면체의 오일러 지표 $\chi$ 는 꼭짓점의 수($V$), 모서리의 수($E$), 면의 수($F$)를 사용하여 다음과 같은 유명한 공식(오일러의 다면체 정리)으로 계산됩니다.
