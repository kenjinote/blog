...
```mermaid
graph TD
    A["Boîte avec une température uniforme (état initial)"] -- "Observation des molécules par le démon" --> B["Le démon ouvre et ferme la porte"]
    B -- "Molécules rapides (haute température)" --> C["Rassemblées dans la chambre gauche (B)"]
    B -- "Molécules lentes (basse température)" --> D["Rassemblées dans la chambre droite (A)"]
    C -- "Augmentation de l'énergie cinétique moyenne" --> E["La chambre gauche devient plus chaude"]
    D -- "Diminution de l'énergie cinétique moyenne" --> F["La chambre droite devient plus froide"]
    E -- "Génération spontanée d'une différence de température" --> G["Diminution de l'entropie (violation de la 2e loi)"]
    F -- "Génération spontanée d'une différence de température" --> G
```
...
```mermaid
graph LR
    A["Mémoire du démon (état vide)"] -- "Mesure réversible de l'information moléculaire" --> B["Mémoire du démon (état enregistré)"]
    B -- "Ouverture/fermeture de la porte et tri" --> C["Diminution de l'entropie dans le système"]
    C -- "Effacement de la mémoire pour le prochain cycle" --> D["Initialisation de l'information (Principe de Landauer)"]
    D -- "Libération inévitable de chaleur" --> E["Augmentation de l'entropie de l'environnement (défense de la 2e loi)"]
    E -- "Cycle terminé" --> A
```
