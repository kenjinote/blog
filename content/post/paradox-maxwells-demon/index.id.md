...
```mermaid
graph TD
    A["Kotak bersuhu seragam (kondisi awal)"] -- "Pengamatan molekul oleh setan" --> B["Setan membuka dan menutup pintu"]
    B -- "Molekul cepat (suhu tinggi)" --> C["Dikumpulkan di ruang kiri (B)"]
    B -- "Molekul lambat (suhu rendah)" --> D["Dikumpulkan di ruang kanan (A)"]
    C -- "Peningkatan energi kinetik rata-rata" --> E["Suhu ruang kiri memanas"]
    D -- "Penurunan energi kinetik rata-rata" --> F["Suhu ruang kanan mendingin"]
    E -- "Timbulnya perbedaan suhu secara spontan" --> G["Penurunan entropi (Pelanggaran Hukum Kedua)"]
    F -- "Timbulnya perbedaan suhu secara spontan" --> G
```
...
```mermaid
graph LR
    A["Ingatan setan (kondisi kosong)"] -- "Mengukur informasi molekul secara reversibel" --> B["Ingatan setan (kondisi terekam)"]
    B -- "Membuka/menutup pintu dan penyortiran" --> C["Penurunan entropi dalam sistem"]
    C -- "Menghapus ingatan untuk siklus berikutnya" --> D["Inisialisasi informasi (Prinsip Landauer)"]
    D -- "Pelepasan panas yang tak terelakkan" --> E["Peningkatan entropi lingkungan (Mempertahankan Hukum Kedua)"]
    E -- "Siklus selesai" --> A
```
