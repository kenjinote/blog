...
## 1. テンプレートメタプログラミングの夜明け：偶然発見された[チューリング](https://kenji.blog/p/turing/)完全性

### 1.1 [チューリング](https://kenji.blog/p/turing/)完全性とは

計算機科学において、「[チューリング](https://kenji.blog/p/turing/)完全（Turing Complete）」であるとは、万能[チューリング](https://kenji.blog/p/turing/)マシンと同じ計算能力を持つことを意味します。平たく言えば、「条件分岐」と「無限ループ（または再帰）」を表現でき、任意のアルゴリズムを記述・実行できるシステムのことです。
...
コンパイラはテンプレートのインスタンス化（実体化）の過程で再帰的な処理を行い、エラーメッセージとしてその計算結果を出力していました。つまり、C++のテンプレート機能が、言語設計者であるビャーネ・ストロヴストルップすら意図していなかった **[チューリング](https://kenji.blog/p/turing/)完全な計算体系** を内包していることが証明された瞬間でした。
...
### 2.2 [フィボナッチ](https://kenji.blog/p/fibonacci/)数列と計算量

次に、[フィボナッチ](https://kenji.blog/p/fibonacci/)数列を計算してみましょう。漸化式は以下の通りです。
