...
解释这种神秘现象的数学定律就是 **[本福特定律](https://kenji.blog/p/benfords-law/) ([Benford's Law](https://kenji.blog/p/benfords-law/))** 。

在本文中，我们将详细解释[本福特定律](https://kenji.blog/p/benfords-law/)的机制、为什么会发生这种现象，以及这一定律是如何被应用于欺诈检测的。

## 什么是[本福特定律](https://kenji.blog/p/benfords-law/)？

[本福特定律](https://kenji.blog/p/benfords-law/)（又称第一数字定律）指出，在许多现实生活中的数值数据集合中，首位数字（最高位的非零数字）出现的概率对于较小的数字来说更高。
...
如果存在一个普遍的自然法则，那么即使改变测量单位，该法则本身也不应该改变。例如，无论是用公里还是英里来测量距离，首位数字的出现概率都必须相同。在数学上，当寻找一种即使乘以一个常数（尺度不变性）分布也不会改变的概率分布时，必然会得出[本福特定律](https://kenji.blog/p/benfords-law/)的对数分布。
...
[本福特定律](https://kenji.blog/p/benfords-law/)不能应用于所有数据。适用的数据和不适用的数据之间存在明显的区别。
...
目前，[本福特定律](https://kenji.blog/p/benfords-law/)最实用的领域之一就是 **欺诈检测 (Fraud Detection)** 。

当人们试图随意编造或篡改数字以创建数据时，他们会无意识地试图平均使用每个数字或避免某些数字。然而，由于自然界的数据遵循[本福特定律](https://kenji.blog/p/benfords-law/)，捏造的数据将显著偏离这一定律。

### 在审计中的使用

税务机关和会计审计公司会扫描公司的账簿和费用报告，以自动检查数字的首位（或第二位）数字是否遵循[本福特定律](https://kenji.blog/p/benfords-law/)。
...
如果大量夸大“虚构费用”，这些金额的分布将变得不自然并从[本福特定律](https://kenji.blog/p/benfords-law/)曲线中凸显出来。这种方法非常强大，事实上，许多贪污案和财务欺诈都是由这一定律引发而被揭露的。

### 选举违规指控

此外，在选举计票数据中，每个投票站的总结果是否遵循[本福特定律](https://kenji.blog/p/benfords-law/)有时会被用作验证选举欺诈的指标（不过，就选举数据而言，根据选区规模的不同，有时很难应用，这一直是争论的话题）。

## 结论

**[本福特定律](https://kenji.blog/p/benfords-law/)** 是隐藏在看似混乱世界中的优美数学秩序之一。
