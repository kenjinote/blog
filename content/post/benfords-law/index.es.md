...
La ley matemática que explica este misterioso fenómeno es la **Ley de Benford (Benford's Law)**.

En este artículo, explicaremos en detalle cómo funciona la Ley de Benford, por qué ocurre este fenómeno y cómo se aplica esta ley para detectar fraudes.

## ¿Qué es la Ley de Benford?

La Ley de Benford (también conocida como la ley del primer dígito) establece que en muchas colecciones de datos numéricos de la vida real, la probabilidad de que aparezca el primer dígito (el dígito distinto de cero más significativo) es mayor para los números más pequeños.
...
La Ley de Benford no se puede aplicar a todos los datos. Existe una clara diferencia entre los datos a los que se aplica y los que no.
...
Actualmente, uno de los campos en los que la Ley de Benford se utiliza de forma más práctica es la **Detección de Fraude (Fraud Detection)**.

Cuando los humanos intentan inventar o manipular números al azar para crear datos, inconscientemente intentan usar cada número por igual o evitar ciertos números. Sin embargo, dado que los datos naturales siguen la Ley de Benford, los datos inventados se desviarán significativamente de esta ley.

### Uso en Auditorías Contables

Las autoridades fiscales y las empresas de auditoría contable escanean los libros de contabilidad y los informes de gastos de las empresas para verificar automáticamente si el primer dígito (o el segundo dígito) de los números sigue la Ley de Benford.
...
Si se infla una gran cantidad de "gastos ficticios", la distribución de esas cantidades se volverá antinatural y sobresaldrá de la curva de la Ley de Benford. Este método es increíblemente poderoso y, de hecho, muchos casos de malversación de fondos y fraudes contables han sido descubiertos a partir de esta ley.

### Alegaciones de Fraude Electoral

Además, en los datos del recuento de votos electorales, si los resultados agregados de cada colegio electoral siguen la Ley de Benford a veces se utiliza como indicador para verificar el fraude electoral (sin embargo, en el caso de los datos electorales, a veces es difícil de aplicar dependiendo del tamaño de los distritos, lo que es tema de debate).

## Conclusión

La **Ley de Benford** es uno de los hermosos órdenes matemáticos ocultos en un mundo aparentemente caótico.
