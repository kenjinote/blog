...
Эту проблему в корне решают контейнерные технологии, такие как **[Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)**, и парадигма **[Infrastructure as Code](https://kenji.blog/ru/p/iac-infrastructure-as-code-terraform/) ([IaC](https://kenji.blog/ru/p/iac-infrastructure-as-code-terraform/))**. Контейнеризация локальной среды разработки обеспечивает изоляцию на уровне ОС и позволяет контролировать версии самой среды вместе с кодовой базой.

В этой статье мы подробно, в том числе с математической точки зрения, рассмотрим глубокие технические механизмы и шаги для создания **«воспроизводимой локальной среды разработки, которая будет абсолютно одинаковой независимо от того, кто, когда и на какой машине ее запускает»**, используя Docker, Docker Compose и VSCode Dev[Container](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)s.
...
- **[Namespace](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)s**: Обеспечивают независимое представление системных ресурсов (PID, сеть, точки монтирования, пользователи и т. д.) для каждого процесса.
...
- **UnionFS (Union File System)**: Технология, которая прозрачно накладывает несколько деревьев каталогов (слоев) друг на друга, представляя их как единую файловую систему. Слои образов [Docker](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/) опираются на эту технологию.
...
Первым шагом к воспроизводимой среде является проектирование `Dockerfile`, определяющего среду выполнения приложения. Здесь на примере Python (FastAPI) мы рассмотрим лучшие практики создания безопасного и легковесного [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)file с использованием **многоэтапных сборок (multi-stage builds)**.
...
### Практический [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)file для Python FastAPI
...
## 4. Оркестрация нескольких контейнеров с помощью [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/) Compose
...
В этот раз мы создадим трехуровневую систему, состоящую из «Web (FastAPI)», «Database (PostgreSQL)» и «Cache ([Redis](https://kenji.blog/ru/p/nosql-database-selection-kvs-document-graph-wide-column/))» в локальной среде.
...
- **Named Volume (Именованный том)**: К ним относятся `postgres_data` и `redis_data`. Это области, внутренне управляемые [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/) (например, `/var/lib/docker/volumes/`), которые превосходят Bind Mount по производительности ввода-вывода и сглаживают различия файловых систем между ОС. Их всегда следует использовать для сохранения баз данных.

### Сеть (Networking) и обнаружение сервисов

По умолчанию [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/) Compose создает собственную мостовую сеть для каждого проекта. В данном случае это `app-network`.
...
По умолчанию [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/) Compose считывает файл `.env` в рабочем каталоге и разворачивает заполнители вида `${VAR_NAME}` в YAML-файле. Этот метод позволяет безопасно управлять различными настройками для разных сред, таких как локальная, staging и production, без изменения кода инфраструктуры.

---

## 6. VSCode Dev[Container](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)s: Идеальный опыт разработки

На данный момент мы создали надежную бэкенд-среду с использованием [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/). Однако можно пойти еще дальше. Используя функцию **VSCode DevContainers (Remote - Containers)**, можно запускать бэкенд самого редактора (VSCode) внутри контейнера.
...
Включив этот файл в репозиторий, при открытии проекта в VSCode сразу же появится запрос «Reopen in [Container](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)» (Открыть в контейнере). По одному щелчку мыши запустятся все необходимые контейнеры, установятся расширения, и вы будете готовы немедленно начать писать код. Это поистине волшебный опыт.
...
- $T_{\text{cache}}$: Время, затрачиваемое на чтение и запись из [Redis](https://kenji.blog/ru/p/nosql-database-selection-kvs-document-graph-wide-column/)
...
В локальной среде разработки (внутри [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)) $T_{\text{net}}$ близко к 0, но стоит обратить внимание на **производительность ввода-вывода (I/O) при Bind Mount**. В частности, при использовании Docker Desktop на Windows/macOS из-за накладных расходов на совместное использование файлов между хост-ОС и виртуальной машиной (контейнером) $T_{\text{app}}$ (например, время загрузки кода) имеет тенденцию увеличиваться. Чтобы устранить это узкое место, настоятельно рекомендуется архитектура, в которой используется описанный ранее Dev[Container](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)s для размещения всего исходного кода внутри именованного тома, или работа движка Docker нативно в среде WSL2 (Windows Subsystem for Linux 2).
...
2. **Нехватка дискового пространства**
   При длительном использовании [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/) накапливаются неиспользуемые образы и тома (Dangling Images / Volumes), что может занять десятки гигабайт дискового пространства. Рекомендуется регулярно очищать систему с помощью следующей команды:
   ```bash
   docker system prune -a --volumes
   ```

3. **Проблемы с правами доступа к файлам**
   При использовании Bind Mount в среде Linux файлы, созданные внутри контейнера, могут принадлежать пользователю `root`, что сделает невозможным их редактирование на стороне хоста. Эту проблему можно решить, создав непривилегированного пользователя в [Docker](https://kenji.blog/ru/p/docker-container-namespace-[cgroups](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)-layers/)file и сопоставив его с вашим собственным UID/GID хост-ОС (например, 1000:1000).

---

## 10. Заключение: Как воспроизводимость ускоряет разработку

Сочетание Docker, Docker Compose и VSCode Dev[Container](https://kenji.blog/ru/p/docker-container-namespace-cgroups-layers/)s позволяет создать надежную локальную среду разработки, в которой «состояние будет абсолютно одинаковым независимо от того, кто ее запускает».

Внедрение парадигмы [IaC](https://kenji.blog/ru/p/iac-infrastructure-as-code-terraform/) в локальную среду не просто сокращает время первоначальной настройки. Это значительно повышает скорость и качество всего цикла разработки, устраняя опасения по поводу изменений конфигурации инфраструктуры, облегчая эксперименты с новыми технологическими стеками и обеспечивая плавный переход к конвейерам (pipelines) [CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/).
