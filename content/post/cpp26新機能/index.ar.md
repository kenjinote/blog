...
في C++26، تم دمج **مطابقة الأنماط (Pattern Matching)** كميزة لغوية (متوافق مع P2688). يتيح ذلك مطابقة بديهية مشابهة للغات الوظيفية (مثل [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) و Haskell).
...
## 6.2 مؤشرات الخطر (Hazard [Pointer](https://kenji.blog/ar/p/c-language-pointers-memory-management-stack-heap/)s) و RCU (التحديث بالنسخ عند القراءة)

كميزات قياسية تدعم تنفيذ هياكل البيانات الخالية من الأقفال ([Lock](https://kenji.blog/ar/p/rdbms-transaction-acid-isolation-level-lock/)-free)، تم توحيد **مؤشرات الخطر (Hazard [Pointer](https://kenji.blog/ar/p/c-language-pointers-memory-management-stack-heap/)s)** (`std::hazard_pointer`) و **RCU** (`std::rcu`). وقد أدى ذلك إلى خفض كبير في العقبات المتمثلة في تنفيذ هياكل بيانات متزامنة وعالية الأداء في C++.
...
*   بفضل **المرسلين/المستقبلين (Senders/Receivers)** و **RCU / مؤشرات الخطر (Hazard [Pointer](https://kenji.blog/ar/p/c-language-pointers-memory-management-stack-heap/)s)** ، تم توحيد المعالجة المتزامنة التي تستخرج أقصى درجات الأداء.
