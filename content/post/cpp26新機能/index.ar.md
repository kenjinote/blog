...
كميزات قياسية تدعم تنفيذ هياكل البيانات الخالية من الأقفال ([Lock](https://kenji.blog/ar/p/rdbms-transaction-acid-isolation-level-lock/)-free)، تم توحيد **مؤشرات الخطر (Hazard Pointers)** (`std::hazard_pointer`) و **RCU** (`std::rcu`). وقد أدى ذلك إلى خفض كبير في العقبات المتمثلة في تنفيذ هياكل بيانات متزامنة وعالية الأداء في C++.
