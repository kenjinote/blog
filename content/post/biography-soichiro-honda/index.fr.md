...
```mermaid
graph TD
    A["Soichiro Honda (本田宗一郎)"] -->|Passion pour la technologie| B["Développement de produits"]
    A -->|Partenaire commercial| C["Takeo Fujisawa (藤沢武夫)"]
    C -->|Gestion et stratégie de vente| B
    
    B --> D["Naissance du Super Cub"]
    B --> E["Participation en F1 (Sport automobile)"]
    B --> F["Moteur CVCC (Respect de la loi Muskie)"]
    
    D --> G["Promotion de la motorisation"]
    E --> H["Mondialisation de la marque"]
    F --> I["Pionnier des technologies environnementales"]
    
    G --> J["Héritage moderne"]
    H --> J
    I --> J
    
    A -->|Philosophie fondamentale| K["Respect de l'individu (Les Trois Joies)"]
    K --> J
```
