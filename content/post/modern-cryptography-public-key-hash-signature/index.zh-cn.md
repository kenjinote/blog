...
公钥密码的安全性，依赖于“正向计算容易，但反向计算极其困难”的 **单向函数** （One-way function），以及只要知道特定信息（陷门：Trapdoor）就能进行逆向计算的 **陷门单向函数** 。这里我们将深入探讨具有代表性的 RSA 密码和椭圆曲线密码（[ECC](/zh-cn/p/elliptic-curve-cryptography-math-cpp/)）。
...
5. 计算满足以下同余式的 $d$。
   ...
   这可以使用扩展[欧几里得算法](/zh-cn/p/euclidean-algorithm/)来计算。
...
[ECC](/zh-cn/p/elliptic-curve-cryptography-math-cpp/) 的安全性依赖于 **椭圆曲线上的离散对数问题** （ECDLP）的困难度。
...
在 [ECC](/zh-cn/p/elliptic-curve-cryptography-math-cpp/) 中，$k$ 即为 **私钥** ， $P$ 即为 **公钥** 。
...
- 依靠 **对称密码** 实现高速的数据加密
- 依靠 **公钥密码** （RSA 或 [ECC](/zh-cn/p/elliptic-curve-cryptography-math-cpp/)）实现安全的密钥交换与非对称性
