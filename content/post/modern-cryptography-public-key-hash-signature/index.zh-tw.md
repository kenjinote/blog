...
公開金鑰密碼的安全性依賴於「某個計算很容易，但其反向計算卻非常困難」的 **單向函數** （One-way function），以及如果知道特定資訊（陷門：Trapdoor）就能進行反向計算的 **具陷門單向函數** 。這裡將深入探討具代表性的 RSA 密碼與橢圓曲線密碼（[ECC](/zh-tw/p/elliptic-curve-cryptography-math-cpp/)）。
...
5. 計算滿足以下同餘式的 $d$。
   ...
   這可以使用擴展[歐幾里得](/zh-tw/p/euclid/)演算法來計算。
...
[ECC](/zh-tw/p/elliptic-curve-cryptography-math-cpp/) 的安全性依賴於 **橢圓曲線上的離散對數問題** （ECDLP）的困難度。
...
在 [ECC](/zh-tw/p/elliptic-curve-cryptography-math-cpp/) 中，$k$ 即為 **私有金鑰** ，而 $P$ 即為 **公開金鑰** 。
...
- 透過 **對稱金鑰密碼** 進行高速的資料加密
- 透過 **公開金鑰密碼** （RSA 或 [ECC](/zh-tw/p/elliptic-curve-cryptography-math-cpp/)）實現安全的金鑰交換與不對稱性
