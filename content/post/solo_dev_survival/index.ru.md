...
В истории разработки программного обеспечения наступила эпоха, беспрецедентно благоприятная для индивидуальных разработчиков (инди-разработчиков). Демократизация облачной инфраструктуры, такой как AWS и GCP, рост популярности BaaS (Backend as a [Service](https://kenji.blog/ru/p/kubernetes-k8s-architecture-pod-service-ingress/)), включая Vercel и Supabase, и, прежде всего, автоматизация написания кода благодаря эволюции [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) ([больших языковых моделей](/ru/p/large-language-models-llm-transformer-prompt-engineering/)) — все это создало почву, на которой отдельный человек может на равных конкурировать с "гигантами", крупными технологическими корпорациями.
...
Системы крупных компаний проектируются с приоритетом на «стабильность» и «масштабируемость», поэтому в них используются [Kubernetes](https://kenji.blog/ru/p/kubernetes-k8s-architecture-pod-service-ingress/) и микросервисная архитектура. Однако, если индивидуальный разработчик попытается сделать то же самое, его ресурсы иссякнут только на поддержке инфраструктуры (Ops).

Девиз технологического стека индивидуального разработчика — **«No-Ops» (Ноль операций)**. Нужно максимально использовать бессерверную архитектуру ([Serverless](https://kenji.blog/ru/p/serverless-architecture-aws-lambda-cold-start/)) и сосредоточиться исключительно на написании бизнес-логики.
...
В стратегии интенсивного использования BaaS и управляемых сервисов всегда обсуждается риск "привязки к поставщику" (Vendor [Lock](https://kenji.blog/ru/p/rdbms-transaction-acid-isolation-level-lock/)-in). Например, если слишком сильно зависеть от Firebase Firestore, то позже будет крайне сложно перейти на RDB (реляционную базу данных).
...
Самым сильным оружием для индивидуальных разработчиков, не имеющих маркетингового бюджета, является «SEO (поисковая оптимизация)». В последние годы большое внимание привлекает «программное SEO» (Programmatic SEO), при котором база данных компании комбинируется с [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) для динамической генерации тысяч и десятков тысяч посадочных страниц.
...
2. **Использовать технологические рычаги ([Serverless](https://kenji.blog/ru/p/serverless-architecture-aws-lambda-cold-start/), BaaS, AI)**
