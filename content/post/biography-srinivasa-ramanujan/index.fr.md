...
```mermaid
graph TD
    A["Naissance à Kumbakonam dans le sud de l'Inde"] -- "Éveil aux mathématiques" --> B["Rencontre avec le recueil de formules mathématiques de Carr"]
    B -- "Pauvreté extrême et recherche solitaire" --> C["Travail à la capitainerie du port et rédaction de carnets"]
    C -- "Lettre à Hardy" --> D["Invitation à l'Université de Cambridge"]
    D -- "Recherche conjointe et maladie" --> E["Retour au pays et décès prématuré"]
    E -- "Découverte sur le lit de mort" --> F["Découverte des fonctions pseudo-thêta et héritage pour la postérité"]
```
...
```mermaid
graph LR
    A["Ramanujan (Intuition et fulgurance)"] -- "Présente des formules non prouvées" --> C["Processus de recherche conjointe"]
    B["G.H. Hardy (Logique et preuve rigoureuses)"] -- "Fournit des preuves mathématiques rigoureuses" --> C
    C -- "Publication d'articles historiques" --> D["Formule asymptotique des nombres de partitions, etc."]
```
