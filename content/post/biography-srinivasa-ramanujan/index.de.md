...
```mermaid
graph TD
    A["Geburt in Kumbakonam, Südindien"] -- "Erwachen zur Mathematik" --> B["Begegnung mit Carrs Formelsammlung"]
    B -- "Extreme Armut und einsame Forschung" --> C["Arbeit bei der Hafenbehörde und Erstellung von Notizbüchern"]
    C -- "Brief an Hardy" --> D["Einladung an die Universität Cambridge"]
    D -- "Gemeinsame Forschung und Krankheit" --> E["Rückkehr und früher Tod"]
    E -- "Entdeckungen auf dem Sterbebett" --> F["Entdeckung der Mock-Thetafunktionen und Vermächtnis an die Nachwelt"]
```
...
```mermaid
graph LR
    A["Ramanujan (Intuition und Blitz)"] -- "Präsentiert unbewiesene Formeln" --> C["Prozess der gemeinsamen Forschung"]
    B["G.H. Hardy (Strikte Logik und Beweis)"] -- "Fügt strenge mathematische Beweise hinzu" --> C
    C -- "Veröffentlichung historischer Papiere" --> D["Asymptotische Formeln für Partitionszahlen usw."]
```
