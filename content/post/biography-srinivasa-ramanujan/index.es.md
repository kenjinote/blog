...
```mermaid
graph TD
    A["Nacimiento en Kumbakonam, sur de la India"] -- "Despertar a las matemáticas" --> B["Encuentro con el libro de fórmulas de Carr"]
    B -- "Pobreza extrema e investigación solitaria" --> C["Trabajo en la Autoridad Portuaria y creación de cuadernos"]
    C -- "Carta a Hardy" --> D["Invitación a la Universidad de Cambridge"]
    D -- "Investigación conjunta y enfermedad" --> E["Regreso a casa y muerte prematura"]
    E -- "Descubrimientos en el lecho de muerte" --> F["Descubrimiento de las funciones theta simuladas y legado a la posteridad"]
```
...
```mermaid
graph LR
    A["Ramanujan (Intuición e inspiración)"] -- "Presentación de fórmulas no demostradas" --> C["Proceso de investigación conjunta"]
    B["G.H. Hardy (Lógica rigurosa y demostración)"] -- "Provisión de demostraciones matemáticas rigurosas" --> C
    C -- "Publicación de artículos históricos" --> D["Fórmula asintótica para particiones, etc."]
```
