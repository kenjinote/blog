...
```mermaid
graph TD
    A["Kelahiran di Kumbakonam, India Selatan"] -- "Kebangkitan matematika" --> B["Pertemuan dengan buku rumus Carr"]
    B -- "Kemiskinan dan penelitian yang sepi" --> C["Bekerja di Otoritas Pelabuhan dan membuat buku catatan"]
    C -- "Surat kepada Hardy" --> D["Undangan ke Universitas Cambridge"]
    D -- "Penelitian bersama dan penyakit" --> E["Kembali ke tanah air dan kematian dini"]
    E -- "Penemuan di ranjang kematian" --> F["Penemuan fungsi mock theta dan warisan untuk generasi mendatang"]
```
...
```mermaid
graph LR
    A["Ramanujan (Intuisi dan kilat)"] -- "Menyajikan rumus yang belum dibuktikan" --> C["Proses penelitian bersama"]
    B["G.H. Hardy (Logika dan pembuktian yang ketat)"] -- "Memberikan pembuktian matematis yang ketat" --> C
    C -- "Publikasi makalah bersejarah" --> D["Rumus asimtotik untuk angka partisi, dll."]
```
