...
Por ser uma multiplicação polinomial, ao usar a "Transformada Numérica de Teoria (Number Theoretic Transform: NTT)", que é análoga à [Transformada Rápida de Fourier (FFT)](/pt/p/fast-fourier-transform-algorithm/), o custo computacional pode ser drasticamente reduzido para $\mathcal{O}(n \log n)$. Além disso, dado que o tamanho da chave pública é reduzido de uma matriz para um único polinômio, o tamanho dos dados é reduzido para $\mathcal{O}(n)$. Isso traz uma vantagem esmagadora na largura de banda de comunicação.
