...
```mermaid
flowchart LR
    S["William Shockley"] -- "Nobelpreisträger, sammelt Talente" --> L["Gründung des Labors"]
    L -- "Schlechtes Management, Paranoia" --> T["Rebellion der jungen Ingenieure"]
    T -- "Unterschrift auf 1-Dollar-Schein" --> E["Acht Verräter"]
    E -- "Unterstützung durch Arthur Rock" --> F["Gründung von Fairchild Semiconductor"]
    F -- "Bildung des Silicon Valley" --> V["Moderne Technologiebranche"]
```
...
graph TD
    ...
    A -- "Spin-off" --> E["Dutzende weitere"]
    B -- "Erfindung des Mikroprozessors" --> F["PC-Revolution, Internetzeitalter"]
    C -- "Wettbewerb und Innovation" --> F
```
