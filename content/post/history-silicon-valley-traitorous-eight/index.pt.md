...
```mermaid
flowchart LR
    S["William Shockley"] -- "Prêmio Nobel, atrai talentos" --> L["Fundação do laboratório"]
    L -- "Gestão ruim, paranoia" --> T["Rebelião dos jovens engenheiros"]
    T -- "Assinatura na nota de 1 dólar" --> E["Oito Traidores"]
    E -- "Apoio de Arthur Rock" --> F["Fundação da Fairchild Semiconductor"]
    F -- "Formação do Vale do Silício" --> V["Indústria moderna de tecnologia"]
```
...
graph TD
    ...
    A -- "Spin-off" --> E["Dezenas de outras empresas"]
    B -- "Invenção do microprocessador" --> F["Revolução do PC / Era da Internet"]
    C -- "Competição e inovação tecnológica" --> F
```
