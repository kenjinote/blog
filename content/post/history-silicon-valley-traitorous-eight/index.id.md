...
```mermaid
flowchart LR
    S["William Shockley"] -- "Penerima Hadiah Nobel, Mengumpulkan Bakat" --> L["Pendirian Laboratorium"]
    L -- "Manajemen Buruk & Paranoid" --> T["Pemberontakan Insinyur Muda"]
    T -- "Tanda Tangan di Uang 1 Dolar" --> E["Delapan Pengkhianat"]
    E -- "Dukungan Arthur Rock" --> F["Pendirian Fairchild Semiconductor"]
    F -- "Menuju Pembentukan Lembah Silikon" --> V["Industri Teknologi Modern"]
```
...
graph TD
    ...
    A -- "Spin-off" --> E["Puluhan Perusahaan Lainnya"]
    B -- "Penemuan Mikroprosesor" --> F["Revolusi PC & Era Internet"]
    C -- "Persaingan & Inovasi Teknologi" --> F
```
