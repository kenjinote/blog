...
```mermaid
flowchart LR
    S["William Shockley"] -- "Wins Nobel Prize, gathers talent" --> L["Establishes Laboratory"]
    L -- "Poor management, paranoia" --> T["Backlash from young engineers"]
    T -- "Signing a $1 bill" --> E["The Traitorous Eight"]
    E -- "Support from Arthur Rock" --> F["Establishes Fairchild Semiconductor"]
    F -- "Towards the formation of Silicon Valley" --> V["Modern technology industry"]
```
...
```mermaid
graph TD
    A["Fairchild Semiconductor"] -- "Spin-offs (Fairchildren)" --> B["Intel (Noyce & Moore)"]
    A -- "Spin-off" --> C["AMD (Sanders)"]
    A -- "Spin-off" --> D["National Semiconductor"]
    A -- "Spin-off" --> E["Dozens of other companies"]
    B -- "Invention of the microprocessor" --> F["PC Revolution and Internet Era"]
    C -- "Competition and technological innovation" --> F
```
