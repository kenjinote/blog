...
```mermaid
flowchart LR
    S["William Shockley"] -- "Gana el Premio Nobel y reúne talento" --> L["Fundación del laboratorio"]
    L -- "Mala gestión y paranoia" --> T["Rebelión de los jóvenes ingenieros"]
    T -- "Firma en un billete de 1 dólar" --> E["Ocho Traidores"]
    E -- "Apoyo de Arthur Rock" --> F["Fundación de Fairchild Semiconductor"]
    F -- "Hacia la formación de Silicon Valley" --> V["Industria tecnológica moderna"]
```
...
```mermaid
graph TD
    A["Fairchild Semiconductor"] -- "Spin-offs (Fairchildren)" --> B["Intel (Noyce y Moore)"]
    A -- "Spin-offs" --> C["AMD (Sanders)"]
    A -- "Spin-offs" --> D["National Semiconductor"]
    A -- "Spin-offs" --> E["Docenas de otras empresas"]
    B -- "Invención del microprocesador" --> F["Revolución de las PC y la era de Internet"]
    C -- "Competencia e innovación tecnológica" --> F
```
