...
```mermaid
flowchart LR
    S["William Shockley"] -- "Prix Nobel, attire les talents" --> L["Création du laboratoire"]
    L -- "Mauvaise gestion, paranoïa" --> T["Rébellion des jeunes ingénieurs"]
    T -- "Signature sur un billet de 1 dollar" --> E["Huit Traîtres"]
    E -- "Soutien d'Arthur Rock" --> F["Création de Fairchild Semiconductor"]
    F -- "Formation de la Silicon Valley" --> V["Industrie technologique moderne"]
```
...
```mermaid
graph TD
    A["Fairchild Semiconductor"] -- "Spin-off (Fairchildren)" --> B["Intel (Noyce & Moore)"]
    A -- "Spin-off" --> C["AMD (Sanders)"]
    A -- "Spin-off" --> D["National Semiconductor"]
    A -- "Spin-off" --> E["Des dizaines d'autres"]
    B -- "Invention du microprocesseur" --> F["Révolution des PC, ère d'Internet"]
    C -- "Concurrence et innovation technologique" --> F
```
