...
В глубоком обучении по мере углубления слоев сети возникает проблема исчезновения или взрыва градиентов во время обучения, из-за чего обучение проходит неудачно. Для предотвращения этого вокруг каждого подслоя (Attention и FFN) в [Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) размещены **Остаточное соединение (Residual Connection)** и ** Нормализация слоя (Layer Normalization)**.
...
Это не проблема для коротких текстов, но если попытаться ввести в [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) длинный контекст, например целую книгу, $N$ достигнет десятков или сотен тысяч, и память GPU мгновенно исчерпается при традиционных вычислениях Attention.
...
В этой статье мы раскрыли математическую структуру, лежащую в глубине модели [Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/).
...
Возможно, в будущем появятся новые архитектуры, превосходящие Transformer (например, Mamba, являющаяся [State](https://kenji.blog/ru/p/iac-infrastructure-as-code-terraform/) Space Model), но математическая база «понимания контекста с помощью Attention», созданная [Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/), навсегда останется в истории ИИ.

Если у вас будет возможность использовать [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/), такие как ChatGPT или Claude в будущем, представьте себе, как в фоновом режиме каждую секунду вычисляются триллионы матричных умножений $Q K^T$, а функция Softmax выдает вероятности. Разрешение вашего понимания технологий повысится, и мир ИИ покажется вам еще более интересным.
