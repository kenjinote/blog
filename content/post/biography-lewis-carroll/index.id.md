...
```mermaid
graph TD
    A["Charles Lutwidge Dodgson"] --> B["Universitas Oxford (Christ Church)"]
    A --> C["Lewis Carroll (Nama Pena)"]
    
    B --> D["Matematika & Logika"]
    B --> E["Fotografi"]
    
    C --> F["Sastra Omong Kosong (Nonsense)"]
    C --> G["Persahabatan dengan Alice Liddell"]
    
    D --> H["Logika Simbolik & Paradoks"]
    G --> I["Alice's Adventures in Wonderland"]
    I --> F
    
    F --> J["Dampak pada Sastra Modern (Surealisme)"]
    F --> K["Budaya Pop & Media"]
    H --> L["Pengaruh pada Filsafat & Sains Kognitif"]
    
    J --> M["Warisan yang Bertahan Lama"]
    K --> M
    L --> M
```
