...
```mermaid
graph TD
    A["Charles Lutwidge Dodgson"] --> B["Universidade de Oxford (Christ Church)"]
    A --> C["Lewis Carroll (Pseudônimo)"]
    
    B --> D["Matemática & Lógica"]
    B --> E["Fotografia"]
    
    C --> F["Literatura do Nonsense"]
    C --> G["Amizade com Alice Liddell"]
    
    D --> H["Lógica Simbólica & Paradoxos"]
    G --> I["Alice no País das Maravilhas"]
    I --> F
    
    F --> J["Impacto na Literatura Moderna (Surrealismo)"]
    F --> K["Cultura Pop & Mídia"]
    H --> L["Influência na Filosofia & Ciência Cognitiva"]
    
    J --> M["Legado Duradouro"]
    K --> M
    L --> M
```
