---
title: '推薦給工程師初學者的必讀書籍 15 選！從程式設計基礎到架構設計'
slug: "工程師初學者に推薦の書籍"
date: 2024-05-05T16:03:33+09:00
tags: ["工程師", "初學者", "書籍"]
draft: false
image: "img.webp"
categories: ["lifestyle-miscellaneous"]
description: '想從程式設計初學者邁向中階的工程師必看！嚴選介紹《無瑕的程式碼》與《程式設計師的自我修養》等 15 本推薦初學者的必讀書籍，讓您學習軟體開發的基礎知識與設計手法。'
---

1. 可讀的程式碼 (Readable Code)
![img_15.png](img_15.webp)
2. 程式設計原理 (Principles of Programming)
![img_14.png](img_14.webp)
3. 程式設計師修煉之道 (The Pragmatic Programmer)
![img_13.png](img_13.webp)
4. 支撐 Web 的技術
![img_12.png](img_12.webp)
5. 安全 Web 應用程式建立指南
![img_11.png](img_11.webp)
7. 網路為何能連接
![img_10.png](img_10.webp)
7. 向達人學習 SQL 徹底指南
![img_9.png](img_9.webp)
8. [Docker](https://kenji.blog/zh-tw/p/docker-container-namespace-[cgroups](https://kenji.blog/zh-tw/p/docker-container-namespace-cgroups-layers/)-layers/) 實踐指南
![img_8.png](img_8.webp)
9. [Kubernetes](https://kenji.blog/zh-tw/p/kubernetes-k8s-architecture-pod-service-ingress/) 完全指南
![img_7.png](img_7.webp)
10. CODE COMPLETE 第2版
![img_5.png](img_5.webp)
![img_6.png](img_6.webp)
11. Clean Code 無瑕的程式碼
![img_4.png](img_4.webp)
12. Clean Architecture 無瑕的程式碼：整潔的軟體設計與架構篇
![img_3.png](img_3.webp)
13. 成為遊戲程式設計師前必須知道的技術
![img_2.png](img_2.webp)
14. 成為極客 (Being Geek)
![img_1.png](img_1.webp)
15. 為什麼要用物件導向？
![img_16.png](img_16.webp)
