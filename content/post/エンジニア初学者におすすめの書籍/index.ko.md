---







title: '엔지니어 초보자에게 추천하는 필독서 15선! 프로그래밍의 기초부터 설계까지'
slug: "エンジニア初学者におすすめの書籍"
date: 2024-05-05T16:03:33+09:00
tags: ["엔지니어", "초보자", "도서"]
draft: false
image: "img.webp"
categories: ["라이프스타일・잡기"]
description: '프로그래밍 초보자에서 중급자로 도약하고 싶은 엔지니어 필견! ''읽기 좋은 코드가 좋은 코드다(The Art of Readable Code)''나 ''실용주의 프로그래머(The Pragmatic Programmer)'' 등 소프트웨어 개발의 기초 지식과 설계 기법을 배울 수 있는, 초보자에게 추천하는 필독서 15권을 엄선하여 소개합니다.'
---








1. 리더블 코드
![img_15.png](img_15.webp)
2. 프로그래밍의 원리
![img_14.png](img_14.webp)
3. 실용주의 프로그래머
![img_13.png](img_13.webp)
4. 웹을 지탱하는 기술
![img_12.png](img_12.webp)
5. 안전한 웹 애플리케이션을 만드는 방법
![img_11.png](img_11.webp)
7. 네트워크는 왜 연결되는가
![img_10.png](img_10.webp)
7. 달인에게 배우는 SQL 철저 지침서
![img_9.png](img_9.webp)
8. Docker 실전 가이드
![img_8.png](img_8.webp)
9. Kubernetes 완전 가이드
![img_7.png](img_7.webp)
10. CODE COMPLETE 제2판
![img_5.png](img_5.webp)
![img_6.png](img_6.webp)
11. Clean Code 애자일 소프트웨어 달인의 기술
![img_4.png](img_4.webp)
12. Clean Architecture 달인에게 배우는 소프트웨어 구조와 설계
![img_3.png](img_3.webp)
13. 게임 프로그래머가 되기 전에 알아두어야 할 기술
![img_2.png](img_2.webp)
14. Being Geek
![img_1.png](img_1.webp)
15. 객체지향으로 왜 만드는가
![img_16.png](img_16.webp)
