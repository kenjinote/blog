...
Поэтому в последние годы стремительную популярность набирают «генераторы статических сайтов» (SSG: Static Site Generator). В этой статье мы подробно рассмотрим « **Hugo** » — один из множества SSG, разработанный на языке [Go](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) и известный своей невероятной скоростью сборки. Мы досконально разберем всё: от сравнения технической архитектуры с динамическими CMS (Content Management System), такими как WordPress, до конкретных шагов по миграции, оценки производительности с использованием математических моделей, а также уникальной структуры каталогов Hugo и порядка поиска шаблонов.
...
2. **Собственные скрипты с использованием WordPress API**
   Метод, при котором с помощью Python, Node.js и т.д. выполняются запросы к [REST API](https://kenji.blog/ru/p/graphql-vs-rest-api-[overfetching](https://kenji.blog/ru/p/graphql-vs-rest-api-overfetching-type-safety/)-type-safety/) WordPress (`/wp-json/wp/v2/posts`), анализируются данные JSON и самостоятельно генерируются файлы Markdown. Этот метод эффективен для сайтов, интенсивно использующих сложные произвольные поля (например, ACF), с которыми плагины не могут справиться в полной мере.
3. **Использование инструмента wp2hugo**
   Также существует подход с использованием инструментов командной строки (CLI), написанных на [Go](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) и т.д., для прямой конвертации из файла экспорта XML WordPress (WXR) в формат Hugo.
...
После завершения процесса миграции настало время собрать сайт с помощью Hugo и опубликовать его для всего мира. Поставляемый в виде бинарного файла на языке [Go](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/), Hugo может похвастаться удивительной скоростью, позволяющей собирать сайты из тысяч или даже десятков тысяч страниц всего за несколько секунд.
...
### 5.3 Автоматизация развертывания ([CI/CD](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/))
...
Например, базовая конфигурация (файл YAML) для развертывания на Cloudflare Pages или GitHub Pages с использованием [GitHub Actions](https://kenji.blog/ru/p/cicd-pipeline-github-actions-best-practices/) выглядит следующим образом:
...
### 6.1 Кардинальное улучшение скорости сайта и показателей Core [Web Vitals](https://kenji.blog/ru/p/web-vitals-frontend-performance-optimization-lcp-fid-cls/)
За счет устранения запросов к базе данных и рендеринга на стороне сервера время загрузки страницы сокращается до миллисекунд. Это напрямую приводит к значительному улучшению показателей «[Core Web Vitals](https://kenji.blog/ru/p/web-vitals-frontend-performance-optimization-lcp-fid-cls/)» ([LCP](https://kenji.blog/ru/p/web-vitals-frontend-performance-optimization-lcp-fid-cls/), [FID](https://kenji.blog/ru/p/web-vitals-frontend-performance-optimization-lcp-fid-cls/)/[INP](https://kenji.blog/ru/p/web-vitals-frontend-performance-optimization-lcp-fid-cls/), [CLS](https://kenji.blog/ru/p/web-vitals-frontend-performance-optimization-lcp-fid-cls/)), которые являются факторами ранжирования Google. Можно ожидать снижения показателя отказов пользователей и улучшения позиций в SEO.
...
В этой статье мы подробно рассмотрели переход с динамических CMS, таких как WordPress, на мощный генератор статических сайтов «Hugo» на базе языка [Go](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/): от различий в технической архитектуре и математического доказательства производительности до конкретных шагов по миграции.
