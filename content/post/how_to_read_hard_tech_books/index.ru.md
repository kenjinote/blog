...
Алгоритм RSA — типичный пример криптографии с открытым ключом, является прекрасным применением элементарной теории чисел (модулярная арифметика, теорема Эйлера, расширенный алгоритм [Евклид](https://kenji.blog/p/euclid/)а).
...
Чтобы найти $d$, удовлетворяющее уравнению $e \cdot d \equiv 1 \pmod{\phi(n)}$, нужно реализовать расширенный алгоритм [Евклид](https://kenji.blog/p/euclid/)а. Таким образом, **при попытке закодировать формулу вы сталкиваетесь с проблемой реализации: «А как конкретно вычислить эту переменную?», и в процессе её решения ваше понимание математики кардинально углубляется**.
