...
- **좋은 카드**: "[페르마의 소정리](https://kenji.blog/p/fermats-little-theorem/) 수식을 답하시오" → "소수 $p$ 와 서로소인 정수 $a$ 에 대하여, $a^{p-1} \equiv 1 \pmod p$"
...
공개키 암호 방식의 대표격인 RSA 암호는, 초등 정수론(합동식, 오일러의 정리, 확장 [[유클리드](https://kenji.blog/p/euclid/) 호제법](https://kenji.blog/p/euclidean-algorithm/))의 아름다운 응용입니다.
...
수식 $e \cdot d \equiv 1 \pmod{\phi(n)}$ 을 만족하는 $d$ 를 찾기 위해, 확장 [[유클리드](https://kenji.blog/p/euclid/) 호제법](https://kenji.blog/p/euclidean-algorithm/)이라는 알고리즘을 구현할 필요가 있습니다. 이처럼, **수식을 코드화하려고 하면 "이 변수는 구체적으로 어떻게 계산하는가?"라는 구현상의 과제에 직면하게 되고, 이를 해결하는 과정에서 수학적 이해가 비약적으로 깊어지는 것** 입니다.
