...
La cryptographie RSA, représentante de la cryptographie à clé publique, est une belle application de la théorie élémentaire des nombres (congruences, théorème d'Euler, algorithme d'[Euclide](https://kenji.blog/p/euclid/) étendu).
...
Pour trouver $d$ satisfaisant la formule $e \cdot d \equiv 1 \pmod{\phi(n)}$, il est nécessaire d'implémenter l'algorithme d'[Euclide](https://kenji.blog/p/euclid/) étendu. Ainsi, **lorsque l'on essaie de coder des formules mathématiques, on est confronté à des problèmes d'implémentation tels que "comment calculer cette variable concrètement ?", et en les résolvant, la compréhension mathématique s'approfondit considérablement**.
