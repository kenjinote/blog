...
이 수식이 의미하는 바는 매우 직관적입니다. 번사이드 보조정리는 **"각 조작마다 '변하지 않는 무늬의 수(고정점의 수)'를 세어 모두 더한 다음, 전체 조작의 수로 나눔(즉, 평균을 구함)"** 으로써 원하는 궤도의 수를 얻을 수 있다고 주장하는 것입니다.

중복에 대한 복잡한 판단을 "각 조작에서 변하지 않는 것을 센다"는 독립적이고 간단한 계산으로 분해할 수 있다는 점이 이 정리의 가장 큰 강점입니다.

## 6. 목걸이 문제로의 적용과 계산

그러면 실제로 번사이드 보조정리를 사용하여 4개의 구슬(빨강, 파랑 2색)로 이루어진 목걸이의 디자인 수를 계산해 봅시다.
