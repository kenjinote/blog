...
Dengan menggunakan [fungsi pembangkit](/id/p/generating-functions/) bilangan pseudo-acak $f(x) = (x^2 + 1) \pmod N$ untuk menghasilkan deret bilangan, ia mencari dua nilai di dalam deret sedemikian rupa sehingga $x_i \equiv x_j \pmod p$ (di mana $p$ adalah faktor prima tak dikenal dari $N$).
...
Untuk bersiap menghadapi ancaman ini, ahli kriptografi di seluruh dunia dan NIST (Institut Standar dan Teknologi Nasional AS) saat ini mempercepat proses standardisasi menuju transisi ke "Kriptografi Pasca-Kuantum (Post-Quantum [Crypto](https://kenji.blog/id/p/cryptocurrency-and-bitcoin/)graphy: PQC)" yang sulit untuk dipecahkan bahkan oleh komputer kuantum sekalipun. Kriptografi berbasis kisi ([Lattice-based cryptography](/id/p/lattice-based-cryptography-math-intuition/)) adalah salah satu contoh yang representatif, di mana mereka mendasarkan keamanan pada kesulitan matematis yang sama sekali berbeda dengan masalah faktorisasi prima (contohnya masalah vektor terpendek).
