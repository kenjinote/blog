...
في مجتمع الإنترنت الحديث، السبب الذي يجعلنا نستطيع التسوق عبر الإنترنت وتبادل المعلومات السرية بأمان هو وجود "تقنية التشفير". والأساس الذي يدعم أمان هذه التقنية (خاصة تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) المستخدم على نطاق واسع) هو الحقيقة الرياضية القائلة بأن "تحليل الأعداد الصحيحة الضخمة إلى عوامل أولية أمر صعب للغاية".
...
2. **$\mathcal{O}(\log N)$ : وقت لوغاريتمي (Logarithmic time)**
   حتى لو تضاعف حجم الإدخال، فإن وقت التنفيذ يزداد بمقدار ثابت فقط، مما يجعلها خوارزمية فعالة للغاية. "البحث الثنائي ([Binary Search](https://kenji.blog/ar/p/search-algorithms-linear-binary-hash-table-principles/))"، الذي يبحث عن قيمة مستهدفة في مصفوفة مرتبة، هو مثال نموذجي. حتى مع وجود مليار نقطة بيانات، يمكن العثور على البيانات المستهدفة في حوالي 30 مقارنة فقط.
...
# آلية تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) ومشكلة التحليل إلى العوامل الأولية
...
## 2. طريقة فيرما للتحليل إلى العوامل ([Fermat](https://kenji.blog/ar/p/fermat/)'s Factorization Method)

هذه الطريقة ابتكرها عالم الرياضيات في القرن السابع عشر [بيير دي فيرما](https://kenji.blog/ar/p/fermat/). بالنظر إلى عدد فردي مؤلف $N$، تحاول الخوارزمية التعبير عن $N$ كفرق بين مربعين.
...
بالمقارنة مع $\mathcal{O}(2^{n/2})$ للقسمة التجريبية، فهي أسرع بشكل كبير، وفعالة للغاية عملياً في تحليل الأرقام متوسطة الحجم (عشرات الخانات). ومع ذلك، فهي لا تزال غير قادرة على التغلب على حاجز "الوقت الأسي" بالنسبة لطول البت $n$، وتقف عاجزة أمام الأعداد الضخمة مثل 2048 بت (حوالي 600 خانة عشرية) المستخدمة في تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/).
...
تنتمي مشكلة التحليل إلى العوامل الأولية إلى فئة NP (فئة المشاكل التي يمكن التحقق من صحتها في وقت متعدد الحدود إذا تم إعطاء إجابة)، ولكن لم يثبت أنها [NP-complet](https://kenji.blog/ar/p/p-vs-np-problem/)e (فئة أصعب المشاكل في NP).
...
يتوقع العديد من الباحثين أن التحليل إلى العوامل الأولية ينتمي إلى فئة وسيطة ليست P ولا NP-complete (تُسمى NP-intermediate). إذا تم اكتشاف خوارزمية تحل التحليل إلى العوامل الأولية في وقت متعدد الحدود (على سبيل المثال، $\mathcal{O}(n^3)$، إلخ) على جهاز كمبيوتر كلاسيكي، فسيكون ذلك حدثاً كبيراً من شأنه أن يطيح بأنظمة التشفير في جميع أنحاء العالم، ولكن حتى الآن لم يتم اكتشاف مثل هذه الخوارزمية. يُقدّر أن كسر تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) بطول 2048 بت سيستغرق وقتاً أطول من عمر الكون، حتى لو تحسن أداء أجهزة الكمبيوتر الكلاسيكية وفقاً لقانون مور.
...
**"خوارزمية شور ([Shor](/ar/p/shors-algorithm-and-rsa-breaking/)'s algorithm)"** ، التي نشرها بيتر شور في عام 1994، هي خوارزمية يمكنها بشكل صادم حل التحليل إلى العوامل الأولية في **وقت متعدد الحدود $\mathcal{O}(n^3)$** (بشكل أكثر دقة، حوالي $\mathcal{O}(n^2 \log n \log \log n)$ في عدد البوابات الكمومية) باستخدام تحويل فورييه الكمومي.
...
عندما يصبح من الممكن تنفيذها على كمبيوتر كمومي ذي نطاق عملي (مع ضوضاء قليلة وعدد كافٍ من البتات الكمومية المنطقية)، فإن تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) بطول 2048 بت والذي يُعتبر آمناً حالياً يمكن أن يتم كسره تماماً في غضون بضع ساعات إلى بضعة أيام.

للاستعداد لهذا التهديد، يسارع خبراء التشفير في جميع أنحاء العالم والمعهد الوطني للمعايير والتكنولوجيا (NIST) في الولايات المتحدة حالياً بأعمال التوحيد القياسي نحو الانتقال إلى "تشفير ما بعد الكم ([PQC](/ar/p/post-quantum-cryptography/))"، والذي يصعب كسره حتى باستخدام أجهزة الكمبيوتر الكمومية. يعد التشفير القائم على الشبكات ([Lattice-based cryptography](/ar/p/lattice-based-cryptography-math-intuition/)) مثالاً تمثيلياً على ذلك، وهي تستند في أمانها إلى صعوبات رياضية مختلفة تماماً عن مشكلة التحليل إلى العوامل الأولية (على سبيل المثال، مشكلة أقصر متجه).
