...
현대 인터넷 사회에서 우리가 안심하고 온라인 쇼핑을 즐기거나 기밀 정보를 주고받을 수 있는 것은 '암호 기술'이 존재하기 때문입니다. 그리고 그 암호 기술(특히 널리 사용되는 [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 암호 등)의 안전성의 근간을 받치고 있는 것이 '거대한 정수의 소인수분해는 매우 어렵다'는 수학적 사실입니다.
...
2. **$\mathcal{O}(\log N)$ : 로그 시간(Logarithmic time)**
   입력 크기가 두 배가 되어도 실행 시간은 상수만큼만 늘어나는 매우 효율적인 알고리즘입니다. 정렬된 배열에서 원하는 값을 찾는 '이진 탐색([Binary Search](https://kenji.blog/ko/p/search-algorithms-linear-binary-hash-table-principles/))'이 대표적인 예입니다. 데이터 양이 10억 개라도 불과 30번 정도의 비교만으로 원하는 데이터를 찾을 수 있습니다.
...
4. **$\mathcal{O}(N \log N)$ : 선형 로그 시간(Linearithmic time)**
   $\mathcal{O}(N)$보다는 조금 느리지만 효율적인 분류에 들어갑니다. 병합 정렬(Merge Sort)이나 퀵 정렬(Quick Sort의 평균 계산량) 등 실용적이고 빠른 [정렬 알고리즘](https://kenji.blog/ko/p/sorting-algorithms/)의 대부분이 이 계산량을 가집니다.
...
# [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 암호의 원리와 소인수분해 문제
...
## 2. 페르마의 소인수분해법 ([Fermat](https://kenji.blog/ko/p/fermat/)'s Factorization Method)

17세기 수학자 [피에르 드 페르마](https://kenji.blog/ko/p/fermat/)에 의해 고안된 기법입니다. 홀수인 합성수 $N$이 주어졌을 때, $N$을 두 평방수의 차이로 표현하려고 시도합니다.
...
이때 $x_i - x_j$는 $p$의 배수가 되므로, 최대공약수 $\gcd(|x_i - x_j|, N)$을 계산함으로써 $p$(즉 $N$의 소인수)를 높은 확률로 추출할 수 있습니다. [로버트 플로이드의 순환 검출법(토끼와 거북이 알고리즘)](https://kenji.blog/ko/p/floyds-cycle-finding/) 등을 조합하여, 메모리 사용량을 $\mathcal{O}(1)$로 억제하면서 효율적으로 계산합니다.
...
시험 나눗셈법의 $\mathcal{O}(2^{n/2})$와 비교하면 극적으로 고속화되었으며, 실용적으로는 중규모(수십 자리) 숫자의 소인수분해에 매우 강력합니다. 그러나 여전히 비트 길이 $n$에 대해서는 '지수 시간'의 벽을 넘지 못하고 있으며, [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 암호에서 사용되는 것과 같은 2048비트(10진수로 약 600자리)의 거대한 숫자에 대해서는 무력합니다.
...
## [P vs NP](https://kenji.blog/ko/p/p-vs-np-problem/) 문제와 소인수분해의 위치
...
* 이 **"다항식 시간에 푸는 고전 알고리즘이 존재하지 않는다(고 강하게 예상된다)"** 는 사실이야말로 [RSA](https://kenji.blog/ko/p/modern-cryptography-public-key-hash-signature/) 암호의 안전성을 담보하며 현대 디지털 사회를 지탱하고 있습니다.
