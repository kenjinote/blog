...
这是由17世纪数学家[皮埃尔·德·费马](https://kenji.blog/p/fermat/)提出的方法。当给定一个奇合数 $N$ 时，它尝试将 $N$ 表示为两个平方数之差：
...
使用某个伪随机数[生成函数](https://kenji.blog/p/generating-functions/) $f(x) = (x^2 + 1) \pmod N$ 生成序列，并找出序列中满足 $x_i \equiv x_j \pmod p$ （$p$ 是 $N$ 的未知素因子）的两个值。
