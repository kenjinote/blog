...
Zu diesem Zeitpunkt ist $x_i - x_j$ ein Vielfaches von $p$, so dass durch die Berechnung des größten gemeinsamen Teilers $\gcd(|x_i - x_j|, N)$ das $p$ (also der Primfaktor von $N$) mit hoher Wahrscheinlichkeit extrahiert werden kann. Durch die Kombination mit dem Zyklusfindungsalgorithmus von Robert Floyd (der [Hase-und-Igel-Algorithmus](/de/p/floyds-cycle-finding/)) kann dies effizient berechnet werden, wobei der Speicherverbrauch auf $\mathcal{O}(1)$ begrenzt wird.
