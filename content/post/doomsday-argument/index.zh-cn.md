...
人类何时会灭亡？这个问题自古以来就被作为宗教、哲学以及科幻题材所探讨。然而，到了20世纪80年代以后，出现了一些使用 **概率论** 和 **贝叶斯推断** 对这个问题进行数学探讨的研究者。这就是这次要介绍的 **末日论证（Doomsday Argument）** 。
...
让我们使用 **贝叶斯推断** ，把这种直觉严格地数学公式化。贝叶斯定理是一个表明当获得新证据（观测数据）时，应如何更新某个假设概率（后验概率）的定理。
...
使用这个，计算 $H_{DOOM}$ 的后验概率。使用全概率定理展开贝叶斯定理如下。
