...
2. **كفاءة الذاكرة وتخصيص الساحة (Arena Allocation)**: نظراً لإمكانية التحكم اليدوي في تخصيص وتحرير الذاكرة، يمكن منع الزيادات غير المتوقعة الناتجة عن جمع القمامة ([Garbage Collection](https://kenji.blog/ar/p/memory-management-garbage-collection/)).
...
في بيئة Windows، بدلاً من `mmap` الخاصة بـ POSIX، يتم استخدام واجهات برمجة تطبيقات [Win32](/ar/p/modern-cpp-win32-api-safe-handling/) `CreateFileMapping` و `MapViewOfFile`.
...
يعد **[GGUF](/ar/p/llama-cpp-quantization-gguf/) (GPT-Generated Unified Format)** ، المحول من تنسيقات مثل `.safetensors` الخاصة بـ Hugging Face، التنسيق النهائي للاستدلال. يحتوي على المخطط الثنائي الصارم التالي:

1. **البتات السحرية (Magic Bytes)**: `0x46554747` ([GGUF](/ar/p/llama-cpp-quantization-gguf/)).
...
تتم معالجة السلسلة المدخلة كتسلسل بايتات UTF-8 وتتم مطابقتها مع مفردات محددة مسبقاً. في C++، لتسريع البحث في المفردات، يتم تنفيذ خوارزميات مثل **شجرة البادئة (Trie [Tree](https://kenji.blog/ar/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/))** أو قوائم الانتظار ذات الأولوية.
