...
В среде Windows вместо POSIX `mmap` используются API [Win32](/ru/p/modern-cpp-win32-api-safe-handling/) `CreateFileMapping` и `MapViewOfFile`.
...
**[GGUF](/ru/p/llama-cpp-quantization-gguf/) (GPT-Generated Unified Format)**, преобразованный из таких форматов, как `.safetensors` на Hugging Face, является идеальным форматом для вывода. Он имеет строгую бинарную структуру, как показано ниже:

1. **Magic Bytes**: `0x46554747` ([GGUF](/ru/p/llama-cpp-quantization-gguf/)).
...
4. **Metadata ([Key-Value](https://kenji.blog/ru/p/nosql-database-selection-kvs-document-graph-wide-column/) Pairs)**: Ключи с префиксом длины строки и типизированные значения.
