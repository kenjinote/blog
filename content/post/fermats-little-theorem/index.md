...
フェルマーの小定理（およびその一般化である **オイラーの定理** ）の最も重要な応用先が、インターネットのセキュリティを支える **RSA暗号** です。
...
オイラーの定理により、暗号化と復号のプロセスにおいて $M^{ed} \equiv M \pmod n$ が成り立つように鍵（$e$ と $d$）が設計されます。ここで、平文 $M$ が元の姿に戻るという魔法のような現象は、本質的にフェルマーの小定理が保証している数学的性質に依存しているのです。
