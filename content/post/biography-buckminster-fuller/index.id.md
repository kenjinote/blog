...
```mermaid
flowchart TD
    A["Pemahaman Lingkungan Global"] -- "Pergeseran Paradigma" --> B["Pesawat Luar Angkasa Bumi"]
    B -- "Karakteristik 1" --> C["Tidak Ada Pasokan dari Luar (Sistem Tertutup)"]
    B -- "Karakteristik 2" --> D["Awak Kapal sebagai Komunitas Nasib Bersama (Umat Manusia)"]
    B -- "Karakteristik 3" --> E["Ketergantungan pada Energi Surya"]
    C -- "Tantangan" --> F["Menghindari Penipisan Sumber Daya dan Perusakan Lingkungan"]
    D -- "Tantangan" --> F
    E -- "Solusi" --> G["Desain Sains Komprehensif"]
```
...
```mermaid
flowchart TD
    A["Pengamatan Struktur Alam"] -- "Inspirasi" --> B["Struktur Rangka Tetrahedron dan Oktahedron"]
    B -- "Aplikasi" --> C["Kubah Geodesik"]
    C -- "Keuntungan 1" --> D["Material Minimal (Efemerisasi)"]
    C -- "Keuntungan 2" --> E["Ruang Besar Tanpa Pilar"]
    C -- "Keuntungan 3" --> F["Kekuatan Sangat Tinggi dan Ketahanan Cuaca"]
    D -- "Hasil" --> G["Model Arsitektur Berkelanjutan"]
    E -- "Hasil" --> G
    F -- "Hasil" --> G
```
