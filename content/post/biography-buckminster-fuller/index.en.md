...
```mermaid
flowchart TD
    A["Perception of Earth's Environment"] -- "Paradigm Shift" --> B["Spaceship Earth"]
    B -- "Feature 1" --> C["No External Supply (Closed System)"]
    B -- "Feature 2" --> D["Crew as a Community of Destiny (Humanity)"]
    B -- "Feature 3" --> E["Dependence on Solar Energy"]
    C -- "Challenge" --> F["Avoiding Resource Depletion and Environmental Destruction"]
    D -- "Challenge" --> F
    E -- "Solution" --> G["Comprehensive Design Science"]
```
...
```mermaid
flowchart TD
    A["Observation of Natural Structures"] -- "Inspiration" --> B["Tetrahedron/Octahedron Truss Structure"]
    B -- "Application" --> C["Geodesic Dome"]
    C -- "Advantage 1" --> D["Minimum Materials (Ephemeralization)"]
    C -- "Advantage 2" --> E["Large Pillar-less Space"]
    C -- "Advantage 3" --> F["Extreme Strength and Weather Resistance"]
    D -- "Result" --> G["Prototype for Sustainable Architecture"]
    E -- "Result" --> G
    F -- "Result" --> G
```
