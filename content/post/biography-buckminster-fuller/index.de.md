...
```mermaid
flowchart TD
    A["Verständnis der globalen Umwelt"] -- "Paradigmenwechsel" --> B["Raumschiff Erde"]
    B -- "Merkmal 1" --> C["Keine Versorgung von außen (geschlossenes System)"]
    B -- "Merkmal 2" --> D["Besatzung als Schicksalsgemeinschaft (Menschheit)"]
    B -- "Merkmal 3" --> E["Abhängigkeit von Sonnenenergie"]
    C -- "Herausforderung" --> F["Vermeidung von Ressourcenerschöpfung und Umweltzerstörung"]
    D -- "Herausforderung" --> F
    E -- "Lösung" --> G["Umfassende Design-Wissenschaft"]
```
...
```mermaid
flowchart TD
    A["Beobachtung natürlicher Strukturen"] -- "Inspiration" --> B["Tetraeder- und Oktaeder-Fachwerkstruktur"]
    B -- "Anwendung" --> C["Geodätische Kuppel"]
    C -- "Vorteil 1" --> D["Minimales Material (Ephemeralisation)"]
    C -- "Vorteil 2" --> E["Großer Raum ohne Säulen"]
    C -- "Vorteil 3" --> F["Extrem hohe Festigkeit und Wetterbeständigkeit"]
    D -- "Ergebnis" --> G["Modell für nachhaltige Architektur"]
    E -- "Ergebnis" --> G
    F -- "Ergebnis" --> G
```
