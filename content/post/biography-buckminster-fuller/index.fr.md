...
```mermaid
flowchart TD
    A["Reconnaissance de l environnement terrestre"] -- "Changement de paradigme" --> B["Vaisseau spatial Terre"]
    B -- "Caracteristique 1" --> C["Aucun approvisionnement externe - systeme ferme"]
    B -- "Caracteristique 2" --> D["Equipage comme communaute de destin - humanite"]
    B -- "Caracteristique 3" --> E["Dependance a l energie solaire"]
    C -- "Defi" --> F["Eviter l epuisement des ressources et la destruction de l environnement"]
    D -- "Defi" --> F
    E -- "Solution" --> G["Science du design anticipative globale"]
```
...
```mermaid
flowchart TD
    A["Observation des structures naturelles"] -- "Inspiration" --> B["Structure en treillis tetraedrique et octaedrique"]
    B -- "Application" --> C["Dome geodesique"]
    C -- "Avantage 1" --> D["Materiaux minimaux - ephemeralisation"]
    C -- "Avantage 2" --> E["Grand espace sans piliers"]
    C -- "Avantage 3" --> F["Resistance et durabilite extremes"]
    D -- "Resultat" --> G["Modele d architecture durable"]
    E -- "Resultat" --> G
    F -- "Resultat" --> G
```
