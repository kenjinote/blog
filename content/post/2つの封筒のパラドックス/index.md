...
任意の事前分布 $P(x)$ を想定した場合、[ベイズの定理](https://kenji.blog/p/bayes-theorem/)を用いて期待値を計算すると、 **どのような現実的な（総和が1になる）確率分布であっても、すべての金額 $X$ において「交換した方が得」になるような魔法の分布は存在しない** ことが数学的に証明されています。
...
この「無限大の期待値」が引き起こすバグは、 **[サンクトペテルブルクのパラドックス](/p/st-petersburg-paradox/)** （無限の期待値を持つギャンブルに、人はいくらまで払えるかという問題）と深く根を同じくしています。
