...
假設存在任意的事前分配 $P(x)$，並使用[貝氏定理](https://kenji.blog/zh-tw/p/bayes-theorem/)來計算期望值，數學上已經證明： **在任何符合現實的（總和為1的）機率分配下，都不存在一種神奇的分配，能讓所有金額 $X$ 都符合「交換比較划算」的結果** 。
...
這種「無限大的期望值」所引發的 bug，與 **[聖彼得堡悖論](/zh-tw/p/st-petersburg-paradox/)** （面對一個具有無限期望值的賭博，人們願意支付多少錢的問題）有著極深的同源性。
