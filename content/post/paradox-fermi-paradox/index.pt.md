...
```mermaid
flowchart TD
    A["Matéria inorgânica"] -- "Possível filtro 1" --> B["Nascimento da vida (procariontes)"]
    B -- "Possível filtro 2" --> C["Nascimento dos eucariontes"]
    C -- "Evolução" --> D["Organismos multicelulares"]
    D -- "Evolução" --> E["Aquisição de inteligência e uso de ferramentas"]
    E -- "Possível filtro 3" --> F["Estabelecimento da civilização tecnológica"]
    F -- "Filtro supremo?" --> G["Civilização interestelar, expansão na galáxia"]
...
```mermaid
flowchart LR
    A["Civilização A\n(Sua sobrevivência é a prioridade máxima)"] -- "Descoberta e observação" --> B["Civilização B\n(Civilização desconhecida)"]
    A -- "Cadeia de suspeita" --> C{"A Civilização B é benigna ou maligna?"}
    C -- "Impossível de julgar" --> D["Se ignorada, pode se tornar uma ameaça\ncom uma explosão tecnológica"]
    D -- "Conclusão lógica" --> E["【Aniquilação por ataque preventivo】"]
