...
```mermaid
flowchart TD
    A["Anorganische Materie"] -- "Filterkandidat 1" --> B["Entstehung des Lebens (Prokaryoten)"]
    B -- "Filterkandidat 2" --> C["Entstehung der Eukaryoten"]
    C -- "Evolution" --> D["Mehrzellige Organismen"]
    D -- "Evolution" --> E["Erwerb von Intelligenz / Werkzeuggebrauch"]
    E -- "Filterkandidat 3" --> F["Etablierung der technologische Zivilisation"]
    F -- "Ultimativer Filter?" --> G["Interstellare Zivilisation / Expansion in die Galaxie"]
    
    style G fill:#ff9999,stroke:#333,stroke-width:2px
    
    H["Die Filter der Vergangenheit, \ndie wir überwunden haben"] -.-> B
    H -.-> C
    I["Die Filter der Zukunft, \ndenen wir gegenüberstehen"] -.-> F
    I -.-> G
```
...
```mermaid
flowchart LR
    A["Zivilisation A\n(Das eigene Überleben hat oberste Priorität)"] -- "Entdeckung / Beobachtung" --> B["Zivilisation B\n(Unbekannte Zivilisation)"]
    A -- "Kette des Verdachts" --> C{"Ist Zivilisation B wohlwollend? Oder feindselig?"}
    C -- "Unmöglich zu beurteilen" --> D["Wenn man sie ignoriert, \nkönnten sie durch eine \nTechnologieexplosion zu \neiner Bedrohung werden"]
    D -- "Logische Schlussfolgerung" --> E["【Auslöschung durch Präventivschlag】"]
