...
```mermaid
flowchart TD
    A["Bahan Anorganik"] -- "Kandidat Filter 1" --> B["Kelahiran Kehidupan (Prokariota)"]
    B -- "Kandidat Filter 2" --> C["Kelahiran Eukariota"]
    C -- "Evolusi" --> D["Organisme Multiseluler"]
    D -- "Evolusi" --> E["Pemerolehan Kecerdasan / Penggunaan Alat"]
    E -- "Kandidat Filter 3" --> F["Pembentukan Peradaban Teknologi"]
    F -- "Filter Utama?" --> G["Peradaban Antarbintang / Ekspansi ke Galaksi"]
    
    style G fill:#ff9999,stroke:#333,stroke-width:2px
    
    H["Filter Masa Lalu yang Telah\nKita Lewati"] -.-> B
    H -.-> C
    I["Filter Masa Depan yang\nAkan Kita Hadapi"] -.-> F
    I -.-> G
```
...
```mermaid
flowchart LR
    A["Peradaban A\n(Kelangsungan hidup sendiri adalah prioritas utama)"] -- "Penemuan/Observasi" --> B["Peradaban B\n(Peradaban tak dikenal)"]
    A -- "Rantai Kecurigaan" --> C{"Apakah Peradaban B bermaksud baik? Atau buruk?"}
    C -- "Tidak Bisa Dinilai" --> D["Jika dibiarkan, dapat menjadi\nancaman dengan ledakan teknologi"]
    D -- "Konsekuensi Logis" --> E["【Pemusnahan Melalui Serangan Preemptive】"]
    
    B -- "Logika yang Sama" --> E
