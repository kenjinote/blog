...
```mermaid
flowchart TD
    A["Matière inorganique"] -- "Candidat filtre 1" --> B["Apparition de la vie (Procaryotes)"]
    B -- "Candidat filtre 2" --> C["Apparition des eucaryotes"]
    C -- "Évolution" --> D["Organismes multicellulaires"]
    D -- "Évolution" --> E["Acquisition de l'intelligence et utilisation d'outils"]
    E -- "Candidat filtre 3" --> F["Établissement de la civilisation technologique"]
    F -- "Le filtre ultime ?" --> G["Civilisation interstellaire et expansion galactique"]
    
    style G fill:#ff9999,stroke:#333,stroke-width:2px
    
    H["Les filtres passés\nque nous avons surmontés"] -.-> B
    H -.-> C
    I["Les filtres futurs\nauxquels nous sommes confrontés"] -.-> F
    I -.-> G
```
...
```mermaid
flowchart LR
    A["Civilisation A\n(Leur propre survie est la priorité absolue)"] -- "Découverte et observation" --> B["Civilisation B\n(Civilisation inconnue)"]
    A -- "Chaîne de suspicion" --> C{"La civilisation B est-elle bienveillante ? Ou malveillante ?"}
    C -- "Impossible à juger" --> D["Si elle est ignorée, elle pourrait devenir\nune menace avec une explosion technologique"]
    D -- "Conclusion logique" --> E["【Annihilation par attaque préventive】"]
