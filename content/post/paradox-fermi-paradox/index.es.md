...
```mermaid
flowchart TD
    A["Materia inorgánica"] -- "Candidato a filtro 1" --> B["Origen de la vida (procariotas)"]
    B -- "Candidato a filtro 2" --> C["Aparición de eucariotas"]
    C -- "Evolución" --> D["Organismos multicelulares"]
    D -- "Evolución" --> E["Adquisición de inteligencia y uso de herramientas"]
    E -- "Candidato a filtro 3" --> F["Establecimiento de una civilización tecnológica"]
    F -- "¿El filtro definitivo?" --> G["Civilización interestelar y expansión en la galaxia"]
    
    style G fill:#ff9999,stroke:#333,stroke-width:2px
    
    H["Filtros pasados\nque hemos superado"] -.-> B
    H -.-> C
    I["Filtros futuros\na los que nos enfrentaremos"] -.-> F
    I -.-> G
```
...
```mermaid
flowchart LR
    A["Civilización A\n(su supervivencia es máxima prioridad)"] -- "Descubrimiento y observación" --> B["Civilización B\n(civilización desconocida)"]
    A -- "Cadena de sospecha" --> C{"¿Es la civilización B bien intencionada? ¿O malintencionada?"}
    C -- "Imposible de determinar" --> D["Si se la ignora, podría convertirse en una\namenaza por una explosión tecnológica"]
    D -- "Consecuencia lógica" --> E["【Aniquilación mediante ataque preventivo】"]
