---
title: 'How to Display a Windows Message Box (MessageBox) Using Rust and winapi'
slug: "RustでMessageBoxを表示する"
date: 2022-09-30T23:54:31+09:00
tags: ["Rust","MessageBox"]
draft: false
image: "images/rust_logo.webp"
categories: ["Programming"]
description: 'Explains how to display a simple message box (MessageBox) by calling the Windows API using the `winapi` and `user32-sys` crates in Rust, following the steps from project creation to code implementation.'
---

You can display a MessageBox in Rust by following these steps.

1. Install Rust. See [Getting Started with Rust](https://kenji.blog/posts/rust%E3%81%AE%E3%81%AF%E3%81%98%E3%82%81%E3%81%8B%E3%81%9F/).
2. Run `cargo new --bin MessageBox` in the command prompt.
3. Navigate to the `MessageBox` directory.
4. Open `Cargo.toml` and modify it as follows:

```toml
[package]
name = "hello_world"
version = "0.1.0"
edition = "2021"

# See more keys and their definitions at https://doc.rust-lang.org/cargo/reference/manifest.html

[dependencies]
winapi = "0.2.7"
user32-sys = "0.2.0"
```

5. Open `src\main.rs` and modify it as follows:
```rust
extern crate user32;
extern crate winapi;

use std::ffi::CString;
use user32::MessageBoxA;
use winapi::winuser::{MB_OK, MB_ICONINFORMATION};

fn main() {
    let lp_text = CString::new("Hello, world!").unwrap();
    let lp_caption = CString::new("MessageBox Example").unwrap();

    unsafe {
        MessageBoxA(
            std::ptr::null_mut(),
            lp_text.as_ptr(),
            lp_caption.as_ptr(),
            MB_OK | MB_ICONINFORMATION
        );
    }
}
```

6. Run `cargo run` in the command prompt.
   ![img.png](img.webp)

7. If you want to build a release version, run `cargo build --release`.

# References
[Hello World MesssageBox example in Rust](https://wesleywiser.github.io/post/rust-windows-messagebox-hello-world/)
