...
1928年，数学家[大卫·希尔伯特](/zh-cn/p/hilbert/)提出了“判定问题（Entscheidungsproblem）”。这是一个问题：“给定一个数学命题，是否存在一个能够机械地判定它是真还是伪的算法？”
...
纯粹的Lambda演算（无类型Lambda演算）虽然强大，但由于任何函数都可以传递任何参数，这可能导致自应用悖论（例如：[罗素悖论](/zh-cn/p/russells-paradox/)）。为了防止这种情况，邱奇后来引入了 **简单类型Lambda演算** （Simply Typed Lambda Calculus）。
