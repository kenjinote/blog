...
1928年，數學家[大衛·希爾伯特](/zh-tw/p/hilbert/)提出了「判定問題（Entscheidungsproblem）」。這是一個提問：「當給定一個數學命題時，是否存在一個演算法能機械性地判定它是真還是偽？」
...
純粹的Lambda演算（無型別Lambda演算）非常強大，但因為任何函數都可以傳遞任何參數，有可能引發自我應用造成的悖論（例如：[羅素悖論](/zh-tw/p/russells-paradox/)）。為了防止這種情況，邱奇後來引入了 **簡單型別Lambda演算** （Simply Typed Lambda Calculus）。
...
在純函數式語言中，如何在保持數學純粹性（參照透明性）的同時，處理輸入輸出或狀態變化等「副作用（Side Effects）」，一直是一個長期的課題。Haskell透過引入範疇論（Category Theory）的概念—— **單子** （[Monad](https://kenji.blog/zh-tw/p/functional-programming-concepts-pure-functions-monads/)），優雅地解決了這個問題。
