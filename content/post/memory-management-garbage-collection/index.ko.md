...
# 메모리 관리의 진실에 오신 것을 환영합니다: C, Java, Rust로 풀어보는 심연
...
Rust에서는 변수를 다른 변수에 대입하거나 함수에 값으로 전달하면 소유권이 "이동( Move )"합니다. 이동된 원래 변수는 그 후로 접근할 수 없게 됩니다(컴파일 오류가 발생합니다). 이로 인해 이중 해제(Double Free)가 구조적으로 불가능해집니다.
...
모든 조작에서 소유권을 이동시키면 프로그래밍이 매우 불편해집니다. 소유권을 빼앗지 않고 데이터에 접근하기 위해, Rust에는 **참조( Reference )** 와 **차용( Borrowing )** 의 개념이 있습니다.
...
- **C / C++ / Rust** : 구조체의 배열( `struct Array[100]` 이나 `Vec<MyStruct>` )을 생성하면, 데이터는 메모리상에 빈틈없이 연속해서 배치됩니다. 배열을 루프 처리할 때 CPU의 하드웨어 프리페처가 완벽하게 기능하여 캐시 적중률이 비약적으로 높아집니다.
...
$ h $ 를 0.99로 만들 것인지(C/Rust적 접근) 아니면 0.5로 떨어뜨려 버릴 것인지(Java적 포인터 체이스)에 따라, 애플리케이션의 루프 실행 속도에서 수십 배의 차이가 발생합니다. 이것이 게임 엔진이나 고빈도 거래 시스템에서 C++나 Rust가 선택되는 진정한 이유입니다.
...
| **Rust** | 소유권 및 차용 검사기 | 런타임 비용 제로의 안전성, 고속 | 가파른 학습 곡선, 라이프타임 설계의 어려움 |
