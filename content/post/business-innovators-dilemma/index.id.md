...
```mermaid
graph TD
    A["Perusahaan Unggulan Saat Ini"] -- "Mendengarkan suara pelanggan" --> B["Inovasi Berkelanjutan"]
    B -- "Fungsionalitas & harga tinggi" --> C["Overshooting (Kinerja Berlebihan)"]
    D["Perusahaan Rintisan"] -- "Standar nilai baru" --> E["Inovasi Disruptif"]
    E -- "Peningkatan kinerja yang cepat" --> F["Menggerogoti dari pasar bawah (low-end)"]
    C -- "Kehilangan daya saing harga" --> G["Kekalahan di pasar"]
    F -- "Merebut arus utama (mainstream)" --> G
```
...
```mermaid
graph LR
    A["Kemunculan teknologi baru"] -- "Evaluasi" --> B{"Apakah sesuai dengan kebutuhan pelanggan saat ini?"}
    B -- "YES" --> C["Investasi agresif sebagai teknologi berkelanjutan"]
    B -- "NO" --> D{"Apakah margin keuntungannya tinggi?"}
    D -- "YES" --> E["Melakukan riset pasar"]
    D -- "NO" --> F["Menunda investasi & membekukan proyek"]
    F -- "Namun teknologi terus berkembang" --> G["Destruksi oleh perusahaan rintisan"]
```
