...
```mermaid
graph TD
    A["Excelentes empresas existentes"] -- "Ouvem a voz do cliente" --> B["Inovação sustentável"]
    B -- "Maior funcionalidade e preço" --> C["Overshooting (Desempenho excessivo)"]
    D["Empresas emergentes"] -- "Novos padrões de valor" --> E["Inovação disruptiva"]
    E -- "Melhoria rápida de desempenho" --> F["Corrói o mercado a partir do low-end"]
    C -- "Perde competitividade de preço" --> G["Derrota no mercado"]
    F -- "Captura do mainstream" --> G
```
...
```mermaid
graph LR
    A["Surgimento de nova tecnologia"] -- "Avaliação" --> B{"Atende às necessidades dos clientes existentes?"}
    B -- "SIM" --> C["Investimento ativo como tecnologia sustentável"]
    B -- "NÃO" --> D{"A margem de lucro é alta?"}
    D -- "SIM" --> E["Realizar pesquisa de mercado"]
    D -- "NÃO" --> F["Adiar investimento / Congelar projeto"]
    F -- "Porém a tecnologia evolui" --> G["Disrupção por empresas emergentes"]
```
