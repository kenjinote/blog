...
```mermaid
graph TD
    A["Existing Excellent Companies"] -- "Listen to customer voice" --> B["Sustaining Innovation"]
    B -- "Higher function & Higher price" --> C["Overshooting (Excessive Performance)"]
    D["Emerging Startups"] -- "New value criteria" --> E["Disruptive Innovation"]
    E -- "Rapid performance improvement" --> F["Erode market from the low-end"]
    C -- "Lose price competitiveness" --> G["Defeat in the market"]
    F -- "Capture the mainstream" --> G
```
...
```mermaid
graph LR
    A["Emergence of new technology"] -- "Evaluate" --> B{"Does it meet existing customer needs?"}
    B -- "YES" --> C["Aggressively invest as a sustaining technology"]
    B -- "NO" --> D{"Is the profit margin high?"}
    D -- "YES" --> E["Conduct market research"]
    D -- "NO" --> F["Pass on investment / Freeze project"]
    F -- "But technology evolves" --> G["Disruption by emerging startups"]
```
