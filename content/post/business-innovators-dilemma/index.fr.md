...
```mermaid
graph TD
    A["Entreprises leaders existantes"] -- "Écoutent les clients" --> B["Innovation de soutien"]
    B -- "Haute fonctionnalité et prix élevés" --> C["Sur-performance (overshooting)"]
    D["Startups"] -- "Nouveaux critères de valeur" --> E["Innovation de rupture"]
    E -- "Amélioration rapide des performances" --> F["Érosion du marché par le bas de gamme"]
    C -- "Perte de compétitivité tarifaire" --> G["Défaite sur le marché"]
    F -- "Conquête du marché principal" --> G
```
...
```mermaid
graph LR
    A["Apparition d'une nouvelle technologie"] -- "Évaluation" --> B{"Répond-elle aux besoins des clients existants ?"}
    B -- "OUI" --> C["Investissement actif comme technologie de soutien"]
    B -- "NON" --> D{"Les marges bénéficiaires sont-elles élevées ?"}
    D -- "OUI" --> E["Étude de marché"]
    D -- "NON" --> F["Abandon d'investissement / Gel du projet"]
    F -- "Mais la technologie évolue" --> G["Rupture par les startups"]
```
