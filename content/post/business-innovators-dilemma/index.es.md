...
```mermaid
graph TD
    A["Empresa excelente existente"] -- "Escuchar la voz del cliente" --> B["Innovación continua"]
    B -- "Alta funcionalidad y alto precio" --> C["Overshooting (Rendimiento excesivo)"]
    D["Empresa emergente (Startup)"] -- "Nuevos criterios de valor" --> E["Innovación disruptiva"]
    E -- "Rápida mejora del rendimiento" --> F["Erosión del mercado desde el segmento inferior"]
    C -- "Pérdida de competitividad en precios" --> G["Derrota en el mercado"]
    F -- "Captura de la corriente principal" --> G
```
...
```mermaid
graph LR
    A["Aparición de nueva tecnología"] -- "Evaluación" --> B{"¿Satisface las necesidades del cliente existente?"}
    B -- "SÍ" --> C["Inversión activa como tecnología continua"]
    B -- "NO" --> D{"¿El margen de beneficio es alto?"}
    D -- "SÍ" --> E["Realizar estudio de mercado"]
    D -- "NO" --> F["Posponer inversión y congelar proyecto"]
    F -- "Pero la tecnología evoluciona" --> G["Disrupción por empresa emergente"]
```
