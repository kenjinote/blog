...
```mermaid
graph TD
    A["Bestehende Top-Unternehmen"] -- "Auf Kunden hören" --> B["Erhaltende Innovation"]
    B -- "Hohe Leistung & Hoher Preis" --> C["Overshooting (Übermäßige Leistung)"]
    D["Start-ups"] -- "Neue Wertmaßstäbe" --> E["Disruptive Innovation"]
    E -- "Schnelle Leistungssteigerung" --> F["Eroberung vom Low-End"]
    C -- "Verlust der preislichen Wettbewerbsfähigkeit" --> G["Niederlage auf dem Markt"]
    F -- "Übernahme des Mainstreams" --> G
```
...
```mermaid
graph LR
    A["Auftreten neuer Technologien"] -- "Bewertung" --> B{"Entspricht es den Bedürfnissen der bestehenden Kunden?"}
    B -- "JA" --> C["Aktive Investition als erhaltende Technologie"]
    B -- "NEIN" --> D{"Ist die Gewinnmarge hoch?"}
    D -- "JA" --> E["Durchführung von Marktforschung"]
    D -- "NEIN" --> F["Investition wird aufgeschoben / Projekt eingefroren"]
    F -- "Aber die Technologie entwickelt sich weiter" --> G["Disruption durch Start-ups"]
```
