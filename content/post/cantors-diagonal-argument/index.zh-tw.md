...
直覺上我們往往會認為「無限就是無限，兩者都同樣沒有盡頭」，但 19 世紀的數學家格奧爾格·康托爾（Georg Cantor）證明了一個令人驚訝的事實： **「無限有大小（勢）的差異」** 。
...
## 通往連續統假設（Continuum Hypothesis）之路
...
主張「不存在中間的勢」的假設，被稱為 **連續統假設（Continuum Hypothesis, CH）** 。康托爾將一生奉獻於這個證明，但始終未能解決。

後來，庫爾特·哥德爾（Kurt Gödel）與保羅·寇恩（Paul Cohen）證明了連續統假設 **「在目前的數學公理系統（ZFC）中，既無法證明也無法證偽（是獨立的）」** 。這是 20 世紀數學中最深奧的發現之一。
...
這種違反直覺卻絕對正確的邏輯之美，可以說是數學這門學問最大的魅力所在。對角線論證後來也被應用於艾倫·圖靈（Alan Turing）的停機問題以及哥德爾不完備定理的證明等，構成了計算機科學與數理邏輯學根基的理論。
