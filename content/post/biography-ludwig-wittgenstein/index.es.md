---
title: "Ludwig Wittgenstein: El genio filósofo que vivió entre el silencio y lo cotidiano"
...
```mermaid
flowchart TD
    A["Nacimiento de Wittgenstein (1889)"] --> B["Interés de la ingeniería a las matemáticas y la lógica"]
    B --> C["Investigación en Cambridge"]
    
    C --- D["Bertrand Russell"]
    D -.->|"Descubre su talento"| C

    C --> E["Servicio en la Primera Guerra Mundial"]
    E --> F["Escritura del 'Tractatus Logico-Philosophicus'"]
    F --> G["'De lo que no se puede hablar, hay que callar'"]
    
    G --> H["Retiro de la filosofía (Maestro, arquitecto)"]
    H --> I["Interacción con el Círculo de Viena"]
    I --> J["Regreso a Cambridge (1929)"]
    
    J --> K["Concepción de las 'Investigaciones filosóficas'"]
    K --> L["Concepto de juegos de lenguaje"]
    L --> M["'El significado es el uso del lenguaje'"]

    F -.->|"Primera filosofía (Límites de la lógica)"| N["Fuerte influencia en el positivismo lógico"]
    K -.->|"Filosofía posterior (Lenguaje cotidiano)"| O["Influencia en la filosofía del lenguaje ordinario y la filosofía analítica"]
```
