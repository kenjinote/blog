...
在密码学理论中，我们使用的不是像实数或复数那样具有连续且无限元素的域，而是元素数量有限的 **有限体（Finite Field）** 或称伽罗瓦域（[Galois](https://kenji.blog/zh-cn/p/galois/) Field）。
...
设直线的斜率为 $\lambda$（[Lambda](https://kenji.blog/zh-cn/p/serverless-architecture-aws-lambda-cold-start/)）。
...
> [!IMPORTANT]
> 这些公式包含了形如 $(x_2 - x_1)^{-1}$ 或 $(2y_1)^{-1}$ 的 **除法（模逆元的计算）** 。模逆元的计算成本非常高，因此在实际实现中，通常会使用诸如 **“雅可比坐标系（[Jacobi](https://kenji.blog/zh-cn/p/jacobi/)an Coordinates）”** 等射影坐标系来延迟除法运算。
...
### 8.1. 模运算与逆元
首先，定义有限体上的运算辅助函数。实现基于[费马小定理](https://kenji.blog/zh-cn/p/fermats-little-theorem/)的逆元计算。
