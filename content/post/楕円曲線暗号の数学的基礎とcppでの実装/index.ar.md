...
في تقنيات التشفير الحديثة، يلعب **تشفير المنحنيات الإهليلجية (Elliptic Curve [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy: ECC)** دورًا بالغ الأهمية. ليس من المبالغة القول إن أساس الثقة في مجتمعنا الرقمي الحديث يعتمد على ECC، بدءًا من اتصالات الإنترنت اليومية (HTTPS/TLS)، والجيوب الآمنة (Secure Enclaves) في الهواتف الذكية، ومصادقة الخوادم عبر SSH، والمصادقة بدون كلمات مرور مثل FIDO، وصولاً إلى الأصول المشفرة مثل بيتكوين وإيثريوم.
...
## 1. لماذا تشفير المنحنيات الإهليلجية؟ (مقارنة مع [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/))
...
بالإضافة إلى ذلك، إذا كانت الزمرة تحقق الشرط التالي (أي أن تغيير ترتيب العملية لا يغير النتيجة)، فإنها تُسمى **زمرة أبيلية ([Abel](https://kenji.blog/ar/p/abel/)ian Group)** أو زمرة إبدالية.
...
في نظرية التشفير، لا نستخدم حقولاً مستمرة ذات عناصر لا نهائية مثل الأعداد الحقيقية أو المركبة، بل نستخدم **الحقول المنتهية (Finite Fields)** أو حقول غالوا ([Galois](https://kenji.blog/ar/p/galois/) Fields) التي تحتوي على عدد محدود من العناصر.
...
> [!IMPORTANT]
> تتضمن هذه المعادلات **عمليات قسمة (حساب المعكوس النمطي)** مثل $(x_2 - x_1)^{-1}$ و $(2y_1)^{-1}$. نظرًا لأن تكلفة حساب المعكوس النمطي عالية جدًا، تُستخدم في التنفيذ الفعلي عادةً أنظمة الإحداثيات الإسقاطية مثل **"إحداثيات جاكوبي ([Jacobi](https://kenji.blog/ar/p/jacobi/)an Coordinates)"** لتأخير عملية القسمة.
...
أولاً، سنعرّف دوال مساعدة للعمليات على الحقول المنتهية. سننفذ حساب المعكوس باستخدام [مبرهنة فيرما الصغرى](https://kenji.blog/ar/p/fermats-little-theorem/).
...
1. **التفوق على [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/)**: يوفر أمانًا قويًا بمفاتيح قصيرة جدًا، مما يجعله مثاليًا لعصر الأجهزة المحمولة وإنترنت الأشياء (IoT) الحديث.
