...
В дополнение к технологиям обнаружения на изображениях (фотографиях), прорывным подходом к выявлению дипфейков в видео является **извлечение биометрических сигналов (Biological [Signals](https://kenji.blog/ru/p/state-management-history-future/))**.
...
2. **Криптографическая подпись ([Digital Signature](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/))**: На манифест и значение хэша самого изображения (сводка пиксельных данных) накладывается цифровая подпись с использованием аппаратного или программного закрытого ключа.
