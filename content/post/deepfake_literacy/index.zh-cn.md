...
**前向扩散过程（Forward Process）** ：针对清晰图像 $x_0$，在每个时间步 $t$ 中加入高斯噪声。该过程作为[马尔可夫链](https://kenji.blog/zh-cn/p/markov-chain/)，用以下公式表示：
...
2. **密码学签名（[Digital Signature](https://kenji.blog/zh-cn/p/modern-cryptography-public-key-hash-signature/)）** ：针对清单以及图像本身的哈希值（像素数据的摘要），使用硬件或软件的私钥施加数字签名。
