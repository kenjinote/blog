---
title: '天才数学者が主人公！おすすめの名作映画と暗号解読の物語'
slug: "数学者が登場する映画"
date: 2022-09-05T10:41:00+09:00
tags: ["映画","数学者"]
draft: false
image: "images/img.webp"
categories: ["math-cryptography-quantum"]
description: '天才数学者が主人公のおすすめ名作映画を紹介します。『ビューティフル・マインド』やエニグマ暗号解読を描いた『イミテーション・ゲーム』、ラマヌジャンの実話『奇跡がくれた数式』など、数学の美しさと人間ドラマが交差する傑作選です。'
---
数学者の主人公が登場する私のおすすめの映画3選です。

# ビューティフル・マインド
![](./images/beautiful_mind.webp)  
実在するアメリカの天才数学者の半生を描いた作品。最後は驚きの展開に。
主演のラッセルクロウがいい味出しています。

# エニグマ
![](./images/enigma.webp)  
> ロバート・ハリスの小説『暗号機エニグマへの挑戦』を映画化した2001年公開の映画。
> wikipediaより

エニグマ暗号機というドイツがつく出した暗号を解読する物語。

# イミテーション・ゲーム
![](./images/imitation_game.webp) 

こちらもエニグマを扱った暗号解読の物語。当時の世界観が好きです。

# 奇跡がくれた数式
![img.png](img.webp)

ケンブリッジ大学の数学者ハーディと、インドの数学者[ラマヌジャン](https://kenji.blog/p/ramanujan/)の交流を描いた作品。数学の美しさを感じることができます。

# 博士と彼女のセオリー
![img_1.png](img_1.webp)

ホーキング博士の半生を描いた作品。

# パーティクル・フィーバー

![img_2.png](img_2.webp)