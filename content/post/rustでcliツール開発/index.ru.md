...
2. **`tokio`**: Стандарт де-факто асинхронной среды выполнения [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/). Позволяет чрезвычайно эффективно обрабатывать асинхронный ввод-вывод в многопоточной среде.
...
4. **`serde` & `serde_json`**: Фреймворк для сериализации и десериализации данных. Незаменим для отображения JSON-ответов API в типобезопасные структуры [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/).
...
Иными словами, в среде, где невозможно избежать сетевой задержки $W$, для повышения пропускной способности системы $\lambda$ единственный выход — увеличить количество одновременно обрабатываемых запросов $L$. Поскольку асинхронные задачи в [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) имеют крайне низкие накладные расходы по памяти в отличие от собственных потоков ОС, масштабирование $L$ происходит очень легко.
...
Отобразим JSON-данные, возвращаемые API GitHub, на структуры [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/). Реализуем `src/models.rs` и `src/api.rs`.
...
В этой статье мы подробно рассмотрели весь процесс разработки CLI-инструмента на [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/).

1. **Принципы проектирования**: Мы подтвердили безопасность и скорость [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/), а также преимущества единого бинарного файла.
...
Разработка CLI на [Rust](https://kenji.blog/ru/p/webassembly-wasm-current-future/) — это потрясающий опыт, который позволяет гарантировать качество программного обеспечения еще на этапе проектирования посредством диалога с компилятором. Возьмите созданный нами базовый код и попробуйте разработать свой собственный, оригинальный CLI-инструмент и поделиться им с миром! Happy [Rust](https://kenji.blog/ru/p/programming-languages-history-paradigm-evolution/) Coding!
