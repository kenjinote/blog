...
2. **`tokio`**: [Rust](https://kenji.blog/p/webassembly-wasm-current-future/)の非同期ランタイムのデファクトスタンダード。マルチスレッドでの非同期I/Oを極めて効率的に処理します。
3. **`reqwest`**: `tokio` 上で動作する高機能なHTTPクライアント。使いやすいAPIを備え、非同期APIリクエストを簡単に実装できます。
4. **`serde` & `serde_json`**: データのシリアライズ・デシリアライズを行うフレームワーク。APIのJSONレスポンスを[Rust](https://kenji.blog/p/webassembly-wasm-current-future/)の型安全な構造体にマッピングするために不可欠です。
5. **`indicatif`**: リッチでカスタマイズ可能なプログレスバーを提供します。[非同期処理](https://kenji.blog/p/event-driven-architecture-async/)の進捗を視覚的に表示し、CLIのUXを劇的に向上させます。
...
## 4. [非同期処理](https://kenji.blog/p/event-driven-architecture-async/)とパフォーマンスの数理的背景
...
つまり、ネットワーク遅延 $W$ が避けられない環境下において、システムのスループット $\lambda$ を向上させるには、同時に処理するリクエスト数 $L$ を増やすしかありません。[Rust](https://kenji.blog/p/webassembly-wasm-current-future/)の非同期タスクはOSのネイティブスレッドと異なり、メモリオーバーヘッドが極めて小さいため、$L$ を容易にスケールさせることができます。
...
GitHub APIから返されるJSONデータを[Rust](https://kenji.blog/p/webassembly-wasm-current-future/)の構造体にマッピングします。`src/models.rs` と `src/api.rs` を実装します。
...
### crates.io への公開
[Rust](https://kenji.blog/p/webassembly-wasm-current-future/)のパッケージマネージャであるCargoを使えば、わずか数コマンドで公式レジストリに公開できます。
