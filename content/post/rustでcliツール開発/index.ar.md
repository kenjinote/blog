...
في تطوير البرمجيات الحديثة، تعتبر أدوات واجهة سطر الأوامر (CLI) ضرورية لتحسين إنتاجية المطورين بشكل كبير. في الماضي، كانت سكربتات الصدفة (Shell scripts) أو Python و Ruby هي السائدة، ولكن في السنوات الأخيرة، رسخت **[Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)** مكانتها كمعيار فعلي لتطوير أدوات CLI.

في هذه المقالة، سنشرح بالتفصيل كيفية بناء أدوات CLI عملية "تعمل بسرعة فائقة ويمكن تطويرها بسرعة فائقة" باستخدام [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/)، من الأساسيات إلى التطبيقات المتقدمة. لن نكتفي بصنع شيء يعمل فقط، بل سنغطي بشكل شامل معالجة الأخطاء القوية والمناسبة للمستوى التجاري، وطلبات API السريعة باستخدام المعالجة غير المتزامنة، وتنفيذ شريط تقدم (Progress bar) لتحسين تجربة المستخدم (UX).
...
2. **`tokio`**: المعيار الفعلي لبيئة التشغيل غير المتزامنة في [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/). يعالج إدخال/إخراج غير متزامن متعدد الخيوط بكفاءة عالية جدًا.
...
4. **`serde` & `serde_json`**: إطار عمل لإجراء عملية التسلسل وإلغاء التسلسل للبيانات. لا غنى عنه لتعيين استجابات JSON الخاصة بـ API إلى هياكل بيانات [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) الآمنة من حيث النوع.
...
بمعنى آخر، في بيئة لا مفر فيها من تأخير الشبكة $W$، الطريقة الوحيدة لتحسين إنتاجية النظام $\lambda$ هي زيادة عدد الطلبات التي تتم معالجتها في نفس الوقت $L$. نظرًا لأن مهام [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) غير المتزامنة، على عكس خيوط نظام التشغيل، لها حمل ذاكرة صغير جدًا، فمن السهل توسيع $L$.
...
سنقوم بتعيين بيانات JSON المرجعة من واجهة برمجة تطبيقات GitHub إلى هياكل [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/). سنقوم بتنفيذ `src/models.rs` و `src/api.rs`.
...
### النشر على crates.io
باستخدام Cargo، مدير حزم [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، يمكنك نشرها في السجل الرسمي ببضعة أوامر فقط.
...
في هذه المقالة، شرحنا بالتفصيل سلسلة تدفق تطوير أدوات CLI باستخدام [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/).

1. **نهج التصميم**: أكدنا على أمان وسرعة [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) وميزة الملفات التنفيذية الأحادية.
...
تطوير CLI بواسطة [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/) هو تجربة رائعة حيث يمكنك ضمان جودة البرنامج منذ مرحلة التصميم من خلال التفاعل مع المترجم. بناءً على الكود الأساسي الذي تم إنشاؤه هذه المرة، يرجى تطوير أداة CLI الأصلية الخاصة بك ونشرها للعالم! برمجة [Rust](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) سعيدة!
