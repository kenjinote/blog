...
このコードスニペットに見られるように、差分プライバシーの実装自体はノイズの加算というシンプルなものですが、実運用においては「プライバシーバジェット（$\epsilon$）」の管理が極めて困難になります。同じデータセットに対して複数回のクエリを発行すると、プライバシーバジェットは消費（合成定理に基づく）され、最終的にはデータセット全体を[ロック](https://kenji.blog/p/rdbms-transaction-acid-isolation-level-lock/)するか、クエリを拒否する仕組み（Privacy Budget Management）を構築する必要があります。
