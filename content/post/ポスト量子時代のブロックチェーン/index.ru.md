...
В этой статье мы глубоко и детально, с технической и математической точек зрения, рассмотрим, с какими конкретными угрозами со стороны квантовых компьютеров сталкивается технология блокчейн, а также новейшие тенденции в области **постквантовой криптографии (PQC: Post-Quantum [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)** как решения этой проблемы, и сценарии миграции криптовалютных сетей.
...
В `secp256k1` [Биткоин](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)а $a = 0, b = 7$, а $p$ — очень большое простое число.
...
### 2.2. Крах из-за алгоритма Шора ([Shor's Algorithm](https://kenji.blog/ru/p/quantum-computing-shors-algorithm/))

Однако **алгоритм Шора**, опубликованный Питером Шором в 1994 году, полностью разрушил это предположение. Изначально алгоритм Шора был предложен для решения задачи факторизации (основы криптографии [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)) за полиномиальное время, но он также применим к задаче дискретного логарифмирования и задаче дискретного логарифмирования на эллиптической кривой.
...
В блокчейне хеш-функции используются для обеспечения целостности данных, генерации адресов, а также в качестве основы для майнинга **[PoW](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/) (Proof of Work)** в [Биткоин](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)е. Обратное вычисление хеш-функции (нахождение прообраза) можно рассматривать как «задачу поиска в неструктурированной базе данных», где ищется входное значение $x$, для которого $H(x) = y$ при заданном выходном значении $y$.
...
## 3. Анализ конкретного влияния на современные криптоактивы ([Bitcoin](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/), Ethereum)

С какими конкретными уязвимостями столкнутся сети криптовалют в мире, где ECDSA можно будет взломать с помощью квантового компьютера? Здесь мы проведем подробный анализ на примере механизма [Биткоин](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)а с точки зрения **«момента раскрытия открытого ключа»**.

### 3.1. Генерация адреса и «непубличность» открытого ключа

Адреса [Биткоин](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)а ([P2P](https://kenji.blog/ru/p/webrtc-realtime-communication-p2p/)KH: Pay-to-Public-Key-Hash или P2WPKH: Pay-to-Witness-Public-Key-Hash) не являются самими открытыми ключами, а представляют собой результат многократного хеширования открытого ключа.

$$
\text{[Bitcoin](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/) Address} = \text{Base58Check}(\text{RIPEMD160}(\text{SHA256}(\text{[Public Key](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)})))
$$
...
### 3.3. Опасность повторного использования адресов и старых адресов ([P2P](https://kenji.blog/ru/p/webrtc-realtime-communication-p2p/)K)
...
## 4. Сценарий перехода на постквантовую криптографию (PQC: Post-Quantum [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)
...
### 4.2. Криптография на решетках (Lattice-Based [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy)
...
Наличие алгоритмов PQC (таких как Dilithium или SPHINCS+) не означает, что их можно внедрить в [Bitcoin](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/) или Ethereum прямо завтра. Существует ряд серьезных проблем, характерных для децентрализованных систем.
...
Если бы [Биткоин](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/) внедрил PQC при текущем пределе размера блока (около 4 МБ с учетом SegWit), количество транзакций, которое можно уместить в одном блоке, резко бы сократилось. Пропускная способность сети (TPS: [Transaction](https://kenji.blog/ru/p/rdbms-transaction-acid-isolation-level-lock/)s Per Second) катастрофически упала бы, а задержки при переводе стали бы нормой.
...
Когда стандартизация PQC от NIST будет полностью закреплена, а отраслевые стандарты библиотек станут доступны и достаточно протестированы, ожидается проведение хардфорков в основных сетях, таких как [Bitcoin](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/) и Ethereum, для полного перехода на PQC в качестве метода цифровой подписи по умолчанию. В течение этого переходного периода будут проводиться масштабные кампании, призывающие пользователей «перевести свои средства со старых кошельков на новые кошельки с поддержкой PQC».
...
* **Следить за технологическими тенденциями**: Будьте в курсе обсуждений перехода на PQC, таких как BIP в [Биткоин](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)е или EIP в Ethereum, а также новостей о хардфорках, чтобы вовремя перевести свои средства на новые кошельки, когда это потребуется.

История блокчейна — это также история обновлений и устойчивости (resilience) перед лицом новых технологических угроз. Подобно тому, как экосистема преодолела проблемы масштабируемости и экологические проблемы (например, переход от [PoW](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/) к [PoS](https://kenji.blog/ru/p/blockchain-technology-smart-contract-distributed-ledger/)), она, несомненно, найдет решения и адаптируется к этой беспрецедентной квантовой угрозе.
...
* National Institute of Standards and Technology (NIST) - Post-Quantum [Crypto](https://kenji.blog/ru/p/cryptocurrency-and-bitcoin/)graphy Standardization Project
