...
في هذا المقال، سنشرح بالتفصيل التهديدات المحددة التي تواجهها تقنية البلوكتشين بسبب أجهزة الكمبيوتر الكمية، وأحدث الاتجاهات في الحلول مثل **التشفير ما بعد الكمي (PQC: Post-Quantum [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)** ، وسيناريوهات انتقال شبكات الأصول المشفرة، وذلك من منظور تقني ورياضي متعمق.
...
تعتمد العديد من البلوكتشين، بما في ذلك البيتكوين والإيثريوم (Ethereum)، على **خوارزمية التوقيع الرقمي للمنحنى الإهليلجي (ECDSA: Elliptic Curve [Digital Signature](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/) Algorithm)** كخوارزمية توقيع رقمي. على وجه التحديد، تستخدم البيتكوين منحنى إهليلجي ذو المعلمة `secp256k1`.
...
### 2.2. الانهيار بسبب خوارزمية شور ([Shor's Algorithm](https://kenji.blog/ar/p/quantum-computing-shors-algorithm/))

ومع ذلك، فإن **خوارزمية شور** التي اقترحها بيتر شور في عام 1994 قد دمرت هذا الافتراض بالكامل. تم اقتراح خوارزمية شور في الأصل لحل مشكلة التحليل إلى العوامل الأولية (أساس تشفير [RSA](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/)) في وقت كثير الحدود، ولكن يمكن تطبيقها أيضًا على مشكلة اللوغاريتم المتقطع ومشكلة اللوغاريتم المتقطع للمنحنى الإهليلجي.
...
في البلوكتشين، تُستخدم دوال التجزئة لضمان سلامة البيانات، وتوليد العناوين، وكأساس لـ **تعدين إثبات العمل ([PoW](https://kenji.blog/ar/p/blockchain-technology-smart-contract-distributed-ledger/): Proof of Work)** في البيتكوين. يمكن اعتبار الحساب العكسي لدالة التجزئة (حساب الصورة المسبقة) كـ "مشكلة بحث في قاعدة بيانات غير مهيكلة"، حيث نبحث عن قيمة الإدخال $x$ التي تحقق $H(x) = y$ لقيمة إخراج معينة $y$.
...
لا تستخدم عناوين البيتكوين ([P2P](https://kenji.blog/ar/p/webrtc-realtime-communication-p2p/)KH: Pay-to-Public-Key-Hash أو P2WPKH: Pay-to-Witness-Public-Key-Hash) المفتاح العام بحد ذاته، بل يتم تجزئة المفتاح العام عدة مرات.

$$
\text{[Bitcoin](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/) Address} = \text{Base58Check}(\text{RIPEMD160}(\text{SHA256}(\text{[Public Key](https://kenji.blog/ar/p/modern-cryptography-public-key-hash-signature/)})))
$$
...
### 3.3. أزمة العناوين المعاد استخدامها والعناوين القديمة ([P2P](https://kenji.blog/ar/p/webrtc-realtime-communication-p2p/)K)
...
## 4. سيناريو الانتقال إلى التشفير المقاوم للكم (PQC: Post-Quantum [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)
...
### 4.2. التشفير المستند إلى الشبكية (Lattice-Based [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy)
...
لتجنب ذلك، من الضروري دمج عقد جديد مترجم مسبقًا للتحقق من PQC (على سبيل المثال: تعيين DilithiumVerify إلى `0x10`) في EVM نفسها من خلال هارد فورك (Hard Fork) للشبكة. سيتطلب هذا عملية طويلة المدى يتعاون فيها المطورون الأساسيون لمختلف عملاء الإيثريوم (Geth و Nethermind و Erigon وما إلى ذلك) لتنفيذ منطق التحقق من تشفير الشبكية الأمثل بلغات مثل C++ و [Go](https://kenji.blog/ar/p/programming-languages-history-paradigm-evolution/) و [Rust](https://kenji.blog/ar/p/webassembly-wasm-current-future/)، وإجراء عمليات تدقيق أمني.
...
بمجرد ترسيخ التوحيد القياسي لـ PQC من قبل NIST بشكل كامل واختبار مكتبات معايير الصناعة بشكل كافٍ، من المتوقع أن يتم تنفيذ هارد فورك للانتقال بالكامل إلى PQC كطريقة توقيع افتراضية على السلاسل الرئيسية مثل [Bitcoin](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/) و Ethereum. خلال هذه الفترة الانتقالية، ستكون هناك إعلانات ضخمة تحث المستخدمين على "نقل الأموال من المحافظ القديمة إلى المحافظ الجديدة المتوافقة مع PQC".
...
تاريخ البلوكتشين هو دائمًا تاريخ من الترقيات والمرونة في مواجهة التهديدات التقنية الجديدة. تمامًا كما تم التغلب على مشاكل قابلية التوسع والمشاكل البيئية (مثل الانتقال من [PoW](https://kenji.blog/ar/p/blockchain-technology-smart-contract-distributed-ledger/) إلى [PoS](https://kenji.blog/ar/p/blockchain-technology-smart-contract-distributed-ledger/))، فإن النظام البيئي بأكمله سيبحث بلا شك عن حلول ويتكيف مع هذا التهديد الكمي غير المسبوق.
...
* National Institute of Standards and Technology (NIST) - Post-Quantum [Crypto](https://kenji.blog/ar/p/cryptocurrency-and-bitcoin/)graphy Standardization Project
