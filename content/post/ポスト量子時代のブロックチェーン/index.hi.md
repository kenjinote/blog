...
इस लेख में, हम गहराई से तकनीकी और गणितीय दृष्टिकोण से व्याख्या करेंगे कि क्वांटम कंप्यूटरों द्वारा ब्लॉकचेन तकनीक को विशेष रूप से किन खतरों का सामना करना पड़ रहा है, और इसके समाधान के रूप में **पोस्ट-क्वांटम क्रिप्टोग्राफी (PQC: Post-Quantum [Crypto](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/)graphy)** के नवीनतम रुझानों और क्रिप्टो एसेट नेटवर्क के संक्रमण परिदृश्यों पर चर्चा करेंगे।
...
बिटकॉइन और एथेरियम (Ethereum) सहित कई ब्लॉकचेन अपने डिजिटल हस्ताक्षर एल्गोरिदम के रूप में **एलिप्टिक कर्व डिजिटल सिग्नेचर एल्गोरिदम (ECDSA: Elliptic Curve [Digital Signature](https://kenji.blog/hi/p/modern-cryptography-public-key-hash-signature/) Algorithm)** का उपयोग करते हैं। विशेष रूप से, बिटकॉइन `secp256k1` नामक मापदंडों वाले एलिप्टिक कर्व का उपयोग करता है।
...
इस वक्र (curve) पर एक बेस पॉइंट (संदर्भ बिंदु) को $G$ मान लेते हैं, और यादृच्छिक रूप से चुने गए 256-बिट के एक विशाल पूर्णांक को प्राइवेट की (Private Key) $k$ मानते हैं। इस स्थिति में, पब्लिक की ([Public Key](https://kenji.blog/hi/p/modern-cryptography-public-key-hash-signature/)) $K$ की गणना बेस पॉइंट को $k$ बार जोड़कर (स्केलर गुणन) की जाती है।
...
### 2.2. शोर का एल्गोरिदम ([Shor's Algorithm](https://kenji.blog/hi/p/quantum-computing-shors-algorithm/)) द्वारा पतन

हालाँकि, 1994 में पीटर शोर द्वारा प्रस्तुत किए गए **शोर के एल्गोरिदम** ने इस धारणा को पूरी तरह से नष्ट कर दिया। शोर का एल्गोरिदम मूल रूप से अभाज्य गुणनखंडन समस्या ([RSA](https://kenji.blog/hi/p/modern-cryptography-public-key-hash-signature/) क्रिप्टोग्राफी का आधार) को पॉलिनॉमियल टाइम (polynomial time) में हल करने के लिए प्रस्तावित किया गया था, लेकिन इसे डिस्क्रीट लॉगरिथम समस्या और एलिप्टिक कर्व डिस्क्रीट लॉगरिथम समस्या पर भी लागू किया जा सकता है।
...
## 3. वर्तमान क्रिप्टो एसेट्स ([Bitcoin](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/), Ethereum) पर विशिष्ट प्रभाव का विश्लेषण

ऐसी दुनिया में जहाँ क्वांटम कंप्यूटरों द्वारा ECDSA को डिक्रिप्ट किया जा सकता है, क्रिप्टो एसेट नेटवर्क विशेष रूप से किन कमजोरियों का सामना करेंगे? यहाँ, हम बिटकॉइन के तंत्र (मैकेनिज्म) का एक उदाहरण के रूप में उपयोग करके **"पब्लिक की के उजागर होने के समय (Timing of [Public Key](https://kenji.blog/hi/p/modern-cryptography-public-key-hash-signature/) Exposure)"** के दृष्टिकोण से एक विस्तृत विश्लेषण करेंगे।

### 3.1. पतों (Addresses) का निर्माण और पब्लिक की की "गोपनीयता"

बिटकॉइन के पते ([P2P](https://kenji.blog/hi/p/webrtc-realtime-communication-p2p/)KH: Pay-to-Public-Key-Hash और P2WPKH: Pay-to-Witness-Public-Key-Hash) सीधे पब्लिक की का उपयोग नहीं करते हैं, बल्कि पब्लिक की के कई बार हैश किए गए संस्करण का उपयोग करते हैं।

$$
\text{[Bitcoin](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/) Address} = \text{Base58Check}(\text{RIPEMD160}(\text{SHA256}(\text{[Public Key](https://kenji.blog/hi/p/modern-cryptography-public-key-hash-signature/)})))
$$
...
### 3.2. लेन-देन ([Transaction](https://kenji.blog/hi/p/rdbms-transaction-acid-isolation-level-lock/)) भेजते समय घातक भेद्यता (फ्रंट-रनिंग हमला)
...
### 3.3. पुन: उपयोग किए गए पतों और पुराने पतों ([P2P](https://kenji.blog/hi/p/webrtc-realtime-communication-p2p/)K) का संकट
...
## 4. पोस्ट-क्वांटम क्रिप्टोग्राफी (PQC: Post-Quantum [Crypto](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/)graphy) में संक्रमण परिदृश्य
...
### 4.2. लैटिस-बेस्ड क्रिप्टोग्राफी (Lattice-Based [Crypto](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/)graphy)

वर्तमान में, PQC की मुख्यधारा के रूप में जिसकी सबसे अधिक उम्मीद की जाती है और जिसे NIST के मुख्य मानक (FIPS 204: ML-DSA / पूर्व में CRYSTALS-Dilithium, और Falcon आदि) के रूप में अपनाया गया है, वह है **लैटिस-बेस्ड क्रिप्टोग्राफी (Lattice-Based [Crypto](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/)graphy)** ।
...
यदि बिटकॉइन अपने मौजूदा ब्लॉक आकार की सीमा (SegWit सहित लगभग 4MB का भार) को बनाए रखते हुए PQC पेश करता है, तो एक ब्लॉक में संग्रहीत किए जा सकने वाले लेन-देन की संख्या में भारी कमी आएगी। नेटवर्क का थ्रूपुट (TPS: [Transaction](https://kenji.blog/hi/p/rdbms-transaction-acid-isolation-level-lock/)s Per Second) विनाशकारी रूप से गिर जाएगा, और हस्तांतरण में देरी (congestion) एक सामान्य बात हो जाएगी।
...
इससे बचने के लिए, नेटवर्क के हार्ड फोर्क (hard fork) के माध्यम से नए PQC सत्यापन के लिए प्रीकंपाइल्ड कॉन्ट्रैक्ट (उदाहरण के लिए: `0x10` को DilithiumVerify आवंटित करना) को EVM में ही बनाना आवश्यक होगा। इसके लिए एक लंबी प्रक्रिया की आवश्यकता होती है जहाँ प्रत्येक एथेरियम क्लाइंट (Geth, Nethermind, Erigon, आदि) के मुख्य विकासकर्ता C++, [Go](https://kenji.blog/hi/p/programming-languages-history-paradigm-evolution/) और [Rust](https://kenji.blog/hi/p/webassembly-wasm-current-future/) जैसी भाषाओं के स्तर पर लैटिस क्रिप्टोग्राफी सत्यापन तर्क को अनुकूलित (optimize) करने और सुरक्षा ऑडिट (security audit) करने के लिए सहयोग करते हैं।

### 5.3. हार्ड फोर्क के माध्यम से आम सहमति ([Consensus](https://kenji.blog/hi/p/blockchain-technology-smart-contract-distributed-ledger/)) बनाने में कठिनाई

अंतर्निहित (underlying) हस्ताक्षर एल्गोरिदम को बदलने के लिए, एक **हार्ड फोर्क (Hard Fork)** आवश्यक है जो पूरे नेटवर्क के प्रोटोकॉल को अपडेट करता है। हालाँकि, बिटकॉइन जैसे समुदाय में जहाँ "नियमों को न बदलने और विकेंद्रीकृत होने" पर ज़ोर दिया जाता है, वहाँ आम सहमति (consensus) प्राप्त करने की प्रक्रिया राजनीतिक रूप से भी बहुत कठिन है। PQC में माइग्रेशन पर एक BIP ([Bitcoin](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/) Improvement Proposal) प्रस्तावित होने से लेकर लागू होने तक, कई वर्षों की चर्चा और परीक्षण की आवश्यकता होगी।
...
ब्लॉकचेन का इतिहास हमेशा नए तकनीकी खतरों के खिलाफ अपग्रेड और लचीलेपन (resilience) का इतिहास रहा है। जिस तरह हमने स्केलेबिलिटी के मुद्दों और पर्यावरणीय समस्याओं (जैसे [PoW](https://kenji.blog/hi/p/blockchain-technology-smart-contract-distributed-ledger/) से [PoS](https://kenji.blog/hi/p/blockchain-technology-smart-contract-distributed-ledger/) में संक्रमण) को पार किया है, उसी तरह संपूर्ण इकोसिस्टम इस अभूतपूर्व क्वांटम खतरे का समाधान भी खोजेगा और खुद को अनुकूलित करेगा।
...
* National Institute of Standards and Technology (NIST) - Post-Quantum [Crypto](https://kenji.blog/hi/p/cryptocurrency-and-bitcoin/)graphy Standardization Project
