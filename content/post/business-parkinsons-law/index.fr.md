...
```mermaid
graph TD
    A["Augmentation du temps alloué"] -- "Provoque" --> B["Complexité du travail et surqualité"]
    A -- "Provoque" --> C["Baisse de la pression psychologique et procrastination"]
    B -- "En conséquence" --> D["Génération de travail inutile"]
    C -- "En conséquence" --> D
    D -- "Finalement" --> E["Baisse de la productivité et achèvement à la dernière minute"]
...
```mermaid
graph LR
    F["Apparition de la tâche"] -- "Suit la loi de Parkinson" --> G["Travail paresseux jusqu'à la limite"]
    F -- "Application de l'approche de surpassement" --> H["Délai provisoire serré et timeboxing"]
    H -- "Amélioration spectaculaire de la concentration" --> I["Achèvement rapide et de haute qualité"]
    I -- "Création de temps libre" --> J["Tâche importante suivante ou repos intentionnel"]
    G -- "Accumulation de fatigue et compromis" --> K["Livraison de mauvaise qualité à la dernière minute"]
