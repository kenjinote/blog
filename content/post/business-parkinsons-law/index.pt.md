...
```mermaid
graph TD
    A["Aumento do tempo disponível"] -- "Causa" --> B["Complexidade do trabalho e excesso de qualidade"]
    A -- "Causa" --> C["Redução da pressão psicológica e procrastinação"]
    B -- "Resulta em" --> D["Geração de trabalho inútil"]
    C -- "Resulta em" --> D
    D -- "Em última análise" --> E["Queda na produtividade e conclusão no limite do prazo"]
...
```mermaid
graph LR
    F["Ocorrência da tarefa"] -- "Segue a Lei de Parkinson" --> G["Trabalho arrastado até o limite do prazo"]
    F -- "Aplicação da abordagem de superação" --> H["Prazo provisório apertado e definição de timebox"]
    H -- "Melhoria drástica da concentração" --> I["Conclusão antecipada e de alta qualidade"]
    I -- "Criação de tempo livre" --> J["Próxima tarefa importante ou descanso intencional"]
    G -- "Acúmulo de fadiga e compromisso" --> K["Entrega de baixa qualidade no último minuto"]
