...
この小さなスクリプトは、ブラウザのレンダリングをブ[ロック](https://kenji.blog/p/rdbms-transaction-acid-isolation-level-lock/)して即座に実行されるため、画面が描画される時点では既に `data-theme` 属性がセットされており、画面のチラつき（FOUC）を完全に防ぐことができます。
