...
본 기사에서는 현대 소프트웨어 개발에서 주류가 되고 있는 **객체 지향 프로그래밍** (OOP), 수학적인 견고함을 가지는 **함수형 프로그래밍** (FP), 그리고 성능과 데이터 분리에 초점을 맞춘 **데이터 지향 프로그래밍** (DOP / DOD)에 대해 각각의 사상, 강점, 그리고 **한계** 를 깊이 파고듭니다. 나아가 현대의 강력한 언어(Rust나 TypeScript 등)가 이것들을 어떻게 **융합** 시키고 있는지 해설합니다.
...
**데이터 지향 설계** (Data-Oriented Design) 또는 **데이터 지향 프로그래밍** 은 게임 개발(특히 C++이나 Rust) 현장에서 태어나, 그 후 엔터프라이즈 영역(Clojure의 사상 등)에도 파급된 패러다임입니다.
...
오늘날, 이들 중 '유일한 정답'을 선택하는 것은 난센스로 여겨집니다. 모던 프로그래밍 언어(Rust, TypeScript, Scala, Go 등)는 이들 패러다임의 **장점만을 취합** 하고 있습니다.
