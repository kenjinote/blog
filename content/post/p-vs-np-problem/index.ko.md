...
2000년 클레이 수학 연구소는 7개의 수학적 미해결 문제에 대해 각각 100만 달러의 현상금을 걸었습니다. 이들은 **밀레니엄 현상 문제** 라고 불립니다. [푸앵카레 추측](/ko/p/poincare-conjecture/)처럼 이미 해결된 것도 있지만, **P vs NP 문제** 는 아직까지 해결의 실마리조차 완전히 보이지 않고 있습니다.

본 기사에서는 이 **P vs NP 문제** 의 전모를, 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 클래스(P, NP, NP-완전, NP-난해)의 기초부터 프로그래밍에서의 실천적인 의의, 나아가 만약 해명되었을 경우 세계에 미칠 영향까지 상세히 깊이 파헤쳐 해설합니다.

---

## 1. 계산 복잡도 이론과 알고리즘의 기초

**P vs NP 문제** 를 이해하기 위해서는 먼저 "알고리즘의 계산량"이라는 개념을 이해해야 합니다. 컴퓨터는 어떤 문제를 풀기 위해 단계별로 계산을 수행하지만, 입력 크기 $n$ 이 커졌을 때 계산에 필요한 시간(스텝 수)이나 메모리(공간)가 어떻게 증가하는지를 나타내는 것이 **계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/)(Computational Complexity)** 입니다.
...
계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 이론에서 $O(n^k)$ ( $k$ 는 상수)로 표현되는 시간을 **다항식 시간(Polynomial Time)** 이라고 부르며, 실용적인 시간 안에 계산 가능하다는 하나의 기준으로 간주합니다. 반면, $O(2^n)$ 등의 지수 시간은 $n$ 이 수십만 되어도 우주의 수명을 초과하는 계산 시간이 필요하기 때문에 실질적으로 "풀 수 없다"고 간주됩니다.
...
아래는 클래스 P의 대표적인 예인 이진 [탐색 알고리즘](/ko/p/search-algorithms-linear-binary-hash-table-principles/)의 Python 구현입니다.
...
현재 전 세계의 암호 시스템(인터넷 쇼핑에 사용되는 SSL/TLS나 비트코인 등의 [블록체인](/ko/p/blockchain-technology-smart-contract-distributed-ledger/))은 **"푸는 데는 엄청난 시간이 걸리지만, 검증은 순식간에 할 수 있다"** 는 비대칭성을 이용하고 있습니다.
...
1. **암호의 붕괴**: RSA 암호나 [타원곡선 암호](/ko/p/elliptic-curve-cryptography-math-cpp/) 등 현대 공개키 암호 체계는 모두 순식간에 깨지고, 디지털 상의 보안은 완전히 붕괴합니다.
...
계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 이론에서는 양자 컴퓨터가 다항식 시간에 풀 수 있는 문제 클래스를 **BQP (Bounded-error Quantum Polynomial time)** 라고 부릅니다. 피터 쇼어가 고안한 "쇼어 알고리즘"에 의해 소인수분해는 BQP에 속한다는 것이 증명되었습니다 (양자 컴퓨터로 빠르게 풀 수 있음).
...
"이 문제의 최적 해답을 내는 시스템을 만들어 줘"라고 비즈니스 측에서 요구했을 때, 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 이론에 대한 지식이 없다면 당신은 영원히 끝나지 않는 프로그램을 작성하게 되어 서버를 다운시키고 말 것입니다.
...
2. **완화와 근사로 우회하기**:
    ...
    - **동적 계획법 ([DP](https://kenji.blog/ko/p/dynamic-programming-dp-introduction-knapsack-fibonacci/))**: 배낭 문제처럼 입력 숫자의 크기에 의존하는(유사 다항식 시간) 해법이 존재할 경우 입력의 제약을 이용한다.
...
향후 연구의 돌파구를 통해 우리가 살아있는 동안 이 문제의 결말을 볼 수 있을 것인지. 아니면 [괴델의 불완전성 정리](/ko/p/godels-incompleteness-theorems/)처럼 "증명도 반증도 불가능하다"는 것이 증명될 것인지. 계산 [복잡도](/ko/p/time-space-complexity-big-o-notation-examples/) 이론의 최전선에서 앞으로도 눈을 뗄 수 없습니다.
