...
flowchart TD
    A["Eiichi Shibusawa"]
    A --> B["Modernisation économique et industrielle"]
    A --> C["Contribution sociale et éducative"]
    
    B --> B1["Première Banque Nationale (Finance)"]
    B --> B2["Oji Paper・Toyobo (Fabrication)"]
    B --> B3["Tokyo Gas・Nippon Yusen (Infrastructure)"]
    
    C --> C1["Université Hitotsubashi・Université des femmes du Japon (Éducation)"]
    C --> C2["Asile de Tokyo (Bien-être)"]
    C --> C3["Croix-Rouge japonaise (Médical)"]
    
    B -. "Redistribution des profits à la société" .-> C
    C -. "Production de talents exceptionnels" .-> B
