...
```mermaid
flowchart TD
  A["Alan Turing (1950s)"] -- "Concepto y Teoría" --> B["Claude Shannon (1950)"]
  B -- "Algoritmo Minimax" --> C["Mac Hack VI (1967)"]
  C -- "Primer torneo jugado" --> D["Belle (1980)"]
  D -- "Aceleración de hardware" --> E["Deep Thought (1988)"]
...
```mermaid
flowchart TD
  A["IBM RS/6000 SP"] -- "Nodo maestro (Lenguaje C)" --> B["Búsqueda y control por software"]
  A -- "30 nodos" --> C["Búsqueda por hardware"]
  C -- "Chips VLSI personalizados" --> D["480 chips de ajedrez"]
  D -- "Función de evaluación" --> E["200 millones de posiciones / seg"]
```
