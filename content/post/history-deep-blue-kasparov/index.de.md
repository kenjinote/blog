...
```mermaid
flowchart TD
  A["Alan Turing (1950er)"] -- "Konzept & Theorie" --> B["Claude Shannon (1950)"]
  B -- "Minimax-Algorithmus" --> C["Mac Hack VI (1967)"]
  C -- "Erstes Turnierspiel" --> D["Belle (1980)"]
  D -- "Hardware-Beschleunigung" --> E["Deep Thought (1988)"]
...
```mermaid
flowchart TD
  A["IBM RS/6000 SP"] -- "Master-Knoten (C-Sprache)" --> B["Software-Suche & Steuerung"]
  A -- "30 Knoten" --> C["Hardware-Suche"]
  C -- "Spezielle VLSI-Chips" --> D["480 Schach-Chips"]
  D -- "Bewertungsfunktion" --> E["200 Millionen Stellungen / Sek"]
```
