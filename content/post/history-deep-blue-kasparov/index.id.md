...
```mermaid
flowchart TD
  A["Alan Turing (1950s)"] -- "Konsep & Teori" --> B["Claude Shannon (1950)"]
  B -- "Algoritma Minimax" --> C["Mac Hack VI (1967)"]
  C -- "Permainan turnamen pertama" --> D["Belle (1980)"]
  D -- "Akselerasi perangkat keras" --> E["Deep Thought (1988)"]
...
```mermaid
flowchart TD
  A["IBM RS/6000 SP"] -- "Node master (Bahasa C)" --> B["Pencarian & Kontrol Perangkat Lunak"]
  A -- "30 node" --> C["Pencarian Perangkat Keras"]
  C -- "Chip VLSI Kustom" --> D["480 Chip Catur"]
  D -- "Fungsi Evaluasi" --> E["200 juta posisi / detik"]
```
