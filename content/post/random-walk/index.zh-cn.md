...
随机漫步（Random Walk）是一个数学概念，指的是下一步的位置由概率随机决定的运动。由于它类似于醉汉摇摇晃晃地左右行走的模样，因此常被称为“醉汉漫步”。乍看之下，这是一种无序且不可预测的运动，但是当步骤数量累积到一定程度后，就会浮现出令人惊叹的、美丽且有规律的数学法则。

在本文中，我们将从最简单的一维随机漫步的基础出发，结合数学公式，深入探讨它如何与物理学中的扩散现象和布朗运动联系起来，以及高维空间中随机漫步的有趣性质。对随机漫步的理解，不仅仅局限于物理学和数学，更已成为金融工程、信息科学等现代众多领域必不可少的素养。

## 历史背景：卡尔·皮尔逊的提问

“随机漫步”这一术语首次在学术上使用，是在 1905 年英国数理统计学家卡尔·皮尔逊（Karl Pearson）向科学杂志《自然》投稿的一篇简短提问文章中。他提出了以下问题：

> “一个人从原点出发，向随机方向走直线距离 $l$。重复 $n$ 次后，其距离出发点在 $r$ 和 $r + dr$ 之间的概率是多少？”

针对这一问题，瑞利男爵（Lord Rayleigh）指出，这完全可以直接套用他自己在声学研究中关于“多个声波叠加”的数学公式。这成为了随机漫步理论被广泛认知的契机。

## 一维随机漫步的严密数学表述

### 概率性移动的定义

让我们考虑最简单的一维随机漫步。假设位于数轴上原点 $x = 0$ 的粒子，每单位时间以概率 $p$ 向右移动 $+1$，以概率 $q = 1 - p$ 向左移动 $-1$。在这里，我们仅处理最简单的 $p = q = 1/2$ 的对称随机漫步（Symmetric Random Walk）。
...
这个结果非常重要。期望值为 $0$ 意味着 **平均而言，粒子会停留在原点**。然而，由于方差与 $n$ 成正比增加，标准差（离散程度的指标）变为 $\sqrt{n}$。换句话说，随着步数 $n$ 的增加，粒子的存在范围会以 $\sqrt{n}$ 的量级逐渐扩大。时间以 $n$ 推进，而移动距离却只以 $\sqrt{n}$ 推进，这种低效性正是随机漫步的最大特征。
...
虽然随机漫步是描述微观粒子运动的模型，但若从宏观的连续极限来看，就可以将其理解为 **扩散方程**。
...
将随机漫步连续化后便得到了 **布朗运动**（Brownian Motion）。

1827 年，植物学家罗伯特·布朗发现，水中花粉释放出的微粒在做不规则运动。这个长期成谜的现象，在 1905 年被阿尔伯特·爱因斯坦通过数学解释为“水分子随机碰撞微粒引起的随机漫步”。爱因斯坦利用扩散方程，推导出微粒的均方位移与时间成正比 $\langle x^2 \rangle = 2Dt$。
...
在无限大的网格空间中进行随机漫步时，重新回到出发点（原点）的概率（复发概率）会因维度的不同而不同。
...
随机漫步的理论并不仅仅停留在物理学中。在有效市场假说下，金融市场的股价波动也被认为服从随机漫步。
...
除了理论之外，通过实际运行程序进行可视化也能加深理解。让我们使用 Python 进行二维随机漫步的模拟吧。
...
在本文中，我们从最简单的“醉汉漫步”出发，详细讲解了与随机漫步相关的数学背景，包括中心极限定理向高斯分布的收敛、扩散方程的推导、爱因斯坦对布朗运动的解释、波利亚定理，以及在金融工程中的应用。

从极其简单、无序的规则的反复中，自然而然地浮现出支配宏观世界的普遍法则（微分方程和正态分布），这是数学和物理学中最具魅力且令人感动的侧面之一。随机漫步这一概念，在未来也必定会继续作为解开各种未知现象的强大武器。
