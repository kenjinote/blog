...
```mermaid
graph TD
    subgraph IbishaPhilosophy ["Ibisha Philosophy"]
        Rook["Right-side Rook (Main Axis of Attack)"] --> |"Breakthrough from the front"| EnemyLeft["Enemy's Left-side Camp"]
        King["Left-side King (Entering the castle)"] --> |"To a safe zone"| LeftCastle["Solid Castles like Yagura and Anaguma"]
    end
```
...
```mermaid
graph TD
    subgraph FuribishaPhilosophy ["Furibisha Philosophy"]
        RookStart["Right-side Rook"] -.-> |"Major movement to the left (Swinging)"| RookEnd["Left-side Rook (Preparing for counterattack)"]
        KingStart["Left-side King"] -.-> |"Escape to the right"| KingEnd["Right-side King (To Mino Castle)"]
    end
```
