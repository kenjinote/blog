...
```mermaid
graph TD
    A["Le voyageur temporel va dans le passé"] -- "Interférence avec le passé" --> B["Tue le grand-père"]
    B -- "Altération de l'histoire" --> C["Le voyageur ne naît pas"]
    C -- "Effacement de l'existence" --> D["Ne peut pas aller dans le passé"]
    D -- "Pas d'interférence avec le passé" --> E["Le grand-père ne meurt pas"]
    E -- "Restauration de l'histoire" --> F["Le voyageur naît"]
    F -- "Une fois de plus" --> A
```
...
```mermaid
graph TD
    A["Ligne temporelle d'origine (Timeline A)"] -- "Voyage dans le temps" --> B["Point de ramification"]
    B -- "Tue le grand-père" --> C["Nouvelle ligne temporelle (Timeline B)"]
    B -- "Ne fait rien" --> D["Continuation de la ligne d'origine"]
    C -- "Monde parallèle" --> E["Futur où le voyageur n'existe pas"]
    D -- "Monde parallèle" --> F["Futur où le voyageur existe"]
    A -. "Inobservable" .- C
```
