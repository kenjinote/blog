...
```mermaid
graph TD
    A["Penjelajah waktu pergi ke masa lalu"] -- "Campur tangan dengan masa lalu" --> B["Membunuh kakek"]
    B -- "Perubahan sejarah" --> C["Diri sendiri tidak lahir"]
    C -- "Penghapusan eksistensi" --> D["Tidak bisa pergi ke masa lalu"]
    D -- "Tidak ada campur tangan" --> E["Kakek tidak mati"]
    E -- "Pemulihan sejarah" --> F["Diri sendiri lahir"]
    F -- "Sekali lagi" --> A
```
...
```mermaid
graph TD
    A["Garis waktu asli (Timeline A)"] -- "Perjalanan waktu" --> B["Titik percabangan"]
    B -- "Membunuh kakek" --> C["Garis waktu baru (Timeline B)"]
    B -- "Tidak melakukan apa-apa" --> D["Kelanjutan garis waktu asli"]
    C -- "Dunia paralel" --> E["Masa depan di mana diri sendiri tidak ada"]
    D -- "Dunia paralel" --> F["Masa depan di mana diri sendiri ada"]
    A -. "Tidak dapat diamati" .- C
```
