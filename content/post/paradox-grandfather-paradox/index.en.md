...
```mermaid
graph TD
    A["Time traveler goes to the past"] -- "Interference with the past" --> B["Kills grandfather"]
    B -- "Alteration of history" --> C["Self is not born"]
    C -- "Erasure of existence" --> D["Cannot go to the past"]
    D -- "No interference with the past" --> E["Grandfather does not die"]
    E -- "Restoration of history" --> F["Self is born"]
    F -- "Once again" --> A
```
...
```mermaid
graph TD
    A["Original timeline (Timeline A)"] -- "Time travel" --> B["Branching point in the past"]
    B -- "Kills grandfather" --> C["New timeline (Timeline B)"]
    B -- "Does nothing" --> D["Continuation of original timeline"]
    C -- "Parallel world" --> E["Future where self does not exist"]
    D -- "Parallel world" --> F["Future where self exists"]
    A -. "Unobservable" .- C
```
