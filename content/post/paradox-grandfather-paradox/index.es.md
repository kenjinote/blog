...
```mermaid
graph TD
    A["El viajero del tiempo va al pasado"] -- "Interferencia con el pasado" --> B["Mata al abuelo"]
    B -- "Alteración de la historia" --> C["El viajero no nace"]
    C -- "Borrado de la existencia" --> D["No puede ir al pasado"]
    D -- "Sin interferencia en el pasado" --> E["El abuelo no muere"]
    E -- "Restauración de la historia" --> F["El viajero nace"]
    F -- "Una vez más" --> A
```
...
```mermaid
graph TD
    A["Línea de tiempo original (Timeline A)"] -- "Viaje en el tiempo" --> B["Punto de bifurcación"]
    B -- "Mata al abuelo" --> C["Nueva línea de tiempo (Timeline B)"]
    B -- "No hace nada" --> D["Continuación de la línea original"]
    C -- "Mundo paralelo" --> E["Futuro donde el viajero no existe"]
    D -- "Mundo paralelo" --> F["Futuro donde el viajero existe"]
    A -. "Inobservable" .- C
```
