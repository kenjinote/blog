...
```mermaid
graph TD
    A["Zeitreisender reist in die Vergangenheit"] -- "Eingriff in die Vergangenheit" --> B["Tötet den Großvater"]
    B -- "Änderung der Geschichte" --> C["Das eigene Selbst wird nicht geboren"]
    C -- "Auslöschung der Existenz" --> D["Kann nicht in die Vergangenheit reisen"]
    D -- "Kein Eingriff in die Vergangenheit" --> E["Großvater stirbt nicht"]
    E -- "Wiederherstellung der Geschichte" --> F["Das eigene Selbst wird geboren"]
    F -- "Erneut" --> A
```
...
```mermaid
graph TD
    A["Ursprüngliche Zeitlinie (Timeline A)"] -- "Zeitreise" --> B["Verzweigungspunkt in der Vergangenheit"]
    B -- "Tötet Großvater" --> C["Neue Zeitlinie (Timeline B)"]
    B -- "Tut nichts" --> D["Fortsetzung der ursprünglichen Zeitlinie"]
    C -- "Parallelwelt" --> E["Zukunft, in der das Selbst nicht existiert"]
    D -- "Parallelwelt" --> F["Zukunft, in der das Selbst existiert"]
    A -. "Nicht beobachtbar" .- C
```
