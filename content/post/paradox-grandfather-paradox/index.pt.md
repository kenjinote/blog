...
```mermaid
graph TD
    A["Viajante do tempo vai ao passado"] -- "Interferência no passado" --> B["Mata o avô"]
    B -- "Alteração da história" --> C["A própria pessoa não nasce"]
    C -- "Apagamento da existência" --> D["Não pode ir ao passado"]
    D -- "Sem interferência no passado" --> E["O avô não morre"]
    E -- "Restauração da história" --> F["A própria pessoa nasce"]
    F -- "Mais uma vez" --> A
```
...
```mermaid
graph TD
    A["Linha do tempo original (Timeline A)"] -- "Viagem no tempo" --> B["Ponto de ramificação"]
    B -- "Mata o avô" --> C["Nova linha do tempo (Timeline B)"]
    B -- "Não faz nada" --> D["Continuação da linha original"]
    C -- "Mundo paralelo" --> E["Futuro onde a pessoa não existe"]
    D -- "Mundo paralelo" --> F["Futuro onde a pessoa existe"]
    A -. "Inobservável" .- C
```
