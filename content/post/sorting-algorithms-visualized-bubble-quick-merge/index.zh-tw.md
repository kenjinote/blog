...
本文將從適合初學者的簡單演算法，到實務上廣泛使用的高速演算法，全面解說具代表性的排序演算法。透過 **Mermaid** 圖解視覺化了解各演算法的機制，以 Python 程式碼確認實際實作，並比較時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)等效能。此外，為了完全掌握演算法的運作，也收錄了使用 50 個元素的陣列進行的完整執行追蹤。藉此，您將能瞭若指掌地理解演算法的細部行為。

## 演算法評估指標

在評估各演算法時，以下指標非常重要。

- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/) (Time Complexity)** : 表示處理時間隨資料元素數量 $n$ 增加而增長的情況。使用 $\text{O}(n^2)$ 或 $\text{O}(n \log n)$ 等大 O 記號（Big-O notation）。在數式中處理文字時使用如 $\text{最佳}$ 的格式。
- **空間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/) (Space Complexity)** : 表示執行時需要多少額外的記憶體。原地（In-place）演算法幾乎不需要額外記憶體。
...
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（最佳）**: $\text{O}(n)$
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（平均）**: $\text{O}(n^2)$
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（最差）**: $\text{O}(n^2)$
- **空間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)**: $\text{O}(1)$
- **穩定性**: 穩定
...
在這個循環中，未排序部分中的最大元素像氣泡一樣浮到了最右側。根據氣泡排序的特性，每次循環至少能保證一個元素被放置在最終的正確位置。因此，隨著循環次數增加，每次探索的範圍可以縮小一個元素，從而減少不必要的比較操作。然而，在資料完全反序的最壞情況下，每對元素都需要進行交換操作，時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)將達到 $\text{O}(n^2)$，效能極低。
...
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（最佳）**: $\text{O}(n \log n)$
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（平均）**: $\text{O}(n \log n)$
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（最差）**: $\text{O}(n^2)$
- **空間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)**: $\text{O}(\log n)$
- **穩定性**: 不穩定
...
快速排序是實務中最頻繁使用的演算法之一。雖然最差[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)為 $\text{O}(n^2)$，但透過使用適當的樞紐選擇策略（例如 Median-of-Three），實質上總能以 $\text{O}(n \log n)$ 運作。Python 內建的排序函式 `list.sort()` 並非使用快速排序，而是採用了結合合併排序與插入排序、稱為 **TimSort** 的演算法。

### 關於樞紐選擇的重要性

快速排序的效能很大程度上取決於樞紐的選擇方式。理想情況下，若總能選擇陣列的中位數（Median）作為樞紐，陣列每次就能精確地分成兩半，遞迴深度將為 $\text{O}(\log n)$，並保證完美的 $\text{O}(n \log n)$ [複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)。然而，嚴密尋找真正的中位數需要額外的計算成本，因此實務上會採用能在常數時間內選擇的近似手法。
...
如果陣列在已排序的狀態下，總是選擇開頭的元素作為樞紐，分割出來的陣列其中一邊元素數為 0，另一邊為 $n-1$，遞迴深度將達到 $n$。這會導致時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)變成最差的 $\text{O}(n^2)$，有時甚至有引發堆疊溢位 ([Stack](https://kenji.blog/zh-tw/p/c-language-pointers-memory-management-stack-heap/) Overflow) 的危險。

如果陣列在已排序的狀態下，總是選擇開頭的元素作為樞紐，分割出來的陣列其中一邊元素數為 0，另一邊為 $n-1$，遞迴深度將達到 $n$。這會導致時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)變成最差的 $\text{O}(n^2)$，有時甚至有引發堆疊溢位 (Stack Overflow) 的危險。
...
由[約翰·馮·紐曼](/zh-tw/p/von-neumann/) (John von Neumann) 所發明，是穩定的外部排序之代表。基於分治法，將陣列細分後，在保持已排序順序的情況下將它們結合（合併）。

### 複雜度與特性

- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（最佳）**: $\text{O}(n \log n)$
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（平均）**: $\text{O}(n \log n)$
- **時間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)（最差）**: $\text{O}(n \log n)$
- **空間[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)**: $\text{O}(n)$
- **穩定性**: 穩定
...
- 需要穩定性，或想保證最差[複雜度](/zh-tw/p/time-space-complexity-big-o-notation-examples/)的情況，會選擇 **合併排序**。
