...
```mermaid
graph TD
    A["Suche nach einem Zeigegerät (1960er Jahre)"] -- "Test und Bewertung" --> B["Lichtgriffel"]
    A -- "Test und Bewertung" --> C["Joystick"]
    A -- "Test und Bewertung" --> D["Trackball"]
    A -- "Test und Bewertung" --> E["Kniesteuerung"]
    A -- "Konzeption des idealen Geräts" --> F["Prototyp der 'Maus'"]
    B -. "Nachteil: Arm wird müde" .-> G["Abgelehnt/Nicht übernommen"]
    C -. "Nachteil: Geringe Präzision" .-> G
    D -. "Vorteil: Gute Bedienbarkeit, aber verbesserungsfähig" .-> H["Erkenntnisse für die Mausentwicklung"]
    E -. "Vorteil: Überraschend gut bewertet" .-> H
    F -- "Design und Herstellung durch Bill English" --> I["Erste hölzerne Maus (1963-1964)"]
```
...
```mermaid
graph TD
    A["The Mother of All Demos (1968)"] -- "Demonstrierte Haupttechnologien" --> B["GUI-Bedienung per Maus"]
    A -- "Demonstrierte Haupttechnologien" --> C["Hypertext und Links"]
    A -- "Demonstrierte Haupttechnologien" --> D["Dynamische Dokumentenbearbeitung (Textverarbeitung)"]
    A -- "Demonstrierte Haupttechnologien" --> E["Outline Processing"]
    A -- "Demonstrierte Haupttechnologien" --> F["Bildschirmaufteilung (Multi-Window)"]
    A -- "Demonstrierte Haupttechnologien" --> G["Videokonferenzsystem"]
    A -- "Demonstrierte Haupttechnologien" --> H["Zusammenarbeit (Kollaboration)"]
```
