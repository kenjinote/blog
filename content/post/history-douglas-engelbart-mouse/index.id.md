...
```mermaid
graph TD
    A["Pencarian Perangkat Penunjuk (Tahun 1960-an)"] -- "Uji coba dan evaluasi" --> B["Light pen"]
    A -- "Uji coba dan evaluasi" --> C["Joystick"]
    A -- "Uji coba dan evaluasi" --> D["Trackball"]
    A -- "Uji coba dan evaluasi" --> E["Knee control"]
    A -- "Gagasan perangkat ideal" --> F["Prototipe \"Mouse\""]
    B -. "Kekurangan: Tangan lelah" .-> G["Ditolak / Tidak Digunakan"]
    C -. "Kekurangan: Akurasi rendah" .-> G
    D -. "Kelebihan: Mudah digunakan tapi perlu perbaikan" .-> H["Wawasan untuk pengembangan mouse"]
    E -. "Kelebihan: Di luar dugaan mendapat nilai tinggi" .-> H
    F -- "Desain dan pembuatan oleh Bill English" --> I["Mouse kayu pertama (1963-1964)"]
```
...
```mermaid
graph TD
    A["The Mother of All Demos (1968)"] -- "Teknologi utama yang didemokan" --> B["Operasi GUI dengan mouse"]
    A -- "Teknologi utama yang didemokan" --> C["Hiperteks dan Tautan"]
    A -- "Teknologi utama yang didemokan" --> D["Pengeditan dokumen dinamis (Word Processor)"]
    A -- "Teknologi utama yang didemokan" --> E["Outline Processing"]
    A -- "Teknologi utama yang didemokan" --> F["Pembagian layar (Multi-window)"]
    A -- "Teknologi utama yang didemokan" --> G["Sistem konferensi video"]
    A -- "Teknologi utama yang didemokan" --> H["Kerja kolaborasi (Collaboration)"]
```
