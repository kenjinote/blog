...
```mermaid
graph TD
    A["Búsqueda de dispositivos apuntadores (Años 1960)"] -- "Pruebas y evaluación" --> B["Lápiz óptico"]
    A -- "Pruebas y evaluación" --> C["Joystick"]
    A -- "Pruebas y evaluación" --> D["Trackball"]
    A -- "Pruebas y evaluación" --> E["Control de rodilla"]
    A -- "Invención del dispositivo ideal" --> F["Prototipo del ratón"]
    B -. "Desventaja: Fatiga en el brazo" .-> G["Rechazado/No adoptado"]
    C -. "Desventaja: Baja precisión" .-> G
    D -. "Ventaja: Buena operatividad, pero mejorable" .-> H["Conocimientos para desarrollar el ratón"]
    E -. "Ventaja: Alta valoración inesperada" .-> H
    F -- "Diseñado y construido por Bill English" --> I["Primer ratón de madera (1963-1964)"]
```
...
```mermaid
graph TD
    A["The Mother of All Demos (1968)"] -- "Tecnologías principales demostradas" --> B["Operación GUI mediante ratón"]
    A -- "Tecnologías principales demostradas" --> C["Hipertexto y enlaces"]
    A -- "Tecnologías principales demostradas" --> D["Edición dinámica de documentos (Procesador de textos)"]
    A -- "Tecnologías principales demostradas" --> E["Procesamiento de esquemas"]
    A -- "Tecnologías principales demostradas" --> F["Pantalla dividida (Multiventana)"]
    A -- "Tecnologías principales demostradas" --> G["Sistema de videoconferencia"]
    A -- "Tecnologías principales demostradas" --> H["Trabajo conjunto (Colaboración)"]
```
...
En 1979, el cofundador de Apple Computer, [Steve Jobs](/es/p/biography-steve-jobs/), tuvo la oportunidad de visitar el Xerox PARC. Jobs quedó impactado por la GUI del Alto y la operatividad del ratón, y se convenció de que "esto es el futuro de las computadoras", integrando ese concepto por la fuerza en los proyectos de desarrollo de su propia empresa.
