...
```mermaid
graph TD
    A["Busca por dispositivos de apontamento (anos 1960)"] -- "Teste e avaliação" --> B["Caneta óptica"]
    A -- "Teste e avaliação" --> C["Joystick"]
    A -- "Teste e avaliação" --> D["Trackball"]
    A -- "Teste e avaliação" --> E["Controle de joelho"]
    A -- "Invenção do dispositivo ideal" --> F["Protótipo do Mouse"]
    B -. "Desvantagem: Braços cansados" .-> G["Rejeitado/Não adotado"]
    C -. "Desvantagem: Baixa precisão" .-> G
    D -. "Vantagem: Boa operabilidade, mas há espaço para melhorias" .-> H["Conhecimento para o desenvolvimento do mouse"]
    E -. "Vantagem: Surpreendentemente bem avaliado" .-> H
    F -- "Projetado e construído por Bill English" --> I["O primeiro mouse de madeira (1963-1964)"]
```
...
```mermaid
graph TD
    A["The Mother of All Demos (1968)"] -- "Principais tecnologias demonstradas" --> B["Operação de GUI via mouse"]
    A -- "Principais tecnologias demonstradas" --> C["Hipertexto e links"]
    A -- "Principais tecnologias demonstradas" --> D["Edição de documentos dinâmicos (Processador de texto)"]
    A -- "Principais tecnologias demonstradas" --> E["Processamento de contorno"]
    A -- "Principais tecnologias demonstradas" --> F["Divisão de tela (Múltiplas janelas)"]
    A -- "Principais tecnologias demonstradas" --> G["Sistema de videoconferência"]
    A -- "Principais tecnologias demonstradas" --> H["Trabalho em grupo (Colaboração)"]
```
