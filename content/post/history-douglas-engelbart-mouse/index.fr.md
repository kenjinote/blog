...
```mermaid
graph TD
    A["Recherche d'un dispositif de pointage (Années 1960)"] -- "Test et évaluation" --> B["Crayon optique"]
    A -- "Test et évaluation" --> C["Joystick"]
    A -- "Test et évaluation" --> D["Trackball"]
    A -- "Test et évaluation" --> E["Contrôle au genou"]
    A -- "Conception de l'appareil idéal" --> F["Prototype de la « souris »"]
    B -. "Défaut : Fatigue le bras" .-> G["Rejeté/Non adopté"]
    C -. "Défaut : Faible précision" .-> G
    D -. "Avantage : Bonne opérabilité mais perfectible" .-> H["Connaissances pour le développement de la souris"]
    E -. "Avantage : Étonnamment bien noté" .-> H
    F -- "Conception et fabrication par Bill English" --> I["Première souris en bois (1963-1964)"]
```
...
```mermaid
graph TD
    A["The Mother of All Demos (1968)"] -- "Principales technologies démontrées" --> B["Opération GUI par souris"]
    A -- "Principales technologies démontrées" --> C["Hypertexte et liens"]
    A -- "Principales technologies démontrées" --> D["Édition dynamique de documents (Traitement de texte)"]
    A -- "Principales technologies démontrées" --> E["Traitement de plans"]
    A -- "Principales technologies démontrées" --> F["Division d'écran (Multi-fenêtres)"]
    A -- "Principales technologies démontrées" --> G["Système de vidéoconférence"]
    A -- "Principales technologies démontrées" --> H["Travail collaboratif (Collaboration)"]
```
