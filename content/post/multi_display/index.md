...
最もスタンダードな構成です。横に並べる場合、ベゼルが中央に来るため、首を常に左右どちらかに傾ける必要があります。これを回避するためには、1台を正面（メイン）、もう1台を斜め（サブ）に配置するか、縦に積む（[スタック](https://kenji.blog/p/c-language-pointers-memory-management-stack-heap/)構成）のが推奨されます。
...
- **デメリット:** 机のスペースを大量に消費。[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)ィックボードの出力端子や帯域幅の制限を受けやすい。
...
## 4. 接続帯域幅とデイジーチェーン：Thunderbolt 4と[DP](https://kenji.blog/p/dynamic-programming-dp-introduction-knapsack-fibonacci/) MSTの限界
