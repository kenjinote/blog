...
```mermaid
graph TD
    A["Algèbre de Boole (0 et 1)"] -- "Application" --> B["Circuits électriques (relais)"]
    B -- "Opérations logiques (AND, OR, NOT)" --> C["Calculs complexes et circuits numériques"]
    C -- "Développement" --> D["Ordinateurs universels modernes"]
```
...
```mermaid
graph LR
    A["Source d'information"] -- "Message" --> B["Émetteur"]
    B -- "Signal" --> C["Canal"]
    N["Source de bruit"] -- "Bruit" --> C
    C -- "Signal reçu" --> D["Récepteur"]
    D -- "Message" --> E["Destination"]
```
