...
```mermaid
graph TD
    A["Aljabar Boolean (0 dan 1)"] -- "Penerapan" --> B["Sirkuit Listrik (Relai)"]
    B -- "Operasi Logika (AND, OR, NOT)" --> C["Komputasi Kompleks dan Sirkuit Digital"]
    C -- "Pengembangan" --> D["Komputer Tujuan Umum Modern"]
```
...
```mermaid
graph LR
    A["Sumber Informasi (Information Source)"] -- "Pesan" --> B["Pemancar (Transmitter)"]
    B -- "Sinyal" --> C["Saluran (Channel)"]
    N["Sumber Kebisingan (Noise Source)"] -- "Kebisingan" --> C
    C -- "Sinyal yang Diterima" --> D["Penerima (Receiver)"]
    D -- "Pesan" --> E["Tujuan (Destination)"]
```
