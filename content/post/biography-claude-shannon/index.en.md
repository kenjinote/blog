...
```mermaid
graph TD
    A["Boolean Algebra (0 and 1)"] -- "Application" --> B["Electrical Circuits (Relays)"]
    B -- "Logical Operations (AND, OR, NOT)" --> C["Complex Calculations / Digital Circuits"]
    C -- "Evolution" --> D["Modern General-Purpose Computers"]
```
...
```mermaid
graph LR
    A["Information Source"] -- "Message" --> B["Transmitter"]
    B -- "Signal" --> C["Channel"]
    N["Noise Source"] -- "Noise" --> C
