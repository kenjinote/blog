...
그래서 DNS는 전 세계에 있는 수십만 대의 서버가 협력하여 데이터를 분산 관리하는 '**계층형 분산 데이터베이스**'로 설계되었습니다. 이것은 컴퓨터 과학 역사상 가장 성공적이고 가장 대규모로 가동되고 있는 분산 시스템이라고 불립니다.

## 3. 도메인 이름의 계층 구조 (트리 구조)

DNS의 작동 원리를 이해하기 위해서는 도메인 이름의 '구조'를 알 필요가 있습니다.
사실 도메인 이름은 오른쪽에서 왼쪽을 향해 계층(트리 구조)으로 되어 있습니다.
...
그러나 1980년대에 폴 모카페트리스(Paul Mockapetris) 등에 의해 설계된 이 계층형 분산 시스템이 없었다면, 현재의 거대한 인터넷은 절대 성립될 수 없었을 것입니다.
