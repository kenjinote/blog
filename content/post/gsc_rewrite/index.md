...
技術ブログや開発者向けのオウンドメディアを運営していく上で、新規記事の継続的な執筆と同じくらい、あるいはそれ以上に重要なのが「過去記事のリライト」です。特にIT・技術系のトピックは情報の陳腐化が早く、数年前に書いたコードスニペットやAPIの仕様が現在では非推奨（Deprecated）になっているケースも珍しくありません。しかし、ただ闇雲に過去の記事を更新するだけでは、[検索エンジン](/p/how-search-engines-work/)からのトラフィック（流入）を最大化することはできません。
...
以下の[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)は、ある技術ブログにおける期待CTRと実際のCTRの乖離を示したイメージです。
...
（※ 折れ線が期待CTR、棒[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)が実際のCTRを示します。4位や8位で大きく下回っていることが確認できます。）
...
この分析により、「実は上位記事は『[Docker](https://kenji.blog/p/docker-container-namespace-cgroups-layers/)コンテナへのデプロイ方法』や『[CI/CD](/p/cicd-pipeline-github-actions-best-practices/)[パイプライン](https://kenji.blog/p/cicd-pipeline-github-actions-best-practices/)の構築』についても言及しているが、自分の記事では触れていない」といった **トピックの抜け漏れ（コンテンツギャップ）** を定量的に発見できます。
...
Google Search Consoleを活用した技術記事のリライトは、単なる文章の修正ではありません。それは、[検索エンジン](/p/how-search-engines-work/)のアルゴリズムというブラックボックスに対して、データと数理モデルを駆使して最適解を提示していく高度なエンジニアリングです。
