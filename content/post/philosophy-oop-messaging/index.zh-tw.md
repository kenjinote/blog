...
在現代軟體開發中，幾乎每天都會聽到「[物件導向程式設計](/zh-tw/p/object-oriented-programming-oop-solid-principles/)（[OOP](/zh-tw/p/object-oriented-programming-oop-solid-principles/): Object-Oriented Programming）」這個詞。Java、C#、Python、Ruby、C++ 等主流程式語言幾乎都引入了物件導向的典範，成為開發者必備的知識。
...
3. **網路上的透通性**
   訊息傳遞這個典範，無論是位在同一個記憶體空間（行程）內的物件之間，還是透過網路位於不同伺服器上的物件之間，都可以用相同的方式來處理。方法呼叫的大前提是必須在同一個記憶體空間內，但訊息傳遞則具有能自然擴展至[分散式系統](/zh-tw/p/cap-theorem-distributed-systems-tradeoff/)的特性。
...
此外，現代的**「[微服務架構](/zh-tw/p/microservices-architecture-bff-api-gateway/) Microservices Architecture」**，本質上也可以說是訊息傳遞導向的物件導向之巨大版。如果將每個微服務視為一個巨大的「物件」，它們會完全隱藏自己的資料庫（內部狀態），並透過 [REST API](/zh-tw/p/graphql-vs-rest-api-overfetching-type-safety/)、gRPC 或 Kafka 等方式進行「訊息」的交換，藉此建構出整個系統。
...
3. **意識到執行時期的靈活性與延遲綁定**。
4. **從行程內到[分散式系統](/zh-tw/p/cap-theorem-distributed-systems-tradeoff/)，以共通的隱喻來理解架構**。
