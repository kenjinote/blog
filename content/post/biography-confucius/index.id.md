---
title: "Kehidupan dan Filosofi Konfusius: Kebijaksanaan 'Ren' dan 'Li' yang Melampaui Zaman"
...
```mermaid
graph TD
    A["Konfusius (Confucius)"] --> B["Pemikiran Inti"]
    A --> C["Prestasi Utama"]
    
    B --> B1["Ren (Empati / Cinta Kemanusiaan)"]
    B --> B2["Li (Norma Sosial / Tatanan)"]
    B --> B3["Aturan Kebajikan (Pemerintahan oleh Moralitas)"]
    
    C --> C1["Pembukaan Sekolah Swasta (Pendidikan tanpa memandang status)"]
    C --> C2["Kompilasi Klasik (Puisi, Buku, Ritus, Musik)"]
    C --> C3["Analek (Catatan ucapan dan tindakan oleh murid-murid)"]
    
    B1 --> D["Aplikasi Modern"]
    B2 --> D
    B3 --> D
    C3 --> E["Pembentukan Konfusianisme"]
    E --> F["Pengaruh Besar pada Lingkup Budaya Asia Timur"]
    
    D --> G["Manajemen Tujuan / Teori Kepemimpinan"]
```
