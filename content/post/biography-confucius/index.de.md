...
```mermaid
graph TD
    A["Konfuzius (Confucius)"] --> B["Kerngedanken"]
    A --> C["Wichtigste Errungenschaften"]
    
    B --> B1["Ren (Mitgefühl / Menschenliebe)"]
    B --> B2["Li (Soziale Normen / Ordnung)"]
    B --> B3["Herrschaft durch Tugend (Regieren durch Moral)"]
    
    C --> C1["Eröffnung von Privatschulen (Bildung unabhängig vom Status)"]
    C --> C2["Zusammenstellung von Klassikern (Poesie, Geschichte, Riten, Musik)"]
    C --> C3["Analekten (Aufzeichnungen der Schüler)"]
    
    B1 --> D["Moderne Anwendung"]
    B2 --> D
    B3 --> D
    C3 --> E["Entstehung des Konfuzianismus"]
    E --> F["Enormer Einfluss auf den ostasiatischen Kulturraum"]
    
    D --> G["Purpose-Driven Management / Führungstheorie"]
```
