---
title: "A Vida e a Filosofia de Confúcio: A Sabedoria Atemporal de 'Ren' e 'Li'"
...
```mermaid
graph TD
    A["Confúcio (Confucius)"] --> B["Pensamento Central"]
    A --> C["Principais Conquistas"]
    
    B --> B1["Ren (Empatia e Amor Humano)"]
    B --> B2["Li (Normas Sociais e Ordem)"]
    B --> B3["Governança baseada na Virtude (Governo pela Moralidade)"]
    
    C --> C1["Estabelecimento de Escola Particular (Educação Sem Distinção de Classe)"]
    C --> C2["Compilação de Clássicos (Poesia, História, Ritos, Música)"]
    C --> C3["Analectos (Registros de Palavras e Ações Pelos Discípulos)"]
    
    B1 --> D["Aplicação na Era Moderna"]
    B2 --> D
    B3 --> D
    C3 --> E["Estabelecimento do Confucionismo"]
    E --> F["Enorme Impacto na Esfera Cultural da Ásia Oriental"]
    
    D --> G["Gestão Orientada por Propósito / Teoria da Liderança"]
```
