...
この「 **限りなく近づく** 」という表現は、私たちの直感に非常にマッチしており、多項式関数や三角関数など、多くの連続関数を扱う上では全く問題なく機能します。[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)を描けば、 $x$ が特定の点に向かって進むとき、 $y$ の値がどこにたどり着くかは一目瞭然だからです。
...
18世紀を通じて微積分は発展を続けましたが、19世紀に入ると、直感だけでは処理できない「病的な関数（pathological functions）」が次々と発見され、数学者たちは危機感を募らせました。[オーギュスタン＝ルイ・コーシー](https://kenji.blog/p/cauchy/)と[カール・ワイエルシュトラス](https://kenji.blog/p/weierstrass/)は、この危機を乗り越えるため、無限小という怪しい概念を追放し、実数の性質と不等式のみを用いて微積分を再構築しました。これが $\varepsilon-\delta$ 論法の誕生です。
...
ここまで読んで、「計算が面倒になっただけではないか？」と思うかもしれません。しかし、 $\varepsilon-\delta$ 論法が真の力を発揮するのは、[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)を描くことが不可能な「病的な関数」を扱うときです。
