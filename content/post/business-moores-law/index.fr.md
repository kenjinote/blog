...
```mermaid
graph TD
    A["Article de Gordon Moore en 1965"] -- "Prédiction" --> B["Doublement du nombre de transistors"]
    B -- "Tous les 18 à 24 mois" --> C["Croissance exponentielle de la puissance de calcul"]
    C -- "Ce qui en découle" --> D["Révolution des PC et Internet"]
    C -- "Ce qui en découle" --> E["Smartphones et cloud"]
    C -- "Ce qui en découle" --> F["IA générative et Deep Learning"]
```
...
```mermaid
graph LR
    A["Arrivée des limites physiques et économiques"] -- "Solutions" --> B["Innovation de l'architecture et du packaging"]
    B -- "Transistors GAA" --> C["Suppression du courant de fuite et poursuite de la miniaturisation"]
    B -- "Technologie des Chiplets" --> D["Amélioration du rendement et optimisation des coûts"]
    B -- "Empilement 3D (TSV/HBM)" --> E["Amélioration spectaculaire de la bande passante mémoire et économie d'énergie"]
    C -- "L'informatique de nouvelle génération" --> F["Évolution dans les domaines IA / HPC"]
    D -- "L'informatique de nouvelle génération" --> F
    E -- "L'informatique de nouvelle génération" --> F
```
