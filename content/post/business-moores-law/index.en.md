...
```mermaid
graph TD
    A["1965 Gordon Moore's Paper"] -- "Prediction" --> B["Doubling of the number of transistors"]
    B -- "Every 18 to 24 months" --> C["Exponential growth of computing power"]
    C -- "Brings about" --> D["PC revolution and the Internet"]
    C -- "Brings about" --> E["Smartphones and the Cloud"]
    C -- "Brings about" --> F["Generative AI and Deep Learning"]
```
...
```mermaid
graph LR
    A["Arrival of physical and economic limits"] -- "Solutions" --> B["Innovation in architecture and packaging"]
    B -- "GAA transistors" --> C["Suppression of leakage current and continuation of miniaturization"]
    B -- "Chiplet technology" --> D["Improvement of yield and cost optimization"]
    B -- "3D stacking (TSV/HBM)" --> E["Dramatic improvement in memory bandwidth and power saving"]
    C -- "Next-generation computing" --> F["Evolution of AI / HPC fields"]
    D -- "Next-generation computing" --> F
    E -- "Next-generation computing" --> F
```
