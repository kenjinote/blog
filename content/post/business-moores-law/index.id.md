...
```mermaid
graph TD
    A["Makalah Gordon Moore 1965"] -- "Prediksi" --> B["Penggandaan jumlah transistor"]
    B -- "Setiap 18-24 bulan" --> C["Pertumbuhan eksponensial kapasitas komputasi"]
    C -- "Membawa" --> D["Revolusi PC dan Internet"]
    C -- "Membawa" --> E["Ponsel pintar dan Cloud"]
    C -- "Membawa" --> F["AI Generatif dan Deep Learning"]
```
...
```mermaid
graph LR
    A["Kedatangan batas fisik dan ekonomi"] -- "Solusi" --> B["Inovasi arsitektur dan pengemasan"]
    B -- "Transistor GAA" --> C["Penekanan arus bocor dan kelanjutan miniaturisasi"]
    B -- "Teknologi Chiplet" --> D["Peningkatan yield rate dan optimalisasi biaya"]
    B -- "Penumpukan 3D (TSV/HBM)" --> E["Peningkatan bandwidth memori dan penghematan daya"]
    C -- "Komputasi generasi berikutnya" --> F["Evolusi bidang AI / HPC"]
    D -- "Komputasi generasi berikutnya" --> F
    E -- "Komputasi generasi berikutnya" --> F
```
