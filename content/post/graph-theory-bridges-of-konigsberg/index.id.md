...
Teka-teki permainan kota yang tampaknya biasa ini diberikan pencerahan matematika yang sama sekali baru oleh matematikawan jenius yang langka, **[Leonhard Euler](/id/p/euler/)**. Pemikirannya tidak hanya memberikan jawaban atas teka-teki tersebut, tetapi juga mendirikan bidang matematika besar yang kemudian dikenal sebagai "[Teori Graf](/id/p/graph-theory-dijkstra-a-star/)" dan "Topologi".

Artikel ini akan menelusuri lintasan epik yang dimulai dari perumusan matematika penemuan bersejarah Euler ini, menuju teori jaringan modern dan [algoritma pencarian](/id/p/search-algorithms-linear-binary-hash-table-principles/) rute navigasi mobil (Algoritma Dijkstra, [Algoritma Pencarian](/id/p/search-algorithms-linear-binary-hash-table-principles/) A*) yang kita gunakan sehari-hari.
...
Menggunakan bahasa [teori graf](/id/p/graph-theory-dijkstra-a-star/), pertanyaan penduduk dapat diutarakan ulang sebagai berikut.
...
Sejak penemuan Euler, [teori graf](/id/p/graph-theory-dijkstra-a-star/) telah berkembang sebagai cabang penting matematika. Banyak masalah sulit telah dibahas di panggung [teori graf](/id/p/graph-theory-dijkstra-a-star/), seperti masalah pewarnaan peta ([Teorema Empat Warna](/id/p/four-color-theorem/)) dan masalah sirkuit Hamilton (rute yang melewati semua simpul tepat satu kali).

Namun, dengan munculnya komputer pada paruh kedua abad ke-20, [teori graf](/id/p/graph-theory-dijkstra-a-star/) melampaui kerangka matematika sederhana dan berevolusi menjadi senjata yang ampuh (algoritma) untuk memecahkan masalah dunia nyata. Banyak infrastruktur masyarakat modern bergantung pada [teori graf](/id/p/graph-theory-dijkstra-a-star/) sebagai fondasinya, seperti perutean jaringan komunikasi, analisis hubungan pertemanan SNS, dan optimalisasi jaringan listrik.
...
Diciptakan oleh [Edsger Dijkstra](/id/p/biography-edsger-dijkstra/) pada tahun 1956, algoritma ini adalah algoritma untuk menemukan jarak terpendek dari titik awal tertentu ke semua simpul dalam graf di mana sisi memiliki bobot (biaya jarak atau waktu).
...
[Algoritma pencarian](/id/p/search-algorithms-linear-binary-hash-table-principles/) A* (A-Star) dirancang untuk mengurangi pencarian sia-sia dari algoritma Dijkstra dan mengarah ke tujuan dengan lebih efisien. Ini dikembangkan di bidang kecerdasan buatan dan secara luas diterapkan pada pergerakan karakter game dan sistem navigasi mobil.
...
Dalam ilmu data dan implementasi algoritma modern, perpustakaan standar untuk menangani [teori graf](/id/p/graph-theory-dijkstra-a-star/) adalah **NetworkX** dari Python.
...
Dengan menjalankan kode ini, Anda dapat mengonfirmasi bahwa algoritma Dijkstra dan [algoritma pencarian](/id/p/search-algorithms-linear-binary-hash-table-principles/) A* menemukan lintasan terpendek yang sama. Dalam jaringan skala besar yang sebenarnya, ada perbedaan yang luar biasa dalam jumlah simpul yang dieksplorasi.

---

## 7. Epilog: Koneksi Membentuk Dunia

Teka-teki kecil yang dinikmati penduduk Königsberg, melalui mata seorang jenius bernama [Leonhard Euler](/id/p/euler/), berubah menjadi lensa baru untuk memahami kembali dunia sebagai "hubungan titik dan garis".
