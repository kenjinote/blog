+++
title = "الدليل الشامل لخوارزميات الفرز: من الفرز الفقاعي إلى Timsort"
description = "شرح كامل لخوارزميات الفرز في البرمجة. يغطي كل شيء من الأساسيات إلى الخوارزميات المتقدمة."
...
# الدليل الشامل لخوارزميات الفرز: من الفرز الفقاعي إلى Timsort

الخوارزميات وهياكل البيانات هي موضوعات غاية في الأهمية وتشكل جوهر علوم الحاسب. من بينها، يعتبر "الفرز" (Sorting) عملية أساسية لا غنى عنها في كل موقف تقريبًا، مثل البحث، وتجميع البيانات، والتصور. في هذا المقال، سنشرح بالتفصيل الممل كيفية العمل، والتنفيذ، والتعقيد الحسابي، ومتى يجب استخدام كل منها، بدءًا من خوارزميات الفرز الأساسية التي يتعلمها المبتدئون، ووصولًا إلى خوارزميات الفرز المتقدمة مثل تلك المستخدمة في المكتبات القياسية للغات البرمجة الحديثة.

## 1. المعرفة الأساسية بخوارزميات الفرز

قبل تعلم خوارزميات الفرز، من الضروري فهم بعض المفاهيم الهامة التي تمثل معايير لتقييم الخوارزميات. فهم هذه المفاهيم سيوضح لك سبب وجود هذا العدد الكبير من خوارزميات الفرز، وأيها يجب أن تختاره بناءً على الموقف.

### 1.1 الاستقرار (Stability)

الاستقرار في خوارزميات الفرز يشير إلى ما إذا كان الترتيب النسبي للعناصر التي تمتلك نفس قيمة مفتاح الفرز محفوظًا قبل الفرز وبعده أم لا.

على سبيل المثال، لنفترض أن لدينا قائمة بيانات تحتوي على درجات اختبار الطلاب وأسمائهم.
`[ (80 نقطة, "الطالب أ"), (70 نقطة, "الطالب ب"), (80 نقطة, "الطالب ج") ]`
عند فرز هذه القائمة تصاعديًا بناءً على الدرجات، إذا كانت خوارزمية الفرز مستقرة، فستكون النتيجة دائمًا كالتالي:
`[ (70 نقطة, "الطالب ب"), (80 نقطة, "الطالب أ"), (80 نقطة, "الطالب ج") ]`
بما أن "الطالب أ" كان قبل "الطالب ج" في الأصل، فسيتم وضع "الطالب أ" أولاً حتى لو كان كلاهما قد حصل على 80 نقطة. هذا هو ** الفرز المستقر ** (Stable Sort).

من ناحية أخرى، في خوارزميات الفرز غير المستقرة، قد ينعكس هذا الترتيب ليصبح `(80 نقطة, "الطالب ج"), (80 نقطة, "الطالب أ")`. يصبح الاستقرار مهمًا للغاية عندما تقوم بالفرز عدة مرات باستخدام مفاتيح متعددة (على سبيل المثال: الفرز أولاً بالاسم، ثم الفرز بالدرجة).

### 1.2 In-place و Out-of-place (في المكان وخارج المكان)

هذا تصنيف يوضح مقدار الذاكرة الإضافية التي تتطلبها الخوارزمية عند تنفيذها.

*   ** In-place (في المكان) **: خوارزمية تقوم بالكتابة فوق مصفوفة الإدخال مباشرة عند إجراء الفرز، ولا تتطلب سوى ذاكرة إضافية بحجم ثابت ($O(1)$) أو بحجم لوغاريتمي ($O(\log n)$) تُستخدم لتبديل العناصر وما إلى ذلك. إنها مفيدة جدًا في البيئات ذات قيود الذاكرة الصارمة.
*   ** Out-of-place (خارج المكان) **: خوارزمية تتطلب مساحة ذاكرة إضافية متناسبة مع حجم الإدخال (على سبيل المثال $O(n)$) بشكل منفصل عن مصفوفة الإدخال من أجل الفرز.

### 1.3 التعقيد الحسابي وتدوين Big O (Big O Notation)

للتعبير عن كفاءة الخوارزمية، نستخدم التدوين المقارب ** Big O Notation ** (تدوين أو الكبرى).
في خوارزميات الفرز، نُقيِّم بشكل أساسي ** التعقيد الزمني ** (وقت التنفيذ) و ** التعقيد المكاني ** (استخدام الذاكرة).

*   $O(1)$: وقت ثابت. لا يعتمد على كمية البيانات ويكون ثابتًا دائمًا.
*   $O(n)$: وقت خطي. يزداد بشكل يتناسب مع كمية البيانات.
*   $O(n \log n)$: وقت خطي لوغاريتمي. السرعة القصوى النظرية في خوارزميات الفرز المعتمدة على المقارنة.
*   $O(n^2)$: وقت تربيعي. إذا تضاعفت كمية البيانات، سيتضاعف الوقت أربع مرات، وهو غير مناسب للبيانات واسعة النطاق.

لقد تم إثبات رياضيًا أن الحد الأدنى لأسوأ حالة للتعقيد في خوارزميات الفرز المستندة إلى المقارنة (وهي طرق فرز عن طريق مقارنة أحجام العناصر) هو $O(n \log n)$.

$$
\text{الحد الأدنى لفرز المقارنة} = \Omega(n \log n)
$$
...
*   ** متوسط التعقيد الزمني **: $O(n \log n)$
    *   عمليًا، الثابت المعامل صغير جدًا، وكفاءة ذاكرة التخزين المؤقت (Cache) عالية للغاية، مما يجعله يعمل أسرع من فرز الدمج وفرز الكومة (Heap Sort).
*   ** التعقيد المكاني **: في المتوسط $O(\log n)$، في أسوأ الحالات $O(n)$
    *   إنها خوارزمية In-place تقوم بالكتابة فوق المصفوفة نفسها بشكل مباشر، لكنها تستهلك مكدس الاستدعاءات (Call Stack) للاستدعاءات المتكررة.
*   ** الاستقرار **: غير مستقر (Unstable)
...
Timsort هي خوارزمية تم تنفيذها لـ Python بواسطة Tim Peters في عام 2002، وهي الآن تهيمن على العالم العملي لاعتمادها في العديد من اللغات مثل `list.sort()` و `sorted()` في Python، ومصفوفات الكائنات في Java، والفرز القياسي في Rust.
...
| ** فرز الكومة (Heap Sort) ** | $O(n \log n)$ | $O(n \log n)$ | $O(n \log n)$ | $O(1)$ | لا (No) | الكومة الثنائية. In-place وقوي. |
...
1. ** فهم المفاهيم الأساسية **: المفاهيم مثل التعقيد الحسابي (Big O Notation) و In-place/Out-of-place و الاستقرار، تشكل أساسًا لتصميم جميع الخوارزميات وتصميم هياكل البيانات، وليس فقط الفرز.
