...
def write_article():
    file_path = r"c:\work\kenji.blog\content\post\sorting-algorithms\index.md"
    
    with open(file_path, "w", encoding="utf-8") as f:
        # Chunk 1: Front matter and Intro
        f.write("""+++
title = "ソートアルゴリズム完全網羅ガイド：バブルソートからTimsortまで"
description = "プログラミングにおけるソートアルゴリズムの完全な解説。基本から高度なものまで網羅。"
...
# ソートアルゴリズム完全網羅ガイド：バブルソートからTimsortまで

アルゴリズムとデータ構造は、コンピュータサイエンスの根幹を成す非常に重要なテーマです。その中でも「ソート（並べ替え）」は、検索、データのグループ化、可視化など、あらゆる場面で必須となる基礎的な操作です。本記事では、初学者が学ぶ基本的なソートアルゴリズムから、現代のプログラミング言語の標準ライブラリで採用されているような高度なソートアルゴリズムまで、その仕組み、実装、計算量、そしてどのような場面で使うべきかを、非常に詳細に解説していきます。

## 1. ソートアルゴリズムの基礎知識

ソートアルゴリズムを学ぶ前に、アルゴリズムの評価基準となるいくつかの重要な概念を理解しておく必要があります。これらを理解することで、なぜこれほど多くのソートアルゴリズムが存在するのか、そして状況に応じてどれを選ぶべきなのかが明確になります。

### 1.1 安定性 (Stability)

ソートアルゴリズムにおける **安定性** （Stability）とは、ソートキーが同じ値を持つ要素同士の相対的な順序が、ソート前後で保存されるかどうかを示す性質です。

例えば、生徒のテストの点数と名前を持つデータのリストがあるとします。
`[ (80点, "Aさん"), (70点, "Bさん"), (80点, "Cさん") ]`
これを点数の昇順でソートしたとき、安定なソートアルゴリズムであれば、必ず以下のようになります。
`[ (70点, "Bさん"), (80点, "Aさん"), (80点, "Cさん") ]`
元々 "Aさん" は "Cさん" より前にいたため、同じ80点でも "Aさん" が前に配置されます。これが **安定なソート** (Stable Sort) です。

一方、不安定なソートアルゴリズムでは、この順序が逆転して `(80点, "Cさん"), (80点, "Aさん")` となる可能性があります。安定性は、複数のキーで複数回ソートを行う場合（例：最初に名前でソートし、次に点数でソートする）に非常に重要になります。

### 1.2 In-place と Out-of-place

アルゴリズムが実行時にどれだけの追加メモリを必要とするかを示す分類です。

*   **In-place (インプレース) **: ソートを行う際に入力配列自体を直接書き換え、要素の交換などに使われる定数サイズ ($O(1)$) または対数サイズ ($O(\\log n)$) の追加メモリしか必要としないアルゴリズムです。メモリ制限が厳しい環境で重宝します。
*   **Out-of-place (アウトオブプレース) **: ソートのために入力配列とは別に、入力サイズに比例した（例えば $O(n)$ の）追加メモリ領域を必要とするアルゴリズムです。

### 1.3 計算量と Big O 記法

アルゴリズムの効率性を表すために、漸近記法である **Big O Notation** （ビッグオー記法）を用います。
ソートアルゴリズムでは主に **時間計算量** （実行時間）と **空間計算量** （メモリ使用量）を評価します。

*   $O(1)$: 定数時間。データ量に依存せず常に一定。
*   $O(n)$: 線形時間。データ量に比例して増大。
*   $O(n \\log n)$: 線形対数時間。比較に基づくソートアルゴリズムにおける理論上の最高速度。
*   $O(n^2)$: 二乗時間。データ量が2倍になると時間は4倍になり、大規模なデータには不向き。

比較ベースのソートアルゴリズム（要素同士の大小を比較して並べ替える手法）では、最悪計算量の下限が $O(n \\log n)$ になることが数学的に証明されています。

$$
\\text{比較ソートの下限} = \\Omega(n \\log n)
$$
...
""")
        ...
        # Chunk 3: Linear time sorts and Advanced sorts
        f.write("""
...
Timsort（ティムソート）は、Tim Peters氏によって2002年にPython向けに実装されたアルゴリズムで、現在ではPythonの `list.sort()` や `sorted()` はもちろん、Javaのオブジェクト配列やRustの標準ソートなど、数多くの言語で採用されている実用界の覇者です。
...
if __name__ == "__main__":
    write_article()
    print("Article completely written.")
