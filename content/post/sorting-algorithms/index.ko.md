+++
title = "정렬 알고리즘 완전 정복 가이드: 버블 정렬부터 Timsort까지"
description = "프로그래밍에서의 정렬 알고리즘에 대한 완벽한 해설. 기초부터 고급까지 망라."
...
# 정렬 알고리즘 완전 정복 가이드: 버블 정렬부터 Timsort까지

알고리즘과 자료 구조는 컴퓨터 과학의 근간을 이루는 매우 중요한 주제입니다. 그중에서도 '정렬'은 검색, 데이터 그룹화, 시각화 등 모든 상황에서 필수가 되는 기초적인 작업입니다. 본 기사에서는 초학자가 배우는 기본적인 정렬 알고리즘부터 현대 프로그래밍 언어의 표준 라이브러리에서 채택하고 있는 고급 정렬 알고리즘까지 그 원리, 구현, 시간 복잡도, 그리고 어떤 상황에서 사용해야 하는지를 매우 상세히 해설합니다.

## 1. 정렬 알고리즘의 기초 지식

정렬 알고리즘을 배우기 전에 알고리즘의 평가 기준이 되는 몇 가지 중요한 개념을 이해해 둘 필요가 있습니다. 이를 이해함으로써 왜 이렇게 많은 정렬 알고리즘이 존재하는지, 그리고 상황에 따라 어떤 것을 선택해야 하는지가 명확해집니다.

### 1.1 안정성 (Stability)

정렬 알고리즘에서의 **안정성** (Stability)이란, 정렬 키가 같은 값을 가지는 요소들 간의 상대적인 순서가 정렬 전후로 보존되는지를 나타내는 성질입니다.

예를 들어, 학생의 시험 점수와 이름을 가진 데이터의 리스트가 있다고 가정해 봅시다.
`[ (80점, "A학생"), (70점, "B학생"), (80점, "C학생") ]`
이를 점수의 오름차순으로 정렬했을 때, 안정적인 정렬 알고리즘이라면 반드시 다음과 같이 됩니다.
`[ (70점, "B학생"), (80점, "A학생"), (80점, "C학생") ]`
원래 "A학생"은 "C학생"보다 앞에 있었기 때문에 같은 80점이라도 "A학생"이 앞에 배치됩니다. 이것이 **안정 정렬** (Stable Sort) 입니다.

반면 불안정한 정렬 알고리즘에서는 이 순서가 역전되어 `(80점, "C학생"), (80점, "A학생")` 이 될 가능성이 있습니다. 안정성은 여러 키로 여러 번 정렬을 수행할 경우(예: 처음에는 이름으로 정렬하고, 다음에는 점수로 정렬) 매우 중요해집니다.

### 1.2 In-place 와 Out-of-place

알고리즘 실행 시 추가 메모리가 얼마나 필요한지를 나타내는 분류입니다.

*   **In-place (제자리 정렬)** : 정렬을 수행할 때 입력 배열 자체를 직접 수정하며, 요소 교환 등에 사용되는 상수 크기 ($O(1)$) 또는 로그 크기 ($O(\log n)$)의 추가 메모리만을 필요로 하는 알고리즘입니다. 메모리 제한이 엄격한 환경에서 유용합니다.
*   **Out-of-place (비제자리 정렬)** : 정렬을 위해 입력 배열과 별도로 입력 크기에 비례하는(예를 들어 $O(n)$ 의) 추가 메모리 공간을 필요로 하는 알고리즘입니다.

### 1.3 복잡도와 Big O 표기법

알고리즘의 효율성을 나타내기 위해 점근 표기법인 **Big O Notation** (빅오 표기법)을 사용합니다.
정렬 알고리즘에서는 주로 **시간 복잡도** (실행 시간)와 **공간 복잡도** (메모리 사용량)를 평가합니다.

*   $O(1)$: 상수 시간. 데이터양에 의존하지 않고 항상 일정.
*   $O(n)$: 선형 시간. 데이터양에 비례하여 증가.
*   $O(n \log n)$: 선형 로그 시간. 비교 기반 정렬 알고리즘의 이론상 최고 속도.
*   $O(n^2)$: 이차 시간. 데이터양이 2배가 되면 시간은 4배가 되어 대규모 데이터에는 부적합.

비교 기반 정렬 알고리즘(요소 간의 크기를 비교하여 정렬하는 기법)에서는 최악의 경우 시간 복잡도의 하한이 $O(n \log n)$ 이 되는 것이 수학적으로 증명되어 있습니다.

$$
\text{비교 정렬의 하한} = \Omega(n \log n)
$$
...
Timsort(팀소트)는 Tim Peters가 2002년에 Python용으로 구현한 알고리즘으로, 현재는 Python의 `list.sort()` 나 `sorted()` 는 물론, Java의 객체 배열이나 Rust의 표준 정렬 등 수많은 언어에서 채택된 실무계의 패자입니다.
