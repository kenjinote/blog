...
Timsort 是由 Tim Peters 在 2002 年为 Python 实现的算法。现在不仅是 Python 的 `list.sort()` 和 `sorted()`，在 Java 的对象数组、[Rust](https://kenji.blog/zh-cn/p/webassembly-wasm-current-future/) 的标准排序等众多语言中也都被采用，是实战领域的霸主。
