+++
title = "Полное руководство по алгоритмам сортировки: от пузырьковой сортировки до Timsort"
description = "Полное руководство по алгоритмам сортировки в программировании. Охватывает всё от основ до продвинутых тем."
...
# Полное руководство по алгоритмам сортировки: от пузырьковой сортировки до Timsort

Алгоритмы и структуры данных — это важнейшие темы, лежащие в основе информатики. Среди них "сортировка" (упорядочивание) — базовая операция, необходимая во многих ситуациях, таких как поиск, группировка данных и визуализация. В этой статье мы подробно рассмотрим алгоритмы сортировки, начиная с базовых, которые изучают новички, и заканчивая продвинутыми, которые используются в стандартных библиотеках современных языков программирования. Мы обсудим их механизм работы, реализацию, временную сложность и ситуации, в которых их следует применять.

## 1. Базовые знания об алгоритмах сортировки

Прежде чем изучать алгоритмы сортировки, необходимо понять несколько важных концепций, которые служат критериями для их оценки. Понимание этих критериев прояснит, почему существует так много алгоритмов сортировки и какой из них следует выбрать в зависимости от ситуации.

### 1.1 Стабильность (Stability)

 **Стабильность**  (Stability) в алгоритмах сортировки означает свойство сохранять относительный порядок элементов с одинаковыми значениями ключа сортировки до и после сортировки.

Например, предположим, у нас есть список данных с оценками студентов за тест и их именами.
`[ (80 баллов, "Студент А"), (70 баллов, "Студент Б"), (80 баллов, "Студент В") ]`
Если мы отсортируем этот список по возрастанию баллов, стабильный алгоритм сортировки всегда выдаст следующий результат:
`[ (70 баллов, "Студент Б"), (80 баллов, "Студент А"), (80 баллов, "Студент В") ]`
Поскольку "Студент А" изначально находился перед "Студент В", он будет расположен перед ним и после сортировки, даже при одинаковых 80 баллах. Это называется  **стабильная сортировка**  (Stable Sort).

С другой стороны, в нестабильном алгоритме сортировки этот порядок может перевернуться, и мы получим `(80 баллов, "Студент В"), (80 баллов, "Студент А")`. Стабильность становится очень важной, когда выполняется несколько сортировок по нескольким ключам (например, сначала сортировка по имени, а затем по оценке).

### 1.2 In-place и Out-of-place

Это классификация, показывающая, сколько дополнительной памяти требуется алгоритму во время выполнения.

*    **In-place (на месте)** : Алгоритм, который при сортировке перезаписывает непосредственно сам входной массив и требует только константного ($O(1)$) или логарифмического ($O(\log n)$) размера дополнительной памяти, используемой для обмена элементов. Он очень полезен в средах с жесткими ограничениями по памяти.
*    **Out-of-place (не на месте)** : Алгоритм, который требует дополнительного объема памяти, пропорционального размеру входных данных (например, $O(n)$), отдельно от входного массива для выполнения сортировки.

### 1.3 Вычислительная сложность и нотация Big O

Для выражения эффективности алгоритмов используется асимптотическая нотация, известная как  **Big O Notation**  (О-большое).
В алгоритмах сортировки в основном оцениваются  **временная сложность**  (время выполнения) и  **пространственная сложность**  (использование памяти).

*   $O(1)$: Константное время. Всегда постоянно и не зависит от объема данных.
*   $O(n)$: Линейное время. Увеличивается пропорционально объему данных.
*   $O(n \log n)$: Линейно-логарифмическое время. Теоретическая максимальная скорость для алгоритмов сортировки, основанных на сравнении.
*   $O(n^2)$: Квадратичное время. При удвоении объема данных время увеличивается в 4 раза, что делает алгоритм непригодным для больших объемов данных.

Математически доказано, что нижняя граница наихудшей вычислительной сложности для алгоритмов сортировки на основе сравнения (методов, сортирующих элементы путем сравнения их значений друг с другом) составляет $O(n \log n)$.

$$
\text{Нижняя граница сортировки сравнением} = \Omega(n \log n)
$$
...
Timsort был реализован Тимом Питерсом (Tim Peters) в 2002 году для Python и сейчас является доминирующим алгоритмом в практическом мире, используемым в функциях `list.sort()` и `sorted()` в Python, а также для сортировки массивов объектов в Java, стандартной сортировки в Rust и многих других языках.
...
1.  **Понимание базовых концепций** : Понятия вычислительной сложности (Big O Notation), In-place/Out-of-place и стабильности лежат в основе проектирования любых алгоритмов и структур данных, а не только сортировки.
