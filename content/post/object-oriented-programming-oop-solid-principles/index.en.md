...
In recent years, the paradigm of [Functional Programming](https://kenji.blog/en/p/functional-programming-concepts-pure-functions-monads/) (FP) has gained prominence, and the benefits of Immutability and Pure Functions are being reconsidered. However, OOP and FP are not in conflict. Modern languages (Scala, Kotlin, Rust, and recent C# and [Java](https://kenji.blog/en/p/programming-languages-history-paradigm-evolution/)) fuse both paradigms, and hybrid designs like "encapsulating state management in OOP classes and executing data transformation pipelines with FP approaches" are becoming mainstream.
