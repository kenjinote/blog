...
この記事では、Gitの内部構造であるDAG（有向非巡回[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)）やコミットハッシュの数学的性質から紐解き、`git merge` と `git rebase` のメカニズムの違いを深く掘り下げます。そして、実務においてどのようにこれらを使い分けるべきか、具体的なワークフローを交えて徹底的に解説します。単なるコマンドの紹介にとどまらず、裏側でGitがどのような計算を行っているのかまで理解することで、コンフリクトへの恐怖をなくし、クリーンで追跡可能な履歴を構築できるようになります。
...
Gitのコミット履歴は、[グラフ](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)理論における「有向非巡回グラフ（Directed Acyclic [Graph](https://kenji.blog/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/), DAG）」としてモデル化されます。
...
## 3.2 トポロジカル[ソート](https://kenji.blog/p/sorting-algorithms/)と履歴の順序

Gitの `git log` コマンドなどで履歴を表示する際、DAGはトポロジカル[ソート](https://kenji.blog/p/sorting-algorithms/)（Topological Sort）アルゴリズムによって1次元のリストとして順序付けされます。DAGにおける任意の有向エッジ $u \to v$ （$u$ は $v$ の子）について、リスト内で $u$ が $v$ よりも前に来るように並べ替えます。
