...
1. **Указатель на объект [Tree](https://kenji.blog/ru/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/)**: снимок структуры каталогов и файлов (Blob) на тот момент времени.
...
Размер хеш-пространства составляет $2^{160}$, и вероятность коллизии (когда разные коммиты имеют одинаковое хеш-значение) $P$ можно приблизительно вычислить, используя [парадокс дней рождения](/ru/p/birthday-paradox/) (Birthday Paradox), следующим образом (где $n$ — количество коммитов):
...
История коммитов Git моделируется как «направленный ациклический граф» (Directed Acyclic [Graph](https://kenji.blog/ru/p/tree-graph-data-structures-search-dfs-bfs-dijkstra/), DAG) в теории графов.
