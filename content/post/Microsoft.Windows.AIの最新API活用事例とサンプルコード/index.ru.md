...
Кроме того, вывод моделей ИИ, особенно [LLM](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) (фаза декодирования), часто **ограничен памятью (Memory-Bound)**. Теоретическая пропускная способность системной памяти $BW$ рассчитывается следующим образом:
...
Используя набор инструментов `Olive (ONNX Live)`, предоставляемый Microsoft, вы можете автоматически оптимизировать модели, такие как PyTorch, для сред Windows. Olive обеспечивает мощную поддержку специализированной оптимизации внимания для моделей [Transformer](https://kenji.blog/ru/p/large-language-models-llm-transformer-prompt-engineering/) и компиляции графов для конкретного оборудования.
