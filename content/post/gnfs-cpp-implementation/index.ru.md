---
title: 'Реализация Общего метода решета числового поля (GNFS) на C++ | Механизм алгоритма взлома RSA-шифрования'
...
description: 'Подробное объяснение механизма алгоритма факторизации «Общий метод решета числового поля (GNFS)», используемого для взлома RSA-шифрования, с кодом реализации на C++. Обязательно к прочтению для любителей программирования, желающих погрузиться в логику вычислений с использованием библиотеки Boost и глубины алгебраической теории чисел.'
---

# "Полная анатомия" понимания сильнейшего алгоритма криптоанализа "GNFS" путем его реализации на C++

«Шифрование [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/)» фундаментально поддерживает современный Интернет. Его надежность опирается на математическое убеждение, что «факторизация огромного составного числа практически невозможна для современных компьютеров».
...
Если мы воспользуемся алгоритмом [Евклид](https://kenji.blog/ru/p/euclid/)а для вычисления ** $\gcd(X-Y, N)$ **, то легко найдем простой множитель $N$.
...
Остается только вычислить $\gcd(X-Y, N)$. За долю секунды 0,001 с нетривиальный множитель выводится на экран, и гордая, неприступная криптография [RSA](https://kenji.blog/ru/p/modern-cryptography-public-key-hash-signature/) полностью рушится.
