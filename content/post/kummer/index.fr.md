---
title: "Ernst Kummer : Père des nombres idéaux et le défi du dernier théorème de Fermat"
description: "Une plongée approfondie dans la vie du grand mathématicien allemand Ernst Kummer, son introduction des nombres idéaux et son défi légendaire au dernier théorème de Fermat."
slug: "kummer"
...
Dans l'histoire des mathématiques, il n'est pas rare que la confrontation à un problème ouvert spécifique ouvre des domaines de recherche entièrement nouveaux. Ernst Eduard Kummer ( **Ernst Eduard Kummer** ) est un géant des mathématiques allemand du XIXe siècle qui a créé exactement un tel tournant historique. Au cours de sa lutte acharnée avec le **dernier théorème de Fermat** ( **[Fermat's Last Theorem](https://kenji.blog/fr/p/fermats-last-theorem/)** ), il a introduit le concept révolutionnaire de **nombres idéaux** ( **Ideal Numbers** ), jetant les bases de la théorie algébrique moderne des nombres.

Dans cet article, nous plongerons profondément dans la vie tumultueuse de Kummer, les anecdotes humaines qui l'entourent, et ses brillantes réalisations qui continuent de briller dans l'histoire des mathématiques.

---

## La vie et l'éducation de Kummer
...
À l'Université de Berlin, Kummer, avec [Karl Weierstrass](https://kenji.blog/fr/p/weierstrass/) ( **[Karl Weierstrass](https://kenji.blog/fr/p/weierstrass/)** ) et son ancien élève Kronecker, a fait de Berlin un centre mondial des mathématiques. Ses cours étaient extrêmement clairs et passionnés, attirant de nombreux étudiants brillants de toute l'Europe.

---

## Anecdote : Le grand mathématicien qui avait des difficultés en calcul

L'une des anecdotes les plus célèbres à propos de Kummer est qu'il était "mauvais en calcul". Bien qu'il ait été un génie ayant construit des mathématiques hautement abstraites et des théories complexes, on dit qu'il avait souvent du mal avec l'arithmétique de base.

Un jour, lors d'un cours, Kummer est resté bloqué en essayant de calculer $7 \times 9$ au tableau.
...
Soulagé, Kummer a approuvé : "Oui, 63. C'est correct," et a continué son cours comme si de rien n'était.
...
Lamé et d'autres supposaient implicitement que "l'unicité de la factorisation en nombres premiers" dans les entiers ordinaires (où tout entier peut être exprimé de manière unique comme un produit de nombres premiers) s'appliquerait également dans ce monde étendu des entiers complexes (entiers cyclotomiques).

Cependant, Kummer avait déjà découvert quelques années auparavant que cette hypothèse était fausse. Par exemple, lorsque $p=23$, l'unicité de la factorisation en nombres premiers échoue. Sans factorisation unique, la preuve de Lamé et Cauchy s'est complètement effondrée.

```mermaid
flowchart TD
    A["Entiers ordinaires"] -->|"Unicité de la factorisation"| B["Valide"]
    C["Entiers cyclotomiques"] -->|"Extension de la factorisation"| D{"L'unicité est-elle valide ?"}
    D -->|"p < 23"| E["Valide"]
    D -->|"p >= 23"| F["Invalide"]
    F -->|"Erreur de Lamé et Cauchy"| G["Effondrement de la preuve du théorème de Fermat"]
```
...
Pour surmonter la situation critique de l'effondrement de la factorisation unique, Kummer a créé un concept entièrement nouveau : les **nombres idéaux** ( **Ideal Numbers** ).

L'idée de Kummer était la suivante : "Si la factorisation en nombres premiers n'est pas unique, peut-être existe-t-il des 'nombres premiers idéaux' invisibles, et en décomposant les éléments en ces nombres premiers idéaux, l'unicité pourrait être restaurée." C'est semblable à la chimie, où la décomposition de la matière des molécules en atomes clarifie sa composition fondamentale.

Kummer a défini des "facteurs idéaux" au sein de l'ensemble des entiers cyclotomiques - des facteurs qui n'existent pas en tant que nombres ordinaires, mais qui peuvent être manipulés avec une parfaite cohérence dans les opérations algébriques. Grâce à cela, il a brillamment ressuscité l'unicité de la factorisation en nombres premiers dans le domaine des entiers cyclotomiques.

```mermaid
graph LR
    A["Échec de la factorisation"] -- "Multiples modèles de factorisation" --> B["Introduction des nombres idéaux"]
    B -- "Subdivision par des facteurs invisibles" --> C["Restauration de la factorisation unique"]
    C -- "Rigueur des opérations algébriques" --> D["Dérivation des congruences de Kummer"]
```

Plus tard, Richard Dedekind ( **Richard Dedekind** ) a généralisé les nombres idéaux de Kummer, les élevant au concept de l'**idéal** ( **Ideal** ) à l'aide de la théorie des ensembles. Cela est devenu le fondement de la géométrie algébrique et de la théorie des anneaux modernes.

---

## Nombres premiers réguliers et preuve partielle du dernier théorème de Fermat

En utilisant la théorie des nombres idéaux, Kummer a porté un coup massif au dernier théorème de Fermat. Il a défini le concept de **nombres premiers réguliers** ( **Regular Primes** ) et a prouvé le résultat étonnant que "si $p$ est un nombre premier régulier, alors le dernier théorème de Fermat s'applique pour $p$."
...
Kummer a en outre découvert un critère puissant pour déterminer si un nombre premier donné est régulier. Il utilise les **nombres de Bernoulli** ( **Bernoulli Numbers** ) $B_k$. Un nombre premier $p$ est régulier s'il ne divise le numérateur d'aucun des nombres de Bernoulli suivants :

$$
B_2, B_4, B_6, \dots, B_{p-3}
$$

En utilisant ce critère, Kummer a prouvé que le dernier théorème de Fermat s'applique à tous les nombres premiers inférieurs à $100$, à l'exception des nombres premiers irréguliers $37, 59$ et $67$. Ce fut une réalisation monumentale qui provoqua une onde de choc au sein de la communauté mathématique de l'époque.

---

## Contribution à la géométrie : La surface de Kummer

En plus de ses travaux révolutionnaires en théorie des nombres, Kummer a fait des découvertes cruciales dans le domaine de la géométrie. La plus représentative d'entre elles est la **surface de Kummer** ( **Kummer Surface** ).

En 1864, Kummer a étudié une classe spécifique de surfaces quartiques dans l'espace tridimensionnel. Cette surface a la propriété profondément fascinante de posséder le nombre maximum possible de points singuliers (points où la surface n'est pas lisse, par exemple, des pics aigus) - exactement $16$.

$$
\text{Caractéristiques de la surface de Kummer :} \quad \text{Une surface quartique possédant 16 points singuliers}
$$
...
```mermaid
flowchart TD
    A["Le défi de Kummer"] --> B["Invention des nombres idéaux"]
    A --> C["Critères pour les nombres premiers réguliers"]
    A --> D["Découverte de la surface de Kummer"]
    B --> E["Vers la théorie des idéaux de Dedekind"]
    C --> F["Avancée majeure de la conjecture de Fermat"]
    D --> G["Vers la géométrie algébrique moderne"]
    E --> H["Impact immense sur les mathématiques modernes"]
    F --> H
    G --> H
```
