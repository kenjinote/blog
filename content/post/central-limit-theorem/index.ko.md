...
중심극한정리의 엄밀한 증명에는 특성함수와 테일러 전개가 사용됩니다. 여기서는 그 개요를 소개합니다.
...
여기서 다루는 기본형은 $X_1,\ldots,X_n$이 독립이고 동일한 분포를 따르며, 유한한 평균 $\mu$와 유한한 양의 분산 $0<\sigma^2<\infty$를 가질 것을 요구합니다. ‘어떤 분포든’이라는 설명은 이 조건 안에서 이해해야 합니다. 정규분포에 가까워지는 것은 표준화한 합이나 표본평균의 분포이며, 개별 관측값의 분포가 바뀌는 것은 아닙니다.

### 부록 3: 표준오차와 큰 수의 법칙

독립성으로부터 표본평균의 기댓값과 분산은 다음과 같습니다. 표준오차는 표본평균의 변동성을 나타내며, 개별 관측값의 표준편차와는 다릅니다.

$$
E[\bar X_n]=\mu,\qquad \operatorname{Var}(\bar X_n)=\frac{\sigma^2}{n},\qquad \operatorname{SE}(\bar X_n)=\frac{\sigma}{\sqrt n}.
$$

표본 크기가 4배가 되면 표준오차는 절반이 됩니다. 큰 수의 법칙은 표본평균이 $\mu$에 가까워진다는 것을, 중심극한정리는 그 주변의 변동을 $\sqrt{n}$배 했을 때의 분포 모양을 설명합니다.

### 부록 4: 특성함수를 이용한 증명의 보충

$Y_i=(X_i-\mu)/\sigma$, $Z_n=n^{-1/2}\sum_{i=1}^nY_i$로 둡니다. $E[Y_i]=0$, $E[Y_i^2]=1$이므로 특성함수는 원점 근처에서 다음과 같이 전개됩니다.

$$
\phi_Y(t)=E[e^{itY}]=1-\frac{t^2}{2}+o(t^2)\quad(t\to0).
$$

독립성을 이용하면 아래 식을 얻습니다. 극한이 표준정규분포의 특성함수이므로 레비의 연속성 정리에 의해 분포수렴이 성립합니다. 특성함수와 적률생성함수는 서로 다르며, 이 증명은 적률생성함수의 존재를 요구하지 않습니다.
...
코시분포는 유한한 평균과 분산을 갖지 않습니다. 독립인 표준 코시 확률변수의 표본평균도 여전히 표준 코시분포를 따릅니다. 모든 $X_i$가 같은 확률변수라면 독립성이 없으므로 평균을 내도 변동성이 줄지 않습니다. $n\ge30$이면 항상 충분하다는 보장도 없습니다. 왜도와 꼬리의 두께에 따라 필요한 표본 크기가 달라집니다. 독립이지만 동일한 분포가 아닌 경우에는 린데베르크 조건이나 리아푸노프 조건 등의 추가 조건을 확인해야 합니다.
