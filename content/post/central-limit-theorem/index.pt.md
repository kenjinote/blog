...
Ao estudar ciência de dados e estatística, não podemos evitar o **Teorema do Limite Central** (TLC). Este teorema tem uma propriedade quase mágica: "Não importa qual seja a distribuição dos dados, a distribuição da média da amostra se aproxima de uma distribuição normal à medida que o tamanho da amostra aumenta."

Neste artigo, explicaremos amplamente o Teorema do Limite Central, desde uma imagem intuitiva até uma rigorosa definição matemática e exemplos práticos de aplicação.

## 2. O que é o Teorema do Limite Central?

O Teorema do Limite Central (TLC) é um dos resultados mais poderosos e surpreendentes na teoria da probabilidade e estatística. Simplificando, a soma (ou média) de um grande número de variáveis aleatórias independentes amostradas aleatoriamente se aproxima de uma distribuição normal, independentemente da distribuição original das variáveis.
...
Seja a média amostral $\bar{X} = \frac{1}{n} \sum_{i=1}^{n} X_i$. De acordo com o Teorema do Limite Central, quando $n$ é suficientemente grande, a variável padronizada $Z$ conforme mostrado abaixo converge para a distribuição normal padrão $\mathcal{N}(0, 1)$.
...
## 3. Visualização do Teorema do Limite Central

Para entender visualmente como o Teorema do Limite Central funciona, aqui está um diagrama de processo usando Mermaid.
...
Por que o Teorema do Limite Central é tão importante? É porque, mesmo que não saibamos exatamente qual distribuição muitos dados do mundo real têm, podemos supor uma distribuição normal ao usar estatísticas como a média da amostra para conduzir testes de hipóteses e construir intervalos de confiança.

### 5.1 Fundamento da Inferência Estatística
Quando inferimos algo dos dados, como em pesquisas de opinião, controle de qualidade ou testes A/B, grande parte do raciocínio baseia-se no Teorema do Limite Central.
...
Funções características e expansão de Taylor são usadas para uma prova estrita do Teorema do Limite Central. Aqui está um breve resumo.
...
### Apêndice 3: Erro Padrão e a Lei dos Grandes Números
...
Quadruplicar o número de amostras reduz pela metade o erro padrão. A lei dos grandes números afirma que a média da amostra se aproxima de $\mu$, e o Teorema do Limite Central descreve a forma da distribuição multiplicando a flutuação ao seu redor por $\sqrt{n}$.
