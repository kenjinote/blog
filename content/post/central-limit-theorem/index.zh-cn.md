...
中心极限定理的严格证明使用了特征函数和泰勒展开。下面介绍其概要。
...
本文的基本形式要求 $X_1,\ldots,X_n$ 独立同分布，且具有有限均值 $\mu$ 和有限的正方差 $0<\sigma^2<\infty$。“任何分布”应在这些条件下理解。趋近正态分布的是标准化后的和或样本均值的分布，单个观测值的分布并不会改变。

### 附录 3：标准误与大数定律

由独立性可得样本均值的期望和方差如下。标准误衡量样本均值的波动，不同于单个观测值的标准差。

$$
E[\bar X_n]=\mu,\qquad \operatorname{Var}(\bar X_n)=\frac{\sigma^2}{n},\qquad \operatorname{SE}(\bar X_n)=\frac{\sigma}{\sqrt n}.
$$

样本量增加到4倍，标准误减半。大数定律说明样本均值趋近 $\mu$；中心极限定理则描述其周围的波动放大 $\sqrt{n}$ 倍后的分布形状。

### 附录 4：特征函数证明的补充

令 $Y_i=(X_i-\mu)/\sigma$、$Z_n=n^{-1/2}\sum_{i=1}^nY_i$。由于 $E[Y_i]=0$、$E[Y_i^2]=1$，特征函数在原点附近有如下展开。

$$
\phi_Y(t)=E[e^{itY}]=1-\frac{t^2}{2}+o(t^2)\quad(t\to0).
$$

利用独立性得到下式。其极限是标准正态分布的特征函数，因此由列维连续性定理可得依分布收敛。特征函数与矩母函数并不相同，这个证明不要求矩母函数存在。
...
柯西分布没有有限均值或方差，独立标准柯西随机变量的样本均值仍服从标准柯西分布。如果所有 $X_i$ 都等于同一个随机变量，就不满足独立性，取平均也不会减小波动。不存在“$n\ge30$ 就一定足够”的普遍保证；所需样本量取决于偏度和尾部厚度。对独立但不同分布的变量，需要额外检查林德伯格条件或李雅普诺夫条件等。
